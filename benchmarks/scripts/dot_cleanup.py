...
def simplify(line: str) -> str:
    ...
    while True:
        ...
        if bracket_end != -1:
            ...
            result = result[:start] + f"loop_redux<{func_name}>" + result[bracket_end + 1:]
    ...
    while True:
        ...
        if bracket_end != -1:
            ...
            result = result[:start] + f"condition_update_kernel<{func_name}>" + result[bracket_end + 1:]
    ...
    result = re.sub(r'((?:loop(?:_redux)?|condition_update_kernel)<[^>]+>)(.*)', clean_after_loop, result)

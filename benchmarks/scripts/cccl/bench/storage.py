...
def get_postgres_config():
    ...
    required_vars = ["CCCL_BENCH_PG_HOST", "CCCL_BENCH_PG_USER", "CCCL_BENCH_PG_DB", "CCCL_BENCH_PG_PASSWORD"]
    config = {}
...
if POSTGRES_AVAILABLE:
    class PostgreSQLStorage(StorageBase):
        ...
        def subbenches(self, algname):
            with self.conn:
                cur = self.conn.execute("SELECT DISTINCT bench FROM subbenches WHERE algorithm=?", (algname,))
                rows = cur.fetchall()
                return [row[0] for row in rows]
        ...
        def store_df(self, algname, df):
            ...
            raise NotImplementedError("DataFrame storage for PostgreSQL not yet implemented")
else:
        ...
        if cls._instance is None:
            ...
            if pg_config and PostgreSQLStorage is not None:
                try:
                    ...
                    print("Using dual storage: SQLite (primary) + PostgreSQL (secondary)")
                except Exception as e:

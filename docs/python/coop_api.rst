...
Reduce and Sum; group descriptors; ``ThreadData``; and ``TempStorage``. It additionally
exposes backend memory namespaces. Both constructors accept the portable
``alignment`` keyword for minimum payload storage alignment. Portable and

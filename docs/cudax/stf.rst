...
In certain applications, tasks are divided into subtasks (forked), which
are executed concurrently, and then their results are combined (joined)
once all subtasks have completed. This fork/join parallelism model is
particularly effective for problems that can be naturally decomposed
into independent components, such as in divide-and-conquer algorithms.

CUDASTF introduces `epochs`, a generalization of the fork/join model.
An epoch object, once defined, can be used as a dependency in ``task`` or
``parallel_for`` calls. Tasks depending on the same epoch are considered
independent and can execute in parallel. To introduce a join point,
increment the epoch using ``operator++``. Any tasks depending on the epoch
after it has been incremented will wait for the tasks that depend on its
previous value to complete. For example:
...
The first two ``parallel_for`` calls will run concurrently because they
depend on the same epoch value. Incrementing ``e`` moves to the next epoch,
causing the third ``parallel_for`` to wait for the completion of the first
two.

At first glance, incrementing an epoch may seem similar to calling
``cudaDeviceSynchronize``, but there are key distinctions:

- ``cudaDeviceSynchronize`` waits for all GPU activity to finish, while
  ``e++`` only affects tasks depending on that specific epoch object.
- ``cudaDeviceSynchronize`` is a global synchronization, whereas epochs
  allow for the definition of multiple, independently managed epochs.
  This makes epochs more flexible than ``cudaDeviceSynchronize``, which
  functions like a singleton global epoch.
- Incrementing an epoch is more efficient than ``cudaDeviceSynchronize``,
  as it does not trigger synchronization but instead introduces a
  dependency in the control graph. Synchronization happens only when
  ``ctx.finalize()`` is called.

Epochs are most beneficial in applications that manage their own memory
and feature simple dependency graphs.

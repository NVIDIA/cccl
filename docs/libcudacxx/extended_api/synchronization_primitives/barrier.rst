...
If ``!(scope == thread_block_scope && is_address_from(address_space::shared, this))``, then the semantics are the same as
...
If ``scope == thread_scope_block && is_address_from(address_space::shared, this)``, then the semantics of `[thread.barrier.class] <http://eel.is/c++draft/thread.barrier.class>`_

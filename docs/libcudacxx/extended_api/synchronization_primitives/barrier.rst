...
If ``!(scope == thread_block_scope && is_address_from(this, address_space::shared))``, then the semantics are the same as
...
If ``scope == thread_scope_block && is_address_from(this, address_space::shared)``, then the semantics of `[thread.barrier.class] <http://eel.is/c++draft/thread.barrier.class>`_

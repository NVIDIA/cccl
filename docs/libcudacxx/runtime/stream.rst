...
:cpp:any:`cuda::stream_ref`
-------------------------------
.. _cccl-runtime-stream-stream-ref:

:cpp:any:`cuda::stream_ref` is a non-owning wrapper around a ``cudaStream_t``. It prevents unsafe implicit constructions from
...
- comparison operators against other :cpp:any:`cuda::stream_ref` or ``cudaStream_t``
...
:cpp:any:`cuda::stream` is an owning wrapper around a ``cudaStream_t`` that manages the lifetime of the underlying CUDA
stream.
It derives from :cpp:any:`cuda::stream_ref`, provides all of its functionality, and can be used anywhere a
:cpp:any:`cuda::stream_ref` is expected.
It can be constructed for a specific :cpp:any:`cuda::device_ref`, moved (but not copied), and converted from or to a

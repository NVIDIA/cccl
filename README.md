...
- [`compute-matrix`](#compute-matrix-yml): Reads the matrix configuration from the [`matrix.yaml`](ci/matrix.yaml) file and computes the necessary build configurations based on the PR or push event.
...
The matrix configuration defines the various combinations of build configurations we want to test. It is specified in the [`matrix.yaml`](ci/matrix.yaml) file. The configurations include:

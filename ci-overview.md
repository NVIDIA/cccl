...
All calls to these scripts must be made from the same working directory and specify relative paths to artifact files in subdirectories.
...
They are good for eg. testing artifact passing, but the non-`packed` versions offer a more convenient raw-zip file downloads.

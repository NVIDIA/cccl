...
struct agent_policy_t
{
  ...
  static constexpr cub::WarpLoadAlgorithm load_algorithm   = LoadAlgorithm;
  static constexpr cub::CacheLoadModifier load_modifier    = LoadModifier;
  static constexpr cub::WarpStoreAlgorithm store_algorithm = StoreAlgorithm;
  static constexpr int max_segments_per_warp               = MaxSegmentsPerWarp;
};

template <int BlockThreads, int ItemsPerThread, int MaxSegmentsPerWorker = 1>
struct policy_wrapper
{
  using warp_segmented_scan_policy_t =
    agent_policy_t<BlockThreads,
                   ...
                   MaxSegmentsPerWorker>;
};

template <int BlockThreads, int ItemsPerThread, int MaxSegmentsPerWorker>

...
DECLARE_LAUNCH_WRAPPER(cub::DeviceReduce::TransformReduce, device_reduce_transform);
...
C2H_TEST("Device transform reduce uses environment", "[reduce][device]", requirements)
{
                  ...
                  || std::is_same_v<determinism_t, cuda::execution::determinism::not_guaranteed_t>)
    {
      REQUIRE(cudaSuccess
              == cub::DeviceReduce::TransformReduce(
                nullptr, expected_bytes_allocated, d_in, d_out.begin(), num_items, op_t{}, transform_t{}, init));
  ...
  device_reduce_transform(d_in, d_out.begin(), num_items, op_t{}, transform_t{}, init, env);

...
C2H_TEST("DeviceSelect::UniqueByKey works with user provided memory and environment",
         ...
         all_types)
{
  ...
  c2h::device_vector<type> keys_in(num_items, thrust::no_init);
  c2h::device_vector<val_type> vals_in(num_items, thrust::no_init);
  c2h::device_vector<type> keys_out(num_items, thrust::no_init);
  c2h::device_vector<val_type> vals_out(num_items, thrust::no_init);

...
__global__ __launch_bounds__(8) void WarpReduceBatchedSumApiKernel(int* out)
{
  ...
  using WarpReduceBatched = cub::WarpReduceBatched<int, 3, 2, true>;
  ...
  thread_data[1] = static_cast<int>(threadIdx.x) + 1;
  thread_data[2] = static_cast<int>(threadIdx.x) - 1;
  // Use a static size spans to pick the first 3 elements as inputs and the last 2 elements as outputs
  cuda::std::span<int, 3> inputs{cuda::std::begin(thread_data), 3};
  cuda::std::span<int, 2> results{cuda::std::begin(thread_data) + 3, 2};
  WarpReduceBatched{temp_storage[logical_warp_id]}.Sum(inputs, results);
  // results across threads (striped!): [[1, -1], [3, ?], [5, 3], [7, ?], [9, 7], [11, ?], [13, 11], [15, ?]]

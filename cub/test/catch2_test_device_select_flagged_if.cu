         ...
         flag_types)
{
  ...
  const int num_items = GENERATE_COPY(take(2, random(1, 1000000)));
  c2h::device_vector<input_type> in(num_items, thrust::no_init);
  c2h::device_vector<input_type> out(num_items, thrust::no_init);
  c2h::gen(C2H_SEED(2), in);

  is_even_t<flag_type> is_even{};

  c2h::device_vector<flag_type> flags(num_items, thrust::no_init);
  c2h::gen(C2H_SEED(1), flags);

...
CUB_TEST("DeviceTransform::Transform fancy input iterator types", "[device][device_transform]")
{
  ...
  c2h::device_vector<type> result(num_items);
  transform_many(num_items, ::cuda::std::make_tuple(a, b), result.begin(), ::cuda::std::plus<type>{});
...
CUB_TEST("DeviceTransform::Transform mixed input iterator types", "[device][device_transform]")
{
  ...
  c2h::device_vector<type> result(num_items);
  transform_many(num_items, ::cuda::std::make_tuple(a, b.begin()), result.begin(), ::cuda::std::plus<type>{});
         ...
         c2h::type_list<std::uint8_t, std::uint16_t, float, double>)
{
  ...
  c2h::device_vector<type> result(num_items);
  transform_many(num_items - offset,
                 ::cuda::std::make_tuple(input.begin() + offset),
                 result.begin() + offset,
                 ::cuda::std::negate<>{});

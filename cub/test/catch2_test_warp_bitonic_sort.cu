...
C2H_TEST("Warp sort keys-only on partial warp-tile works", "[sort][warp]", key_types, items_per_thread_list)
{
  using params = params_t<TestType>;
  using type   = typename params::type;

...
struct full_op_t
{
  ...
  {
    using warp_topk_t = cub::detail::WarpBitonicTopK<MaxK, KeyT, warp_threads, cub::NullType, Algo>;
    __shared__ typename warp_topk_t::TempStorage temp_storage[WarpsPerBlock];
    warp_topk_t{temp_storage[threadIdx.x / warp_threads]}.TopK(keys, CustomLess{}, calc_k(MaxK));
  }
};

template <Mode mode, WarpBitonicTopKAlgorithm Algo, typename KeyT, int MaxK, int Len>
void full(
  nvbench::state& state,
  nvbench::
    type_list<nvbench::enum_type<mode>, nvbench::enum_type<Algo>, KeyT, nvbench::enum_type<MaxK>, nvbench::enum_type<Len>>)
{
  constexpr int warps_per_block = calc_run_params<mode>().block_dim / warp_threads;
  run_topk_bench<full_op_t<warps_per_block, MaxK, Algo>, mode, KeyT, void, Len, MaxK>(state);
}

NVBENCH_BENCH_TYPES(full, NVBENCH_TYPE_AXES(modes, algos, key_types, max_k_values, len_values))
  .set_type_axes_names({"mode", "algo", "KeyT", "max_k", "len"});
...
struct partial_op_t
{
  ...
  __device__ __forceinline__ void operator()(KeyT (&keys)[ItemsPerThread]) const
  {
    ...
    warp_topk_t{temp_storage[threadIdx.x / warp_threads]}.TopK(keys, CustomLess{}, calc_k(MaxK), valid_nums);
  }
};
...
{
  constexpr int warps_per_block = calc_run_params<mode>().block_dim / warp_threads;
  run_topk_bench<partial_op_t<warps_per_block, MaxK, Algo>, mode, KeyT, void, Len, MaxK>(state);
}

NVBENCH_BENCH_TYPES(partial, NVBENCH_TYPE_AXES(modes, algos, key_types, max_k_values, len_values))
  .set_type_axes_names({"mode", "algo", "KeyT", "max_k", "len"});

template <int WarpsPerBlock, int MaxK, WarpBitonicTopKAlgorithm Algo, typename KeyT>
__global__ void iterator_kernel(int num_iterations, KeyT* keys_in, int k, int num_items)
{
  using warp_topk_t = cub::detail::WarpBitonicTopK<MaxK, KeyT, warp_threads, cub::NullType, Algo>;
  ...
  for (int i = 0; i < num_iterations; ++i)
  {
    ...
    warp_topk.TopK(keys_in + offset, CustomLess{}, k, num_items, keys_out);
    sink(keys_out);
  }
}

template <Mode mode, WarpBitonicTopKAlgorithm Algo, typename KeyT, int MaxK>
void iterator(nvbench::state& state,
              nvbench::type_list<nvbench::enum_type<mode>, nvbench::enum_type<Algo>, KeyT, nvbench::enum_type<MaxK>>)
{
  ...
  constexpr int warps_per_block = calc_run_params<mode>().block_dim / warp_threads;
  const auto kernel             = iterator_kernel<warps_per_block, MaxK, Algo, KeyT>;
  ...
  state.exec(nvbench::exec_tag::gpu | nvbench::exec_tag::no_batch, [&](nvbench::launch& launch) {
    kernel<<<run_params.grid_dim, run_params.block_dim, 0, launch.get_stream()>>>(
      run_params.num_iterations, thrust::raw_pointer_cast(keys_in.data()), k, num_items);
  });
}

NVBENCH_BENCH_TYPES(
  iterator,
  NVBENCH_TYPE_AXES(modes, nvbench::enum_type_list<WarpBitonicTopKAlgorithm::buffered>, key_types, max_k_values))
  .set_type_axes_names({"mode", "algo", "KeyT", "max_k"})

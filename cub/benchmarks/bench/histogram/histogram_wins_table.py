...
NAME = {
    ...
    "GPC": "gmem-priv cuckoo",
    "GPS": "gmem-priv single-probe",
    "GND": "gmem-priv no-cache + direct spill",
    "GSC": "gmem-priv single-probe + coalesced spill",
    "GSR": "gmem-priv single-probe + RLE spill",
    "GNR": "gmem-priv no-cache + RLE spill",
    "DAN": "direct-atomic nocache",
    "DAC": "direct-atomic cuckoo",
    "DAS": "direct-atomic single-probe",
    "GN0": "gmem-priv gather + no coalesce",
    "GC0": "gmem-priv cuckoo + no coalesce",
    "GS0": "gmem-priv single-probe + no coalesce",
    "DN0": "direct-atomic + no cache + no coalesce",
    "DC0": "direct-atomic + cuckoo + no coalesce",
    "DS0": "direct-atomic + single-probe + no coalesce",
}
...
for binary in d:
    ...
    for k in keys:
        vals = {
            c: d[binary][c][k]
            for c in cols
            if k in d[binary][c] and d[binary][c][k] and d[binary][c][k] > 0
        }
        if not vals:
            continue
        ncells += 1
...
print(
    "  coalesce-ON and coalesce-OFF (*0 tags) are separate entries. default/main excluded from contest."
)
print("=" * 92)
print(
...
for binary in d:
    ...
    parts = [
        f"{TAG[c]}:{win.get(c, 0)}"
        for c in order(ALL_COLS)
        if c in d[binary] and pres.get(c, 0) > 0
    ]
    print(f"\n{binary} ({ncells} cells):")
    print("   " + "  ".join(parts))

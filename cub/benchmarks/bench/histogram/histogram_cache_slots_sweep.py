...
AXES_SAMPLE = "[I32,F64]"
LAUNCH_RE = re.compile(r"\[launch\].*->\s*([a-z_]+)(?:\(gated\))?\s*\(smem=(\d+) bytes, slots/ch=(\d+)\)")


def run(binname, algo, slots, is_multi):
    binexe = BINDIR / binname
    cmd = [str(binexe), "--benchmark", "base",
           "--axis", f"SampleT{{ct}}={AXES_SAMPLE}",
    ...
    if p.returncode != 0:
        sys.stderr.write(f"[A] {binname} {algo} slots={slots} exit={p.returncode}\n{p.stderr[-800:]}\n")
        return {}, 0, 0, 0
    ...
    ran_forced = (algo in launched) or (algo == "direct_atomic_cuckoo" and "direct_atomic_cuckoo" in launched)
    cells = {}
...
def main():
    ...
    for label, binname in BINARIES:
        ...
        for algo in ALGOS:
            for slots in SLOT_LADDER:
                ...
                results[label][tag] = {
                    "requested_slots": slots, "launched_slots": slots_seen,
                    "launched_smem_bytes": smem_seen, "ran_forced": ran, "cells": cells,
                }
        ...
        for algo in COMPARE_ALGOS:
            ...
            results[label][tag] = {
                "requested_slots": "ref", "launched_slots": slots_seen,
                "launched_smem_bytes": smem_seen, "ran_forced": ran, "cells": cells,
            }
    ...
    for label, av in results.items():
        for tag, rec in av.items():
            sys.stderr.write(f"[A] {label} {tag}: launched_slots={rec['launched_slots']} "

...
static [[nodiscard]] _CCCL_DEVICE_API int makeWarpUniform(int x)
{
  return __reduce_min_sync(~0, x);
}
static [[nodiscard]] _CCCL_DEVICE_API uint32_t makeWarpUniform(uint32_t x)
{
  return __reduce_min_sync(~0, x);
}
static [[nodiscard]] _CCCL_DEVICE_API uint64_t makeWarpUniform(uint64_t x)
{

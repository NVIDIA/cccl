...
[[nodiscard]] _CCCL_DEVICE_API static int makeWarpUniform(int x)
{
  return __reduce_min_sync(~0, x);
}
[[nodiscard]] _CCCL_DEVICE_API static uint32_t makeWarpUniform(uint32_t x)
{
  return __reduce_min_sync(~0, x);
}
[[nodiscard]] _CCCL_DEVICE_API static uint64_t makeWarpUniform(uint64_t x)
{

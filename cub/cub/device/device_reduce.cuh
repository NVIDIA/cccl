...
//!
//! - ``run_to_run`` (the default) is reproducible because, for a given GPU, every launch with the same
//!   input, build, and launch configuration selects the *same* tuning policy and therefore performs the
//!   *same* fixed reduction tree: the input is partitioned into the same tiles, mapped onto the same
//!   thread blocks, and the partial results are combined in the same fixed order on every run — no
//!   atomics or other run-dependent ordering are involved. Because floating-point addition is only
//!   pseudo-associative, that fixed combining order is what makes the result bitwise-identical from one
//!   run to the next. The combining order is tied to the tuning and the partition, so it can change on a
//!   *different* GPU architecture (or under a different user-provided tuning); that is exactly the
//!   cross-architecture reproducibility that ``gpu_to_gpu`` adds on top.
...
//!   are grouped into a fixed number of exponent-range bins and accumulated in that fixed,
//!   hardware-independent order, so the same inputs yield the same bits on any GPU architecture. For
//!   exactly-associative cases — integral types with a known CUDA binary operator, and
//!   ``float``/``double`` with ``min``/``max`` — the result is already identical across GPUs, so the
//!   request is satisfied by the (faster) ``run_to_run`` path. Other type/operator combinations under
//!   ``gpu_to_gpu`` are rejected at compile time.
...
//!   type equal to the accumulator type. Atomics combine partial results in whatever order the hardware
//!   schedules them, which can differ between runs — hence no run-to-run guarantee, but typically the
//!   fastest option. When those conditions are not met, the call falls back to ``run_to_run`` rather
//!   than failing.
//!
//! @endrst
struct DeviceReduce

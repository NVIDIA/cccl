...
template <typename KeyT, bool IsFundamental = detail::radix::is_fundamental_type<KeyT>>
...
{
  if constexpr (detail::radix::is_fundamental_type<KeyT>)
  {
    using bits_t        = typename Traits<KeyT>::UnsignedBits;
...
struct AgentTopK
{
    ...
    int pass)
  {
    ...
    if (__syncthreads_or(is_last_block))
    {
      ...
      if constexpr (detail::radix::is_fundamental_type<key_in_t>)
      {

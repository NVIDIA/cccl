...
/**
 * @brief A benchmark evaluating `cudax::cuco::bloom_filter::contains_async` performance.
 */
                     ...
                     nvbench::enum_type<VerticalLayout>>)
{
  ...
  {
    // `add` is fully horizontal so only the `contains` layout is swept here
    constexpr auto add_horizontal_layout = words_per_block;
    constexpr auto add_vertical_layout   = 1;
    ...
    // Add the false-positive-rate-optimal number of keys
    const auto num_build_keys = static_cast<cuda::std::int64_t>(filter_size_mb) * 1024 * 1024 * 8 / (2 * PatternBits);
    const auto build_keys     = cuda::counting_iterator<Key>{0};
    filter.add(stream, build_keys, build_keys + num_build_keys);

    auto result = cuda::make_device_buffer<bool>(stream, device, num_keys, cuda::no_init);

    // Probe with keys that were not added, so the reported hits are exactly the false positives
    const auto probe_keys = build_keys + num_build_keys;
    filter.contains(stream, probe_keys, probe_keys + num_keys, result.begin());

    const auto false_positives = static_cast<double>(thrust::count(exec_policy, result.begin(), result.end(), true));

    auto& fpr_summary = state.add_summary("FalsePositiveRate");
    fpr_summary.set_string("hint", "FPR");
    fpr_summary.set_string("short_name", "FalsePositiveRate");
    fpr_summary.set_string("description", "False-positive rate of the Bloom filter.");
    fpr_summary.set_float64("value", false_positives / static_cast<double>(num_keys));

    // The CUPTI-based collectors (`collect_dram_throughput`, `collect_l2_hit_rates`) that the CUCO
    // benchmark uses are deliberately omitted: they require elevated profiling privileges. The
    // memory-traffic annotations below give nvbench enough information to report bandwidth
    // utilization without them.
    auto keys = cuda::make_device_buffer<Key>(stream, device, num_keys, cuda::no_init);
    thrust::sequence(exec_policy, keys.begin(), keys.end(), Key{0});
    stream.sync();

    state.add_element_count(num_keys);
    state.add_global_memory_reads<char>(
      num_keys * static_cast<cuda::std::int64_t>((words_per_block * sizeof(Word)) + sizeof(Key)));
    state.add_global_memory_writes<char>(num_keys * static_cast<cuda::std::int64_t>(sizeof(bool)));

    state.exec([&](nvbench::launch& launch) {
      filter.contains_async({launch.get_stream()}, keys.begin(), keys.end(), result.begin());
    });
  }
}

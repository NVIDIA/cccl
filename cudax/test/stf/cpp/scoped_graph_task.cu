    ...
    for (int iter = 0; iter < NITER; iter++) {
        ...
        ctx.task(exec_place::current_device(), handle_X.rw(), handle_Y.rw(), handle_TMP.write())
                ...
                [&](cudaGraph_t child_graph, auto d_x, auto d_y, auto d_tmp) {
                    ...
                    cuda_try(cudaGraphAddMemcpyNode1D(&cpy_tmp_to_x, child_graph, nullptr, 0, d_tmp.data_handle(),
                            d_x.data_handle(), n * sizeof(double), cudaMemcpyDeviceToDevice));

                    // X = Y
                    cudaGraphNode_t cpy_x_to_y;
                    cuda_try(cudaGraphAddMemcpyNode1D(&cpy_x_to_y, child_graph, &cpy_tmp_to_x, 1, d_x.data_handle(),
                            d_y.data_handle(), n * sizeof(double), cudaMemcpyDeviceToDevice));

                    // Y = TMP
                    cudaGraphNode_t cpy_tmp_to_y;
                    cuda_try(cudaGraphAddMemcpyNode1D(&cpy_tmp_to_y, child_graph, &cpy_x_to_y, 1, d_y.data_handle(),
                            d_tmp.data_handle(), n * sizeof(double), cudaMemcpyDeviceToDevice));
                };
    }

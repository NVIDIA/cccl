...
int main() {
    ...
    for (size_t k = 0; k < niter; k++) {
        ...
        ctx.parallel_for(ltmp.shape(), ltmp.write())->*[k] __device__(size_t i, auto tmp) { tmp(i) = (k % 2) * i; };
    ...
    for (size_t k = 0; k < niter; k++) {
        auto ltmp = run_once(k)->*[&](size_t k) {
            ...
            ctx.parallel_for(out.shape(), out.write())->*[k] __device__(size_t i, auto tmp) { tmp(i) = (k % 2) * i; };
            return out;
        };

...
void write_vtk_2D(const std::string& filename, double* Ez_array, double dx, double dy, double dz, size_t SIZE_X,
    ...
    for (size_t y = 0; y < SIZE_Y; y++)
        for (size_t x = 0; x < SIZE_X; x++) {
            ...
            fprintf(f, "4 %d %d %d %d\n", (int) (point_offset + 0), (int) (point_offset + 1), (int) (point_offset + 2),
                    (int) (point_offset + 3));
        ...
        for (size_t k = 1; k < SIZE_Z - 1; k++)
            for (size_t j = 1; j < SIZE_Y - 1; j++)
                for (size_t i = 1; i < SIZE_X - 1; i++) {
                    Ex(i, j, k) = Ex(i, j, k) + (DT / (epsilon(i, j, k) * DX)) *
...
#pragma acc parallel loop collapse(3) async(2)
        for (size_t k = 1; k < SIZE_Z - 1; k++)
            for (size_t j = 1; j < SIZE_Y - 1; j++)
                for (size_t i = 1; i < SIZE_X - 1; i++) {
                    Ey(i, j, k) = Ey(i, j, k) + (DT / (epsilon(i, j, k) * DY)) *
...
#pragma acc parallel loop collapse(3) async(3)
        for (size_t k = 1; k < SIZE_Z - 1; k++)
            for (size_t j = 1; j < SIZE_Y - 1; j++)
                for (size_t i = 1; i < SIZE_X - 1; i++) {
                    Ez(i, j, k) = Ez(i, j, k) + (DT / (epsilon(i, j, k) * DZ)) *
        ...
        for (size_t k = 0; k < SIZE_Z - 1; k++)
            for (size_t j = 0; j < SIZE_Y - 1; j++)
                for (size_t i = 0; i < SIZE_X - 1; i++) {
                    Hx(i, j, k) = Hx(i, j, k) - (DT / (mu(i, j, k) * DY)) *
                                                        (Ez(i, j + 1, k) - Ez(i, j, k) - Ey(i, j, k + 1) + Ey(i, j, k));
                }
...
#pragma acc parallel loop collapse(3) async(2)
        for (size_t k = 0; k < SIZE_Z - 1; k++)
            for (size_t j = 0; j < SIZE_Y - 1; j++)
                for (size_t i = 0; i < SIZE_X - 1; i++) {
                    Hy(i, j, k) = Hy(i, j, k) - (DT / (mu(i, j, k) * DZ)) *
...
#pragma acc parallel loop collapse(3) async(3)
        for (size_t k = 0; k < SIZE_Z - 1; k++)
            for (size_t j = 0; j < SIZE_Y - 1; j++)
                for (size_t i = 0; i < SIZE_X - 1; i++) {
                    Hz(i, j, k) = Hz(i, j, k) - (DT / (mu(i, j, k) * DX)) *
                                                        (Ey(i + 1, j, k) - Ey(i, j, k) - Ex(i, j + 1, k) + Ex(i, j, k));
                }

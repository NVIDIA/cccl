...
int main() {
    ...
    for (int i = 0; i < K; i++) {
        ...
        ctx.parallel_for(lX[i].shape(), lX[i].write()).set_symbol("INIT")->*
                [] __device__(size_t i, auto x) { x(i) = 2.0 * i + 12.0; };
    }
    ...
    for_each_batched<slice<double>, slice<double>>(context(ctx), K, BATCH_SIZE, [&](size_t i) {
        return std::make_tuple(lX[i].read(), lY[i].write());
    })->*[](context inner_ctx, size_t, auto lxi, auto lyi) { COPY(inner_ctx, lxi, lyi); };

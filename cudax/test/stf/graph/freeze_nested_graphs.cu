...
//  };
  ...
  cuda_safe_call(cudaGraphAddChildGraphNode(&child_graph_node, ctx.get_graph(), fX_ready_nodes.data(), fX_ready_nodes.size(), sub_ctx.get_graph()));


  event_list child_graph_event;
  reserved::fork_from_graph_node(ctx, child_graph_node, ctx.get_graph(), ctx.stage(), child_graph_event, "child graph done");

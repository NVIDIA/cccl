...
logical_slice LITERAL(Ctx& ctx, size_t n, int, std::string out_symbol = "undefined") {
    ...
    ctx.task(z.write()).set_symbol("LITERAL")->*
            [](cudaStream_t stream, auto /*unused*/) { cuda_sleep(sleep_time, stream); };
...
template <typename Ctx>
logical_slice ARRAY_INDEX(
        Ctx& ctx, logical_slice x, logical_slice /*unused*/, size_t sz, std::string out_symbol = "undefined") {
    auto z = ctx.logical_data(shape_of<slice<double>>(sz));
    z.set_symbol(out_symbol);

    ctx.task(x.read(), z.write()).set_symbol("ARRAY INDEX")
                    ->*[](cudaStream_t stream, auto /*unused*/, auto /*unused*/) { cuda_sleep(sleep_time, stream); };
...
logical_slice NOT(Ctx& ctx, logical_slice x, std::string out_symbol = "undefined") {
    ...
    ctx.task(x.read(), y.write()).set_symbol("NOT")->*
            [](cudaStream_t stream, auto /*unused*/, auto /*unused*/) { cuda_sleep(sleep_time, stream); };
...
logical_slice CONCAT(Ctx& ctx, size_t sz, std::vector<logical_slice> inputs, std::string out_symbol = "undefined") {
    ...
    t->*[](cudaStream_t stream) { cuda_sleep(sleep_time, stream); };

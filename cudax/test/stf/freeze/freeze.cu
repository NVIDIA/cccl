...
int main() {
    ...
    ctx.parallel_for(lX.shape(), lX.rw()).set_symbol("X=2X")->*[] __device__(size_t i, auto x) { x(i) *= 2; };
    ...
    ctx.parallel_for(lX.shape(), lX.read(), lY.write()).set_symbol("Y=X")->*
            [] __device__(size_t i, auto x, auto y) { y(i) = x(i); };

    fx.unfreeze(stream);

    ctx.parallel_for(lX.shape(), lX.rw()).set_symbol("X+=1")->*[] __device__(size_t i, auto x) { x(i) += 1; };

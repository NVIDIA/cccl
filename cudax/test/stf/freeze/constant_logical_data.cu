...
int main() {
    ...
    ctx.parallel_for(ld_cst.shape(), ld_cst.write())->*[] __device__(size_t i, slice<int> res) { res(i) = 18 * i - 9; };
    ...
    for (size_t iter = 0; iter < 4; iter++) {
        ...
        ctx.parallel_for(lX.shape(), lX.rw()).set_symbol("X+=cst")->*
                [cst_slice] __device__(size_t i, auto x) { x(i) += cst_slice(i); };

        auto cst2 = run_once()->*[&]() {
            auto ld = ctx.logical_data(shape_of<slice<int>>(N));
            ctx.parallel_for(ld.shape(), ld.write())->*[] __device__(size_t i, slice<int> res) { res(i) = 4 * i - 2; };
            return constant_logical_data(ctx, mv(ld));
        };

        auto cst2_slice = cst2.get();
        ctx.parallel_for(lX.shape(), lX.rw()).set_symbol("X+=cst2")->*
                [cst2_slice] __device__(size_t i, auto x) { x(i) += cst2_slice(i); };
    }

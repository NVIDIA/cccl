...
int log2Int(int n) {
    int result = 0;
    while (n >>= 1) {  // Divide n by 2 until n becomes 0
        ++result;
    }
    return result;
}
...
void bench(context& ctx, test_id id, size_t width, size_t nsteps, size_t repeat_cnt) {
    ...
    double sq_sum = std::accumulate(
            t.begin(), t.end(), 0.0, [mean](double acc, double val) { return acc + std::pow(val - mean, 2); });
    double variance = sq_sum / t.size();
    double standardDeviation = std::sqrt(variance);

    fprintf(stderr, "[%s] Elapsed: %.3lf+-%.4lf us per task (%ld tasks, %ld deps, %lf deps/task (avg)\n",
            test_name(id).c_str(), mean, standardDeviation, task_cnt, deps_cnt, (1.0 * deps_cnt) / task_cnt);
}

...
cublasHandle_t& get_cublas_handle() {
    ...
    if (result == cublasHandle_t()) {  // not found, default value inserted
...
cusolverDnHandle_t& get_cusolver_handle() {
    ...
    if (result == cusolverDnHandle_t()) {  // not found, default value inserted
        // Lazy initialization, and save the handle for future use
...
public:
    ...
    int get_preferred_devid(int row, int col) { return (row % grid_p) + (col % grid_q) * grid_p; }

    auto& handle(int row, int col) { return handles[row + col * mt]; }
...
void DPOTRF(cublasFillMode_t uplo, class matrix<double>& A, int A_row, int A_col) {
    ...
    auto t = ctx.task(
            exec_place::device(A.get_preferred_devid(A_row, A_col)), Akk.rw(), potrf_buffer.write(), devInfo.write());
    t.set_symbol("DPOTRF");
    t->*[uplo](cudaStream_t s, auto sAkk, auto buffer, auto info) {
        auto& h = get_cusolver_handle();
        cuda_safe_call(cusolverDnSetStream(h, s));

        cuda_safe_call(cusolverDnDpotrf(h, uplo, sAkk.extent(0), sAkk.data_handle(), sAkk.stride(1),
                buffer.data_handle(), buffer.extent(0), info.data_handle()));
    };
}

void DGEMM(cublasOperation_t transa, cublasOperation_t transb, double alpha, class matrix<double>& A, int A_row,
        int A_col, class matrix<double>& B, int B_row, int B_col, double beta, class matrix<double>& C, int C_row,
        int C_col) {
    ...
    auto t = ctx.task(exec_place::device(A.get_preferred_devid(C_row, C_col)), A.handle(A_row, A_col).read(),
            B.handle(B_row, B_col).read(), dep_c);
    t.set_symbol("DGEMM");
    t->*[transa, transb, alpha, beta](cudaStream_t s, auto sA, auto sB, auto sC) {
        ...
        cuda_safe_call(cublasDgemm(h, transa, transb, sC.extent(0), sC.extent(1), k, &alpha, sA.data_handle(),
                sA.stride(1), sB.data_handle(), sB.stride(1), &beta, sC.data_handle(), sC.stride(1)));
    };
}

void DSYRK(cublasFillMode_t uplo, cublasOperation_t trans, double alpha, class matrix<double>& A, int A_row, int A_col,
        double beta, class matrix<double>& C, int C_row, int C_col) {
    auto t = ctx.task(exec_place::device(A.get_preferred_devid(C_row, C_col)), A.handle(A_row, A_col).read(),
    ...
    t->*[uplo, trans, alpha, beta](cudaStream_t s, auto sA, auto sC) {
        ...
        cuda_safe_call(cublasDsyrk(
                h, uplo, trans, n, k, &alpha, sA.data_handle(), sA.stride(1), &beta, sC.data_handle(), sC.stride(1)));
    };
}

void DTRSM(cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t transa, cublasDiagType_t diag, double alpha,
        class matrix<double>& A, int A_row, int A_col, class matrix<double>& B, int B_row, int B_col) {
    auto t = ctx.task(exec_place::device(A.get_preferred_devid(B_row, B_col)), A.handle(A_row, A_col).read(),
    ...
    t->*[side, uplo, transa, diag, alpha](cudaStream_t s, auto sA, auto sB) {
        ...
        cuda_safe_call(cublasDtrsm(h, side, uplo, transa, diag, sB.extent(0), sB.extent(1), &alpha, sA.data_handle(),
                sA.stride(1), sB.data_handle(), sB.stride(1)));
    };
}
...
void PDPOTRF(matrix<double>& A) {
    ...
    for (int K = 0; K < NBLOCKS; K++) {
        ...
        for (int row = K + 1; row < NBLOCKS; row++) {
            ...
            DTRSM(CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_T, CUBLAS_DIAG_NON_UNIT, 1.0, A, K, K, A, row,
                    K);
...
void PDTRSM(cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, double alpha,
        class matrix<double>& A, class matrix<double>& B) {
                ...
                for (int k = 0; k < B.mt; k++) {
                    ...
                    for (int m = k + 1; m < B.mt; m++) {
                        for (int n = 0; n < B.nt; n++) {
                            ...
                            DGEMM(trans, CUBLAS_OP_N, -1.0, A, B.mt - k - 1, B.mt - 1 - m, B, B.mt - k - 1, n, lalpha,
                                    B, B.mt - 1 - m, n);
                        }
                    }
                }
...
void PDPOTRS(matrix<double>& A, class matrix<double>& B, cublasFillMode_t uplo) {
    ...
    PDTRSM(CUBLAS_SIDE_LEFT, uplo, uplo == CUBLAS_FILL_MODE_UPPER ? CUBLAS_OP_T : CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
            1.0, A, B);
    ...
    PDTRSM(CUBLAS_SIDE_LEFT, uplo, uplo == CUBLAS_FILL_MODE_UPPER ? CUBLAS_OP_N : CUBLAS_OP_T, CUBLAS_DIAG_NON_UNIT,
            1.0, A, B);
 ...
 ******************************************************************************/
void PDGEMM(cublasOperation_t transa, cublasOperation_t transb, double alpha, class matrix<double>& A,
        class matrix<double>& B, double beta, class matrix<double>& C) {
#ifdef HAVE_DOT
...
int main(int argc, char** argv) {
    ...
    auto hilbert = [=] __host__ __device__(
                           size_t row, size_t col) { return 1.0 / (col + row + 1.0) + 2.0 * N * (col == row); };

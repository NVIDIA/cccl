...
template <typename T>
void copy_task(size_t cnt, logical_data<slice<T>>& dst, size_t offset_dst, logical_data<slice<T>>& src,
        size_t offset_src, int dev) {
    ctx.task(exec_place::device(dev), dst.rw(), src.read())->*[&](cudaStream_t stream, auto dstS, auto srcS) {
        int nblocks = (cnt > 64) ? 32 : 1;
        copy_kernel<T>
                <<<nblocks, 64, 0, stream>>>(cnt, dstS.data_handle() + offset_dst, srcS.data_handle() + offset_src);
    };
}
...
int main(int argc, char** argv) {
    ...
    for (int b = 0; b < NBLOCKS; b++) {
        ...
        ctx.task(exec_place::host, Un[b].handle.rw(), Un1[b].handle.rw())
                        ->*[&](cudaStream_t stream, auto sUn, auto sUn1) {
                                cuda_safe_call(cudaStreamSynchronize(stream));

...
template <typename T>
void copy_task(stream_ctx& ctx, size_t cnt, logical_data<slice<T>>& dst, size_t offset_dst, int dst_dev,
        logical_data<slice<T>>& src, size_t offset_src, int src_dev) {
    ...
    t->*[&](cudaStream_t stream, auto dst_array, auto src_array) {
        ...
        copy_kernel<T><<<nblocks, 64, 0, stream>>>(
                cnt, dst_array.data_handle() + offset_dst, src_array.data_handle() + offset_src);
    };
}

...
int main() {
    ...
    for (int iter = 0; iter < niters; iter++) {
        ...
        for (int i = 0; i < N; i++) {
            ctx.parallel_for(var_handle.shape(), var_handle.redux(op))->*[=] CUDASTF_DEVICE(size_t ind, auto d_var) {
                atomicAdd(d_var.data_handle(), i);
            };
        }

        // Check that we have the expected value, and reset it so that we can perform another reduction
        ctx.parallel_for(var_handle.shape(), var_handle.rw())->*[=] CUDASTF_DEVICE(size_t ind, auto d_var) {

...
struct OR_op {
    static void init_host(bool& out) { out = false; };
    static __device__ void init_gpu(bool& out) { out = false; };
    static void op_host(const bool& in, bool& inout) { inout |= in; };
    static __device__ void op_gpu(const bool& in, bool& inout) { inout |= in; };
};
...
int main() {
    ...
    ctx.task(lC.redux(op), lA.read())->*[](auto stream, auto sC, auto sA) {
        cudaMemcpyAsync(
                sC.data_handle(), sA.data_handle(), sA.extent(0) * sizeof(bool), cudaMemcpyDeviceToDevice, stream);
    };

    // C |= B
    ctx.task(lC.redux(op), lB.read())->*[](auto stream, auto sC, auto sB) {
        cudaMemcpyAsync(
                sC.data_handle(), sB.data_handle(), sB.extent(0) * sizeof(bool), cudaMemcpyDeviceToDevice, stream);
    };

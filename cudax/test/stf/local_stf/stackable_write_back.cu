...
int main()
{
 ...
 /* Start to use a graph */
  ...
  lA2.push(access_mode::rw);
  lA3.push(access_mode::rw);
  lA4.push(access_mode::rw);
  ...
  // XXX (it seems that this write is not written back to the copy of array4 on the device when popping before we write back ?)

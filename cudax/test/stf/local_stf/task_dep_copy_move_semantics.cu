...
void test_compile_time_type_traits()
{
  ...
  static_assert(::std::is_copy_constructible_v<task_dep_untyped>, 
                "task_dep_untyped must be copy constructible");
  static_assert(::std::is_copy_assignable_v<task_dep_untyped>,
                "task_dep_untyped must be copy assignable");
  static_assert(::std::is_move_constructible_v<task_dep_untyped>,
                "task_dep_untyped must be move constructible");  
  static_assert(::std::is_move_assignable_v<task_dep_untyped>,
                "task_dep_untyped must be move assignable");

  // Test task_dep<T> for common types
  using int_task_dep = task_dep<slice<int>>;
  static_assert(::std::is_copy_constructible_v<int_task_dep>,
                "task_dep<slice<int>> must be copy constructible");
  static_assert(::std::is_copy_assignable_v<int_task_dep>,
                "task_dep<slice<int>> must be copy assignable");
  static_assert(::std::is_move_constructible_v<int_task_dep>,
                "task_dep<slice<int>> must be move constructible");
  static_assert(::std::is_move_assignable_v<int_task_dep>,
                "task_dep<slice<int>> must be move assignable");

  using double_task_dep = task_dep<slice<double>>;
  static_assert(::std::is_copy_constructible_v<double_task_dep>,
                "task_dep<slice<double>> must be copy constructible");
  static_assert(::std::is_copy_assignable_v<double_task_dep>,
                "task_dep<slice<double>> must be copy assignable");
  static_assert(::std::is_move_constructible_v<double_task_dep>,
                "task_dep<slice<double>> must be move constructible");
  static_assert(::std::is_move_assignable_v<double_task_dep>,
                "task_dep<slice<double>> must be move assignable");
...
int main()
{
  ...
  // Test functional usage with actual tasks to ensure copy/move semantics work in practice
  {
    ...
    task->*[&task](cudaStream_t stream) {
      ...
      double_values_kernel<<<grid_size, block_size, 0, stream>>>(
        output.data_handle(), input.data_handle(), N);
    };
    ...
    ctx.host_launch(ltest.read())->*[](auto data) {
      for (size_t i = 0; i < data.size(); i++) {
        EXPECT(data(i) == static_cast<int>(i * 2), 
               "Expected ", i * 2, " but got ", data(i), " at index ", i);
      }
    };
  }

...
int main() {
    ...
    sctx.parallel_for(lA.shape(), lA.write())->*[] __device__(size_t i, auto a) { a(i) = 42 + 2 * i; };
    ...
    sctx.parallel_for(lB.shape(), lB.write())->*[] __device__(size_t i, auto b) { b(i) = 17 - 3 * i; };

    lC.push(access_mode::rw);
    lA.push(access_mode::read);
    lA2.push(access_mode::write, data_place::current_device());

    sctx.parallel_for(lA2.shape(), lA2.write())->*[] __device__(size_t i, auto a2) { a2(i) = 5 * i + 4; };

    sctx.parallel_for(lB.shape(), lA.read(), lB.rw())->*[] __device__(size_t i, auto a, auto b) { b(i) += a(i); };

    sctx.parallel_for(lB.shape(), lB.read(), lC.rw())->*[] __device__(size_t i, auto b, auto c) { c(i) += b(i); };

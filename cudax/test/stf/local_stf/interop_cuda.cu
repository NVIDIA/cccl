...
int main() {
    ...
    for (size_t i = 0; i < NITER; i++) {
        ...
        ctx.parallel_for(lA.shape(), lA.write())->*[] __device__(size_t i, auto a) { a(i) = sin(double(i)); };
        ctx.parallel_for(lB.shape(), lB.write())->*[] __device__(size_t i, auto b) { b(i) = cos(double(i)); };

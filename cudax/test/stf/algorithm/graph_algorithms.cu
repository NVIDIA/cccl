...
void lib_call(ctx_t& ctx, logical_data<T> a, logical_data<T2> b) {
    ...
    // a = a + b
    // b = a
    ctx.parallel_for(b.shape(), b.rw())->*[] __device__(size_t i, auto sb) { sb(i) *= 2; };

    ctx.parallel_for(a.shape(), a.rw(), b.read())->*[] __device__(size_t i, auto sa, auto sb) { sa(i) += sb(i); };

    ctx.parallel_for(a.shape(), a.read(), b.write())->*[] __device__(size_t i, auto sa, auto sb) { sb(i) = sa(i); };
}

template <typename T>
void init(context& ctx, logical_data<T> l, int val) {
    ctx.parallel_for(l.shape(), l.write())->*[=] __device__(size_t i, auto s) { s(i) = val; };
}

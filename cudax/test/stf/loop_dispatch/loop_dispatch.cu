...
#include "cudastf/__stf/places/loop_dispatch.h"
...
int main() {
    ...
    ctx.parallel_for(lB.shape(), lB.write())->*[] __device__(size_t i, auto b) { b(i) = 42; };
    ...
    loop_dispatch(ctx, exec_place::all_devices(), 0, n, [&](size_t) {
        ...
        ctx.parallel_for(ctx.current_exec_place(), lA.shape(), lA.rw(), lB.read())
                        ->*[] __device__(size_t i, auto a, auto b) { a(i) += b(i); };
    });

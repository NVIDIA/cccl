...
C2H_TEST("graph fill_bytes sets every byte to the requested value", "[graph][fill_bytes]")
{
  ...
  const test::pinned_array<int> mem{N, static_cast<int>(0xDEADBEEF)};
...
C2H_TEST("graph fill_bytes with non-zero value", "[graph][fill_bytes]")
{
  ...
  const test::pinned_array<unsigned char> mem{N};
...
C2H_TEST("graph copy_bytes copies data from source to destination", "[graph][copy_bytes]")
{
  ...
  const test::pinned_array<int> src{N};
  const test::pinned_array<int> dst{N, -1};
...
C2H_TEST("graph copy_bytes can be chained after fill_bytes", "[graph][fill_bytes][copy_bytes]")
{
  ...
  const test::pinned_array<unsigned char> src{N};
  const test::pinned_array<unsigned char> dst{N};
...
{
  const cudax::stream s{cuda::device_ref{0}};
  const test::pinned_array<int> mem{1};
...
{
  const cudax::stream s{cuda::device_ref{0}};
  const test::pinned_array<int> mem{1, 5}; // will be decremented to 0

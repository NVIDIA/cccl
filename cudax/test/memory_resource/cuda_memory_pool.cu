...
TEST_CASE("cuda_memory_pool construction", "[memory_resource]")
{
  ...
  SECTION("Construct from device id")
  {
    ...
    ::cudaMemPool_t native_handle = from_device.native_handle();
  ...
  SECTION("Construct with empty properties")
  {
    ...
    ::cudaMemPool_t native_handle = from_defaulted_properties.native_handle();
    CHECK(native_handle != current_default_pool);
...
TEST_CASE("cuda_memory_pool comparison", "[memory_resource]")
{
  ...
  { // comparison against a cudaMemPool_t
    CHECK(first == first.native_handle());
    CHECK(first.native_handle() == first);

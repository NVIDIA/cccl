...
static_assert(!cuda::std::is_trivial<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(!cuda::std::is_trivially_default_constructible<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_default_constructible<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_copy_constructible<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_move_constructible<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_copy_assignable<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_move_assignable<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(cuda::std::is_trivially_destructible<cudax::mr::cuda_async_memory_resource>::value, "");
static_assert(!cuda::std::is_empty<cudax::mr::cuda_async_memory_resource>::value, "");
...
TEST_CASE("cuda_async_memory_resource construction", "[memory_resource]")
{
  int current_device{};
  {
  ...
  using async_resource = cuda::experimental::mr::cuda_async_memory_resource;
  SECTION("Default construction")
  {
    {
      async_resource default_constructed{};
      CHECK(default_constructed.native_handle() == current_default_pool);
    }
    ...
    _CCCL_TRY_CUDA_API(
      ::cudaMallocAsync,
      "Failed to allocate with pool passed to cuda::experimental::mr::cuda_async_memory_resource",
      &ptr,
      42,
      current_default_pool,
      ::cudaStream_t{0});
    ...
    _CCCL_ASSERT_CUDA_API(
      ::cudaFreeAsync,
      "Failed to deallocate with pool passed to cuda::experimental::mr::cuda_async_memory_resource",
      ptr,
      ::cudaStream_t{0});
  }
  ...
  SECTION("Construct from mempool handle")
  {
...

    {
      ...
      CHECK(from_cudaMemPool.native_handle() == cuda_pool_handle);
      CHECK(from_cudaMemPool.native_handle() != current_default_pool);
    }
  ...
  SECTION("Construct with initial pool size")
  {
    ...
    cuda::experimental::mr::cuda_memory_pool pool{current_device, props};
    async_resource from_initial_pool_size{pool};

    ::cudaMemPool_t native_handle = from_initial_pool_size.native_handle();
    CHECK(native_handle != current_default_pool);

    // Ensure we use the right release threshold
    CHECK(ensure_release_threshold(native_handle, 0));

    // Ensure that we disable reuse with unsupported drivers
    CHECK(ensure_disable_reuse(native_handle, driver_version));

    // Ensure that we disable export
    CHECK(ensure_export_handle(native_handle, ::cudaMemHandleTypeNone));
  }
  ...
  SECTION("Construct with release threshold")
  {
    ...
    ::cudaMemPool_t native_handle = with_threshold.native_handle();
    CHECK(native_handle != current_default_pool);

    // Ensure we use the right release threshold
    CHECK(ensure_release_threshold(native_handle, props.release_threshold));
  ...
  SECTION("Construct with allocation handle")
  {
    ...
    ::cudaMemPool_t native_handle = with_allocation_handle.native_handle();
    CHECK(native_handle != current_default_pool);
    ...
    // Ensure that we disable export
    CHECK(ensure_export_handle(native_handle, static_cast<cudaMemAllocationHandleType>(props.allocation_handle_type)));
  }
...
TEST_CASE("cuda_async_memory_resource allocation", "[memory_resource]")
{
...
TEST_CASE("cuda_async_memory_resource comparison", "[memory_resource]")
{
  int current_device{};
  {
    _CCCL_TRY_CUDA_API(::cudaGetDevice, "Failed to query current device with cudaGetDevice.", &current_device);
  ...
  cuda::experimental::mr::cuda_async_memory_resource first{};
  { // comparison against a plain cuda_async_memory_resource
    cuda::experimental::mr::cuda_async_memory_resource second{};
    CHECK(first == second);
    CHECK(!(first != second));
  }

  { // comparison against a plain cuda_async_memory_resource with a different pool
    cudaMemPool_t cuda_pool_handle{};
    {
      ::cudaMemPoolProps pool_properties{};
      pool_properties.allocType     = ::cudaMemAllocationTypePinned;
    ...
    cuda::experimental::mr::cuda_async_memory_resource second{cuda_pool_handle};
    CHECK(first != second);
    CHECK(!(first == second));
  }

  { // comparison against a cuda_async_memory_resource wrapped inside a resource_ref<device_accessible>
    cuda::experimental::mr::cuda_async_memory_resource second{};
  ...
  { // comparison against a cuda_async_memory_resource wrapped inside a resource_ref<>
    cuda::experimental::mr::cuda_async_memory_resource second{};
  ...
  { // comparison against a cuda_async_memory_resource wrapped inside a async_resource_ref
    cuda::experimental::mr::cuda_async_memory_resource second{};

...
TEST_CASE("cuda_async_memory_resource construction", "[memory_resource]")
{
  ...
  {
    {
      ...
      CHECK(default_constructed.pool_handle() == current_default_pool);
    }
  ...
  SECTION("Construct from mempool handle")
  {
...

    {
      ...
      CHECK(from_cudaMemPool.pool_handle() == cuda_pool_handle);
      CHECK(from_cudaMemPool.pool_handle() != current_default_pool);
    }

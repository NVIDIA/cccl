...
_CCCL_HOST_DEVICE_API void check_hash_reduction()
{
  ...
  static_assert(cuco::detail::__reduce_hash(-1, Index{17}) == Index{1});
  CHECK(cuco::detail::__reduce_hash(-1, Index{17}) == Index{1});
  static_assert(cuco::detail::__reduce_hash(cuda::std::int8_t{-128}, Index{257}) == Index{128});
  CHECK(cuco::detail::__reduce_hash(cuda::std::int8_t{-128}, Index{257}) == Index{128});
  static_assert(cuco::detail::__reduce_hash(cuda::std::uint8_t{255}, Index{257}) == Index{255});
  CHECK(cuco::detail::__reduce_hash(cuda::std::uint8_t{255}, Index{257}) == Index{255});
  static_assert(cuco::detail::__reduce_hash(i64_min, Index{17}) == static_cast<Index>(min_magnitude % 17));
  CHECK(cuco::detail::__reduce_hash(i64_min, Index{17}) == static_cast<Index>(min_magnitude % 17));
  static_assert(cuco::detail::__reduce_hash(i64_min, limit) == static_cast<Index>(min_magnitude % limit));
  CHECK(cuco::detail::__reduce_hash(i64_min, limit) == static_cast<Index>(min_magnitude % limit));
  static_assert(cuco::detail::__reduce_hash(u64_max, limit) == static_cast<Index>(u64_max % limit));
  CHECK(cuco::detail::__reduce_hash(u64_max, limit) == static_cast<Index>(u64_max % limit));
  static_assert(cuco::detail::__reduce_hash(u64_max, Index{17}) == static_cast<Index>(u64_max % 17));
  CHECK(cuco::detail::__reduce_hash(u64_max, Index{17}) == static_cast<Index>(u64_max % 17));
  static_assert(cuco::detail::__reduce_hash(wide, Index{13}) == Index{12});
  CHECK(cuco::detail::__reduce_hash(wide, Index{13}) == Index{12});
  static_assert(cuco::detail::__reduce_hash(array_hash, Index{13}) == Index{12});
  CHECK(cuco::detail::__reduce_hash(array_hash, Index{13}) == Index{12});
  static_assert(cuco::detail::__reduce_hash(i64_min, Index{1}) == Index{0});
  CHECK(cuco::detail::__reduce_hash(i64_min, Index{1}) == Index{0});
  ...
  static_assert(cuco::detail::__reduce_hash(wide128, Index{13}) == Index{6});
  CHECK(cuco::detail::__reduce_hash(wide128, Index{13}) == Index{6});
  static_assert(cuco::detail::__reduce_hash(signed_wide128, Index{13}) == Index{6});
  CHECK(cuco::detail::__reduce_hash(signed_wide128, Index{13}) == Index{6});
  static_assert(cuco::detail::__reduce_hash(i128_min, Index{17}) == Index{9});
  CHECK(cuco::detail::__reduce_hash(i128_min, Index{17}) == Index{9});
  static_assert(cuco::detail::__reduce_hash(u128_max, Index{17}) == Index{0});
  CHECK(cuco::detail::__reduce_hash(u128_max, Index{17}) == Index{0});
  static_assert(cuco::detail::__reduce_hash(u128_max, limit) == static_cast<Index>(u128_max % limit));
  CHECK(cuco::detail::__reduce_hash(u128_max, limit) == static_cast<Index>(u128_max % limit));
  static_assert(cuco::detail::__reduce_hash(i128_min, Index{1}) == Index{0});
  CHECK(cuco::detail::__reduce_hash(i128_min, Index{1}) == Index{0});
...
{
  const auto first_bucket = static_cast<unsigned>(first_magnitude % 7);
  const auto step         = static_cast<unsigned>(second_magnitude % 6 + 1);
  cuda::launch(
    stream,
...
C2H_TEST("cuco probing accepts public 128-bit hash results", "[probing][hash][cg]", index_types)
{
  ...
  constexpr auto first       = static_cast<unsigned>(first_hash % 7);
  constexpr auto step        = static_cast<unsigned>(second_hash % 6 + 1);

  CHECK(cuco::detail::__reduce_hash(hasher{0}(42), index_type{7}) == static_cast<index_type>(first));
  CHECK(cuco::detail::__reduce_hash(hasher{42}(42), index_type{6}) == static_cast<index_type>(step - 1));
  const cuda::stream stream{cuda::device_ref{0}};
  cuda::launch(
    stream,

...
__device__ void test_cg_interop(Config config)
{
  ...
  // Test cg::this_grid().
  {
    ...
    static_assert(noexcept(cudax::make_cg_equivalent_group(cg_group, config)));
    auto cccl_group = cudax::make_cg_equivalent_group(cg_group, config);

    static_assert(cuda::std::is_same_v<cudax::this_grid<Hierarchy>, decltype(cccl_group)>);
...
#if defined(_CG_HAS_CLUSTER_GROUP)
  {
    ...
    static_assert(cuda::std::is_same_v<cudax::this_cluster<Hierarchy>, decltype(cccl_group)>);
    REQUIRE(cuda::gpu_thread.count(cccl_group) == cg_group.size());
    REQUIRE(cuda::gpu_thread.rank(cccl_group) == cg_group.thread_rank());
    cccl_group.sync();
  }
  ...
  // Test cg::this_thread_block().
  {
    ...
    static_assert(cuda::std::is_same_v<cudax::this_block<Hierarchy>, decltype(cccl_group)>);
    REQUIRE(cuda::gpu_thread.count(cccl_group) == cg_group.size());
    REQUIRE(cuda::gpu_thread.rank(cccl_group) == cg_group.thread_rank());
    cccl_group.sync();
  }
  ...
  // Test cg::this_thread().
  {
    ...
    static_assert(cuda::std::is_same_v<cudax::this_thread<Hierarchy>, decltype(cccl_group)>);
    REQUIRE(cuda::gpu_thread.count(cccl_group) == cg_group.size());
    REQUIRE(cuda::gpu_thread.rank(cccl_group) == cg_group.thread_rank());
    cccl_group.sync();
  }
  ...
  cuda::static_for<cuda::ilog2(16)>([&](auto i) {
    ...
    const cudax::this_warp this_warp{config};
    ...
    static_assert(
      cuda::std::is_same_v<
        decltype(cudax::generic_group{cuda::gpu_thread, this_warp, cudax::group_by<n>{}, cudax::lane_synchronizer{}}),
        decltype(cccl_group)>);
    REQUIRE(cuda::gpu_thread.count(cccl_group) == cg_group.size());
    REQUIRE(cuda::gpu_thread.rank(cccl_group) == cg_group.thread_rank());
    cccl_group.sync();
  ...
  // Test cg::tiled_partition<N>(cg::this_thread_block()) with N == 32.
  {
    ...
    static_assert(cuda::std::is_same_v<cudax::this_warp<Hierarchy>, decltype(cccl_group)>);
    REQUIRE(cuda::gpu_thread.count(cccl_group) == cg_group.size());
    REQUIRE(cuda::gpu_thread.rank(cccl_group) == cg_group.thread_rank());
    cccl_group.sync();
  }

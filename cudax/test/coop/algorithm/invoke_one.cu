...
__device__ void test_invoke_one(const Group& group)
{
  // invoke_one callable with void return type
  {
    ...
    static_assert(cuda::std::is_same_v<void, decltype(cudax::coop::invoke_one(group, callable))>);
    static_assert(!noexcept(cudax::coop::invoke_one(group, callable)));

    cudax::coop::invoke_one(group, callable);
  ...
  // invoke_one callable with value return type
  {
    ...
    static_assert(cuda::std::is_same_v<cuda::std::optional<int>, decltype(cudax::coop::invoke_one(group, callable))>);
    static_assert(!noexcept(cudax::coop::invoke_one(group, callable)));

    const auto ret = cudax::coop::invoke_one(group, callable);
    REQUIRE(ret.has_value() == cudax::coop::__elect_one(group));
    if (ret.has_value())
    {
      REQUIRE(ret == 1);
    }
  ...
  // Check that invoke_one correctly forwards the arguments for invocable with void return type.
  {
    ...
    int arg1{2};
    unsigned arg2{20};
    cudax::coop::invoke_one(group, callable, arg1, cuda::std::move(arg2));
  }
  ...
  // Check that invoke_one correctly forwards the arguments for invocable with non-void return type.
  {
    ...
    int arg1{2};
    unsigned arg2{20};
    const auto ret = cudax::coop::invoke_one(group, callable, arg1, cuda::std::move(arg2));
...
struct TestKernel
{
  ...
  __device__ void operator()(const Config& config)
  {
    ...
    {
      test_invoke_one(cudax::coop::this_thread{config});
    }
    if (cuda::warp.rank(cuda::grid, config) == 0)
    {
      test_invoke_one(cudax::coop::this_warp{config});
    }
    if (cuda::block.rank(cuda::grid, config) == 0)
    {
      test_invoke_one(cudax::coop::this_block{config});
    }
    if (cuda::cluster.rank(cuda::grid, config) == 0)
    {
      test_invoke_one(cudax::coop::this_cluster{config});
    }
    test_invoke_one(cudax::coop::this_grid{config});

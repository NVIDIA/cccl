...
{
  static_assert(cudax::coop::group<Group>);
...
struct TestKernel1
{
  ...
  {
    auto parent_group = cudax::coop::make_this_group(cuda::warp, config);
    const auto rank   = cuda::gpu_thread.rank_as<cuda::std::uint32_t>(parent_group);

    const cudax::coop::virtual_group group1{
      cuda::gpu_thread, parent_group, cudax::coop::group_by{cuda::std::integral_constant<cuda::std::size_t, 16>{}}};
    test_group_membership(group1, 16, rank % 16);
    REQUIRE(group1.count(parent_group) == 2);
    REQUIRE(group1.rank(parent_group) == rank / 16);

    const cudax::coop::virtual_group group2{
      cuda::gpu_thread, group1, cudax::coop::group_as{cuda::std::integer_sequence<cuda::std::size_t, 8, 8>{}}};
    test_group_membership(group2, 8, rank % 8);
    REQUIRE(group2.count(group1) == 2);
    REQUIRE(group2.rank(group1) == (rank % 16) / 8);

    const cudax::coop::virtual_group group3{cuda::gpu_thread, group2, cudax::coop::group_by{4}};
...
struct TestKernel2
{
  ...
  __device__ void operator()(Config config) const
  {
    auto parent_group = cudax::coop::make_this_group(cuda::block, config);
    const auto rank   = cuda::gpu_thread.rank_as<cuda::std::uint32_t>(parent_group);

    const cudax::coop::virtual_group group1{
      cuda::gpu_thread, parent_group, cudax::coop::group_by{cuda::std::integral_constant<cuda::std::size_t, 32>{}}};
    test_group_membership(group1, 32, rank % 32);
    REQUIRE(group1.count(parent_group) == 4);
    REQUIRE(group1.rank(parent_group) == rank / 32);

    const cudax::coop::virtual_group group2{
      cuda::gpu_thread, group1, cudax::coop::group_as{cuda::std::integer_sequence<cuda::std::size_t, 16, 16>{}}};
    test_group_membership(group2, 16, rank % 16);
    REQUIRE(group2.count(group1) == 2);
    REQUIRE(group2.rank(group1) == (rank % 32) / 16);

    const cudax::coop::virtual_group group3{
      cuda::gpu_thread, group2, cudax::coop::group_by{cuda::std::integral_constant<cuda::std::size_t, 8>{}}};
    test_group_membership(group3, 8, rank % 8);
    REQUIRE(group3.count(group2) == 2);
    REQUIRE(group3.rank(group2) == (rank % 16) / 8);

    const cudax::coop::virtual_group group4{cuda::gpu_thread, group3, cudax::coop::identity_mapping{}};
...
struct TestKernel3
{
  ...
  __device__ void operator()(Config config) const
  {
    ...
    const cudax::coop::virtual_group group1{
      cuda::warp, parent_group, cudax::coop::group_by{cuda::std::integral_constant<cuda::std::size_t, 16>{}}};
    ...
    const cudax::coop::virtual_group group2{
      cuda::warp, group1, cudax::coop::group_by{cuda::std::integral_constant<cuda::std::size_t, 8>{}}};

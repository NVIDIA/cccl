...
__device__ cuda::std::optional<T> sum(const cudax::coop::this_thread<Hierarchy>& group, T (&array)[N])
{
  return {cub::ThreadReduce(array, cuda::std::plus<T>{})};
}

template <class Hierarchy, class T, cuda::std::size_t N>
__device__ cuda::std::optional<T> sum(const cudax::coop::this_warp<Hierarchy>& group, T (&array)[N])
{
...
template <class Hierarchy, class T, cuda::std::size_t N>
__device__ cuda::std::optional<T> sum(const cudax::coop::this_block<Hierarchy>& group, T (&array)[N])
{
...
template <class Hierarchy, class T, cuda::std::size_t N>
__device__ cuda::std::optional<T> sum(const cudax::coop::this_cluster<Hierarchy>& group, T (&array)[N])
{
  using BlockExts = decltype(cuda::gpu_thread.extents(cuda::block, group.hierarchy()));
  static_assert(BlockExts::rank_dynamic() == 0, "This algorithm requires all static extents.");
  ...
  NV_IF_TARGET(NV_PROVIDES_SM_90, ({
                 ...
                 cudax::coop::this_block this_block{group.hierarchy()};
...
__device__ cuda::std::optional<T> sum(const Group& group, T (&array)[N])
{
  ...
  const auto unit_group  = cudax::coop::make_this_group(Unit{}, group.hierarchy());
...
struct TestKernel
{
  ...
  {
    test_cooperative_algorithm(cudax::coop::this_thread{config});
    test_cooperative_algorithm(cudax::coop::this_warp{config});
    test_cooperative_algorithm(cudax::coop::this_block{config});

...
struct functor_taking_config
{
  ...
  {
    static_assert(cuda::gpu_thread.count(cuda::block, config.dims) == BlockSize);
    CUDAX_REQUIRE(cuda::block.count(cuda::grid, config.dims) == grid_size);
    kernel_run_proof = true;
  }
};
...
void launch_smoke_test(StreamOrPathBuilder& dst)
{
  ...
  // Lambda
  {
    cudax::launch(dst, cuda::block_dims<256>() & cuda::grid_dims(1), [] __device__(auto config) {
      if (cuda::gpu_thread.rank(cuda::block, config.dims) == 0)
      {
...
void test_default_config()
{
  ...
  auto verify_lambda = [] __device__(auto config) {
    static_assert(cuda::gpu_thread.count(cuda::block, config.dims) == 256);
    CUDAX_REQUIRE(cuda::block.count(cuda::grid, config.dims) == 4);

...
TEMPLATE_TEST_CASE("cudax::async_vector insert",
                   ...
                   (cuda::std::tuple<cuda::mr::host_accessible, cuda::mr::device_accessible>) )
{
  ...
  SECTION("cudax::async_vector::insert(const_iter, const T&)")
  {
    { // cudax::async_vector::insert(const_iter, const T&), empty without allocation
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, const T&), empty sufficient capacity
    ...
    { // cudax::async_vector::insert(const_iter, const T&), non-empty sufficient capacity
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1), T(1337)}};
      vec.resize(5);
    ...
    { // cudax::async_vector::insert(const_iter, const T&), non-empty growing
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1)}};
      const T to_be_inserted = 42;
      auto res               = vec.insert(vec.cbegin() + 1, to_be_inserted);
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec));
    ...
    { // cudax::async_vector::insert(const_iter, const T&), empty at back
      Vector vec{env};
      const T to_be_inserted = 3;
    ...
    { // cudax::async_vector::insert(const_iter, const T&), non-empty at back
  ...
  SECTION("cudax::async_vector::insert(const_iter, T&&)")
  {
    { // cudax::async_vector::insert(const_iter, T&&), empty without allocation
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, T&&), empty sufficient capacity
      Vector vec{env, 10, (-2)};
      vec.clear();
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::insert(const_iter, T&&), non-empty sufficient capacity
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1), T(1337)}};
      vec.resize(5);
    ...
    { // cudax::async_vector::insert(const_iter, T&&), non-empty growing
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1)}};
      auto res = vec.insert(vec.cbegin() + 1, T{42});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec));
    ...
    { // cudax::async_vector::insert(const_iter, T&&), empty at back
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, T&&), non-empty at back
      Vector vec{env, {T(1), T(42), T(1337), T(0), T(12)}};
  ...
  SECTION("cudax::async_vector::insert(const_iter, count, value)")
  {
    { // cudax::async_vector::insert(const_iter, count, value), no allocation, zero count
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, count, value), with allocation, zero count
      Vector vec{env, 10, (-2)};
      vec.clear();
      auto res = vec.insert(vec.begin(), 0, T{3});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
    ...
    { // cudax::async_vector::insert(const_iter, count, value), empty, sufficient capacity
      Vector vec{env, 10, (-2)};
      vec.clear();
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::insert(const_iter, count, value), non-empty sufficient capacity
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1), T(1337)}};
      vec.resize(5);
    ...
    { // cudax::async_vector::insert(const_iter, count, value), non-empty growing
      Vector vec{env, {T(1), T(1337), T(0), T(12), T(-1)}};
      auto res = vec.insert(vec.cbegin() + 1, 1, T{42});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec));
    ...
    { // cudax::async_vector::insert(const_iter, count, value), non-empty at back
      Vector vec{env, {T(1), T(42), T(1337), T(0), T(12)}};
  ...
  SECTION("cudax::async_vector::insert(const_iter, iter, iter), input iterators")
  {
    const cuda::std::array<T, 6> input{T(1), T(42), T(1337), T(0), T(12), T(-1)};
    using iter = cpp17_input_iterator<const T*>;

    { // cudax::async_vector::insert(iter, iter, iter), no allocation empty rnge
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(iter, iter, iter), from empty no allocation
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(iter, iter, iter), from empty sufficient capacity
      Vector vec{env, 10, T(-2)};
      vec.clear();
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::insert(iter, iter, iter), non-empty sufficient capacity
      Vector vec{env, {T(1), T(-1), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(2);
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::insert(iter, iter, iter), non-empty growing
      Vector vec{env, {T(1), T(-1)}};
  ...
  SECTION("cudax::async_vector::insert(const_iter, iter, iter), forward iterators")
  {
    const cuda::std::array<T, 6> input{T(1), T(42), T(1337), T(0), T(12), T(-1)};
    ...
    { // cudax::async_vector::insert(iter, iter, iter), no allocation
      Vector vec{env};
      auto res = vec.insert(vec.begin(), iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec));
    ...
    { // cudax::async_vector::insert(iter, iter, iter), empty sufficient capacity
      Vector vec{env, 10, T(-2)};
      vec.clear();
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin, iter{input.begin()}, iter{input.end()});
    ...
    { // cudax::async_vector::insert(iter, iter, iter), non-empty sufficient capacity, at end
      Vector vec{env, {T(1), T(-2), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(1);
    ...
    { // cudax::async_vector::insert(iter, iter, iter), non-empty growing, at end
      Vector vec{env, {T(1)}};
  ...
  SECTION("cudax::async_vector::insert(const_iter, initializer_list)")
  {
    { // cudax::async_vector::insert(const_iter, initializer_list), no allocation, empty input
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), with allocation, empty input
      Vector vec{env, {T(0), T(5)}};
      vec.clear();
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), no allocation, non-empty input
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), non-empty sufficient capacity
      Vector vec{env, {T(1), T(-1), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin + 1, {T(42), T(1337), T(0), T(12)});
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), non-empty, growing
      Vector vec{env, {T(1), T(-1)}};
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), non-empty sufficient capacity, at end
      Vector vec{env, {T(1), T(-2), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(1);
      auto old_end = vec.end();
    ...
    { // cudax::async_vector::insert(const_iter, initializer_list), non-empty, growing, at end
      Vector vec{env, {T(1)}};
  ...
  SECTION("cudax::async_vector::insert_range, input range")
  {
    { // cudax::async_vector::insert(const_iter, range), no allocation, empty input
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, range), with allocation, empty input
      Vector vec{env, {T(0), T(5)}};
      vec.clear();
    ...
    { // cudax::async_vector::insert(const_iter, range), no allocation, non-empty input
      Vector vec{env};
    ...
    { // cudax::async_vector::insert(const_iter, range), with allocation, non-empty input, sufficient capacity
      Vector vec{env, {T(1), T(-1), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(2);
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::insert(const_iter, range), with allocation, non-empty input, growing
      Vector vec{env, {T(1), T(-1)}};
    ...
    { // cudax::async_vector::insert(const_iter, range), with allocation, non-empty input, sufficient capacity, at end
      Vector vec{env, {T(1), T(42), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(2);
      auto old_end = vec.end();
    ...
    { // cudax::async_vector::insert(const_iter, range), with allocation, non-empty input, growing, at end
      Vector vec{env, {T(1), T(42)}};
  ...
  {
    { // cudax::async_vector::append_range(range), no allocation, empty input
      Vector vec{env};
    ...
    { // cudax::async_vector::append_range(range), with allocation, empty input
    ...
    { // cudax::async_vector::append_range(range), with allocation, non-empty input, sufficient capacity
      Vector vec{env, {T(1), T(42), T(1337), T(0), T(-2), T(-2), T(-2), T(-2), T(-2)}};
      vec.resize(4);
      auto old_begin = vec.begin();
    ...
    { // cudax::async_vector::append_range(range), with allocation, non-empty input, growing
  ...
  SECTION("cudax::async_vector::insert_range, uncommon range")
  {
    { // cudax::async_vector::insert(const_iter, range), no allocation, empty input
      Vector vec{env};
  ...
  SECTION("cudax::async_vector::insert_range, sized uncommon range")
  {
    { // cudax::async_vector::insert(const_iter, range), no allocation, empty input
      Vector vec{env};
  ...
  SECTION("cudax::async_vector::insert_range, random_access range")
  {
    { // cudax::async_vector::insert(const_iter, range), no allocation, empty input
      Vector vec{env};
...
{ // insert throws std::bad_alloc
  using Vector = cudax::async_vector<int, 2>;

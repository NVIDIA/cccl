...
TEMPLATE_TEST_CASE("cudax::synchronous_buffer comparison",
                   ...
                   (cuda::std::tuple<cuda::mr::host_accessible, cuda::mr::device_accessible>) )
{
  ...
  SECTION("cudax::synchronous_buffer equality")
  {
    { // without allocation
      Buffer buf{resource};
      Buffer other{resource, {T(0), T(1), T(2), T(3), T(4)}};
    ...
    Buffer lhs{resource, {T(1), T(42), T(1337), T(0)}};
    ...
    { // lhs is shorter
      Buffer rhs{resource, {T(1), T(42), T(1337), T(0), T(-1)}};
      CUDAX_CHECK(!(lhs == rhs));
      CUDAX_CHECK(lhs != rhs);
    }

    { // rhs is shorter
      Buffer rhs{resource, {T(1), T(42), T(1337)}};
      CUDAX_CHECK(!(lhs == rhs));
      CUDAX_CHECK(lhs != rhs);
    }

    { // rhs is different
      Buffer rhs{resource, {T(1), T(42), T(1337), T(-1)}};
      CUDAX_CHECK(!(lhs == rhs));
      CUDAX_CHECK(lhs != rhs);
    }
  }

...
constexpr bool equal_range(const Buffer& buf)
{
  ...
  else
  {
    return buf.size() == cuda::std::size(device_data)
        && thrust::equal(thrust::cuda::par, buf.begin(), buf.end(), cuda::get_device_address(device_data[0]));
  }
}
...
constexpr bool equal_size_value(const Buffer& buf, const size_t size, const int value)
{
  ...
  else
  {
    return buf.size() == size
        && thrust::equal(
             thrust::cuda::par, buf.begin(), buf.end(), cuda::std::begin(device_data), equal_to_value{value});
  }
}
...
constexpr bool equal_range(const Range1& range1, const Range2& range2)
{
  ...
  else
  {
    return range1.size() == range2.size()
        && thrust::equal(thrust::cuda::par, range1.begin(), range1.end(), range2.begin());
  }
}
...
struct extract_properties<cuda::std::tuple<Properties...>>
{
  ...
  using matching_buffer       = cudax::synchronous_buffer<int, other_property, Properties...>;
  using matching_resource     = memory_resource_wrapper<other_property, Properties...>;
  using matching_any_resource = cudax::any_resource<other_property, Properties...>;
};

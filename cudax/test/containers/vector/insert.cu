//===----------------------------------------------------------------------===//
//
// Part of CUDA Experimental in CUDA C++ Core Libraries,
// under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
// SPDX-FileCopyrightText: Copyright (c) 2024 NVIDIA CORPORATION & AFFILIATES.
//
//===----------------------------------------------------------------------===//

#include <cuda/memory_resource>
#include <cuda/std/__algorithm_>
#include <cuda/std/array>
#include <cuda/std/cassert>
#include <cuda/std/initializer_list>
#include <cuda/std/tuple>
#include <cuda/std/type_traits>

#include <cuda/experimental/vector>

#include <stdexcept>

#include "types.h"
#include <catch2/catch.hpp>

TEMPLATE_TEST_CASE(
  "cudax::vector insert", "[container][vector]", cuda::std::tuple<>, cuda::std::tuple<cuda::mr::host_accessible>)
{
  using Resource     = typename extract_properties<TestType>::resource;
  using Resource_ref = typename extract_properties<TestType>::resource_ref;
  using Vector       = typename extract_properties<TestType>::vector;
  using T            = typename Vector::value_type;
  using reference    = typename Vector::reference;
  using iterator     = typename extract_properties<TestType>::iterator;

  Resource raw_resource{};
  Resource_ref resource{raw_resource};

  SECTION("cudax::vector::insert(const_iter, const T&)")
  {
    { // cudax::vector::insert(const_iter, const T&), empty without allocation
      Vector vec             = {resource};
      const T to_be_inserted = 3;
      auto res               = vec.insert(vec.begin(), to_be_inserted);
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 1>{T(3)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert(const_iter, const T&), empty with allocation
      Vector vec = {resource, {T(0), T(3), T(5)}};
      vec.clear();
      const T to_be_inserted = 3;
      auto old_begin         = vec.begin();
      auto res               = vec.insert(old_begin, to_be_inserted);
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 1>{T(3)}));
      CHECK(res == old_begin);
    }

    { // cudax::vector::insert(const_iter, const T&), non-empty sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(7)}};
      vec.resize(2);
      const T to_be_inserted = 3;
      auto res               = vec.insert(vec.cbegin() + 1, to_be_inserted);
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(0), T(3), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert(const_iter, const T&), non-empty growing
      Vector vec             = {resource, {T(0), T(5)}};
      const T to_be_inserted = 3;
      auto res               = vec.insert(vec.cbegin() + 1, to_be_inserted);
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(0), T(3), T(5)}));
      CHECK(res == vec.begin() + 1);
    }
  }

  SECTION("cudax::vector::insert(const_iter, T&&)")
  {
    { // cudax::vector::insert(const_iter, T&&), empty without allocation
      Vector vec = {resource};
      auto res   = vec.insert(vec.begin(), T{3});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 1>{T(3)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert(const_iter, T&&), empty with allocation
      Vector vec = {resource, {T(0), T(3), T(5)}};
      vec.clear();
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin, T{3});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 1>{T(3)}));
      CHECK(res == old_begin);
    }

    { // cudax::vector::insert(const_iter, T&&), non-empty sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(7)}};
      vec.resize(2);
      auto res = vec.insert(vec.cbegin() + 1, T{3});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(0), T(3), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert(const_iter, T&&), non-empty growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert(vec.cbegin() + 1, T{3});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(0), T(3), T(5)}));
      CHECK(res == vec.begin() + 1);
    }
  }

  SECTION("cudax::vector::insert(const_iter, iter, iter), input iterators")
  {
    const cuda::std::array<T, 3> input{T(42), T(3), T(1337)};
    using iter = cpp17_input_iterator<const T*>;

    { // cudax::vector::insert(iter, iter, iter), no allocation empty rnge
      Vector vec = {resource};
      auto res   = vec.insert(vec.begin(), iter{input.begin()}, iter{input.begin()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert(iter, iter, iter), no allocation
      Vector vec = {resource};
      auto res   = vec.insert(vec.begin(), iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert(iter, iter, iter), with allocation
      Vector vec = {resource, {T(0), T(42), T(3), T(1337), T(5)}};
      vec.clear();
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == old_begin);
    }

    { // cudax::vector::insert(iter, iter, iter), non-empty sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(0), T(42), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin + 1, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert(iter, iter, iter), non-empty growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert(vec.cbegin() + 1, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::vector::insert(const_iter, iter, iter), forward iterators")
  {
    const cuda::std::array<T, 3> input{T(42), T(3), T(1337)};
    using iter = forward_iterator<const T*>;

    { // cudax::vector::insert(iter, iter, iter), no allocation empty rnge
      Vector vec = {resource};
      auto res   = vec.insert(vec.begin(), iter{input.begin()}, iter{input.begin()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert(iter, iter, iter), no allocation
      Vector vec = {resource};
      auto res   = vec.insert(vec.begin(), iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert(iter, iter, iter), with allocation
      Vector vec = {resource, {T(0), T(42), T(3), T(1337), T(5)}};
      vec.clear();
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == old_begin);
    }

    { // cudax::vector::insert(iter, iter, iter), non-empty sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(0), T(42), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert(old_begin + 1, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert(iter, iter, iter), non-empty growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert(vec.cbegin() + 1, iter{input.begin()}, iter{input.end()});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::vector::insert(const_iter, initializer_list)")
  {
    { // cudax::vector::insert_range(iter, initializer_list), no allocation, empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), cuda::std::initializer_list<T>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert_range(iter, initializer_list), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, cuda::std::initializer_list<T>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(iter, initializer_list), no allocation, non-empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), cuda::std::initializer_list<T>{T(42), T(3), T(1337)});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert_range(iter, initializer_list), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert_range(old_begin + 1, cuda::std::initializer_list<T>{T(42), T(3), T(1337)});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert_range(iter, initializer_list), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, cuda::std::initializer_list<T>{T(42), T(3), T(1337)});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(const_iter, initializer_list)
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.cbegin() + 1, cuda::std::initializer_list<T>{T(42), T(3), T(1337)});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::vector::insert_range, input range")
  {
    { // cudax::vector::insert_range(iter, range), no allocation, empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), input_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, input_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    using Range = input_range<T, 3>;
    { // cudax::vector::insert_range(iter, range), no allocation, non-empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert_range(old_begin + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(const_iter, range)
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.cbegin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::template::append_range, input_range")
  {
    using Range = input_range<T, 3>;
    { // cudax::vector::append_range(range), no allocation, empty input
      Vector vec = {resource};
      vec.append_range(input_range<T, 0>{});
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::append_range(range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      vec.append_range(input_range<T, 0>{});
      auto old_begin = vec.begin();
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(vec.begin() == old_begin);
    }

    { // cudax::vector::append_range(range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(42), T(3), T(1337)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() == old_begin);
    }

    { // cudax::vector::append_range(range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() != old_begin);
    }
  }

  SECTION("cudax::vector::insert_range, uncommon range")
  {
    { // cudax::vector::insert_range(iter, range), no allocation, empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), uncommon_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, uncommon_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    using Range = uncommon_range<T, 3>;
    { // cudax::vector::insert_range(iter, range), no allocation, non-empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert_range(old_begin + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(const_iter, range)
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.cbegin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::template::append_range, uncommon range")
  {
    { // cudax::vector::append_range(range), no allocation, empty input
      Vector vec = {resource};
      vec.append_range(uncommon_range<T, 0>{});
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::append_range(range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      vec.append_range(uncommon_range<T, 0>{});
      auto old_begin = vec.begin();
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(vec.begin() == old_begin);
    }

    using Range = uncommon_range<T, 3>;
    { // cudax::vector::append_range(range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(42), T(3), T(1337)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() == old_begin);
    }

    { // cudax::vector::append_range(range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() != old_begin);
    }
  }

  SECTION("cudax::vector::insert_range, sized uncommon range")
  {
    { // cudax::vector::insert_range(iter, range), no allocation, empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), sized_uncommon_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, sized_uncommon_range<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    using Range = sized_uncommon_range<T, 3>;
    { // cudax::vector::insert_range(iter, range), no allocation, non-empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert_range(old_begin + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(const_iter, range)
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.cbegin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::template::append_range, sized uncommon range")
  {
    { // cudax::vector::append_range(range), no allocation, empty input
      Vector vec = {resource};
      vec.append_range(sized_uncommon_range<T, 0>{});
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::append_range(range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      vec.append_range(sized_uncommon_range<T, 0>{});
      auto old_begin = vec.begin();
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(vec.begin() == old_begin);
    }

    using Range = sized_uncommon_range<T, 3>;
    { // cudax::vector::append_range(range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(42), T(3), T(1337)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() == old_begin);
    }

    { // cudax::vector::append_range(range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() != old_begin);
    }
  }

  SECTION("cudax::vector::insert_range, random access range")
  {
    { // cudax::vector::insert_range(iter, range), no allocation, empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), cuda::std::array<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, cuda::std::array<T, 0>{});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    using Range = cuda::std::array<T, 3>;
    { // cudax::vector::insert_range(iter, range), no allocation, non-empty input
      Vector vec = {resource};
      auto res   = vec.insert_range(vec.begin(), Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 3>{T(42), T(3), T(1337)}));
      CHECK(res == vec.begin());
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(3), T(1337), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      auto res       = vec.insert_range(old_begin + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == old_begin + 1);
    }

    { // cudax::vector::insert_range(iter, range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.begin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.begin() + 1);
    }

    { // cudax::vector::insert_range(const_iter, range)
      Vector vec = {resource, {T(0), T(5)}};
      auto res   = vec.insert_range(vec.cbegin() + 1, Range{{T(42), T(3), T(1337)}});
      static_assert(cuda::std::is_same<decltype(res), iterator>::value, "");
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(42), T(3), T(1337), T(5)}));
      CHECK(res == vec.cbegin() + 1);
    }
  }

  SECTION("cudax::template::append_range, random access range")
  {
    { // cudax::vector::append_range(range), no allocation, empty input
      Vector vec = {resource};
      vec.append_range(cuda::std::array<T, 0>{});
      CHECK(vec.empty());
      CHECK(vec.data() == nullptr);
    }

    { // cudax::vector::append_range(range), with allocation, empty input
      Vector vec = {resource, {T(0), T(5)}};
      vec.append_range(cuda::std::array<T, 0>{});
      auto old_begin = vec.begin();
      CHECK(equal_range(vec, cuda::std::array<T, 2>{T(0), T(5)}));
      CHECK(vec.begin() == old_begin);
    }

    using Range = cuda::std::array<T, 3>;
    { // cudax::vector::append_range(range), with allocation, non-empty input, sufficient capacity
      Vector vec = {resource, {T(0), T(5), T(42), T(3), T(1337)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() == old_begin);
    }

    { // cudax::vector::append_range(range), with allocation, non-empty input, growing
      Vector vec = {resource, {T(0), T(5)}};
      vec.resize(2);
      auto old_begin = vec.begin();
      vec.append_range(Range{{T(42), T(3), T(1337)}});
      CHECK(equal_range(vec, cuda::std::array<T, 5>{T(0), T(5), T(42), T(3), T(1337)}));
      CHECK(vec.begin() != old_begin);
    }
  }

#if 0 // Implement exceptions
#  ifndef TEST_HAS_NO_EXCEPTIONS
{ // insert throws std::bad_alloc
  using Vector = cudax::vector<int, 2>;
  Vector too_small{1, 2};

  try
  {
    const int input = 5;
    too_small.insert(too_small.begin(), input);
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert(too_small.begin(), 1);
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert(too_small.begin(), 5, 42);
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    using iter = cpp17_input_iterator<const int*>;
    cuda::std::array<int, 3> input{42, 3, 1337};
    too_small.insert(too_small.begin(), iter{input.begin()}, iter{input.end()});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    cuda::std::array<int, 3> input{42, 3, 1337};
    too_small.insert(too_small.begin(), input.begin(), input.end());
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert(too_small.begin(), {42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert_range(too_small.begin(), input_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert_range(too_small.begin(), uncommon_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert_range(too_small.begin(), sized_uncommon_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.insert_range(too_small.begin(), cuda::std::array<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.append_range(input_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.append_range(uncommon_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.append_range(sized_uncommon_range<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }

  try
  {
    too_small.append_range(cuda::std::array<int, 3>{42, 3, 1337});
  }
  catch (const std::bad_alloc&)
  {}
  catch (...)
  {
    CHECK(false);
  }
}
#  endif // !TEST_HAS_NO_EXCEPTIONS
#endif // Implement exceptions
}

...
struct no_send_sync
{
  ...
  template <class Tp>
  void recv_sync(group_token_type&, const Tp*, ::cuda::std::size_t, ::cuda::std::int32_t);
};

struct send_sync_returns_int : no_send_sync
{
  template <class Tp>
...
_CCCL_HOST_DEVICE_API constexpr bool test()
{
  ...
  static_assert(!cudax::__has_synchronous_send<no_send_sync>);

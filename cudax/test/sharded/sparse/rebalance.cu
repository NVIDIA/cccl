...
void test_rebalance_moves_toward_equal_time(place_group& group, const host_csr& m)
{
  ...
  const auto times = spmm_shard_times(group, A, d_B, C, n_cols, 1.0, 0.0, /* warmup */ 2, /* iters */ 6);
...
void test_spmv_shard_times_sane(place_group& group, const host_csr& m)
{
  ...
  const auto times = spmv_shard_times(group, A, d_x, y, 1.0, 0.0, /* warmup */ 1, /* iters */ 4);
  EXPECT(times.size() == group.size());
  for (double t : times)
  {
    EXPECT(t > 0.0);

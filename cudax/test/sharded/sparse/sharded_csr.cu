...
// Zero-copy adoption: colinds/values shards alias the caller's device
// arrays; only the rebased offsets are container-owned. The caller's arrays
// are the storage: in-place writes to them ARE writes to the matrix.
void test_adopt(place_group& group)
{
  const auto m = make_mixed_csr(20011, 512, 6);
  ...
  {
    auto A = sharded_csr<double>::adopt(group, m.rows, m.cols, d_off, d_col, d_val);

    // Slice-and-rebase contract holds through the aliased views.
    check_shards_match_reference(m, A);

    // Alias identity: shard views ARE slices of the caller's arrays, and the
    // whole arrays are reachable through the caller's base pointers.
    for (size_t d = 0; d < A.num_shards(); d++)
    {
      const auto& sh = A.shard(d);
      EXPECT(sh.colinds == d_col + sh.nnz_begin);
      EXPECT(sh.values == d_val + sh.nnz_begin);
    }
    EXPECT(A.values_contiguous());
    EXPECT(A.contiguous_values() == d_val);
    EXPECT(A.contiguous_colinds() == d_col);

    // The caller's array is the storage: a write through it is visible in
    // the shard views (same bytes).
    const double sentinel = 12345.5;
    cuda_safe_call(cudaMemcpy(d_val, &sentinel, sizeof(double), cudaMemcpyDefault));
    double read_back = 0;
    cuda_safe_call(cudaMemcpy(&read_back, A.shard(0).values, sizeof(double), cudaMemcpyDefault));
    EXPECT(read_back == sentinel);
  } // container death never touches the caller's arrays...

  // ...which remain fully usable afterwards.
  double still = 0;
  cuda_safe_call(cudaMemcpy(&still, d_val, sizeof(double), cudaMemcpyDefault));
  EXPECT(still == 12345.5);

...
TEST_CASE("copy d2d 2D row-major to column-major", "[copy][d2d][2d][basic]")
{
  ...
  for (int i = 0; i < M; ++i)
  {
    for (int j = 0; j < N; ++j)
    {
      expected[i + static_cast<std::size_t>(j) * M] = static_cast<std::size_t>(i) * N + j;
    }
  }
...
TEST_CASE("copy d2d 2D column-major to row-major", "[copy][d2d][2d][basic]")
{
  ...
  for (int i = 0; i < M; ++i)
  {
    for (int j = 0; j < N; ++j)
    {
      expected[static_cast<std::size_t>(i) * N + j] = i + static_cast<std::size_t>(j) * M;
    }
  }
    ...
    for (int j = 0; j < D1; ++j)
    {
      for (int k = 0; k < D2; ++k)
      {
        expected[i + static_cast<std::size_t>(j) * D0 + static_cast<std::size_t>(k) * D0 * D1] =
          static_cast<std::size_t>(i) * (D1 * D2) + static_cast<std::size_t>(j) * D2 + k;
      }
    }
  }
...
TEST_CASE("copy d2d 2D strided padded row-major", "[copy][d2d][2d][stride][row]")
{
  ...
  for (int i = 0; i < M; ++i)
  {
    for (int j = 0; j < N; ++j)
    {
      data[static_cast<std::size_t>(i) * Ld + j] = static_cast<std::size_t>(i) * N + j;
    }
  }
...
TEST_CASE("copy d2d 2D strided padded column-major", "[copy][d2d][2d][stride][column]")
{
  ...
  for (int i = 0; i < M; ++i)
  {
    for (int j = 0; j < N; ++j)
    {
      data[i + static_cast<std::size_t>(j) * Ld] = static_cast<std::size_t>(i) * N + j;
    }
  }
  test_copy_strided(data, shape, strides);
}

...
struct _CCCL_TYPE_VISIBILITY_DEFAULT stream_scheduler
{
  ...
  private:
    _CCCL_HOST_API void __host_start() noexcept
    {
      ...
      constexpr int __block_threads = cuda::gpu_thread.count(cuda::block, __launch_dims);
      int const __grid_blocks       = cuda::block.count(cuda::grid, __launch_dims);
    ...
    {
      using __launch_dims_t = decltype();
      constexpr int __block_threads =
        cuda::gpu_thread.count(cuda::block, get_launch_config(execution::get_env(__rcvr_)).dims);

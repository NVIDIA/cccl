...
struct __type_try__<false>
{
  ...
  using __call_q _CCCL_NODEBUG = _Fn<_Ts...>;

  template <class _Fn, class... _Ts>
  using __call _CCCL_NODEBUG = typename _Fn::template __call<_Ts...>;
};
...
{
  template <template <class...> class _Fn, class... _Ts>
  using __call_q _CCCL_NODEBUG = __type_find_error<_Ts...>;

  template <class _Fn, class... _Ts>
  using __call _CCCL_NODEBUG = __type_find_error<_Fn, _Ts...>;
};

template <class _Fn, class... _Ts>
using __type_try_call _CCCL_NODEBUG =
  typename __type_try__<__type_contains_error<_Fn, _Ts...>>::template __call<_Fn, _Ts...>;

template <template <class...> class _Fn, class... _Ts>
using __type_try_call_quote _CCCL_NODEBUG =
...
{
  template <class... _Ts>
  using __call _CCCL_NODEBUG = __type_try_call<_Fn, _Ts...>;
};
...
{
  template <class... _Ts>
  using __call _CCCL_NODEBUG = typename __type_try__<__type_contains_error<_Ts...>>::template __call_q<_Fn, _Ts...>;
};
...
struct __type_function
{
  ...
  using __call _CCCL_NODEBUG = _Return(_Args...);
};
...
{
  template <class _Arg>
  using __call _CCCL_NODEBUG = _Return(_Arg);
};
...
{
  template <class... _Ts>
  using __call _CCCL_NODEBUG = _Second<_First<_Ts...>>;
};

struct __type_count
{
  template <class... _Ts>
  using __call _CCCL_NODEBUG = ::cuda::std::integral_constant<size_t, sizeof...(_Ts)>;
};
...
struct __type_self_or
{
  ...
  using __call _CCCL_NODEBUG = _Uy;
};

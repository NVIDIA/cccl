...
class context {
    ...
    class unified_scope {
    public:
        unified_scope(T1 arg) : payload(mv(arg)) {}
        unified_scope(T2 arg) : payload(mv(arg)) {}

        /// Get the string attached to the task for debugging purposes
        const ::std::string& get_symbol() const {
            return ::std::visit([&](auto& self) { return self.get_symbol(); }, payload);
        }

        auto& set_symbol(::std::string s) & {
            ::std::visit([&](auto& self) { self.set_symbol(mv(s)); }, payload);
            return *this;
        }
    ...
    class unified_task {
    public:
        unified_task(stream_task<Deps...> task) : payload(mv(task)) {}
        unified_task(graph_task<Deps...> task) : payload(mv(task)) {}
        ...
        unified_task& add_deps(Args&&... args) {
            ::std::visit([&](auto& self) { self.add_deps(::std::forward<Args>(args)...); }, payload);
            return *this;
        }
        ...
        decltype(auto) get(size_t submitted_index) const {
            return ::std::visit(
                    [&](auto& self) -> decltype(auto) { return self.template get<T>(submitted_index); }, payload);
        }

        template <typename Fun>
        void operator->*(Fun&& f) {
            ::std::visit([&](auto& self) { self->*f; }, payload);
        }
...
public:
     ...
     */
    context(async_resources_handle handle) : payload(stream_ctx(handle)) {
        // The default choice is stream_ctx, otherwise we should assign a graph_ctx with the appropriate parameters
    }
     ...
     */
    context(stream_ctx ctx) : payload(mv(ctx)) {}
     ...
     * @param ctx The context to be assigned.
     */
    context(graph_ctx ctx) : payload(mv(ctx)) {}
    ...
    ::std::string to_string() const {
        ...
        return ::std::visit([&](auto& self) { return self.to_string(); }, payload);
    }
    ...
    auto logical_data(size_t elements, Sizes... othersizes) {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        return ::std::visit(
                [&](auto& self) { return self.template logical_data<T>(elements, othersizes...); }, payload);
    }
    ...
    auto logical_data(P0&& p0, Ps&&... ps) {
        ...
        if constexpr (::std::is_integral_v<T0>) {
            ...
            return ::std::visit(
                    [&](auto& self) { return self.logical_data(::std::forward<P0>(p0), ::std::forward<Ps>(ps)...); },
                    payload);
        }
    }

    template <typename T>
    frozen_logical_data<T> freeze(::cuda::experimental::stf::logical_data<T> d, access_mode m = access_mode::read,
            data_place where = data_place::invalid) {
        return ::std::visit([&](auto& self) { return self.freeze(mv(d), m, mv(where)); }, payload);
    }
    ...
    auto logical_data(T* p, size_t n, data_place dplace = data_place::host) {
        ...
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        return ::std::visit([&](auto& self) { return self.logical_data(make_slice(p, n), mv(dplace)); }, payload);
    }
    ...
    unified_task<Deps...> task(exec_place e_place, task_dep<Deps>... deps) {
        ...
        return ::std::visit(
                [&](auto& self) { return unified_task<Deps...>(self.task(mv(e_place), ::std::move(deps)...)); },
                payload);
    }
    ...
    auto parallel_for(exec_place e_place, S shape, task_dep<Deps>... deps) {
        ...
        return ::std::visit(
                [&](auto& self) { return result_t(self.template parallel_for(mv(e_place), mv(shape), deps...)); },
                payload);
    }
    ...
    auto host_launch(task_dep<Deps>... deps) {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        using result_t = unified_scope<host_launch_scope<stream_ctx, false, Deps...>,
                host_launch_scope<graph_ctx, false, Deps...>>;
        return ::std::visit([&](auto& self) { return result_t(self.host_launch(deps...)); }, payload);
    }
    ...
    auto cuda_kernel(task_dep<Deps>... deps) {
        ...
        using result_t = unified_scope<cuda_kernel_scope<stream_ctx, false, Deps...>,
                cuda_kernel_scope<graph_ctx, false, Deps...>>;
        return ::std::visit([&](auto& self) { return result_t(self.cuda_kernel(deps...)); }, payload);
    }
    ...
    auto cuda_kernel(exec_place e_place, task_dep<Deps>... deps) {
        ...
        using result_t = unified_scope<cuda_kernel_scope<stream_ctx, false, Deps...>,
                cuda_kernel_scope<graph_ctx, false, Deps...>>;
        return ::std::visit([&](auto& self) { return result_t(self.cuda_kernel(e_place, deps...)); }, payload);
    }
    ...
    auto cuda_kernel_chain(task_dep<Deps>... deps) {
        ...
        using result_t = unified_scope<cuda_kernel_scope<stream_ctx, true, Deps...>,
                cuda_kernel_scope<graph_ctx, true, Deps...>>;
        return ::std::visit([&](auto& self) { return result_t(self.cuda_kernel_chain(deps...)); }, payload);
    }
    ...
    auto cuda_kernel_chain(exec_place e_place, task_dep<Deps>... deps) {
        ...
        using result_t = unified_scope<cuda_kernel_scope<stream_ctx, true, Deps...>,
                cuda_kernel_scope<graph_ctx, true, Deps...>>;
        return ::std::visit([&](auto& self) { return result_t(self.cuda_kernel_chain(e_place, deps...)); }, payload);
    }
    ...
    cudaStream_t task_fence() {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        return ::std::visit([&](auto& self) { return self.task_fence(); }, payload);
    }

    void finalize() {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        ::std::visit([](auto& self) { self.finalize(); }, payload);
    }

    void submit() {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        ::std::visit([](auto& self) { self.submit(); }, payload);
    }

    void set_allocator(block_allocator_untyped custom_allocator) {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        ::std::visit([&](auto& self) { self.set_allocator(mv(custom_allocator)); }, payload);
    }

    void attach_allocator(block_allocator_untyped custom_allocator) {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        ::std::visit([&](auto& self) { self.attach_allocator(mv(custom_allocator)); }, payload);
    }

    void update_uncached_allocator(block_allocator_untyped custom) {
        ::std::visit([&](auto& self) { self.update_uncached_allocator(mv(custom)); }, payload);
    }

    void change_epoch() {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        ::std::visit([](auto& self) { self.change_epoch(); }, payload);
    }

    ::std::shared_ptr<per_ctx_dot> get_dot() {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
        return ::std::visit([](auto& self) { return self.get_dot(); }, payload);
    }
    ...
    void set_parent_ctx(parent_ctx_t& parent_ctx) {
        ...
        ::std::visit([&](auto& self) { self.set_parent_ctx(parent_ctx.get_dot()); }, payload);
    }
    ...
    async_resources_handle& async_resources() const {
        ...
        return ::std::visit([&](auto& self) -> async_resources_handle& { return self.async_resources(); }, payload);
    }

    // Shortcuts to manipulate the current affinity stored in the async_resources_handle of the ctx
    void push_affinity(::std::vector<::std::shared_ptr<exec_place>> p) const { async_resources().push_affinity(mv(p)); }
    void push_affinity(::std::shared_ptr<exec_place> p) const { async_resources().push_affinity(mv(p)); }
    void pop_affinity() const { async_resources().pop_affinity(); }
    ...
    bool has_affinity() const { return async_resources().has_affinity(); }
...
private:
    template <typename Fun>
    auto visit(Fun&& fun) -> decltype(::std::visit(
            ::std::forward<Fun>(fun), ::std::declval<::std::variant<stream_ctx, graph_ctx>&>())) {
        assert(payload.index() != ::std::variant_npos && "Context is not initialized");
...
UNITTEST("context to make generic code") {
    auto f = [](context ctx) { ctx.task_fence(); };
...
inline void unit_test_context_pfor() {
    context ctx;
    SCOPE(exit) { ctx.finalize(); };
    auto lA = ctx.logical_data(shape_of<slice<size_t>>(64));
    ctx.parallel_for(lA.shape(), lA.write())->*[] CUDASTF_DEVICE(size_t i, slice<size_t> A) { A(i) = 2 * i; };
...
inline void unit_test_recursive_apply() {
    ...
    ctx.launch(spec3, exec_place::current_device(), lB.write())->*[] CUDASTF_DEVICE(auto th, slice<size_t> B) {
        for (auto i:
                th.apply_partition(shape(B), ::std::tuple<blocked_partition, blocked_partition, cyclic_partition>())) {
            B(i) = 2 * i + 7;
        }
    };
...
UNITTEST("logical data slice const") {
    ...
    ctx.task(lA.read())->*
            [](cudaStream_t, auto A) { static_assert(::std::is_same_v<decltype(A), slice<const double>>); };
    ctx.finalize();
};
...
private:
    ...
    public:
        runner_impl(context_t& _ctx, algorithm& _alg, task_dep<Deps>... _deps)
                : ctx(_ctx), alg(_alg), deps(::std::make_tuple(mv(_deps)...)) {};
...
public:
    algorithm(::std::string _symbol = "algorithm") : symbol(mv(_symbol)) {}
    ...
    void run_in_graph(Fun fun, parent_ctx_t& parent_ctx, cudaGraph_t graph, Args... args) {
        ...
        if (auto search = graph_cache.find(hashValue); search != graph_cache.end()) {
            ...
            auto logicalArgsTuple = ::std::apply(
                    [&](auto&&... args) { return ::std::tuple(logify(::std::forward<decltype(args)>(args))...); },
                    argsTuple);
    ...
    void run_as_task(Fun fun, stream_ctx& ctx, task_dep<Deps>... deps) {
        ctx.task(deps...).set_symbol(symbol)->*
                [this, &fun, &ctx](cudaStream_t stream, Deps... args) { this->run(fun, ctx, stream, args...); };
    }
    ...
    void run_as_task(Fun fun, graph_ctx& ctx, task_dep<Deps>... deps) {
        ctx.task(deps...).set_symbol(symbol)->*
                [this, &fun, &ctx](cudaGraph_t g, Deps... args) { this->run_in_graph(fun, ctx, g, args...); };
    }
    ...
    void run_as_task(Fun fun, context& ctx, task_dep<Deps>... deps) {
        ::std::visit([&](auto& actual_ctx) { this->run_as_task(fun, actual_ctx, deps...); }, ctx.payload);
    }
    ...
    void run_as_task_dynamic(Fun fun, stream_ctx& ctx, const ::std::vector<task_dep_untyped>& deps) {
        ...
        t->*[this, &fun, &ctx, &t](cudaStream_t stream) { this->run_dynamic(fun, ctx, stream, t); };
    }
    ...
    void run_as_task_dynamic(Fun fun, context& ctx, const ::std::vector<task_dep_untyped>& deps) {
        ::std::visit([&](auto& actual_ctx) { this->run_as_task_dynamic(fun, actual_ctx, deps); }, ctx.payload);
    }
    ...
    void run(Fun fun, parent_ctx_t& parent_ctx, cudaStream_t stream, Args... args) {
        ...
        if (!found) {
            auto cudaGraphExecDeleter = [](cudaGraphExec_t* pGraphExec) { cudaGraphExecDestroy(*pGraphExec); };
...
public:
    for_each_batched(
            ...
            : ctx(ctx), cnt(cnt), batch_size(batch_size), df(mv(df)) {}
    ...
    void batched_iterations(Fun&& fun, size_t start, size_t end) {
        ...
        auto fn = [this, start, end, &fun](context gctx, stream_task<> t) {
            ...
            auto logify = [](auto& dest_ctx, auto x) {
                return dest_ctx.logical_data(
                        rw_type_of<decltype(x)>(x), exec_place::current_device().affine_data_place());
            };
            ...
            for (size_t i = start; i < end; i++) {
                ...
                auto logified_instances_tuple = ::std::apply(
                        [&logify, &gctx](auto&&... args) { return ::std::make_tuple(logify(gctx, args)...); },
                        instance_tuple);

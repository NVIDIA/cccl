...
public:
    ...
    parallel_for_scope(context& ctx, exec_place e_place, shape_t shape, task_dep<deps_t>... deps)
            : ctx(ctx), e_place(mv(e_place)), deps { deps... }, shape(mv(shape)) {
     ...
     */
    const task_dep_vector_untyped& get_task_deps() const { return deps; }
...
#    pragma diag_suppress no_device_stack
                auto h = [&](auto... coords) { f(coords..., data...); };
#    pragma diag_default no_device_stack
        ...
        static const auto conf = [] {
            ...
            // optimize occupancy.
            compute_kernel_limits(&reserved::loop<false, false, decltype(explode_deps), ::std::tuple<deps_t...>>,
                    min_grid_size, max_block_size, 0, false, block_size_limit);
        ...
        if constexpr (::std::is_same_v<context, stream_ctx>) {
            if (blocks * block_size == n) {
                if (blocks == max_blocks) {
                    reserved::loop<true, true>
                            <<<blocks, block_size, 0, t.get_stream()>>>(n, mv(explode_deps), deps.instance(t));
                } else {
                    reserved::loop<false, true>
                            <<<blocks, block_size, 0, t.get_stream()>>>(n, mv(explode_deps), deps.instance(t));
                }
            } else {
                if (blocks == max_blocks) {
                    reserved::loop<true, false>
                            <<<blocks, block_size, 0, t.get_stream()>>>(n, mv(explode_deps), deps.instance(t));
                } else {
            ...
            if (blocks * block_size == n) {
                if (blocks == max_blocks) {
                    kernel_params.func =
                            (void*) reserved::loop<true, true, decltype(explode_deps), ::std::tuple<deps_t...>>;
                } else {
                    kernel_params.func =
                            (void*) reserved::loop<false, true, decltype(explode_deps), ::std::tuple<deps_t...>>;
                }
            } else {
                if (blocks == max_blocks) {
                    kernel_params.func =
                            (void*) reserved::loop<true, false, decltype(explode_deps), ::std::tuple<deps_t...>>;
                } else {
                    kernel_params.func =
                            (void*) reserved::loop<false, false, decltype(explode_deps), ::std::tuple<deps_t...>>;
                }
            }

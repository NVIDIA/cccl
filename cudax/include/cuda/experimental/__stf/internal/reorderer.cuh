...
struct reorderer_payload {
    reorderer_payload(::std::string s, int id, ::std::unordered_set<int> succ, ::std::unordered_set<int> pred,
            task_dep_vector_untyped d)
            : symbol(mv(s)), mapping_id(id), successors(mv(succ)), predecessors(mv(pred)), deps(mv(d)) {}
    ...
    size_t num_successors() const { return successors.size(); }
    size_t num_predecessors() const { return predecessors.size(); }
...
private:
    void calculate_upward_ranks(
            const ::std::vector<int>& tasks, ::std::unordered_map<int, reorderer_payload>& task_map) const {
    ...
    void rearrange_tasks(::std::vector<int>& tasks, ::std::unordered_map<int, reorderer_payload>& task_map) const {
        using task_priority = ::std::pair<int,
                double>;  // the double is the upward rank, needed for sorting. Should I just use the reorderer_payload
        auto cmp = [](const task_priority& p1, const task_priority& p2) { return p1.second < p2.second; };
...
public:
    post_mortem_reorderer(const char* order_file) : reorderer() { read_order_file(order_file); }

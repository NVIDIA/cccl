...
public:
   ...
   struct entry {
       entry(executable_graph_cache *cache, ::std::shared_ptr<cudaGraphExec_t> exec_g_, size_t footprint) : cache(cache), exec_g(mv(exec_g_)), footprint(footprint) {
   ...
   void save(size_t nnodes, size_t nedges, ::std::shared_ptr<cudaGraphExec_t> exec_g) {
       ...
       fprintf(stderr, "Caching graph %s (total %s)\n", pretty_print_bytes(footprint).c_str(), pretty_print_bytes(total_cache_footprint).c_str());
   }
   ...
   ::std::pair<bool, ::std::shared_ptr<cudaGraphExec_t>> query(size_t nnodes, size_t nedges, ::std::shared_ptr<cudaGraph_t> g)
   {
      ...
      for (auto it = range.first; it != range.second; ++it)
      {
          ...
          if (reserved::try_updating_executable_graph(*e.exec_g, *g)) {
              ...
              // We have succesfully updated the graph, this is a cache hit

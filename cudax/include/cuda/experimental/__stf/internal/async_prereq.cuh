...
public:
     ...
     */
    bool operator==(const event_impl& e) const { return unique_prereq_id == e.unique_prereq_id; }
     ...
     */
    virtual bool factorize(reserved::event_vector& /*unused*/) { return false; }
...
public:
    ...
    event_list(event e) : payload { mv(e) } {}

    event_list(event_vector& payload_) : payload(mv(payload_)), optimized(false) {}
    ...
    void optimize() {
        ...
        if (!factorized) {
            ...
            ::std::sort(payload.begin(), payload.end(), [](auto& a, auto& b) { return *a < *b; });
            auto new_end = unstable_unique(payload.begin(), payload.end(), [](auto& a, auto& b) { return *a == *b; });
        ...
        each_in_pack(
                [&](auto&& event) {
                    if constexpr (::std::is_lvalue_reference_v<decltype(event)>) {
                        ...
                        payload.insert(payload.end(), ::std::make_move_iterator(event.payload.begin()),
    ...
    auto begin() { return payload.begin(); }
    auto end() { return payload.end(); }
    auto begin() const { return payload.begin(); }
    auto end() const { return payload.end(); }
        ...
        if (!typechecked) {
            ...
            if (!from) {
                fprintf(stderr, "Internal error: cannot dynamically cast event \"%s\" from %s to %.*s.\n",
                        item->get_symbol().c_str(), typeid(*item).name(),
                        static_cast<int>(type_name<some_event>.size()), type_name<some_event>.data());
                abort();
            }
            typechecked = true;
        } else {

...
public:
    post_mortem_scheduler(const char* schedule_file) { read_schedule_file(schedule_file); }
...
public:
    heft_scheduler() : gpu_loads(num_devices, 0.0), msi(num_devices) {
...
private:
    class msi_protocol {
    public:
        msi_protocol(int num_devices) : num_devices(num_devices) {}
        ...
        double when_available(int device_id, const task_dep_untyped& dep) {
            ...
            case msi_state::shared: return device_info.second;
            default: break;
            }
        ...
        void update_msi_for_dep(int device_id, const task_dep_untyped& dep, double task_end) {
            ...
            switch (state) {
            case msi_state::modified: device_info.second = task_end; break;
            ...
            for (int i = 0; i < num_devices; i++) {
                ...
                case msi_state::shared:
                    ...
                    break;
                default: assert(state == msi_state::invalid); break;
                }
            }
        }

    private:
        enum class msi_state : unsigned int { modified = 0, shared = 1, invalid = 2 };

        int num_devices;
        const double bandwidth =
                250 *
                1e5;  // Bytes/ms, Obtained by running cuda-samples/Samples/5_Domain_Specific/p2pBandwidthLatencyTest

...
public:
     ...
     */
    virtual void data_allocate(backend_ctx_untyped& ctx, block_allocator_untyped& custom_allocator,
            const data_place& memory_node, instance_id_t instance_id, ssize_t& s, void** extra_args,
     ...
     * @param prereqs Prerequisite event list, will be updated as a side effect
     */
    virtual void data_deallocate(backend_ctx_untyped& ctx, block_allocator_untyped& custom_allocator,
            const data_place& memory_node, instance_id_t instance_id, void* extra_args, event_list& prereqs) = 0;
     ...
     * @param prereqs Prerequisite event list, will be updated as a side effect
     */
    virtual void data_copy(backend_ctx_untyped& ctx, const data_place& dst_memory_node, instance_id_t dst_instance_id,
            const data_place& src_memory_node, instance_id_t src_instance_id, event_list& prereqs) = 0;
     ...
     */
    virtual bool pin_host_memory(instance_id_t /*instance_id*/) { return false; }

    virtual ::std::optional<cudaMemoryType> get_memory_type(instance_id_t) { return ::std::nullopt; };
...
private:
     ...
     */
    virtual void* get_instance_impl(
            instance_id_t instance_id, const ::std::type_info& asked_ti, ::std::string_view tname) = 0;
     ...
     */
    virtual const void* get_instance_impl(
            instance_id_t instance_id, const ::std::type_info& asked_ti, ::std::string_view tname) const = 0;
};
...
public:
    ...
    explicit data_impl_base(T object) : shape(shape_t(object)), prototype(mv(object)) {}

    explicit data_impl_base(shape_of<T> shape) : shape(mv(shape)), prototype() {}
    ...
    size_t data_footprint() const final { return shape.size(); }
...
private:
    const void* get_common_impl(const ::std::type_info& asked_ti, ::std::string_view tname) const final override {
        if (typeid(shape_t) != asked_ti) {
            fprintf(stderr, "Shape type mismatch.\nAssumed: %.*s\nActual:  %.*s\n",
                    static_cast<int>(type_name<shape_t>.size()), type_name<shape_t>.data(),
                    static_cast<int>(tname.size()), tname.data());
            abort();
        }
        return &shape;
    }
    ...
    void* get_instance_impl(instance_id_t instance_id, const ::std::type_info& ti, ::std::string_view tname) final {
        ...
        if (ti != typeid(rw_type_of<T>) && ti != typeid(void)) {
            fprintf(stderr, "Data interface type mismatch.\nAssumed: %.*s\nActual:  %.*s\n",
                    static_cast<int>(type_name<T>.size()), type_name<T>.data(), static_cast<int>(tname.size()),
                    tname.data());
            abort();
        }
    ...
    const void* get_instance_impl(
            instance_id_t instance_id, const ::std::type_info& ti, ::std::string_view tname) const final {
        // We pass types where we removed const qualifiers in instance()
        if (ti != typeid(rw_type_of<T>) && ti != typeid(void)) {
            fprintf(stderr, "Data interface type mismatch.\nAssumed: %.*s\nActual:  %.*s\n",
                    static_cast<int>(type_name<T>.size()), type_name<T>.data(), static_cast<int>(tname.size()),

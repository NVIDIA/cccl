...
struct decorated_stream {
    decorated_stream(cudaStream_t stream = nullptr, ssize_t id = -1, int dev_id = -1)
            : stream(stream), id(id), dev_id(dev_id) {}
...
struct stream_pool {
    ...
    stream_pool(size_t n, ssize_t dev_id, CUcontext cuctx = nullptr)
            : payload(n, decorated_stream(nullptr, -1, dev_id)), dev_id(dev_id), primary_ctx(cuctx) {}
    ...
    iterator begin() { return iterator(payload.data()); }
    iterator end() { return iterator(payload.data() + payload.size()); }
...
private:
    ...
    class id_pool {
    public:
        ~id_pool() { assert(released.load() == current.load()); }
...
public:
    async_resources_handle() : pimpl(::std::make_shared<impl>()) {}

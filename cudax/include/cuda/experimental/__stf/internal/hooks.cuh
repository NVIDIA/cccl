...
public:
    static int get() { return instance().cnt++; }
...
inline void create_dump_dir(const ::std::string& dump_dir) {
    ...
    if (::std::filesystem::create_directories(dump_dir)) {
        ...
        if (!::std::filesystem::exists(dump_dir)) {
            ::std::cerr << "An error occurred while trying to create the dump_dir \"" << dump_dir << "\"."
                        << ::std::endl;
            abort();
        }
    }
            ...
            auto h = [ctx, ro_dep, dump_dir, hash_only, compare]() {
                ...
                if (compare) {
                    ...
                    ctx->task(exec_place::host, ro_dep).set_symbol("compare " + ::std::to_string(cnt))
                                    ->*[filePath, hash_only](cudaStream_t stream, auto s) {
                                            ...
                                            if (computed_hash != saved_hash) {
                                                ::std::cerr << "Hash mismatch : computed = " << computed_hash
                                                            << ", saved = " << saved_hash << " in " << filePath
                                                            << ::std::endl;
                    ...
                    ctx->host_launch(ro_dep).set_symbol("dump " + ::std::to_string(cnt))
                                    ->*[filePath, hash_only](auto s) {

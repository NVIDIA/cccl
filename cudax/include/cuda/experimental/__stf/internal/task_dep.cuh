...
public:
    ...
    task_dep_untyped(logical_data_untyped& d, access_mode m, data_place dplace,
            ::std::shared_ptr<reduction_operator_base> redux_op = nullptr)
            : data(pack_state(d)), m(m), dplace(mv(dplace)), redux_op(mv(redux_op)) {}

    task_dep_untyped(const logical_data_untyped& d, data_place dplace,
            ::std::shared_ptr<reduction_operator_base> redux_op = nullptr)
    ...
    task_dep_untyped(
            logical_data_untyped& d, access_mode m, ::std::shared_ptr<reduction_operator_base> redux_op = nullptr)
    ...
    instance_id_t get_instance_id() const { return instance_id; }
    ...
    access_mode get_access_mode() const { return m; }
    ...
    bool operator!=(const task_dep_untyped& rhs) const { return !(*this == rhs); }

    void set_symbol(::std::string s) const { symbol = mv(s); }

    const ::std::string& get_symbol() const { return symbol; }

    void set_data_footprint(size_t f) const { data_footprint = f; }

    size_t get_data_footprint() const { return data_footprint; }

    void reset_logical_data() { data = nullptr; }
...
public:
    ...
    task_dep(Pack&&... pack) : task_dep_untyped(::std::forward<Pack>(pack)...) {
    ...
    task_dep<T> as_read_mode() const { return task_dep<T>(as_read_mode_untyped()); }
...
public:
     ...
     */
    template <typename T>
    task_dep_vector(task_dep<T> d) : task_dep_vector_untyped(1, mv(d)) {
     ...
     */
    task_dep_vector(task_dep<Data>... deps) : task_dep_vector_untyped { mv(deps)... } {}
    ...
    ::std::tuple<Data...> instance(task& t) {
        return make_tuple_indexwise<sizeof...(Data)>([&](auto i) { return at<i>().instance(t); });
    }

...
public:
    ///@{ @name Constructors
    KeyValue() = default;
    CUDASTF_HOST_DEVICE KeyValue(uint32_t key, uint32_t value) : key(key), value(value) {}
    ///@}
    uint32_t key;
    uint32_t value;
};
...
public:
    ...
    hashtable(uint32_t capacity = kHashTableCapacity) : capacity(capacity) { init(); }
     ...
     *
     */
    CUDASTF_HOST_DEVICE void insert(uint32_t key, uint32_t value) { insert(KeyValue(key, value)); }
...
public:
     ...
     */
    shape_of(uint32_t capacity = kHashTableCapacity) : capacity(capacity) {}
     ...
     * All `shape_of` specializations must define this constructor.
     */
    shape_of(const hashtable& h) : shape_of(h.get_capacity()) {}
    ...
    CUDASTF_HOST_DEVICE
    size_t size() const { return capacity * sizeof(uint32_t); }

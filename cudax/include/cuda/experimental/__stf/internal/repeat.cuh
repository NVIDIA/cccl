...
public:
    ...
    repeat_scope(context_t& ctx, size_t count) : ctx(ctx), condition(count) {}
    repeat_scope(context_t& ctx, ::std::function<bool()> condition) : ctx(ctx), condition(mv(condition)) {}
...
private:
    bool next() {
        return condition.index() == 0 ? ::std::get<size_t>(condition)-- > 0

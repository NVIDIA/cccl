...
class thread_hierarchy {
    ...
    static constexpr size_t depth = [](auto x, auto y, auto...) {
        ...
        static_assert(
                ::std::is_same_v<decltype(x), bool>, "You must use bool for the odd arguments of thread_hierarchy.");
...
public:
     ...
     */
    thread_hierarchy(int devid, interpreted_execution_policy<spec...>& p) : devid(devid) {
    ...
    CUDASTF_HOST_DEVICE
    const ::std::array<size_t, 3>& get_config() const { return launch_config; }
    ...
    CUDASTF_HOST_DEVICE
    size_t size(int level = int(depth) - 1) const { return size(level, -1); }

    CUDASTF_HOST_DEVICE
    size_t rank(int level = int(depth) - 1) const { return rank(level, -1); }
    ...
    CUDASTF_HOST_DEVICE auto apply_partition(const shape_t& s, const ::std::tuple<P, sub_partitions...>& t) const {
        ...
        auto sans_first =
                make_tuple_indexwise<sizeof...(sub_partitions)>([&](auto index) { return ::std::get<index + 1>(t); });
     ...
     */
    CUDASTF_HOST_DEVICE
    auto inner() const { return typename inner_t<spec...>::type(*this); }
    ...
    void set_device_tmp(void* addr) { device_tmp = addr; }

    void set_system_tmp(void* addr) { system_tmp = addr; }

    void set_devid(int d) { devid = d; }
...
private:
    ...
    CUDASTF_HOST_DEVICE
    bool may_sync(int level) const { return may_sync_impl<0, spec...>(level); }
};
...
UNITTEST("thread hierarchy sync") {
    ...
    cuda_safe_call(cudaLaunchCooperativeKernel(
            (void*) reserved::unit_test_thread_hierarchy_sync<true, size_t(0), true, size_t(1)>, config[1], config[2],
            args, 0, 0));

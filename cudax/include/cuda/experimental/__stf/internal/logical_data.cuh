...
public:
    ...
    bool empty() const { return task_ids.empty(); }

    const event_list& get_done_prereqs() const { return done_prereqs; }

    /* Get a reference to the vector of tasks IDs */
    const auto& get_ids() const { return task_ids; }
...
public:
    logical_data_untyped_impl(
            ...
            : ctx(mv(ctx_)), dinterface(mv(dinterface_)) {
        assert(this->ctx);
    ...
    ~logical_data_untyped_impl() { erase(); }
    ...
    const auto& get_allocator() const { return get_allocator(*this); }
    auto& get_allocator() { return get_allocator(*this); }

    void set_allocator(block_allocator_untyped a) { custom_allocator = mv(a); }
...
// @@@@ TODO @@@@ reimplement !
    ...
    bool has_interface() const { return dinterface != nullptr; }
    ...
    data_instance& get_data_instance(instance_id_t instance_id) { return used_instances[size_t(instance_id)]; }

    size_t get_data_instance_count() const { return used_instances.size(); }

    void allocate(const data_place& memory_node, instance_id_t instance_id, ssize_t& s, void** extra_args,
            event_list& prereqs) {
    ...
    void data_copy(const data_place& dst_node, instance_id_t dst_instance_id, const data_place& src_node,
            instance_id_t src_instance_id, event_list& prereqs) {
    ...
    void reclaim_update_state(const data_place& memory_node, instance_id_t instance_id, event_list& prereqs) {
        ...
        default: assert(!"Corrupt MSIR state value found."); abort();
        }
    }
    ...
    void enforce_msi_protocol(instance_id_t instance_id, access_mode mode, event_list& prereqs) {
        ...
        case access_mode::read: {
            ...
            default: assert(!"Corrupt MSIR state detected."); abort();
            }
            ...
            case msir_state_id::invalid: {
                ...
                if (mode == access_mode::rw) {
                    ...
                    src_avail_prereq.merge(get_data_instance(dst_instance_id).get_read_prereq(),
                            get_data_instance(dst_instance_id).get_write_prereq(), prereqs);
        ...
        case access_mode::redux: current_instance.set_msir(msir_state_id::reduction); break;
        default: assert(!"Corrupt MSIR state detected."); abort();
        }
    }
...
public:
    ...
    logical_data_untyped(::std::shared_ptr<reserved::logical_data_untyped_impl> p) : pimpl(mv(p)) {}
     ...
     * events.
     */
    logical_data_untyped(
            backend_ctx_untyped ctx, ::std::shared_ptr<data_interface> backend, const data_place& memory_node)
            : pimpl(::std::make_shared<reserved::logical_data_untyped_impl>(mv(ctx), mv(backend), memory_node)) {}
    ///@}

    ///@{ @name Symbol getter/setter
    const ::std::string& get_symbol() const { return pimpl->get_symbol(); }
    void set_symbol(::std::string str) { pimpl->symbol = mv(str); }
    ///@}

    ///@{ @name allocator setter
    void set_allocator(block_allocator_untyped a) { pimpl->set_allocator(mv(a)); }
    ///@}
    ...
    void freeze(access_mode freeze_mode, data_place place = data_place::invalid) { pimpl->freeze(freeze_mode, place); }
    ...
    void unfreeze(task& fake_task, event_list prereqs = event_list()) { pimpl->unfreeze(fake_task, mv(prereqs)); }

    void set_automatic_unfreeze(task& fake_task, bool flag) { pimpl->set_automatic_unfreeze(fake_task, flag); }

    /**
     * @brief Indicates whether a logical data is frozen or not, and what is the corresponding mode
     *
     */
    ::std::pair<bool, access_mode> is_frozen() const { return pimpl->is_frozen(); }
     ...
     */
    instance_id_t find_unused_instance_id(const data_place& dplace) { return pimpl->find_unused_instance_id(dplace); }
     ...
     * @return The found instance_id
     */
    instance_id_t find_instance_id(const data_place& dplace) { return pimpl->find_instance_id(dplace); }
     ...
     * @param instance_id
     */
    void release_instance_id(instance_id_t instance_id) { pimpl->release_instance_id(instance_id); }
    ...
    data_instance& get_data_instance(instance_id_t instance_id) { return pimpl->get_data_instance(instance_id); }
    ///@}

    ///@{ @name Reference count query and manipulation
    bool has_ref() const { return pimpl->has_ref(); }
    void add_ref() { pimpl->add_ref(); }
    void remove_ref() { pimpl->remove_ref(); }
    ///@}
    ...
    task_dep_untyped rw(data_place dp = data_place::affine) { return task_dep_untyped(*this, access_mode::rw, mv(dp)); }
    ...
    void apply_redux_op(const data_place& memory_node, const exec_place& e_place, instance_id_t inout_instance_id,
            instance_id_t in_instance_id, event_list& prereqs) {
    ...
    void apply_redux_on_node(const data_place& memory_node, const exec_place& e_place,
            ::std::vector<instance_id_t>& ids, event_list& prereqs) {
        if (ids.size() < 2)
            return;
    ...
    void reconstruct_after_redux(instance_id_t instance_id, const exec_place& e_place, event_list& prereqs) {
        ...
        for (size_t i = 0; i < nnodes; i++) {
            ...
            fprintf(stderr, "make_reduction_plan :: instance %d : ops %p dplace %d\n", i,
                    instance_i.get_redux_op().get(), int(instance_i.get_dplace()));
        }
#endif
...
#ifdef REDUCTION_DEBUG
            fprintf(stderr, "make_reduction_plan :: instance %d status %s\n", i,
                    status_to_string(instance_i.get_msir()).c_str());
#endif
    ...
    size_t get_data_instance_count() const { return pimpl->get_data_instance_count(); }
    ...
    // Enable or disable write-back. Enabling write-back will cause an error if there is no reference data instance.
    void set_write_back(bool flag) { pimpl->set_write_back(flag); }

    // Explicitly enable or disable auto dump for this logical data
    // If true, the logical data will be dumped with CUDASTF_AUTO_DUMP is set,
    // if false, CUDASTF_AUTO_DUMP will be ignored for that logical data.
    void set_auto_dump(bool flag) { pimpl->set_auto_dump(flag); }

    bool get_auto_dump() const { return pimpl->get_auto_dump(); }

    reserved::logical_data_state& get_state() { return pimpl->get_state(); }

    auto& get_ctx() const { return pimpl->ctx; }

    bool is_initialized() const { return pimpl.get() != nullptr; }

    bool operator==(const logical_data_untyped& other) const { return pimpl == other.pimpl; }

    friend inline ::std::shared_ptr<void> pack_state(const logical_data_untyped& d) { return d.pimpl; }
    ...
    auto& get_mutex() { return pimpl->get_mutex(); }

private:
    int get_unique_id() const { return pimpl->get_unique_id(); }
...
inline void reserved::logical_data_untyped_impl::erase() {
    ...
    if (get_unique_id() == -1  // this logical_data_untyped was moved to another logical_data_untyped
            ...
            || was_erased) {   // the logical_data_untyped was already erased
        return;
    }
...
// This implementation is deferred because we need the logical_data_untyped type in it
template <typename T>
::std::pair<T, event_list> reserved::logical_data_untyped_impl::get_frozen(
        task& fake_task, const data_place& dplace, access_mode m) {
    event_list prereqs;
...
public:
    ...
    logical_data(logical_data_untyped&& u) : logical_data_untyped(u) {}
    ...
    const shape_t& shape() const { return logical_data_untyped::common<shape_t>(); }
    ///@}

    ///@{ @name Instance getter for a given instance or the default instance
    decltype(auto) instance(instance_id_t instance_id) { return logical_data_untyped::instance<T>(instance_id); }
    decltype(auto) instance(task& tp) {
    ...
    ///@{ @name Get hash value
    size_t hash() const { return logical_data_untyped::hash(); }
    ///@}
 ...
 * reclaimed.
 */
inline void reclaim_memory(backend_ctx_untyped& ctx, const data_place& place, size_t requested_s, size_t& reclaimed_s,
        event_list& prereqs) {
    ...
    SCOPE(exit) { ctx.get_dot()->set_current_color("white"); };
    ...
    for (int pass = first_pass; (reclaimed_s < requested_s) && (eligible_data_size < requested_s) && (pass <= 2);
            pass++) {
    ...
    if (!str || atoi(str) == 0) {
        ::std::sort(eligible_data.begin(), eligible_data.end(),

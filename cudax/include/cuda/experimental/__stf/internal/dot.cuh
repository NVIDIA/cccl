...
public:
    per_ctx_dot(bool _is_tracing, bool _is_tracing_prereqs)
            : _is_tracing(_is_tracing), _is_tracing_prereqs(_is_tracing_prereqs) {}
    ~per_ctx_dot() = default;

    void finish() { prev_oss.push_back(mv(oss)); }

    const auto& get_streams() const { return prev_oss; }

    void set_ctx_symbol(::std::string s) { ctx_symbol = mv(s); }
    ...
    void add_vertex(task_type t) {
        ...
        // Add an entry in the DOT file
        oss << "\"NODE_" << t.get_unique_id() << "\" [style=\"filled\" fillcolor=\"" << get_current_color()
            << "\" label=\"" << t.get_symbol();
    ...
    const char* get_current_color() { return current_color; }
    ...
    void set_tracing(bool enable) { tracing_enabled = enable; }
    ...
    const ::std::string get_ctx_symbol() const { return ctx_symbol; }
...
public:
    ...
    bool is_tracing() const { return _is_tracing; }
    bool _is_tracing;
    bool is_tracing_prereqs() const { return _is_tracing_prereqs; }
    ...
    // A palette of colors
    const char* const colors[8] = { "#ff5500", "#66ccff", "#9933cc", "#00cc66", "#ffcc00", "#00b3e6", "#cc0066",
        "#009933" };
...
public:
    void print_one_context(
            ::std::ofstream& outFile, size_t& ctx_cnt, bool display_clusters, ::std::shared_ptr<per_ctx_dot> pc) {
    ...
    bool is_tracing() const { return !dot_filename.empty(); }

    bool is_tracing_prereqs() { return tracing_prereqs; }

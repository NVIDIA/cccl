 ...
 *
 */
enum class task_id_t : int { invalid = -1 };
...
public:
    ...
    const auto& get_leaf_tasks() const { return leaf_tasks; }

    auto& get_leaf_tasks() { return leaf_tasks; }
    ...
    bool has_start_events() const { return (start_events.size() > 0); }
    ...
    const event_list& get_start_events() const { return start_events; }

...
public:
    host_launch_scope(Ctx& ctx, task_dep<Deps>... deps) : ctx(ctx), deps(mv(deps)...) {}
...
struct cuda_kernel_desc {
    ...
    cuda_kernel_desc(Fun func, dim3 gridDim_, dim3 blockDim_, size_t sharedMem_, Args... args)
            : func((const void*) func), gridDim(gridDim_), blockDim(blockDim_), sharedMem(sharedMem_) {
...
public:
    cuda_kernel_scope(Ctx& ctx, task_dep<Deps>... deps) : ctx(ctx), deps(mv(deps)...) {}

    // Provide an explicit execution place
    cuda_kernel_scope(Ctx& ctx, exec_place e_place, task_dep<Deps>... deps)
            : ctx(ctx), e_place(mv(e_place)), deps(mv(deps)...) {}
    ...
    void operator->*(Fun&& f) {
        ...
        if constexpr (chained) {
            ...
            if constexpr (::std::is_same_v<Ctx, graph_ctx>) {
                ...
                for (auto& k: res) {
                    cuda_safe_call(cudaLaunchKernel(
                            k.func, k.gridDim, k.blockDim, k.args_ptr.data(), k.sharedMem, t.get_stream()));
                }
            }
            ...
            if constexpr (::std::is_same_v<Ctx, graph_ctx>) {
                ...
                cuda_safe_call(cudaLaunchKernel(
                        res.func, res.gridDim, res.blockDim, res.args_ptr.data(), res.sharedMem, t.get_stream()));
            }
        }
    }
...
protected:
    ...
    public:
        ...
        virtual cudaGraph_t graph() const { return nullptr; }
        ...
        virtual size_t epoch() const { return size_t(-1); }

        auto& get_default_allocator() { return default_allocator; }

        auto& get_uncached_allocator() { return uncached_allocator; }
         ...
         */
        void attach_allocator(block_allocator_untyped a) { attached_allocators.push_back(mv(a)); }
        ...
        ::std::shared_ptr<per_ctx_dot>& get_dot() { return dot; }

        const ::std::shared_ptr<per_ctx_dot>& get_dot() const { return dot; }

        auto get_phase() const { return ctx_phase; }

        void set_phase(backend_ctx_untyped::phase p) { ctx_phase = p; }

        bool has_start_events() const { return stack.has_start_events(); }

        const event_list& get_start_events() const { return stack.get_start_events(); }
...
public:
    ...
    backend_ctx_untyped(::std::shared_ptr<impl> impl) : pimpl(mv(impl)) { assert(pimpl); }

    explicit operator bool() const { return pimpl != nullptr; }

    bool operator==(const backend_ctx_untyped& rhs) const { return pimpl == rhs.pimpl; }

    bool operator!=(const backend_ctx_untyped& rhs) const { return !(*this == rhs); }
    ...
    auto& get_composite_cache() { return pimpl->composite_cache; }
    ...
    void increment_task_count() { ++pimpl->total_task_cnt; }

    size_t task_count() const { return pimpl->total_task_cnt; }

    /* Customize the allocator used by all logical data */
    void set_allocator(block_allocator_untyped custom) { pimpl->custom_allocator = mv(custom); }

    /* Customize the uncached allocator used by other allocators */
    void set_uncached_allocator(block_allocator_untyped custom) { pimpl->uncached_allocator = mv(custom); }

    auto& get_allocator() { return pimpl->custom_allocator; }
    const auto& get_allocator() const { return pimpl->custom_allocator; }

    auto& get_default_allocator() { return pimpl->get_default_allocator(); }

    auto& get_uncached_allocator() { return pimpl->get_uncached_allocator(); }
    ...
    void attach_allocator(block_allocator_untyped a) { pimpl->attach_allocator(mv(a)); }
    ...
    bool is_tracing() const { return pimpl->is_tracing; }

    bool is_tracing_prereqs() const { return pimpl->is_tracing_prereqs; }

    bool generate_event_symbols() const { return pimpl->generate_event_symbols; }
    ...
    size_t epoch() const { return pimpl->epoch(); }
    ...
    auto get_phase() const { return pimpl->get_phase(); }

    void set_phase(backend_ctx_untyped::phase p) { pimpl->set_phase(p); }

    bool has_start_events() const { return pimpl->has_start_events(); }

    const event_list& get_start_events() const { return pimpl->get_start_events(); }
    ...
    // execution place
    auto pick_dstream() { return default_exec_place().get_stream_pool(async_resources(), true).next(); }
...
public:
    backend_ctx(::std::shared_ptr<impl> impl) : backend_ctx_untyped(mv(impl)) {
    ...
    auto repeat(size_t count) { return repeat_scope(self(), count); }

    auto repeat(::std::function<bool()> condition) { return repeat_scope(self(), mv(condition)); }

...
public:
    ...
    class level {
    public:
        level(::std::pair<hw_scope, size_t> desc, size_t local_mem = 0) : local_mem(local_mem) {
            level_desc.push_back(mv(desc));
        }
        level(::std::initializer_list<::std::pair<hw_scope, size_t>> desc, size_t local_mem = 0)
                : level_desc(desc.begin(), desc.end()), local_mem(local_mem) {}
        ...
        void set_mem(size_t size) { local_mem = size; }
        size_t get_mem() const { return local_mem; }

        void set_sync(bool sync) { may_sync = sync; }
        bool get_sync() const { return may_sync; }

        const auto& get_desc() const { return level_desc; }
...
public:
    ...
    void add_level(level l) { levels.push_back(mv(l)); }
    ...
    const auto& get_levels() const { return levels; }
    ...
    ::std::array<size_t, 3> get_config() const {
        ...
        for (auto& l: levels) {
            for (auto& e: l.get_desc()) {
                switch (e.first) {
                case hw_scope::device: config[0] *= e.second; break;

                case hw_scope::block: config[1] *= e.second; break;

                case hw_scope::thread: config[2] *= e.second; break;

                default: assert(!"Corrupt hw_scope value."); abort();
                }
            }
        }
    ...
    hw_scope last_level_scope() const { return (levels[0].get_desc())[0].first; }
    ...
    void set_system_mem(void* addr) { system_mem = addr; }
    void* get_system_mem() const { return system_mem; }

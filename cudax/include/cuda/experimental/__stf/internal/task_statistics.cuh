...
public:
    ...
    bool is_calibrating_to_file() const { return !calibration_file.empty(); }

    /**
     * @brief Enable online calibration
     */
    void enable_calibration() { calibrating = true; }
    ...
    public:
        statistic() = delete;
        explicit statistic(double initial_time) : num_calls(1), mean(initial_time), squares(0.0) {}

        // Constructor when reading from file
        statistic(int num_calls, double mean, double stddev) : num_calls(num_calls), mean(mean), stddev(stddev) {}
        ...
        int get_num_calls() const { return num_calls; }
        double get_mean() const { return mean; }
        double get_variance() const { return num_calls == 1 ? 0 : squares / (num_calls - 1); }
...
private:
    ...
    void write_stats() const {
        ...
        for (const auto& [key, value]: statistics) {
            ...
            file << key.first << "," << key.second << "," << value.get_num_calls() << "," << value.get_mean() << ","
                 << stddev << '\n';
        }

...
public:
    ...
    graph_data_interface(T p) : base(mv(p)) {}

    graph_data_interface(shape_of<T> s) : base(mv(s)) {}

    virtual cudaGraphNode_t graph_data_copy(cudaMemcpyKind kind, instance_id_t src_instance_id,
            instance_id_t dst_instance_id, cudaGraph_t graph, const cudaGraphNode_t* input_nodes, size_t input_cnt) = 0;

    // Returns prereq
    void data_copy(backend_ctx_untyped& ctx, const data_place& dst_memory_node, instance_id_t dst_instance_id,
        ...
        cudaGraphNode_t out =
                graph_data_copy(kind, src_instance_id, dst_instance_id, graph, nodes.data(), nodes.size());

...
public:
    ...
    void* allocate(backend_ctx_untyped& bctx, const data_place& memory_node, ssize_t& s, event_list& prereqs) override {
        ...
        if (memory_node == data_place::host) {
            ...
            SCOPE(fail) { cudaFreeHost(&result); };
    ...
    void deallocate(backend_ctx_untyped& bctx, const data_place& memory_node, event_list& prereqs, void* ptr,
            size_t /*sz*/) override {
        const cudaGraph_t graph = bctx.graph();
        const size_t graph_epoch = bctx.epoch();
        cudaGraphNode_t out = nullptr;
    ...
    ::std::string to_string() const override { return "uncached graph allocator"; }
...
private:
    void* add_mem_alloc_node(const data_place& memory_node, cudaGraphNode_t& out, cudaGraph_t graph, size_t s,
        ...
        static cudaMemAllocNodeParams params = { .poolProps = { .allocType = cudaMemAllocationTypePinned,
...
class graph_ctx : public backend_ctx<graph_ctx> {
    ...
    public:
        impl(async_resources_handle _async_resources = async_resources_handle(nullptr))
                : backend_ctx<graph_ctx>::impl(mv(_async_resources)), _graph(shared_cuda_graph()) {
        ...
        impl(cudaGraph_t g) : _graph(wrap_cuda_graph(g)), explicit_graph(true) {
            reserved::backend_ctx_setup_allocators<impl, uncached_graph_allocator>(*this);
        }
        ...
        size_t epoch() const override { return graph_epoch; }
...
public:
    ...
    graph_ctx(cudaGraph_t g) : backend_ctx<graph_ctx>(::std::make_shared<impl>(g)) {}
    ///@}

    ::std::string to_string() const { return "graph backend context"; }
    ...
    void finalize() {
        ...
        fprintf(stderr, "[STATS CUDA GRAPHS] updated=%lu success=%ld failed=%ld\n",
                "cudaGraphExecUpdate"_counter.load(), "cudaGraphExecUpdate_success"_counter.load(),
    ...
    ::std::shared_ptr<cudaGraph_t> get_shared_graph() const { return state()._graph; }
    ...
    size_t get_graph_epoch() const { return state().graph_epoch; }

    auto get_exec_graph() const { return state().exec_graph; }
...
private:
    ...
    static ::std::shared_ptr<cudaGraph_t> shared_cuda_graph() {
        auto cudaGraphDeleter = [](cudaGraph_t* pGraph) { cudaGraphDestroy(*pGraph); };
...
public:
    ...
    friend inline cudaGraph_t ctx_to_graph(backend_ctx_untyped& ctx) { return ctx.graph(); }

    friend inline size_t ctx_to_graph_epoch(backend_ctx_untyped& ctx) { return ctx.epoch(); }
};

...
public:
  ...
  graph_task& start()
  {
    ...
    if (is_capture_enabled())
    {
      ...
      capture_stream = get_exec_place().getStream(ctx.async_resources().stream_pools(), true).stream;
...
public:
  ...
  void operator->*(Fun&& f)
  {
    ...
    if constexpr (fun_invocable_stream_deps || fun_invocable_stream_non_void_deps)
    {
      ...
      // Get a stream from the pool associated to the execution place
      cudaStream_t capture_stream = get_exec_place().getStream(ctx.async_resources().stream_pools(), true).stream;

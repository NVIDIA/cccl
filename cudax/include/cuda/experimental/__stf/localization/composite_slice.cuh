...
public:
    ...
    // TODO pass mv(place)
    template <typename F>
    localized_array(exec_place_grid grid, get_executor_func_t mapper, F&& delinearize, size_t total_size,
            size_t elemsize, dim4 data_dims)
            : grid(mv(grid))
            , mapper(mv(mapper))
        ...
        EXPECT(supportsVMM == 1,
                "Cannot create a localized_array object on this machine because it does not support VMM.");
        ...
        size_t alloc_granularity_bytes =
                cuda_try<cuMemGetAllocationGranularity>(&prop, CU_MEM_ALLOC_GRANULARITY_MINIMUM);
        ...
        for (size_t i = 0; i < nblocks; i++) {
            owner.push_back(block_to_grid_pos(
                    i * block_size_bytes / elemsize, alloc_granularity_bytes / elemsize, delinearize));
        }
    ...
    void* get_base_ptr() const { return reinterpret_cast<void*>(base_ptr); }
    ...
    void merge(const event_list& source) { prereqs.merge(source); }
...
private:
    int grid_pos_to_dev(pos4 grid_pos) { return device_ordinal(grid.get_place(grid_pos).affine_data_place()); }
...
public:
    ...
    [[nodiscard]] event_list deinit() {
        event_list result;
        cache.each([&](auto& obj) { obj.merge_into(result); });
        return result;
    }
    ...
    template <typename F>
    ::std::unique_ptr<localized_array> get(const data_place& place, get_executor_func_t mapper, F&& delinearize,
            size_t total_size, size_t elem_size, dim4 data_dims) {

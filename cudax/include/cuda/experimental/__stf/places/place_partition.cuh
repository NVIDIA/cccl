...
inline ::std::string place_partition_scope_to_string(place_partition_scope scope) {
    switch (scope) {
    case place_partition_scope::cuda_device: return "cuda_device";
    case place_partition_scope::green_context: return "green_context";
    case place_partition_scope::cuda_stream: return "cuda_stream";
    }
...
public:
    ...
    place_partition(async_resources_handle& handle, ::std::vector<::std::shared_ptr<exec_place>> places,
    ...
    iterator begin() { return iterator(sub_places.data()); }
    iterator end() { return iterator(sub_places.data() + sub_places.size()); }

    /** @brief Number of subplaces in the partition */
    size_t size() const { return sub_places.size(); }

    /** @brief Get the i-th place */
    exec_place& get(size_t i) { return sub_places[i]; }

    /** @brief Get the i-th place (const reference) */
    const exec_place& get(size_t i) const { return sub_places[i]; }
...
// Green contexts are only supported since CUDA 12.4
        ...
        if (place.is_grid() && scope == place_partition_scope::green_context) {
            ...
            for (auto& device_p: place_partition(handle, place, place_partition_scope::cuda_device)) {
                auto device_p_places =
                        place_partition(handle, device_p, place_partition_scope::green_context).sub_places;
                sub_places.insert(sub_places.end(), device_p_places.begin(), device_p_places.end());
            }
            return;
        }

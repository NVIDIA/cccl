...
public:
    ...
    ~cudaCallbackStateCtx() { pthread_key_delete(cudaCallbackStateCtxKeys::instance().cb_key_once); }

    void set_current_cb(cb* cb) { pthread_setspecific(cudaCallbackStateCtxKeys::instance().cb_key, cb); }

    cb* get_current_cb() { return ((cb*) pthread_getspecific(cudaCallbackStateCtxKeys::instance().cb_key)); }
...
cudaError_t cudaStreamAddCallbackWithQueue(cudaStream_t stream, cudaStreamCallback_t callback, void* userData,
        unsigned int flags, class callback_queue* q);
...
public:
    ...
    void push(int* flag_ptr) { stack.push(flag_ptr); }
...
public:
    ...
    void async_destroy(cudaStream_t stream) { cudaStreamAddCallbackWithQueue(stream, NULL, NULL, 0, this); }
    ...
    void wait_worker() { pthread_join(progress_thread, NULL); }
 ...
 * @return cudaError_t indicating if the submission was successful
 */
inline __host__ cudaError_t cudaLaunchHostFuncWithQueue(
        cudaStream_t stream, cudaHostFn_t fn, void* userData, class callback_queue* q) {
    assert(q);
    // We store the arguments in a structure that will be destroyed later on
 ...
 */
inline __host__ cudaError_t cudaGraphAddHostNodeWithQueue(cudaGraphNode_t* node, cudaGraph_t graph,
        cudaGraphNode_t* deps, size_t ndeps, const cudaHostNodeParams* params, class callback_queue* q) {
    assert(q);

...
public:
    ...
    green_context_helper(int sm_count, int devid = cuda_try<cudaGetDevice>()) : devid(devid), numsm(sm_count) {
        assert(devid >= 0);
        ...
        for (int i = 0; i < nbGroups; i++) {
            if (resources[i].type != CU_DEV_RESOURCE_TYPE_INVALID) {
                ...
                pools.push_back(::std::make_shared<stream_pool>(
                        async_resources_handle::pool_size, ssize_t(devid), green_primary));
            }
        }
    }
...
public:
    size_t get_device_id() const { return devid; }
    CUgreenCtx partition(size_t partition = 0) { return ctxs[partition]; }

    green_ctx_view get_view(size_t id) { return green_ctx_view(ctxs[id], pools[id], devid); }

    size_t get_count() const { return ctxs.size(); }
...
public:
    ...
    public:
        ...
        impl(CUcontext saved_context) : driver_context(saved_context) {}
...
#    endif
        ...
        virtual ::std::string to_string() const override {
            return "green ctx ( id=" + ::std::to_string(get_cuda_context_id(g_ctx)) +
                   " dev_id =" + ::std::to_string(devid) + ")";
        }

        virtual stream_pool& get_stream_pool(async_resources_handle&, bool) const { return *pool; }
...
public:
    exec_place_green_ctx(green_ctx_view gc_view) : exec_place(::std::make_shared<impl>(mv(gc_view))) {

...
public:
    ...
    CUDASTF_HOST_DEVICE cyclic_shape(const ::std::array<size_t, dimensions>& begins_,
            const ::std::array<size_t, dimensions>& ends_, const ::std::array<size_t, dimensions>& strides_)
            : begins(begins_), ends(ends_), strides(strides_) {}
    ...
    CUDASTF_HOST_DEVICE size_t get_rank() const { return dimensions; }
    ...
    public:
        CUDASTF_HOST_DEVICE iterator(cyclic_shape& s, size_t idx = 0) : shape(&s), index(idx), current(s.begins) {}
        ...
        CUDASTF_HOST_DEVICE bool operator==(
                const iterator& other) const { /*printf("EQUALITY TEST index %d %d shape equal ? %s\n", index,
        ...
        CUDASTF_HOST_DEVICE bool operator!=(const iterator& other) const { return !(*this == other); }
    };

    // Functions to create the begin and end iterators
    CUDASTF_HOST_DEVICE iterator begin() { return iterator(*this); }

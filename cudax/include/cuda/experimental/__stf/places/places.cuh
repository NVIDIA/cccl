...
class data_place {
    ...
    explicit data_place(int devid) : devid(devid) {}
...
public:
     ...
     */
    static data_place current_device() { return device(cuda_try<cudaGetDevice>()); }
    ...
    bool operator!=(const data_place& rhs) const { return !(*this == rhs); }

    /// checks if this data place is a composite data place
    bool is_composite() const { return (composite_desc != nullptr); }
    ...
    bool is_device() const { return !is_composite() && !is_green_ctx() && (devid >= 0); }
...
public:
    ...
    public:
        ...
        explicit impl(data_place place) : affine(mv(place)) {}
        ...
        virtual const data_place& affine_data_place() const { return affine; }

        virtual ::std::string to_string() const { return "exec(" + affine.to_string() + ")"; }

        virtual bool is_device() const { return affine.is_device(); }

        virtual bool is_grid() const { return false; }

        virtual size_t size() const { return 1; }

        virtual void set_affine_data_place(data_place place) { affine = mv(place); };

        virtual bool operator==(const impl& rhs) const { return affine == rhs.affine; }
    ...
    protected:
        ...
        explicit impl(int devid) : affine(data_place::device(devid)) {}
    ...
    bool operator==(const exec_place& rhs) const { return *pimpl == *rhs.pimpl; }
    bool operator!=(const exec_place& rhs) const { return !(*this == rhs); }
    ...
    class iterator {
    public:
        iterator(::std::shared_ptr<impl> impl, size_t index) : it_impl(mv(impl)), index(index) {}
    ...
    iterator begin() { return iterator(pimpl, 0); }
    iterator end() { return iterator(pimpl, pimpl->size()); }
     ...
     * @return std::string
     */
    ::std::string to_string() const { return pimpl->to_string(); }

    /**
     * @brief Returns the `data_place` naturally associated with this execution place.
     */
    const data_place& affine_data_place() const { return pimpl->affine_data_place(); }

    void set_affine_data_place(data_place place) { pimpl->set_affine_data_place(mv(place)); };
    ...
    // TODO make protected !
    const ::std::shared_ptr<impl>& get_impl() const { return pimpl; }
     ...
     */
    exec_place activate(backend_ctx_untyped& state) const { return pimpl->activate(state); }
     ...
     */
    void deactivate(backend_ctx_untyped& state, const exec_place& p) const { pimpl->deactivate(state, p); }

    bool is_device() const { return pimpl->is_device(); }

    bool is_grid() const { return pimpl->is_grid(); }
...
// Green contexts are only supported since CUDA 12.4
     ...
     * @return exec_place
     */
    static exec_place current_device() { return exec_place::device(cuda_try<cudaGetDevice>()); }
...
public:
    exec_place(::std::shared_ptr<impl> pimpl) : pimpl(mv(pimpl)) {}
...
public:
    ...
    public:
        impl() : exec_place::impl(data_place::host) {}
        exec_place activate(backend_ctx_untyped&) const override { return exec_place(); }                     // no-op
        void deactivate(backend_ctx_untyped&, const exec_place& p) const override { EXPECT(!p.get_impl()); }  // no-op
        virtual const data_place& affine_data_place() const override { return data_place::host; }
        virtual stream_pool& get_stream_pool(
                async_resources_handle& async_resources, bool for_computation) const override {
...
private:
     ...
     * @brief Constructor
     */
    exec_place_host() : exec_place(make()) {
...
public:
    ...
    public:
        ...
        impl(::std::vector<exec_place> _places) : dims(_places.size(), 1, 1, 1), places(mv(_places)) {
            assert(!places.empty());
            assert(dims.x > 0);
        ...
        // With a "dim4 shape"
        impl(::std::vector<exec_place> _places, const dim4& _dims) : dims(_dims), places(mv(_places)) {
            assert(dims.x > 0);
            assert(affine == data_place::invalid);
        }

        // TODO improve with a better description
        ::std::string to_string() const final { return ::std::string("GRID place"); }
        ...
        const ::std::vector<exec_place>& get_places() const { return places; }
        ...
        const exec_place& get_current_place() { return get_places()[current_p_1d]; }
        ...
        ssize_t current_place_id() const { return current_p_1d; }

        dim4 get_dims() const { return dims; }

        int get_dim(int axis_id) const { return dims.get(axis_id); }

        size_t size() const override { return dims.size(); }

        /* Get the place associated to this position in the grid */
        const exec_place& get_place(pos4 p) const { return coords_to_place(p); }

        const exec_place& get_place(size_t p_index) const { return coords_to_place(p_index); }
    ...
    ///@{ @name Constructors
    dim4 get_dims() const { return get_impl()->get_dims(); }

    int get_dim(int axis_id) const { return get_dims().get(axis_id); }

    size_t size() const { return get_dims().size(); }

    explicit operator bool() const { return get_impl() != nullptr; }
     ...
     */
    bool operator==(const exec_place_grid& rhs) const { return *get_impl() == *(rhs.get_impl()); }

    ssize_t current_place_id() const { return get_impl()->current_place_id(); }

    const exec_place& get_place(pos4 p) const { return get_impl()->get_place(p); }

    const ::std::vector<exec_place>& get_places() const { return get_impl()->get_places(); }
    ...
    const exec_place& get_current_place() { return get_impl()->get_current_place(); }

    // Set the current place, given the position in the grid
    void set_current_place(backend_ctx_untyped& ctx, pos4 p) { return get_impl()->set_current_place(ctx, p); }

    void unset_current_place(backend_ctx_untyped& ctx) { return get_impl()->unset_current_place(ctx); }
    ...
    // private:
    exec_place_grid(::std::shared_ptr<impl> p) : exec_place(mv(p)) {}

    exec_place_grid(::std::vector<exec_place> p, const dim4& d) : exec_place(::std::make_shared<impl>(mv(p), d)) {}
};
...
inline exec_place_grid partition_tile(const exec_place_grid& e_place, dim4 tile_sizes, pos4 tile_id) {
    ...
    dim4 size = dim4(end_coords.x - begin_coords.x, end_coords.y - begin_coords.y, end_coords.z - begin_coords.z,
...
public:
    ...
    composite_state(exec_place_grid grid, get_executor_func_t partitioner_func)
            : grid(mv(grid)), partitioner_func(mv(partitioner_func)) {}

    const exec_place_grid& get_grid() const { return grid; }
    const get_executor_func_t& get_partitioner() const { return partitioner_func; }
 ...
 *
 */
enum class instance_id_t : size_t { invalid = static_cast<size_t>(-1) };
...
::std::pair<int /*min_grid_size*/, int /*block_size*/> compute_occupancy(
        Kernel&& f, size_t dynamicSMemSize = 0, int blockSizeLimit = 0) {
 ...
 */
template <typename Fun>
void compute_kernel_limits(const Fun&& f, int& min_grid_size, int& max_block_size, size_t shared_mem_bytes,
        bool cooperative, int& block_size_limit) {
...
interpreted_execution_policy<spec...>::interpreted_execution_policy(
    ...
    if constexpr (pdepth == 1) {
        ...
        this->add_level({ ::std::make_pair(hw_scope::device, ndevs), ::std::make_pair(hw_scope::block, grid_size),
        ...
        if (l1_size == 0) {
            ...
            if (l1_size > block_size_limit) {
                fprintf(stderr, "Unsatisfiable spec: Maximum block size %d threads, requested %ld (level 1)\n",
                        block_size_limit, l1_size);
                abort();
            }
        }
        ...
        this->add_level(
                { ::std::make_pair(hw_scope::device, ndevs), ::std::make_pair(hw_scope::block, l0_size / ndevs) });
        this->set_level_mem(0, size_t(p.get_mem(0)));
        this->set_level_sync(0, l0_sync);
        ...
        if (l2_size == 0) {
            ...
            if (l2_size > block_size_limit) {
                fprintf(stderr, "Unsatisfiable spec: Maximum block size %d threads, requested %ld (level 2)\n",
                        block_size_limit, l2_size);
                abort();
            }
        }

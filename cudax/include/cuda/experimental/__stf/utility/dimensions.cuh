...
public:
    ...
    CUDASTF_HOST_DEVICE constexpr explicit pos4(int x = 0, int y = 0, int z = 0, int t = 0) : x(x), y(y), z(z), t(t) {}
    ...
    CUDASTF_HOST_DEVICE constexpr int get(size_t axis_id) const {
        ...
        case 1: return y;
        case 2: return z;
        default: assert(axis_id == 3); return t;
        }
    }

    /// Get the position along a specific axis
    CUDASTF_HOST_DEVICE constexpr int operator()(int axis_id) const { return get(axis_id); }
...
public:
    ...
    CUDASTF_HOST_DEVICE constexpr explicit dim4(int x, int y = 1, int z = 1, int t = 1) : pos4(x, y, z, t) {}
...
public:
    ...
    CUDASTF_HOST_DEVICE box(const ::std::array<::std::pair<Int1, Int2>, dimensions>& s) : s(s) {}
    ...
    CUDASTF_HOST_DEVICE ssize_t get_extent(size_t dim) const { return s[dim].second - s[dim].first; }

    /// Get the first coordinate (included) in a specific dimension
    CUDASTF_HOST_DEVICE ssize_t get_begin(size_t dim) const { return s[dim].first; }

    /// Get the last coordinate (excluded) in a specific dimension
    CUDASTF_HOST_DEVICE ssize_t get_end(size_t dim) const { return s[dim].second; }
    ...
    CUDASTF_HOST_DEVICE constexpr size_t get_rank() const { return dimensions; }
    ...
    public:
        CUDASTF_HOST_DEVICE iterator(const box& b, bool at_end = false) : iterated(b) {
            if (at_end) {
        ...
        CUDASTF_HOST_DEVICE bool operator==(
                const iterator& rhs) const { /*printf("EQUALITY TEST index %d %d shape equal ? %s\n", index,
    ...
    CUDASTF_HOST_DEVICE iterator end() { return iterator(*this, true); }
    ...
    CUDASTF_HOST_DEVICE bool operator!=(const box& rhs) const { return !(*this == rhs); }

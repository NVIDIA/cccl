...
public:
  ...
  _CCCL_HOST_DEVICE constexpr explicit pos4(int x = 0, int y = 0, int z = 0, int t = 0)
      : x(x)
      , y(y)
      , z(z)
      , t(t)
...
public:
  ...
  _CCCL_HOST_DEVICE ssize_t get_extent(size_t dim) const
  {
    return s[dim].second - s[dim].first;
  }

  /// Get the first coordinate (included) in a specific dimension
  _CCCL_HOST_DEVICE ssize_t get_begin(size_t dim) const
  {
    return s[dim].first;
  }

  /// Get the last coordinate (excluded) in a specific dimension
  _CCCL_HOST_DEVICE ssize_t get_end(size_t dim) const
  {
    return s[dim].second;
  }

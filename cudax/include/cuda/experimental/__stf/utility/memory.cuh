...
enum : size_t { maxPoolEntries = 16 * 1024 };
 ...
 */
inline void deallocateHostMemory(void* p, size_t sz,
 ...
 * @param loc location of the call, defaulted
 */
inline void deallocateManagedMemory(void* p, size_t sz,
        ::std::experimental::source_location loc = ::std::experimental::source_location::current()) {
    ::std::ignore = loc;
    assert([&] {
...
inline cudaGraphNode_t deallocateHostMemory(
    ...
    const cudaHostNodeParams params = { .fn =
                                                [](void* vp) {
                                                    auto args = static_cast<::std::pair<size_t, void*>*>(vp);
                                                    deallocateHostMemory(args->second, args->first);
                                                    delete args;
...
public:
    ...
    // Iterators
    iterator begin() noexcept { return is_small() ? small() : big().data(); }
    const_iterator begin() const noexcept { return is_small() ? small() : big().data(); }

    const_iterator cbegin() const noexcept { return is_small() ? small() : big().data(); }
    ...
    // Capacity
    bool empty() const noexcept { return is_small() ? small_length == 0 : big().empty(); }

    size_t size() const noexcept { return is_small() ? small_length : big().size(); }

    size_t max_size() const noexcept { return big().max_size(); }
    ...
    size_t capacity() const noexcept { return is_small() ? small_cap : big().capacity(); }
    void shrink_to_fit();
        ...
        if (is_small()) {
            if (new_size <= small_length) {
                ...
                if (new_size <= small_cap) {
                    ...
                    copy.reserve(new_size);
                    copy.insert(
                            copy.end(), ::std::move_iterator(small()), ::std::move_iterator(small() + small_length));
    ...
    void swap(small_vector& other) noexcept {
        if (is_small()) {
            if (other.is_small()) {
                ...
                auto b = small(), e = small() + small_length, ob = other.small(),
                     oe = other.small() + other.small_length;
...
private:
    auto small() { return reinterpret_cast<T*>(&small_); }

    auto small() const { return reinterpret_cast<const T*>(&small_); }

    auto& big() { return *reinterpret_cast<::std::vector<T>*>(&big_); }

    auto& big() const { return *reinterpret_cast<const ::std::vector<T>*>(&big_); }

    bool is_small() const { return small_length <= small_cap; }

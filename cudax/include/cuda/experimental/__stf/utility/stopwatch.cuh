...
public:
    ...
    stopwatch(stopwatch&& rhs) noexcept : _start(rhs._start), _stop(rhs._stop), _state(rhs._state) {
    ...
    stopwatch(autostart_t, cudaStream_t stream = nullptr) : stopwatch() { start(stream); }
...
private:
    cudaEvent_t _start;
    cudaEvent_t _stop;
    enum class state { invalid, idle, started, stopped } _state = state::invalid;
};

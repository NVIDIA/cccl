...
public:
    ...
    handle(handle<T1, f1> rhs) : impl(mv(rhs.impl)) {
        if constexpr (f & handle_flags::non_null) {
    ...
    handle(Args&&... args) : impl(make(::std::forward<Args>(args)...)) {
    ...
    template <typename T1, handle_flags f1>
    handle(const handle<T1, f1>& src, decltype(use_static_cast)) : handle(::std::static_pointer_cast<T>(src.impl)) {
        if constexpr (f & handle_flags::non_null) {
            EXPECT(src.impl, "Pointer of static type ", type_name<T1>,
                    " was null upon construction of non-null handle.");
            assert(impl);
        }
    }
    ...
    /// @brief Similar constructor as above but for const handle.
    template <typename T1, handle_flags f1>
    handle(const handle<T1, f1>& src, decltype(use_dynamic_cast)) : handle(::std::dynamic_pointer_cast<T>(src.impl)) {
        if constexpr (f & handle_flags::non_null) {
            EXPECT(src.impl, "Pointer of static type ", type_name<T1>,
                    " was null upon construction of non-null handle.");
    ...
    explicit operator bool() const { return impl.get() != nullptr; }
...
private:
    ...
    struct Derived : public U {
        template <typename... Args>
        Derived(Args&&... args) : U(::std::forward<Args>(args)...) {}
    };
...
UNITTEST("Weak handle") {
    ...
    EXPECT(h->a == 42);
    auto w = h.weak();
    handle<test>::if_valid(w, [](handle<test> x) { x->a++; });
    EXPECT(h->a == 43);
};

...
void __copy_impl(__nvhpc_exec<__blas_exec_space>&& /* __exec */
    ...
    if (__is_use_1d_api) {
        ...
        __bl::__blas_copy_2d<_ElementType_y>::__copy(__op_x, __ex::linalg::__get_mem_row_count(__x),
                __ex::linalg::__get_mem_col_count(__x), __x.data_handle(), __ex::linalg::__get_leading_dim(__x),
                __y.data_handle(), __ex::linalg::__get_leading_dim(__y));

        if (__conj_x) {
            __bl::__blas_conj_2d<_ElementType_y>::__conj(__ex::linalg::__get_mem_row_count(__y),
                    __ex::linalg::__get_mem_col_count(__y), __y.data_handle(), __ex::linalg::__get_leading_dim(__y));
        }

        if constexpr (__is_scaled<__x_t>()) {
            __bl::__blas_scal_2d<_ElementType_y>::__scal(__ex::linalg::__get_mem_row_count(__y),
                    __ex::linalg::__get_mem_col_count(__y), __extract_scaling_factor(__x), __y.data_handle(),

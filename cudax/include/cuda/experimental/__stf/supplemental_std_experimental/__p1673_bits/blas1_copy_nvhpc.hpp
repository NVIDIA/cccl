...
#ifndef INCLUDE_EXPERIMENTAL___P1673_BITS_BLAS1_COPY_NVHPC_HPP_
#define INCLUDE_EXPERIMENTAL___P1673_BITS_BLAS1_COPY_NVHPC_HPP_
...
template <class _exec_space, class _ElementType_x, class _SizeType_x, ::std::size_t... _ext_x, class _Layout_x,
        ...
        class _Accessor_y>
void copy(__nvhpc_exec<_exec_space>&& __exec,
    ...
    constexpr bool __types_supported =
            __data_types_supported(__stdblas_data_type<_ElementType_x>, __stdblas_output_data_type<_ElementType_y>);
    ...
    if constexpr (__types_supported && __x_supported && __y_supported) {
        __copy_impl(std::forward<__nvhpc_exec<_exec_space>>(__exec), __x, __y);
    } else {
#ifdef STDBLAS_VERBOSE
        __STDBLAS_COMPILE_TIME_FALLBACK_MESSAGE(copy);
#endif
        __ex::linalg::copy(std::execution::seq, __x, __y);
    }
}

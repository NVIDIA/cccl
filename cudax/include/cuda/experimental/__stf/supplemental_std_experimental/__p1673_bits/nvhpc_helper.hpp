...
constexpr inline bool __input_supported() {
    return ((__is_supported_layout<_Layout>() || __is_supported_layout_transpose<_Layout>())  // layout supported
            && (__is_directly_scaled<_Accessor>() || __is_nested_scaled<_Accessor>() ||
                       __is_not_scaled<_Accessor>())  // in-place ops
...
constexpr bool __extract_conj() {
    ...
    if constexpr (!__la::is_complex<__val_t>::value || __is_not_conjugated<__acc_t>()) {
        ...
        static_assert(
                sizeof(__acc_t) == 0, "Internal error, nested in-place transformations not supported for acceleration");
...
template <class _ValueType>
void __conjugate(
        __nvhpc_exec<__blas_exec_space> /* __exec */, int __length, _ValueType const* __in, _ValueType* __out) {
...
auto __create_conjugate(
        _Exec&& __exec, __ex::mdspan<_ElementType, __ex::extents<_SizeType, _ext...>, _Layout, _Accessor> __A) {
    ...
    std::unique_ptr<__val_t, std::function<void(__val_t*)>> __ptr(
            __allocate<__val_t>(__base_exec {}, __length), [=](__val_t* p) { __deallocate(__exec, p); });

    __conjugate(__exec, __length, __A.data_handle(), __ptr.get());

    __ex::mdspan<__val_t, __ex::extents<_SizeType, _ext...>, _Layout, _Accessor> __out { __ptr.get(), __A.mapping(),
        __A.accessor() };

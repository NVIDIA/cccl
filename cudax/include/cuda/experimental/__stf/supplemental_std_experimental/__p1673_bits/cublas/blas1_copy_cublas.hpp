...
void __copy_impl(__nvhpc_exec<__cublas_exec_space<_SyncType>>&& __exec,
    ...
    if constexpr (__x.rank() == 1 && !__is_scaled<__x_t>()) {
        __cb::__check_cublas_status(
                __cb::__cublas_copy(__handle, __x.extent(0),
                        (__conj_x ? std::get<1>(__work_x).data_handle() : __x.data_handle()), __y.data_handle()),
                "copy", "cublas_copy");
    } else {
        ...
        __cb::__check_cublas_status(
                __cb::__cublas_geam(__handle, __cb::__op_to_cublas_op(__op_x), CUBLAS_OP_N,
                        __ex::linalg::__get_row_count(__x, __is_transposed),
                        __ex::linalg::__get_col_count(__x, __is_transposed), &__alpha,
                        (__conj_x ? std::get<1>(__work_x).data_handle() : __x.data_handle()),
                        __ex::linalg::__get_leading_dim(__x), &__beta, __y.data_handle(),
                        __ex::linalg::__get_leading_dim(__y), __y.data_handle(), __ex::linalg::__get_leading_dim(__y)),
                "copy", "cublas_geam");
    }

    if (__conj_y) {
        __cb::__check_cublas_status(
                __cb::__cublas_conj(__handle, __lengthConj, __y.data_handle()), "copy", "cublas_conj");
    }

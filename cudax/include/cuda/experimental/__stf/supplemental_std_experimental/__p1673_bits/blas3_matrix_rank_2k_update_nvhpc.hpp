...
template <class _exec_space, class _ElementType_A, class _SizeType_A, ::std::size_t _numRows_A,
        ::std::size_t _numCols_A, class _Layout_A, class _Accessor_A, class _ElementType_B, class _SizeType_B,
        ::std::size_t _numRows_B, ::std::size_t _numCols_B, class _Layout_B, class _Accessor_B, class _ElementType_C,
        class _SizeType_C, ::std::size_t _numRows_C, ::std::size_t _numCols_C, class _Layout_C, class _Accessor_C,
        class _Triangle>
void symmetric_matrix_rank_2k_update(__nvhpc_exec<_exec_space>&& __exec,
    ...
    constexpr bool __types_supported = __data_types_supported(__stdblas_data_type<_ElementType_A>,
            __stdblas_data_type<_ElementType_B>, __stdblas_output_data_type<_ElementType_C>);
    ...
    if constexpr (__types_supported && __A_supported && __B_supported && __C_supported) {
        ...
        using __B_t = typename __ex::mdspan<_ElementType_B, __ex::extents<_SizeType_B, _numRows_B, _numCols_B>,
                _Layout_B, _Accessor_B>;
        ...
        if (__op_A == __op_B) {
            try {
                ...
                __symmetric_matrix_rank_2k_update_impl(std::forward<__nvhpc_exec<_exec_space>>(__exec),
                        _ElementType_C { __extract_scaling_factor(__A) * __extract_scaling_factor(__B) },
                        _ElementType_C { __extract_scaling_factor(__C) }, (__conj_A ? std::get<1>(__work_A) : __A),
                        (__conj_B ? std::get<1>(__work_B) : __B), __C, __op_A, __ex::linalg::__get_fill_mode(__C, __t));

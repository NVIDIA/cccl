...
public:
    ...
    virtual void deallocate(
            backend_ctx_untyped&, const data_place& memory_node, event_list& prereqs, void* ptr, size_t sz) = 0;
...
public:
    ...
    block_allocator_untyped(::std::shared_ptr<block_allocator_interface> ptr) : pimpl(mv(ptr)) {}

    template <typename ctx_t>
    block_allocator_untyped(ctx_t& ctx, ::std::shared_ptr<block_allocator_interface> ptr) : pimpl(mv(ptr)) {
    ...
    void deallocate(
            backend_ctx_untyped& bctx, const data_place& memory_node, event_list& prereqs, void* ptr, size_t sz) {
        return pimpl->deallocate(bctx, memory_node, prereqs, ptr, sz);
    }

    event_list deinit(backend_ctx_untyped& bctx) { return pimpl->deinit(bctx); }
...
public:
    ...
    block_allocator(::std::shared_ptr<block_allocator_interface> ptr) : block_allocator_untyped(mv(ptr)) {}
};

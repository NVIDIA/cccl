...
public:
    block_data_pool(backend_ctx_untyped& ctx, block_allocator_untyped root_allocator_, event_list& prereqs,
            data_place place, const size_t block_size, size_t nentries, double max_ratio)
            : place(mv(place)), block_size(block_size), root_allocator(mv(root_allocator_)) {
        if (this->place == data_place::host || this->place == data_place::managed) {
            ...
            if (nentries * block_size > (size_t) (max_ratio * max_mem)) {
                ...
                fprintf(stderr, "Capping pool size at %f %%: nentries = %zu of %s...\n", max_ratio * 100.0, nentries,
...
struct pooled_allocator_config {
    ...
    double get_max_ratio() const { return max_ratio.has_value() ? max_ratio.value() : 0.9; };
};
...
public:
    ...
    void* get_pool_entry(backend_ctx_untyped& ctx, block_allocator_untyped& root_allocator,
            const data_place& memory_node, size_t block_size, event_list& prereqs) {
    ...
    void set_config(pooled_allocator_config _config) { config = mv(_config); }
...
private:
    ...
    block_data_pool* get_pool(backend_ctx_untyped& ctx, block_allocator_untyped& root_allocator, event_list& prereqs,
            const data_place& memory_node, size_t sz) {
        ...
        return &map.emplace(key, block_data_pool(ctx, root_allocator, prereqs, memory_node, padded_sz,
                                         config.get_max_entries_per_place(), config.get_max_ratio()))
                        ->second;
    }
...
public:
    ...
    pooled_allocator(pooled_allocator_config config) { pool_set.set_config(mv(config)); };
    ...
    event_list deinit(backend_ctx_untyped& ctx) override { return pool_set.deinit_pools(ctx); }

    ::std::string to_string() const override { return "pooled"; }
...
public:
    fixed_size_allocator(size_t block_size) : block_size(block_size) {}

    fixed_size_allocator(size_t block_size, pooled_allocator_config config) : block_size(block_size) {
    ...
    ::std::string to_string() const override { return "fixed size "; }

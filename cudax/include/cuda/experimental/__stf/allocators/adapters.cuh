...
class stream_adapter {
     ...
     */
    struct raw_buffer {
        raw_buffer(void* ptr_, size_t sz_, data_place memory_node_) : ptr(ptr_), sz(sz_), memory_node(memory_node_) {}
    ...
    public:
        adapter_allocator(cudaStream_t stream_, stream_adapter* sa_) : stream(stream_), sa(sa_) {}

        // these are events from a graph_ctx
        void* allocate(
                backend_ctx_untyped&, const data_place& memory_node, ssize_t& s, event_list& /* prereqs */) override {
        ...
        void deallocate(backend_ctx_untyped&, const data_place& memory_node, event_list& /* prereqs */, void* ptr,
                size_t sz) override {
        ...
        ::std::string to_string() const override { return "stream_adapter"; }

...
public:
    ...
    void* allocate(backend_ctx_untyped& ctx, const data_place& memory_node, ssize_t& s, event_list& prereqs) override {
        ...
        if (memory_node == data_place::host) {
            ...
            if (memory_node.is_green_ctx()) {
                fprintf(stderr, "Pretend we use cudaMallocAsync on green context (using device %d in reality)\n",
    ...
    event_list deinit(backend_ctx_untyped& /* ctx */) override { return event_list(); }

    ::std::string to_string() const override { return "uncached stream allocator"; }
};
...
public:
    ...
    ::std::string to_string() const { return "stream backend context"; }
    ...
    class deferred_parallel_for_scope : public deferred_stream_task<> {
        struct payload_t : public deferred_stream_task<>::payload_t {
            ...
            void set_symbol(::std::string s) override { task.set_symbol(mv(s)); }

            const ::std::string& get_symbol() const override { return task.get_symbol(); }

            int get_mapping_id() const override { return task.get_mapping_id(); }

            void run() override { todo(task); }
            ...
            const task_dep_vector_untyped& get_task_deps() const override { return task.get_task_deps(); }

            void set_exec_place(exec_place e_place) override { task.set_exec_place(mv(e_place)); }
        };
    ...
    public:
        ...
        void populate_deps_scheduling_info() { payload->populate_deps_scheduling_info(); }

        template <typename Fun>
        void operator->*(Fun fun) {
            my_payload().todo = [f = mv(fun)](parallel_for_scope<stream_ctx, shape_t, P, Data...>& task) { task->*f; };
        }
    };
    ...
    class deferred_host_launch_scope : public deferred_stream_task<> {
        struct payload_t : public deferred_stream_task<>::payload_t {
            payload_t(stream_ctx& ctx, task_dep<Data>... deps) : task(ctx, mv(deps)...) {}

            host_launch_scope<stream_ctx, false, Data...> task;
            ::std::function<void(host_launch_scope<stream_ctx, false, Data...>&)> todo;

            void set_symbol(::std::string s) override { task.set_symbol(s); }
    ...
    public:
        ...
        template <typename Fun>
        void operator->*(Fun fun) {
            my_payload().todo = [f = mv(fun)](host_launch_scope<stream_ctx, false, Data...>& task) { task->*f; };
        }
    };
    ...
    void finalize() {
        ...
        const auto alive = "cuda_events alive"_counter.load();
        if (alive != 0) {
            fprintf(stderr, "WARNING!!! %lu CUDA events leaked (approx %lu created vs. %lu destroyed).\n", alive,
                    "cuda_events created"_counter.load(), "cuda_events destroyed"_counter.load());
        }
        ...
        fprintf(stderr, "[STATS CUDA EVENTS] created=%lu destroyed=%lu alive=%lu high_water_mark=%lu\n",
                "cuda_events created"_counter.load(), "cuda_events destroyed"_counter.load(), alive,
                "cuda_events"_high_water_mark.load());
        fprintf(stderr, "[STATS CUDA EVENTS] cuda_stream_wait_event=%lu\n", "cuda_stream_wait_event"_counter.load());
#endif
    }
    ...
    auto deferred_parallel_for(exec_place e_place, S shape, task_dep<Deps>... deps) {
        auto result =
                deferred_parallel_for_scope<S, null_partition, Deps...>(*this, mv(e_place), mv(shape), mv(deps)...);
        int id = result.get_mapping_id();
        state().deferred_tasks.push_back(id);
        state().task_map.emplace(id, result);
...
private:
    ...
    impl& state() { return dynamic_cast<impl&>(get_state()); }
    const impl& state() const { return dynamic_cast<const impl&>(get_state()); }
...
UNITTEST("stream context basics") {
    ...
    for (int k = 0; k < 10; k++) {
        ctx.task(handle_X.rw())->*[&](cudaStream_t s, auto /*unuased*/) { reserved::dummy<<<1, 1, 0, s>>>(); };
    }
...
UNITTEST("stream context after logical data") {
    ...
    for (int k = 0; k < 10; k++) {
        ctx.task(handle_X.rw())->*[&](cudaStream_t s, auto /*unused*/) { reserved::dummy<<<1, 1, 0, s>>>(); };
    }
...
UNITTEST("const class containing a stream_ctx") {
    struct foo {
        foo() = default;
        stream_ctx& get_ctx() const { return ctx; }
...
inline void unit_test_host_pfor() {
    stream_ctx ctx;
    auto lA = ctx.logical_data(shape_of<slice<size_t>>(64));
    ctx.parallel_for(exec_place::host, lA.shape(), lA.write())->*[](size_t i, slice<size_t> A) { A(i) = 2 * i; };
    ctx.host_launch(lA.read())->*[](auto A) {
        for (size_t i = 0; i < 64; i++) {
            EXPECT(A(i) == 2 * i);
        }
...
inline void unit_test_pfor_mix_host_dev() {
    ...
    ctx.parallel_for(lx.shape(), lx.write())->*[=] CUDASTF_DEVICE(size_t pos, auto sx) { sx(pos) = 17 * pos + 4; };

    ctx.parallel_for(exec_place::host, lx.shape(), lx.rw())->*[=](size_t pos, auto sx) { sx(pos) = sx(pos) * sx(pos); };

    ctx.parallel_for(lx.shape(), lx.rw())->*[=] CUDASTF_DEVICE(size_t pos, auto sx) { sx(pos) = sx(pos) - 7; };
...
inline void unit_test_pfor_grid() {
    ...
    ctx.parallel_for(blocked_partition(), where, lA.shape(), lA.write())
                    ->*[] CUDASTF_DEVICE(size_t i, slice<size_t> A) { A(i) = 2 * i; };
    ctx.host_launch(lA.read())->*[](auto A) {
        for (size_t i = 0; i < 64; i++) {
            EXPECT(A(i) == 2 * i);
        }

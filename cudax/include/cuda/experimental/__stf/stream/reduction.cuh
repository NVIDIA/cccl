...
public:
  ...
  {
    auto dstream  = inout_memory_node.getDataStream(d.get_ctx().async_resources().stream_pools());
...
#  endif
  ...
  void init_op_untyped(logical_data_untyped& d,
                       ...
                       event_list& prereqs) override
  {
    auto dstream  = out_memory_node.getDataStream(d.get_ctx().async_resources().stream_pools());
    auto async_op = stream_async_op(d.get_ctx(), dstream, prereqs);
    if (d.get_ctx().generate_event_symbols())
    {

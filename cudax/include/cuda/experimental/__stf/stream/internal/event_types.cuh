...
inline event join_with_stream(backend_ctx_untyped& bctx, decorated_stream dstream, event_list& prereq_in,
        ::std::string string, bool record_event);
...
protected:
    ...
    stream_and_event(const decorated_stream& dstream, bool do_insert_event) : dstream(dstream) {
...
public:
    ...
    bool factorize(reserved::event_vector& events) override {
        ...
        ::std::sort(proxy.begin(), proxy.end(), [](const auto& a, const auto& b) {
            return a->dstream.stream != b->dstream.stream ? a->dstream.stream < b->dstream.stream
        ...
        proxy.erase(unstable_unique(proxy.begin(), proxy.end(),
                            [](const auto& a, const auto& b) { return a->dstream.stream == b->dstream.stream; }),
                proxy.end());
    ...
    cudaStream_t get_stream() const { return dstream.stream; }

    decorated_stream get_decorated_stream() const { return dstream; }

    ssize_t get_stream_id() const { return dstream.id; }

    cudaEvent_t get_cuda_event() const { return cudaEvent; }
...
public:
    ...
    stream_async_op(backend_ctx_untyped& bctx, decorated_stream dstream, event_list& prereq_in) : dstream(mv(dstream)) {
        setup(bctx, prereq_in);
    }

    // Async operation associated to a data place, we automatically get a stream attached to that place
    stream_async_op(
            backend_ctx_untyped& bctx, const data_place& place, cudaStream_t* target_stream, event_list& prereq_in) {
    ...
    void set_symbol(::std::string s) { symbol = mv(s); }
    ...
    event_list end(backend_ctx_untyped& bctx) { return event_list(end_as_event(bctx)); }
...
private:
    ...
    void setup(backend_ctx_untyped& bctx, event_list& prereq_in) {
        ...
        for (const auto& e: prereq_in) {
            ...
            if (dstream.stream != se->get_stream()) {
                bool skip = bctx.async_resources().validate_sync_and_update(
                        dstream.id, se->get_stream_id(), se->unique_prereq_id);
                if (!skip) {
     ...
     * prereq list */
    static decorated_stream device_lookup_in_event_list(
            backend_ctx_untyped& /* bctx */, event_list& prereq_in, int devid) {

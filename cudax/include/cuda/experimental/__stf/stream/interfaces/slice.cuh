...
public:
    ...
    slice_stream_interface(T* p) : base(slice<T, dimensions>(p)) {
        static_assert(dimensions == 0, "This constructor is reserved for 0-dimensional data.");
    }

    slice_stream_interface(slice<T, dimensions> s) : base(mv(s)) {}

    slice_stream_interface(shape_t s) : base(mv(s)) {}
    ...
    void data_deallocate(backend_ctx_untyped& bctx, block_allocator_untyped& custom_allocator,
            const data_place& memory_node, instance_id_t instance_id, void* extra_args, event_list& prereqs) override {
    ...
    void data_copy(backend_ctx_untyped& bctx, const data_place& dst_memory_node, instance_id_t dst_instance_id,
        ...
        if constexpr (dimensions == 0) {
            ...
            cuda_safe_call(cudaMemcpy2DAsync(dst_ptr, dst_instance.stride(1) * sizeof(T), src_ptr,
                    src_instance.stride(1) * sizeof(T), b.extent(0) * sizeof(T), b.extent(1), kind, s));
        } else {
    ...
    void unpin_host_memory(instance_id_t instance_id) override { unpin(this->instance(instance_id)); }

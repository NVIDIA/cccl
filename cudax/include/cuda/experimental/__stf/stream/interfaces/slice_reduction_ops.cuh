...
__global__ void slice_reduction_op_kernel(
        const slice<element_type, dimensions> in, const slice<element_type, dimensions> inout) {
...
public:
    ...
    void op(const instance_t& in, instance_t& inout, const exec_place& e, cudaStream_t s) override {
        if (e.affine_data_place() == data_place::host) {
            ...
            auto [gridsize, threadblocksize] =
                    compute_occupancy(slice_reduction_op_kernel<element_type, dimensions, ReduxOp>);
            slice_reduction_op_kernel<element_type, dimensions, ReduxOp>
                    <<<gridsize, threadblocksize, 0, s>>>(in, inout);
        }
    }
    ...
    void init_op(instance_t& out, const exec_place& e, cudaStream_t s) override {
        if (e.affine_data_place() == data_place::host) {
            ...
            // this is not the host, so this has to be a device ... (XXX)
            auto [gridsize, threadblocksize] =
                    compute_occupancy(slice_reduction_op_init_kernel<element_type, dimensions, ReduxOp>);
...
public:
    static void init_host(element_type& out) { out = element_type(0); };
    static __device__ void init_gpu(element_type& out) { out = element_type(0); };

    static void op_host(const element_type& in, element_type& inout) { inout += in; };
    static __device__ void op_gpu(const element_type& in, element_type& inout) { inout += in; };
};

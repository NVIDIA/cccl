...
public:
    ...
    void set_current_place(pos4 p) { get_exec_place().as_grid().set_current_place(ctx, p); }

    void unset_current_place() { return get_exec_place().as_grid().unset_current_place(ctx); }

    const exec_place& get_current_place() { return get_exec_place().as_grid().get_current_place(); }
...
public:
     ...
     * @param deps A list of `task_dep` objects that this task depends on
     */
    stream_task(stream_ctx* ctx, task_dep<Data>... deps) : stream_task(exec_place::host, ctx, mv(deps)...) {}
...
private:
    auto typed_deps() {
        return make_tuple_indexwise<sizeof...(Data)>(
                [&](auto i) { return this->get<::std::tuple_element_t<i, ::std::tuple<Data...>>>(i); });
    }
};
...
protected:
    ...
    struct payload_t {
        ...
        void add_successor(int succ) { successors.insert(succ); }
        void add_predecessor(int pred) { predecessors.insert(pred); }

        const ::std::unordered_set<int>& get_successors() const { return successors; }
        const ::std::unordered_set<int>& get_predecessors() const { return predecessors; }
    ...
    deferred_stream_task(::std::shared_ptr<payload_t> payload) : payload(mv(payload)) {}
...
public:
    ...
    void set_symbol(::std::string s) const { return payload->set_symbol(mv(s)); }
    ...
    reorderer_payload get_reorderer_payload() const {
        ...
        return reorderer_payload(payload->get_symbol(), payload->get_mapping_id(), payload->get_successors(),
                payload->get_predecessors(), payload->get_task_deps());
    }
...
class deferred_stream_task : public deferred_stream_task<> {
    struct payload_t : public deferred_stream_task<>::payload_t {
        ...
        payload_t(backend_ctx_untyped ctx, exec_place e_place, task_dep<Deps>... deps) : task(mv(ctx), mv(e_place)) {
        ...
        void populate_deps_scheduling_info() override { task.populate_deps_scheduling_info(); }

        const task_dep_vector_untyped& get_task_deps() const override { return task.get_task_deps(); }

        void set_exec_place(exec_place e_place) override { task.set_exec_place(e_place); }
    };

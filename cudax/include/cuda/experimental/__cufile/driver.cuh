...
public:
  ...
  template <class _Attr>
  [[nodiscard]] typename _Attr::type attribute(const _Attr& __attr) const
  {
  ...
  //! @throws cuda_error if a CUDA driver error occurs.
  template <class _Attr>
  [[nodiscard]] cufile_driver_attribute_range<_Attr> attribute_range(const _Attr& __attr) const
  {
    static_assert(_Attr::__has_queryable_range, "Attribute does not have a queryable range");
  ...
  template <class _Attr>
  void set_attribute(const _Attr& __attr, typename _Attr::type __value)
  {

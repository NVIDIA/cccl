 ...
 *
 * 2. The adapter — `make_communicators`, `mgmn_envs`, and the `reserved`
...
//! @brief The MGMN environment of a non-allocating sharded environment: its
//! stream only. The rank-local MGMN algorithms (`transform`) allocate
//! nothing, so the map family accepts any `sharded_env_range`, as before.
...
{
  ::std::vector<decltype(__make(::std::size_t{0}))> __result;
...
__mgmn_map(const _S& __data, const _Envs& __envs, const _CallEnv& __call_env, const char* __what, _Body __body)
{
  ...
  const auto __comms = places_communicator::create(__mgmn_per_lane(__lanes, [&](::std::size_t __g) {

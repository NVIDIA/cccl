...
/**
 * @file
 * @brief LEGACY reference implementation of the sharded scans (`inclusive_scan`, `exclusive_scan`,
 *        `inclusive_sum`, `exclusive_sum`) — kept
 ...
 *        communicator, see `engine/mgmn.cuh`). Not maintained for features;
 *        to be removed. Not included by `cuda/experimental/sharded.cuh`;
 *        namespace `cuda::experimental::sharded::reserved::legacy`.
 *
 * Original description: In-place scans over sharded arrays: each place runs the device-scope
...
_CCCL_HOST_API void inclusive_sum(_S&& data, const _Envs& envs, const _CallEnv& call_env = {})
{
  ...
  legacy::inclusive_scan(::cuda::std::forward<_S>(data), envs, ::cuda::std::plus<elem_t>{}, elem_t{0}, call_env);
}

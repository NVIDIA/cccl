...
/**
 * @file
 * @brief LEGACY reference implementation of the sharded reductions (`reduce`, `reduce_into`,
 *        `reduce_into_lanes`, `sum`/`min`/`max`) — kept
 *        TEMPORARILY for parity and overhead measurement against the live
 *        verbs (whose engine is the MGMN algorithms over the places
 *        communicator, see `mgmn_adapter.cuh`). Not maintained for features;
 *        to be removed. Not included by `cuda/experimental/sharded.cuh`;
 *        namespace `cuda::experimental::sharded::reserved::legacy`.
 *
 * Original description: Reduction over sharded arrays: each place runs the device-scope
  ...
  const _S& data, const _Envs& envs, _OutIt out, _ReduceOp reduce_op, _Tp init_value, const _CallEnv& call_env)
{
  ...
  if (num_shards > legacy::__max_fold_shards)
  {
  ...
  if (num_shards == 0)
  {
    ...
    legacy::__fold_partials_kernel<<<1, 1, 0, call_stream.get()>>>(
...
reduce_into(const _S& data, _OutIt out, _ReduceOp reduce_op, _Tp init_value, const _CallEnv& call_env)
{
  ...
  legacy::reduce_into(data, envs, out, reduce_op, init_value, call_env);
}
...
reduce_into_lanes(const _S& data, const _Envs& envs, _OutIt outs, _ReduceOp reduce_op, _Tp init_value)
{
  ...
  legacy::__partial_slots<_Tp> slots{};
  ::cuda::std::uint64_t mask = 0;
  legacy::__lane_events reduced(num_shards); // E_g: lane g's partial is written
  legacy::__lane_events folded(num_shards); // F_g: lane g's fold has read every slot
  ...
  for (const auto g : each(num_shards))
  {
    ...
    legacy::__fold_partial_slots_kernel<<<1, 1, 0, lane_stream>>>(
...
{
  const auto envs = default_envs(data);
  legacy::reduce_into_lanes(data, envs, outs, reduce_op, init_value);
}
...
{
  const auto envs = default_envs(data);
  return legacy::reduce(data, envs, reduce_op, init_value, call_env);
}
...
[[nodiscard]] _CCCL_HOST_API view_element_t<_S> sum(const _S& data, const _Envs& envs, const _CallEnv& call_env = {})
{
  ...
  return legacy::reduce(data, envs, ::cuda::std::plus<elem_t>{}, elem_t{0}, call_env);
}
...
{
  const auto envs = default_envs(data);
  return legacy::sum(data, envs, call_env);
}
...
{
  using elem_t = view_element_t<_S>;
  return legacy::reduce(data, envs, ::cuda::minimum<elem_t>{}, ::cuda::std::numeric_limits<elem_t>::max(), call_env);
}
...
{
  const auto envs = default_envs(data);
  return legacy::min(data, envs, call_env);
}
...
{
  using elem_t = view_element_t<_S>;
  return legacy::reduce(data, envs, ::cuda::maximum<elem_t>{}, ::cuda::std::numeric_limits<elem_t>::lowest(), call_env);
}
...
{
  const auto envs = default_envs(data);
  return legacy::max(data, envs, call_env);
}
} // namespace cuda::experimental::sharded::reserved::legacy

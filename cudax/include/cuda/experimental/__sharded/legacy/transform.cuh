...
/**
 * @file
 * @brief LEGACY reference implementation of the sharded transforms (`transform`, `zip_transform`;
 *        thrust / `cub::DeviceTransform` bodies) — kept
 ...
 *
 * Original description: Elementwise transforms over sharded arrays (in-place, unary,
...
_CCCL_HOST_API void transform(_S&& data, _UnaryOp op, const _CallEnv& call_env = {})
{
  ...
  legacy::transform(::cuda::std::forward<_S>(data), envs, op, call_env);
}
  ...
  __detail::__generic_map(
    out, envs, call_env, "sharded::zip_transform", [&](::std::size_t g, const auto& s_out, cudaStream_t) {
      cuda_safe_call(cub::DeviceTransform::Transform(
        ...
        s_out.size,
        legacy::__tuple_result_op<_Op>{op},
        envs[g]));
    });
}
...
_CCCL_HOST_API void zip_transform(_SOut&& out, _Op op, const _SIn&... ins)
{
  ...
  legacy::zip_transform(::cuda::std::forward<_SOut>(out), envs, op, default_call_env{}, ins...);
}
} // namespace cuda::experimental::sharded::reserved::legacy

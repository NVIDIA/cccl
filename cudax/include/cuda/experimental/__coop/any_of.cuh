...
template <bool _Broadcasted, class _Hierarchy, ::cuda::std::size_t _Np>
[[nodiscard]] _CCCL_DEVICE_API auto
__any_of_impl(::cuda::std::bool_constant<_Broadcasted>, const this_thread<_Hierarchy>&, bool (&__thread_data)[_Np])
{
  bool __result = false;
  for (auto __v : __thread_data)
  {
    __result = __result || __v;
  }
...
_CCCL_TEMPLATE(bool _Broadcasted, class _Group, ::cuda::std::size_t _Np)
_CCCL_REQUIRES(is_group<_Group> _CCCL_AND ::cuda::std::is_same_v<typename _Group::level_type, warp_level>)
[[nodiscard]] _CCCL_DEVICE_API auto
__any_of_impl(::cuda::std::bool_constant<_Broadcasted>, const _Group& __group, bool (&__thread_data)[_Np]) noexcept
{
  const auto __thread_result = ::cuda::experimental::coop::__any_of_impl(
    ::cuda::std::true_type{}, this_thread{__group.hierarchy()}, __thread_data);

  const auto& __mapping_result = __group.__mapping_result();
  const auto __result          = static_cast<bool>(::__any_sync(__mapping_result.lane_mask().value(), __thread_result));
  if constexpr (_Broadcasted)
  {
    return __result;
  }

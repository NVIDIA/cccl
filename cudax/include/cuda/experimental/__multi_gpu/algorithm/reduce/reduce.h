  ...
  _BinaryOp __op)
{
  ...
  auto __resource              = __resource_from_env(__env, __logical_device.underlying_device());
...
_CCCL_REQUIRES(__range_of_communicators<_CommRange> _CCCL_AND ::cuda::std::ranges::forward_range<_EnvRange> _CCCL_AND
                 __detail::__range_of_sized_random_access_ranges<_InputRangeOfRanges> _CCCL_AND
                   __detail::__range_of_output_iters<_RangeOfOutputIt, _Tp>)
  ...
  for (auto&& [__comm, __env, __inputs] : ::cuda::std::ranges::views::zip(__comms, __envs, __range_of_inputs))
  {
    __partials.emplace_back(
      ::cuda::experimental::__detail::__reduce::__local_reduction<typename __partial_type::__buffer_type>(
        /*__ROOT_RANK=*/0, __comm, __env, __inputs, __init, __op));
  }

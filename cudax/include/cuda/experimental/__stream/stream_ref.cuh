...
struct stream_ref : ::cuda::stream_ref
{
  ...
  {
    const auto __result = _CUDA_VDRIVER::__streamQueryNoThrow(__stream);
    switch (__result)
    {
  ...
  {
    return _CUDA_VDRIVER::__streamGetPriority(__stream);
  }
  ...
  {
    return stream_id{_CUDA_VDRIVER::__streamGetId(__stream)};
  }
  ...
  {
    _CUDA_VDRIVER::__streamSynchronize(__stream);
  }
  ...
  _CCCL_HOST_API void wait(event_ref __ev) const
  {
    ...
    _CUDA_VDRIVER::__streamWaitEvent(get(), __ev.get());
  }
  ...
  _CCCL_HOST_API logical_device logical_device() const
  {
    ...
    {
      auto __ctx = _CUDA_VDRIVER::__streamGetCtx_v2(__stream);
      if (__ctx.__ctx_kind == _CUDA_VDRIVER::__ctx_from_stream::__kind::__green)
      {
        __stream_ctx = _CUDA_VDRIVER::__ctxFromGreenCtx(__ctx.__ctx_ptr.__green);
    ...
    {
      __stream_ctx = _CUDA_VDRIVER::__streamGetCtx(__stream);
      __ctx_kind   = ::cuda::experimental::logical_device::kinds::device;
    }

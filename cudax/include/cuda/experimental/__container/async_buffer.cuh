...
public:
  ...
  //! @brief Copy-assigns a async_buffer
  //! @param __other The other async_buffer.
  //! @note Even if the old async_buffer would have enough storage available, we may have to reallocate if the stored
  //! memory resource is not equal to the new one. In that case no memory is allocated if \p __other is empty.
  _CCCL_HIDE_FROM_ABI async_buffer& operator=(const async_buffer& __other)
  {
  ...
  //! @brief Move-assigns a async_buffer
  //! @param __other The other async_buffer.
  //! Clears the async_buffer and swaps the contents with \p __other.
  _CCCL_HIDE_FROM_ABI async_buffer& operator=(async_buffer&& __other) noexcept
  {
    if (this == _CUDA_VSTD::addressof(__other))
    {
      return *this;
  ...
  //! @brief Assigns an initializer_list to a async_buffer, replacing its content with that of the initializer_list
  //! @param __ilist The initializer_list to be assigned
  _CCCL_HIDE_FROM_ABI async_buffer& operator=(_CUDA_VSTD::initializer_list<_Tp> __ilist)
  {
  ...
  //! @brief Copy-assigns from a different async_buffer
  //! @param __other The other async_buffer.
  _CCCL_TEMPLATE(class... _OtherProperties)
  _CCCL_REQUIRES(__properties_match<_OtherProperties...>)
  _CCCL_HIDE_FROM_ABI async_buffer& operator=(const async_buffer<_Tp, _OtherProperties...>& __other)
  {
    if (this == reinterpret_cast<const async_buffer*>(_CUDA_VSTD::addressof(__other)))
    {
      return *this;
    }
  ...
  //! @brief Move-assigns from a different async_buffer
  //! @param __other The other async_buffer.
  _CCCL_TEMPLATE(class... _OtherProperties)
  _CCCL_REQUIRES(__properties_match<_OtherProperties...>)
  _CCCL_HIDE_FROM_ABI async_buffer& operator=(async_buffer<_Tp, _OtherProperties...>&& __other)
  {
    if (this == reinterpret_cast<async_buffer*>(_CUDA_VSTD::addressof(__other)))
    {
      return *this;
    }

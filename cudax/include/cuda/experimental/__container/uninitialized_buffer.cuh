...
private:
  ...
  _CCCL_TEMPLATE()
  [[nodiscard]] _CCCL_HIDE_FROM_ABI friend auto
  __cudax_launch_transform(::cuda::stream_ref, const uninitialized_buffer& __self) noexcept -> _CCCL_TRAILING_REQUIRES(
    _CUDA_VSTD::span<const _Tp>)(_CUDA_VSTD::__is_included_in_v<device_accessible, _Properties...>)
  {
    return {__self.__get_data(), __self.size()};
  }

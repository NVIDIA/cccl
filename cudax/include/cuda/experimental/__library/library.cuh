...
struct library : public library_ref
{
  ...
  static library from_native_handle(_CUDA_VSTD::nullptr_t) = delete;
  ...
  ~library()
  {
    ...
    {
      [[maybe_unused]] const auto __status = __detail::driver::libraryUnload(__library_);
    }
  }
  ...
  library& operator=(library&& __other) noexcept
  {
    ...
    {
      library __tmp{_CUDA_VSTD::move(__other)};
      _CUDA_VSTD::swap(__library_, __tmp.__library_);
    }
    return *this;
  }
  ...
  {
    return _CUDA_VSTD::exchange(__library_, value_type{});
  }

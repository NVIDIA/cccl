...
//! @brief Information about a global symbol in a CUDA library
struct library_global_symbol_info
{
  void* ptr; //!< Device pointer to the global symbol
  _CUDA_VSTD::size_t size; //!< Size of the global symbol in bytes
};

//! @brief Information about a managed symbol in a CUDA library
struct library_managed_symbol_info
{
  void* ptr; //!< Device pointer to the managed symbol
  _CUDA_VSTD::size_t size; //!< Size of the managed symbol in bytes
};
...
public:
  ...
  library_ref(_CUDA_VSTD::nullptr_t) = delete;
  ...
  {
    return kernel_ref<_Signature>{__detail::driver::libraryGetKernel(__library_, __name)};
  }
  ...
  //!
  //! @return A pair containing a pointer to the global symbol and its size
  //!
  ...
  [[nodiscard]] library_global_symbol_info global(const char* __name) const
  {
    ...
    __detail::driver::libraryGetGlobal(__dptr, __size, __library_, __name);
    return library_global_symbol_info{reinterpret_cast<void*>(__dptr), __size};
  }
  ...
  [[nodiscard]] library_managed_symbol_info managed(const char* __name) const
  {
    CUdeviceptr __dptr{};
    _CUDA_VSTD::size_t __size{};
    __detail::driver::libraryGetManaged(__dptr, __size, __library_, __name);
    return library_managed_symbol_info{reinterpret_cast<void*>(__dptr), __size};
  }

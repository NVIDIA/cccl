...
#ifndef __CUDAX_GRAPH_GRAPH_EXEC
#define __CUDAX_GRAPH_GRAPH_EXEC
...
struct _CCCL_TYPE_VISIBILITY_DEFAULT graph_exec
{
  ...
  _CCCL_HOST_API constexpr graph_exec(graph_exec&& __other) noexcept
  ...
  //! \throws None
  _CCCL_HOST_API _CCCL_CONSTEXPR_CXX20 ~graph_exec()
  {
    reset();
  }
  ...
  //! \throws None
  //! \post `__other.get() == nullptr`
  _CCCL_HOST_API constexpr auto operator=(graph_exec&& __other) noexcept -> graph_exec&
  {
    swap(__other);
    __other.reset();
    return *this;
  ...
  //! \throws None
  _CCCL_HOST_API constexpr void swap(graph_exec& __other) noexcept
  {
  ...
  //! \brief Constructs a `graph_exec` object from a native CUDA graph execution handle.
  //! \param __exec The native CUDA graph execution handle to construct the `graph_exec` object from.
  //! \throws None
  //! \post `get() == __exec`
  [[nodiscard]] _CCCL_HOST_API static _CCCL_CONSTEXPR_CXX20 auto from_native_handle(cudaGraphExec_t __exec) noexcept
    -> graph_exec
  {
    return graph_exec{__exec};
  }
...
private:
  ...
  //! \param __exec The native CUDA graph execution handle to construct the `graph_exec` object from.
  //! \throws None
  _CCCL_HOST_API explicit constexpr graph_exec(cudaGraphExec_t __exec) noexcept
      : __exec_{__exec}
  {}
...
#endif // __CUDAX_GRAPH_GRAPH_EXEC

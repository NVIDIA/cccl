...
{
  detail::__equal_result __state; ///< Equal result
...
private:
  ...
  detail::__equal_wrapper<__key_type, __key_equal, __allows_duplicates> __predicate; ///< Key equality
...
public:
    ...
    while (true)
    {
      ...
      for (auto& __slot_content : __bucket_slots)
      {
        ...
        if constexpr (!__allows_duplicates)
        {
          ...
          if (__eq_res == detail::__equal_result::__equal)
          {
            return false;
          }
        }
        if (__eq_res == detail::__equal_result::__available)
        {
  ...
  [[nodiscard]] _CCCL_DEVICE_API bool contains(_ProbeKey __key) const noexcept
  {
    ...
    while (true)
    {
      ...
      {
        switch (__predicate.template operator()<detail::__is_insert::__no>(__key, __extract_key(__bucket_slots[i])))
        {
  ...
  __probe_bucket(const _ProbeKey& __key, __bucket_type __bucket_slots) const noexcept
  {
    ...
    for (::cuda::std::int32_t __i = 0; __i < __bucket_size; ++__i)
    {
      __res = __predicate.template operator()<detail::__is_insert::__no>(__key, __extract_key(__bucket_slots[__i]));
      if (__res != detail::__equal_result::__unequal)
      {
        return __res;
      }
  ...
  [[nodiscard]] _CCCL_DEVICE_API constexpr auto __heterogeneous_value(_Value __value) const noexcept
  {
    if constexpr (__has_payload && !::cuda::std::is_same_v<_Value, __value_type>)
    {
      ...
      if constexpr (detail::__is_pair_like_v<_Value>)
      {
  ...
  packed_cas(__value_type* __address, __value_type __expected, _Value __desired) noexcept
  {
    ...
    auto* const __slot_ptr      = reinterpret_cast<packed_type*>(__address);
    ...
    const auto success = ::cuda::experimental::cuco::detail::__atomic_compare_exchange<_Scope>(
      __slot_ptr, __expected_packed, __desired_packed);
  ...
  back_to_back_cas(__value_type* __address, __value_type __expected, _Value __desired) noexcept
  {
    ...
    if (__predicate.__equal_to(__desired.first, __expected_key) == detail::__equal_result::__equal)
    {

...
//! @brief \c cuda_memory_pool is an owning wrapper around a
//! <a href="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY__POOLS.html">cudaMemPool_t</a>
//!
//! It handles creation and destruction of the underlying pool utilizing the provided \c cuda_memory_pool_properties
...
private:
  ...
  //! @param __device_id The id of the current device.
...
#    endif //_CCCL_CUDACC_BELOW_11_3
  ...
  __create_cuda_mempool(const int __device_id, cuda_memory_pool_properties __properties) noexcept
  {
    ...
    if (__properties.initial_pool_size != 0)
    {
      ...
      _CCCL_TRY_CUDA_API(
        ::cudaMallocAsync,
        "cuda_memory_pool failed to allocate the initial pool size",
        &__ptr,
        __properties.initial_pool_size,
        __temp_stream.get());

      _CCCL_ASSERT_CUDA_API(
        ::cudaFreeAsync, "cuda_memory_pool failed to free the initial pool allocation", __ptr, __temp_stream.get());
    }
...
public:
  ...
  explicit cuda_memory_pool(const ::cuda::experimental::device_ref __device_id,
                            cuda_memory_pool_properties __properties = {})

...
// to add `inline` to the function declaration. In cudax we solve this problem with two
// attributes:
// - `_CUDAX_API` declares the function host/device and hides the symbol from the ABI
// - `_CUDAX_TRIVIAL_API` does the same while also forcing inlining and hiding the
//   function from debuggers
...
// point objects.
#define _CUDAX_TRIVIAL_API        _CUDAX_API _CCCL_FORCEINLINE _CCCL_ARTIFICIAL _CCCL_NODEBUG
#define _CUDAX_TRIVIAL_HOST_API   _CUDAX_HOST_API _CCCL_FORCEINLINE _CCCL_ARTIFICIAL _CCCL_NODEBUG
#define _CUDAX_TRIVIAL_DEVICE_API _CUDAX_DEVICE_API _CCCL_FORCEINLINE _CCCL_ARTIFICIAL _CCCL_NODEBUG

// _CUDAX_DEFAULTED_API is used on `= default` functions to ensure they are hidden from
// the ABI.

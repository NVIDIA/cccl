...
struct __rcvr_with_env_t : _Rcvr
{
  ...
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto __rcvr() noexcept -> _Rcvr&
  {
    return *this;
  }

  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto __rcvr() const noexcept -> const _Rcvr&
  {
    return *this;
  }

  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto get_env() const noexcept -> __env_t
  {
    return __env_t{*this};
  }

  template <class _Query>
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE constexpr decltype(auto) __get_1st(_Query) const noexcept
  {
  ...
  template <class _Query>
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE constexpr auto query(_Query __query) const
...
struct __rcvr_with_env_t<_Rcvr*, _Env>
{
  ...
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto __rcvr() const noexcept -> _Rcvr*
  {
    return __rcvr_;
  }

  template <class... _As>
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE void set_value(_As&&... __as) && noexcept
  {
    __async::set_value(__rcvr_, static_cast<_As&&>(__as)...);
  }

  template <class _Error>
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE void set_error(_Error&& __error) && noexcept
  {
    __async::set_error(__rcvr_, static_cast<_Error&&>(__error));
  }

  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE void set_stopped() && noexcept
  {

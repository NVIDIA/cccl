...
struct prop
{
  ...
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE constexpr auto query(_Query) const noexcept -> const _Value&
  {
    return __value;
  }
...
{
template <class _Ty>
_CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto get_env(_Ty* __ty) noexcept //
...
{
  template <class _Ty>
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE auto operator()(_Ty* __ty) const noexcept //
...
struct get_env_t
{
  ...
  _CUDAX_HIDDEN_INLINE _CCCL_HOST_DEVICE empty_env operator()(__ignore) const noexcept
  {
    return {};
  }
};

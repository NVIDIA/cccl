...
public:
    ...
    const logical_data<slice<char>>& data() const { return l; }

    logical_data<slice<char>>& data() { return l; }
...
class ciphertext {
public:
    ciphertext(const context& ctx) : ctx(ctx) {}
    ...
    ciphertext operator|(const ciphertext& other) const {
        ...
        ctx.parallel_for(data().shape(), data().read(), other.data().read(), result.data().write()).set_symbol("OR")->*
                [] CUDASTF_DEVICE(size_t i, auto d_c1, auto d_c2, auto d_res) { d_res(i) = d_c1(i) | d_c2(i); };
    ...
    ciphertext operator~() const {
        ...
        ctx.parallel_for(data().shape(), data().read(), result.data().write()).set_symbol("NOT")->*
                [] CUDASTF_DEVICE(size_t i, auto d_c, auto d_res) { d_res(i) = ~d_c(i); };

        return result;
    }

    const logical_data<slice<uint64_t>>& data() const { return l; }

    logical_data<slice<uint64_t>>& data() { return l; }

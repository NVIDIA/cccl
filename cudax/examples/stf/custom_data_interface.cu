...
public:
    ...
    CUDASTF_HOST_DEVICE coords_t index_to_coords(size_t index) const { return coords_t(index % m, index / m); }
...
public:
    ...
    matrix_stream_interface(typename base::shape_t s) : base(s) {}
    ...
    void stream_data_allocate(backend_ctx_untyped& /*unused*/, const data_place& memory_node, instance_id_t instance_id,
            ssize_t& s, void** /*unused*/, cudaStream_t stream) override {
    ...
    void stream_data_deallocate(backend_ctx_untyped& /*unused*/, const data_place& memory_node,
            instance_id_t instance_id, void* /*unused*/, cudaStream_t stream) override {
        matrix<T>& instance = this->instance(instance_id);
        if (memory_node == data_place::host) {

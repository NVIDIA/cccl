    ...
    for (int iter = 0; iter < niter; iter++) {
        ...
        ctx.task(lY.rw(), lX.rw(), tmp.write())->*[](cudaStream_t s, auto sY, auto sX, auto sTMP) {
            ...
            cuda_safe_call(cudaMemcpyAsync(
                    sTMP.data_handle(), sX.data_handle(), n * sizeof(int), cudaMemcpyDeviceToDevice, s));
            // X = Y
            cuda_safe_call(
                    cudaMemcpyAsync(sX.data_handle(), sY.data_handle(), n * sizeof(int), cudaMemcpyDeviceToDevice, s));
            // Y = TMP
            cuda_safe_call(cudaMemcpyAsync(
                    sY.data_handle(), sTMP.data_handle(), n * sizeof(int), cudaMemcpyDeviceToDevice, s));
        };
    }

...
int main(int argc, char** argv) {
    ...
    for (size_t k = 0; k < 100; k++) {
        ...
        ctx.parallel_for(laux.shape(), laux.write(data_place::managed)).set_symbol("init_aux")
                        ->*[] CUDASTF_DEVICE(size_t i, auto aux) { aux(i) = 0.0; };
        ...
        for (size_t b = 0; b < NBLOCKS; b++) {
            ...
            ctx.parallel_for(exec_place::device(0), box({ b, b + 1 }), lX[b].read(data_place::device(b % ndevs)),
                       laux.rw(data_place::managed))
                            .set_symbol("store sum X_" + std::to_string(b))
                            ->*[] CUDASTF_DEVICE(size_t ind, auto Xb, auto aux) { aux(ind) = Xb(Xb.extent(0) - 1); };
        }
        ...
        for (size_t b = 1; b < NBLOCKS; b++) {
            ...
            ctx.parallel_for(lX[b].shape(), lX[b].rw(), laux.read(data_place::managed))
                            ...
                            ->*[=] CUDASTF_DEVICE(size_t i, auto Xb, auto aux) { Xb(i) += aux(b - 1); };
        }
    }

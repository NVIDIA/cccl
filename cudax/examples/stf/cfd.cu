...
void writeplotfile(int m, int n, int scale) {
    ...
    SCOPE(exit) { EXPECT(fclose(gnuplot) == 0); };
...
void writedatafiles(context& ctx, logical_data<slice<double, 2>> lpsi, int m, int n, int scale) {
    ...
    ctx.host_launch(lpsi.read(), lvel.write(), lrgb.write()).set_symbol("writedatafiles")
            ...
            [=](auto psi, auto vel, auto rgb) {
                ...
                SCOPE(exit) { fclose(cfile); };
                FILE* vfile = EXPECT(fopen("velocity.dat", "w"));
                SCOPE(exit) { fclose(vfile); };
...
void jacobistep(
        context& ctx, logical_data<slice<double, 2>> lpsinew, logical_data<slice<double, 2>> lpsi, int m, int n) {
...
void jacobistepvort(context& ctx, logical_data<slice<double, 2>> lzetnew, logical_data<slice<double, 2>> lpsinew,
        logical_data<slice<double, 2>> lzet, logical_data<slice<double, 2>> lpsi, int m, int n, double re) {
...
T transfer_host(context& ctx, logical_data<slice<T>>& ldata) {
    ...
    if (is_graph) {
        ctx.host_launch(ldata.read()).set_symbol("transfer_host")->*[&](auto data) { out = data(0); };
...
double deltasq(
        context& ctx, logical_data<slice<double, 2>> lnewarr, logical_data<slice<double, 2>> loldarr, int m, int n) {
    ...
    ctx.launch(spec, ldsq.write(), lnewarr.read(), loldarr.read()).set_symbol("deltasq")->*
            [m, n] __device__(auto th, auto dsq, auto newarr, auto oldarr) {
                ...
                for (auto [i, j]: th.apply_partition(
                             box<2>({ 1, m + 1 }, { 1, n + 1 }), std::tuple<blocked_partition, cyclic_partition>())) {
...
void boundarypsi(context& ctx, logical_data<slice<double, 2>> lpsi, int m, int /*n*/, int b, int h, int w) {
    // BCs on bottom edge
    ctx.parallel_for(box({ b + 1, b + w }), lpsi.rw()).set_symbol("boundary_bottom")
                    ->*[=] __device__(size_t i, auto psi) { psi(i, 0) = double(i - b); };

    ctx.parallel_for(box<1>({ b + w, m + 1 }), lpsi.rw()).set_symbol("boundary_bottom")
                    ->*[=] __device__(size_t i, auto psi) { psi(i, 0) = double(w); };
    ...
    ctx.parallel_for(box({ h + 1, h + w }), lpsi.rw()).set_symbol("boundary_right")
                    ->*[=] __device__(size_t j, auto psi) { psi(m + 1, j) = (double) (w - j + h); };
}
...
int main(int argc, char** argv) {
    ...
    // zero the psi array
    ctx.parallel_for(lpsi.shape(), lpsi.write()).set_symbol("InitPsi")->*
            [] __device__(size_t i, size_t j, auto psi) { psi(i, j) = 0.0; };
    ...
    if (!irrotational) {
        ...
        // zero the zeta array
        ctx.parallel_for(lzet.shape(), lzet.write()).set_symbol("InitZet")->*
                [] __device__(size_t i, size_t j, auto zet) { zet(i, j) = 0.0; };
    }
    ...
    for (; iter <= numiter; iter++) {
        ...
        ctx.parallel_for(box<2>({ 1, m + 1 }, { 1, n + 1 }), lpsi.rw(), lpsitmp.read()).set_symbol("SwitchPsi")
                        ->*[] __device__(size_t i, size_t j, auto psi, auto psitmp) { psi(i, j) = psitmp(i, j); };

        if (!irrotational) {
            ctx.parallel_for(box<2>({ 1, m + 1 }, { 1, n + 1 }), lzet.rw(), lzettmp.read()).set_symbol("SwitchZet")
                            ->*[] __device__(size_t i, size_t j, auto zet, auto zettmp) { zet(i, j) = zettmp(i, j); };
        }

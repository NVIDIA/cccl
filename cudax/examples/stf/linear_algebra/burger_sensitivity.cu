...
template <typename ctx_t>
void detect_shock(ctx_t& ctx, vector_t<double>& U, stackable_logical_data<scalar_view<double>>& max_gradient, size_t N, double h)
{
...
struct BurgerResidualCallback {
  ...
  template<typename ctx_t>
  void operator()(ctx_t& ctx, const vector_t<double>& x, const vector_t<double>& x_prev, vector_t<double>& residual) const {
  ...
  const char* filename = "shock_solutions.dat")
{
  ctx.host_launch(U.read()).set_symbol("dump shock solution")
    ...
    if (fp)
    {
      fprintf(fp, "# Sample %zu: nu=%.6e, shock_time=%.6e, max_gradient=%.2f\n", 
  ...
  double& final_time)
{
  ...
  ctx.parallel_for(U.shape(), U.write()).set_symbol("reset_initial_condition")
      ->*[h, N] __device__(size_t i, auto dU) {
            double x = i * h;
  ...
  ctx.parallel_for(box(1), current_time.write(), max_grad_global.write(), shock_detected.write())
      .set_symbol("init_tracking")->*[] __device__(size_t i, auto dtime, auto dmax_grad, auto dshock) {
        *dtime = 0.0;
        *dmax_grad = 0.0;
  ...
  // Time evolution loop with shock detection
  {
    ...
    newton_solver(ctx, U, csr_values, csr_row_offsets, csr_col_ind, 
                  residual_callback, jacobian_callback);

    // Update time
    ctx.parallel_for(box(1), current_time.rw()).set_symbol("update_time")
        ->*[dt] __device__(size_t i, auto dtime) {
              *dtime += dt;
            };
    ...
    while_guard.update_cond(current_time.read(), shock_detected.read())
        ->*[max_time] __device__(auto dtime, auto dshock) {
              return (*dtime < max_time) && (*dshock == 0);
            };
  }

  // Extract results to host variables
  ctx.host_launch(current_time.read(), max_grad_global.read(), shock_detected.read())
      .set_symbol("extract_results")
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) 
{
  ...
  if (argc > 2) nu_target = atof(argv[2]);
  if (argc > 3) nu_std = atof(argv[3]);
  if (argc > 4) num_samples = atoi(argv[4]);
  if (argc > 5) shock_threshold = atof(argv[5]);
  ...
  for (size_t i = 0; i < nu_values.size(); ++i) {
    ...
    run_single_nu_simulation(ctx, nu, U, csr_values, csr_row_offsets, csr_col_ind,
                             N, h, dt, max_time, shock_threshold, i+1,
                             shock_time, max_gradient, final_time);

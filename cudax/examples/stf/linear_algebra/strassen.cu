...
void MULT_CLASSIC(context& ctx, logical_matrix& A, logical_matrix& B, logical_matrix& C) {
    ctx.task(A.read(), B.read(), C.write()).set_symbol("MULT")->*[](cudaStream_t s, auto a, auto b, auto c) {
        ...
        cuda_safe_call(cublasDgemm(get_cublas_handle(), CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &one, a.data_handle(),
                a.stride(1), b.data_handle(), b.stride(1), &zero, c.data_handle(), c.stride(1)));
    };
}
...
void COPY_FROM_SUBMATRICES(context& ctx, logical_data<slice<T, 2>>& A, logical_data<slice<T, 2>> subA[2][2]) {
    ...
    ctx.task(A.write(), subA[0][0].read(), subA[0][1].read(), subA[1][0].read(), subA[1][1].read())
                    .set_symbol("COPY_FROM")
...
void strassen_test(context& ctx, size_t N) {
    ...
    ctx.host_launch(descC.read())->*[&](auto /* ignored */) { start = std::chrono::steady_clock::now(); };

    MULT(ctx, descA, descB, descC);

    ctx.host_launch(descC.read())->*[&](auto /* ignored */) { stop = std::chrono::steady_clock::now(); };

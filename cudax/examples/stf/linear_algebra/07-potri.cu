...
public:
    ...
    auto& get_handle(int row, int col) { return handles[row + col * mt]; }
...
void DPOTRF(cublasFillMode_t uplo, matrix<double>& A, int A_row, int A_col) {
    ...
    t->*[&](auto s, auto sAkk, auto buffer, auto info) {
        ...
        cuda_try(cusolverDnDpotrf(h, uplo, sAkk.extent(0), sAkk.data_handle(), sAkk.stride(1), buffer.data_handle(),
                buffer.extent(0), info.data_handle()));
    };
}
...
void DTRTRI(cublasFillMode_t uplo, cublasDiagType_t diag, matrix<double>& A, int A_row, int A_col) {
    ...
    cuda_try(cusolverDnXtrtri_bufferSize(get_cusolver_handle(), uplo, diag, m_a00, CUDA_R_64F /* DTRTRI */, nullptr,
            m_a00, &workspaceInBytesOnDevice, &workspaceInBytesOnHost));
    ...
    auto t = ctx.task(
            A.get_handle(A_row, A_col).rw(), d_buffer.write(), h_buffer.write(data_place::managed), devInfo.write());
    t.set_symbol("DTRTRI");
    t->*[&](auto s, auto sA, auto dbuffer, auto hbuffer, auto info) {
        ...
        // DTRTRI(...)
        cuda_try(cusolverDnXtrtri(h, uplo, diag, sA.extent(0), CUDA_R_64F /* DTRTRI */, sA.data_handle(), sA.stride(1),
                (double*) dbuffer.data_handle(), workspaceInBytesOnDevice, (double*) hbuffer.data_handle(),
                workspaceInBytesOnHost, info.data_handle()));
    };
}
...
cusolverStatus_t cusolverDnXlacpy(cublasFillMode_t uplo,  // "UPPER", B = upper(A)
                                                          // "LOWER", B = lower(A)
                                                          // otherwise, B = A
        int m, int n, const T_ELEM_SRC* A, int lda, T_ELEM_DST* B, int ldb, cudaStream_t stream) {
...
cusolverStatus_t cusolverDnDlacpy(cublasFillMode_t uplo,  // "UPPER", B = upper(A)
                                                          // "LOWER", B = lower(A)
                                                          // otherwise, B = A
        int m, int n, const double* A, int lda, double* B, int ldb, cudaStream_t stream) {
...
// Lower : A = A^T * A
// Upper : A = A A^T
void cublasDnDlaaum(cublasHandle_t cublas_handle, cublasFillMode_t uplo, int m, int n, double* A, int ldA,
        double* Workspace_d, size_t Workspace_size) {
    cudaStream_t stream;
...
void DLAAUM(cublasFillMode_t uplo, matrix<double>& A, int A_row, int A_col) {
    ...
    t->*[&](auto s, auto sA, auto buffer) {
        ...
        cublasDnDlaaum(h, uplo, sA.extent(0), sA.extent(1), sA.data_handle(), sA.stride(1),
                (double*) buffer.data_handle(), Lwork);
    };
}

void DGEMM(cublasOperation_t transa, cublasOperation_t transb, double alpha, matrix<double>& A, int A_row, int A_col,
        matrix<double>& B, int B_row, int B_col, double beta, matrix<double>& C, int C_row, int C_col) {
    auto ignored = get_cublas_handle();
    auto t = ctx.task(
            A.get_handle(A_row, A_col).read(), B.get_handle(B_row, B_col).read(), C.get_handle(C_row, C_col).rw());
    t.set_symbol("DGEMM");
    t->*[&](auto s, auto sA, auto sB, auto sC) {
        ...
        cuda_try(cublasDgemm(h, transa, transb, sC.extent(0), sC.extent(1), k, &alpha, sA.data_handle(), sA.stride(1),
                sB.data_handle(), sB.stride(1), &beta, sC.data_handle(), sC.stride(1)));
    };
}

void DSYMM(cublasSideMode_t side, cublasFillMode_t uplo, double alpha, matrix<double>& A, int A_row, int A_col,
        matrix<double>& B, int B_row, int B_col, double beta, matrix<double>& C, int C_row, int C_col) {
    auto ignored = get_cublas_handle();
    auto t = ctx.task(
            A.get_handle(A_row, A_col).read(), B.get_handle(B_row, B_col).read(), C.get_handle(C_row, C_col).rw());
    ...
    t->*[&](auto s, auto sA, auto sB, auto sC) {
        ...
        cuda_try(cublasDsymm(h, side, uplo, sC.extent(0), sC.extent(1), &alpha, sA.data_handle(), sA.stride(1),
                sB.data_handle(), sB.stride(1), &beta, sC.data_handle(), sC.stride(1)));
    };
}

void DSYRK(cublasFillMode_t uplo, cublasOperation_t trans, double alpha, matrix<double>& A, int A_row, int A_col,
        double beta, matrix<double>& C, int C_row, int C_col) {
    auto ignored = get_cublas_handle();
...
void DTRSM(cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t transa, cublasDiagType_t diag, double alpha,
        matrix<double>& A, int A_row, int A_col, matrix<double>& B, int B_row, int B_col) {
    auto ignored = get_cublas_handle();
    ...
    t->*[&](auto s, auto sA, auto sB) {
        ...
        cuda_try(cublasDtrsm(h, side, uplo, transa, diag, sB.extent(0), sB.extent(1), &alpha, sA.data_handle(),
                sA.stride(1), sB.data_handle(), sB.stride(1)));
    };
}

void DTRMM(cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t transa, cublasDiagType_t diag, double alpha,
        matrix<double>& A, int A_row, int A_col, matrix<double>& B, int B_row, int B_col) {
    auto ignored = get_cublas_handle();
    auto t = ctx.task(A.get_handle(A_row, A_col).read(), B.get_handle(B_row, B_col).rw());
    t.set_symbol("DTRMM");
    t->*[&](auto s, auto sA, auto sB) {
        ...
        cuda_try(cublasDtrmm(get_cublas_handle(), side, uplo, transa, diag, sB.extent(0), sB.extent(1), &alpha,
                sA.data_handle(), sA.stride(1), sB.data_handle(), sB.stride(1), sB.data_handle(),
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
void PDGEMM(cublasOperation_t transa, cublasOperation_t transb, double alpha, matrix<double>& A, matrix<double>& B,
...
void PDSYMM(cublasSideMode_t side, cublasFillMode_t uplo, double alpha, matrix<double>& A, matrix<double>& B,
        double beta, matrix<double>& C) {
    int k, m, n;
    double zbeta;
...
void PDTRMM(cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, double alpha,
        matrix<double>& A, matrix<double>& B) {
    if (side == CUBLAS_SIDE_LEFT) {
        if (uplo == CUBLAS_FILL_MODE_UPPER) {
            //===========================================
...
void run(int N, int NB) {
    ...
    auto hilbert = [=] CUDASTF_HOST_DEVICE(
    ...
    auto rhs_vals = [] CUDASTF_HOST_DEVICE(size_t row, size_t /*unused*/) { return 1.0 * (row + 1); };
    ...
    if (check_result) {
        ...
        auto zero_vals = [] CUDASTF_HOST_DEVICE(size_t /* unused */, size_t /*unused*/) { return 0.0; };

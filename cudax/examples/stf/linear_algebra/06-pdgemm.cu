...
class matrix {
public:
    matrix(stream_ctx& ctx, size_t NROWS, size_t NCOLS, size_t BLOCKSIZE_ROWS, size_t BLOCKSIZE_COLS,
    ...
    logical_data<slice<T, 2>>& get_handle(int row, int col) { return handles[row + col * mt]; }
...
void DGEMM(stream_ctx& ctx, cublasOperation_t transa, cublasOperation_t transb, double alpha, matrix<double>& A,
        int A_row, int A_col, matrix<double>& B, int B_row, int B_col, double beta, matrix<double>& C, int C_row,
        int C_col) {
    ...
    t->*[&](cudaStream_t stream, auto tA, auto tB, auto tC) {
        ...
        cuda_safe_call(cublasDgemm(get_cublas_handle(), transa, transb, tC.extent(0), tC.extent(1), k, &alpha,
                tA.data_handle(), tA.stride(1), tB.data_handle(), tB.stride(1), &beta, tC.data_handle(), tC.stride(1)));
    };
}

void PDGEMM(stream_ctx& ctx, cublasOperation_t transa, cublasOperation_t transb, double alpha, matrix<double>& A,
        matrix<double>& B, double beta, matrix<double>& C) {
...
void run(stream_ctx& ctx, size_t N, size_t NB) {
    ...
    std::cout << "[PDDGEMM] ELAPSED: " << milliseconds << " ms, GFLOPS: " << gflops_pdgemm / (milliseconds / 1000.0)
              << '\n';
}

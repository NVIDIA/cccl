...
class vector {
public:
    vector(size_t N) { handle = ctx.logical_data(shape_of<slice<double>>(N)); }
    ...
    static void copy_vector(const vector& from, vector& to) {
        ctx.parallel_for(to.handle.shape(), to.handle.write(), from.handle.read()).set_symbol("copy_vector")
                        ->*[] CUDASTF_DEVICE(size_t i, slice<double> dto, slice<double> dfrom) { dto(i) = dfrom(i); };
    }
    ...
    vector& operator-=(const vector& rhs) {
        ctx.parallel_for(handle.shape(), handle.rw(), rhs.handle.read()).set_symbol("-= vector")
                        ->*[] CUDASTF_DEVICE(size_t i, auto dthis, auto drhs) { dthis(i) -= drhs(i); };

        return *this;
    }

    size_t size() const { return handle.shape().size(); }
...
class scalar {
public:
    scalar() { handle = ctx.logical_data(shape_of<slice<double>>(1)); }
    ...
    scalar operator/(scalar const& rhs) const {
        ...
        ctx.parallel_for(handle.shape(), handle.read(), rhs.handle.read(), res.handle.write())
                        ->*
                [] CUDASTF_DEVICE(size_t i, auto dthis, auto drhs, auto dres) { dres(i) = dthis(i) / drhs(i); };
    ...
    scalar operator-() const {
        ...
        ctx.parallel_for(handle.shape(), handle.read(), res.handle.write())
                        ->*[] CUDASTF_DEVICE(size_t i, auto dthis, auto dres) { dres(i) = -dthis(i); };
...
scalar DOT(vector& a, vector& b) {
    ...
    ctx.parallel_for(box(1), res.handle.write())->*[] CUDASTF_DEVICE(size_t, auto dres) { dres(0) = 0.0; };
...
void AXPY(const scalar& alpha, vector& x, vector& y) {
    ctx.parallel_for(x.handle.shape(), alpha.handle.read(), x.handle.read(), y.handle.rw())
                    ->*[] CUDASTF_DEVICE(size_t i, auto dalpha, auto dx, auto dy) { dy(i) += dalpha(0) * dx(i); };
};
...
void SCALE_AXPY(const scalar& alpha, const vector& x, vector& y) {
    ctx.parallel_for(x.handle.shape(), alpha.handle.read(), x.handle.read(), y.handle.rw())
                    ->*
            [] CUDASTF_DEVICE(size_t i, auto dalpha, auto dx, auto dy) { dy(i) = dalpha(0) * dy(i) + dx(i); };
};

void SPMV(csr_matrix& a, vector& x, vector& y) {
    ctx.parallel_for(y.handle.shape(), a.val_handle.read(), a.col_handle.read(), a.row_handle.read(), x.handle.read(),
            y.handle.write())
...
int main(int argc, char** argv) {
    ...
    // RHS
    ctx.parallel_for(B.handle.shape(), B.handle.write())->*[] CUDASTF_DEVICE(size_t i, auto dB) { dB(i) = 1.0; };

    // Initial guess
    ctx.parallel_for(X.handle.shape(), X.handle.write())->*[] CUDASTF_DEVICE(size_t i, auto dX) { dX(i) = 1.0; };

...
int main(int, char**)
{
  ...
  ctx.parallel_for(box(N), lsum.reduce(reducer::sum<size_t>{}))->*[] __device__(size_t i, auto& sum) {
    ...
    const double x = curand_uniform_double(&local_state); // Random x in [0, 1)
    const double y = curand_uniform_double(&local_state); // Random y in [0, 1)
  ...
  const double pi_val = (4.0 * static_cast<double>(res)) / static_cast<double>(N);

 ...
 */
__device__ void calculating_pagerank(int idx, slice<const int> loffsets, slice<const int> lnonzeros,
        slice<const float> lpage_rank, slice<float> lnew_page_rank, float init_rank) {
    float rank_sum = 0.0;
    ...
    for (int iter = 0; iter < NITER; ++iter) {
        ...
        ctx.parallel_for(box(num_vertices), loffsets.read(), lnonzeros.read(), lpage_rank.rw(), lnew_page_rank.rw())
                        ->*[init_rank] __device__(
                                   size_t idx, auto loffsets, auto lnonzeros, auto lpage_rank, auto lnew_page_rank) {
                                calculating_pagerank(idx, loffsets, lnonzeros, lpage_rank, lnew_page_rank, init_rank);
                            };

        // Calculate Current Iteration Error
        ctx.parallel_for(box(1), lmax_diff.write())->*[] __device__(size_t, auto lmax_diff) { lmax_diff(0) = 0.0f; };

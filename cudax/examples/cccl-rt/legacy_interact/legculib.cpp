...
//! @brief Array of datatype sizes. Should be indexed by \c legculib_datatype.
...
extern "C" legculib_result legculib_init(int ordinal)
{
  ...
  CALL_CUDA(cuDeviceGet(&device, device));
...
extern "C" __global__ void kernel(T* result, const T* a, const T* b, unsigned n)
{
  ...
  if (tid < 32) warp_reduce<T, block_size>(sdata, tid);
...
static legculib_result make_sum_of_products_kernel(legculib_datatype type, CUfunction& kernel)
{
  ...
  // Make the compilation options.
  const auto type_name = datatype_name[type];
  ...
  const char* opts[]{
    ...
    "-dopt", // Enable device optimizations.
    "-D",
    type_name, // Pass the compiled type.
    "-arch",
  ...
  CALL_NVRTC(nvrtcCompileProgram(prog, 4, opts));
  ...
  cudaStream_t stream, void* result, const void* a, const void* b, unsigned n, legculib_datatype type)
{
  ...
  // Get the kernel.
  CUfunction kernel{};
  if (const auto result = make_sum_of_products_kernel(type, kernel); result.error != LEGCULIB_SUCCESS)
  {
    return result;
  }

...
struct kernel_functor_with_config
{
  ...
  __device__ void operator()(cudax::kernel_config<Dims, Opts...> config, name_buffer name)
  {
    ...
    static_assert(decltype(config.dims.extents(cudax::block, cudax::grid))::static_extent(0) == 1);
    static_assert(decltype(config.dims.extents(cudax::block, cudax::grid))::static_extent(1) == 1);

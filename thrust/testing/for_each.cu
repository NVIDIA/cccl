...
void TestForEachSimpleAnySystem()
{
  ...
  const cuda::counting_iterator<int> result =
...
void TestForEachNSimpleAnySystem()
{
  ...
  const cuda::counting_iterator<int> result = thrust::for_each_n(cuda::make_counting_iterator(0), 5, f);
...
{
  const cuda::counting_iterator<unsigned long long> begin(0);
  const cuda::counting_iterator<unsigned long long> end = begin + static_cast<std::ptrdiff_t>(1ull << magnitude);

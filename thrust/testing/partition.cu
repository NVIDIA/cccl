...
void TestPartitionSimple()
{
  ...
  Iterator iter = thrust::partition(data.begin(), data.end(), c2h::is_even);
...
void TestPartitionStencilSimple()
{
  ...
  Iterator iter = thrust::partition(data.begin(), data.end(), stencil.begin(), c2h::is_even);
...
void TestPartitionCopySimple()
{
  ...
  cuda::std::pair<typename Vector::iterator, typename Vector::iterator> ends =
    thrust::partition_copy(data.begin(), data.end(), true_results.begin(), false_results.begin(), c2h::is_even);
...
void TestPartitionCopyStencilSimple()
{
  ...
  cuda::std::pair<typename Vector::iterator, typename Vector::iterator> ends = thrust::partition_copy(
    data.begin(), data.end(), stencil.begin(), true_results.begin(), false_results.begin(), c2h::is_even);
...
void TestStablePartitionSimple()
{
  ...
  Iterator iter = thrust::stable_partition(data.begin(), data.end(), c2h::is_even);
...
void TestStablePartitionCopySimple()
{
  ...
  cuda::std::pair<typename Vector::iterator, typename Vector::iterator> ends =
    thrust::stable_partition_copy(data.begin(), data.end(), true_results.begin(), false_results.begin(), c2h::is_even);
...
struct TestPartition
{
  void operator()(const size_t n)
  {
    ...
    typename thrust::host_vector<T>::iterator h_iter   = thrust::partition(h_data.begin(), h_data.end(), c2h::is_even);
    typename thrust::device_vector<T>::iterator d_iter = thrust::partition(d_data.begin(), d_data.end(), c2h::is_even);
...
struct TestPartitionStencil
{
  void operator()(const size_t n)
  {
    ...
    typename thrust::host_vector<T>::iterator h_iter =
      thrust::partition(h_data.begin(), h_data.end(), h_stencil.begin(), c2h::is_even);
    typename thrust::device_vector<T>::iterator d_iter =
      thrust::partition(d_data.begin(), d_data.end(), d_stencil.begin(), c2h::is_even);
  ...
  void operator()(const size_t n)
  {
    ...
    cuda::std::pair<typename thrust::host_vector<T>::iterator, typename thrust::host_vector<T>::iterator> h_ends =
      thrust::partition_copy(
        h_data.begin(), h_data.end(), h_true_results.begin(), h_false_results.begin(), c2h::is_even);

    cuda::std::pair<typename thrust::device_vector<T>::iterator, typename thrust::device_vector<T>::iterator> d_ends =
      thrust::partition_copy(
        d_data.begin(), d_data.end(), d_true_results.begin(), d_false_results.begin(), c2h::is_even);
...
struct TestPartitionCopyStencil
{
  void operator()(const size_t n)
  {
    ...
    cuda::std::pair<typename thrust::host_vector<T>::iterator, typename thrust::host_vector<T>::iterator> h_ends =
      thrust::partition_copy(
        h_data.begin(), h_data.end(), h_stencil.begin(), h_true_results.begin(), h_false_results.begin(), c2h::is_even);

    cuda::std::pair<typename thrust::device_vector<T>::iterator, typename thrust::device_vector<T>::iterator> d_ends =
      thrust::partition_copy(
        d_data.begin(), d_data.end(), d_stencil.begin(), d_true_results.begin(), d_false_results.begin(), c2h::is_even);
...
struct TestStablePartitionCopyStencil
{
  void operator()(const size_t n)
  {
    ...
    std::ptrdiff_t n_true  = thrust::count_if(h_stencil.begin(), h_stencil.end(), c2h::is_even);
    std::ptrdiff_t n_false = n - n_true;
...
struct TestPartitionCopyToDiscardIterator
{
  void operator()(const size_t n)
  {
    ...
    cuda::std::pair<thrust::discard_iterator<>, thrust::discard_iterator<>> h_result1 = thrust::partition_copy(
      h_data.begin(), h_data.end(), thrust::make_discard_iterator(), thrust::make_discard_iterator(), c2h::is_even);

    cuda::std::pair<thrust::discard_iterator<>, thrust::discard_iterator<>> d_result1 = thrust::partition_copy(
      d_data.begin(), d_data.end(), thrust::make_discard_iterator(), thrust::make_discard_iterator(), c2h::is_even);
    ...
    cuda::std::pair<typename thrust::host_vector<T>::iterator, thrust::discard_iterator<>> h_result2 =
      thrust::partition_copy(
        h_data.begin(), h_data.end(), h_trues.begin(), thrust::make_discard_iterator(), c2h::is_even);

    cuda::std::pair<typename thrust::device_vector<T>::iterator, thrust::discard_iterator<>> d_result2 =
      thrust::partition_copy(
        d_data.begin(), d_data.end(), d_trues.begin(), thrust::make_discard_iterator(), c2h::is_even);
    ...
    cuda::std::pair<thrust::discard_iterator<>, typename thrust::host_vector<T>::iterator> h_result3 =
      thrust::partition_copy(
        h_data.begin(), h_data.end(), thrust::make_discard_iterator(), h_falses.begin(), c2h::is_even);

    cuda::std::pair<thrust::discard_iterator<>, typename thrust::device_vector<T>::iterator> d_result3 =
      thrust::partition_copy(
        d_data.begin(), d_data.end(), thrust::make_discard_iterator(), d_falses.begin(), c2h::is_even);
...
struct TestPartitionCopyStencilToDiscardIterator
{
  void operator()(const size_t n)
  {
    ...
    cuda::std::pair<typename thrust::host_vector<T>::iterator, thrust::discard_iterator<>> h_result2 =
      thrust::partition_copy(
        h_data.begin(), h_data.end(), h_stencil.begin(), h_trues.begin(), thrust::make_discard_iterator(), c2h::is_even);

    cuda::std::pair<typename thrust::device_vector<T>::iterator, thrust::discard_iterator<>> d_result2 =
      thrust::partition_copy(
        d_data.begin(), d_data.end(), d_stencil.begin(), d_trues.begin(), thrust::make_discard_iterator(), c2h::is_even);
...
struct TestStablePartition
{
  void operator()(const size_t n)
  {
    ...
    typename thrust::host_vector<T>::iterator h_iter =
      thrust::stable_partition(h_data.begin(), h_data.end(), c2h::is_even);
    typename thrust::device_vector<T>::iterator d_iter =
      thrust::stable_partition(d_data.begin(), d_data.end(), c2h::is_even);
...
struct TestStablePartitionStencil
{
  void operator()(const size_t n)
  {
    ...
    typename thrust::host_vector<T>::iterator h_iter =
      thrust::stable_partition(h_data.begin(), h_data.end(), h_stencil.begin(), c2h::is_even);
    typename thrust::device_vector<T>::iterator d_iter =
      thrust::stable_partition(d_data.begin(), d_data.end(), d_stencil.begin(), c2h::is_even);

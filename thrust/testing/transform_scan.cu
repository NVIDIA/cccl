...
template <class Vector>
void TestBinaryTransformExclusiveScanSimple()
{
  using T = typename Vector::value_type;
  ...
  thrust::transform_exclusive_scan(
    v1.begin(), v1.end(), v2.begin(), output.begin(), T(0), ::cuda::std::plus<T>(), ::cuda::std::multiplies<T>());

  // Expected: [0, 0+1*5, 0+1*5+2*6, 0+1*5+2*6+3*7] = [0, 5, 17, 38]
  ASSERT_EQUAL(output[0], T(0));
  ASSERT_EQUAL(output[1], T(5));
  ASSERT_EQUAL(output[2], T(17));
  ASSERT_EQUAL(output[3], T(38));
}
DECLARE_VECTOR_UNITTEST(TestBinaryTransformExclusiveScanSimple);

template <class Vector>
void TestBinaryTransformExclusiveScanWithCustomOps()
{
  using T = typename Vector::value_type;
  ...
  thrust::transform_exclusive_scan(
    v1.begin(), v1.end(), v2.begin(), output.begin(), T(10), ::cuda::std::plus<T>(), ::cuda::std::plus<T>());

  // Expected: [10, 10+(1+5), 10+(1+5)+(2+6), 10+(1+5)+(2+6)+(3+7)] = [10, 16, 24, 34]
  ASSERT_EQUAL(output[0], T(10));
  ASSERT_EQUAL(output[1], T(16));
  ASSERT_EQUAL(output[2], T(24));
  ASSERT_EQUAL(output[3], T(34));
}
DECLARE_VECTOR_UNITTEST(TestBinaryTransformExclusiveScanWithCustomOps);

template <typename T>
void TestBinaryTransformExclusiveScan(const size_t n)
{
...
DECLARE_VARIABLE_UNITTEST(TestBinaryTransformExclusiveScan);

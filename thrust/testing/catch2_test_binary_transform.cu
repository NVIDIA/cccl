...
OutputIterator
transform(my_system& system, InputIterator1, InputIterator1, InputIterator2, OutputIterator result, BinaryFunction)
{
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename BinaryFunction>
OutputIterator transform(my_tag, InputIterator1, InputIterator1, InputIterator2, OutputIterator result, BinaryFunction)
{
  *result = 13;
  return result;
}
...
TEMPLATE_LIST_TEST_CASE("BinaryCountingIterator", "[transform]", generic_list)
{
  ...
  const auto first = cuda::make_counting_iterator(T{0});

  thrust::host_vector<T> h_result(n);
  thrust::device_vector<T> d_result(n);

  thrust::transform(thrust::host, first, first + n, first, h_result.begin(), ::cuda::std::plus<T>());
  thrust::transform(thrust::device, first, first + n, first, d_result.begin(), ::cuda::std::plus<T>());

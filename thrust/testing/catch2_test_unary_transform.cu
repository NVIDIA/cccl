...
TEMPLATE_LIST_TEST_CASE("UnaryCountingIterator", "[transform]", generic_list)
{
  ...
  thrust::transform(thrust::host, first, first + n, h_result.begin(), ::cuda::std::identity{});
  thrust::transform(thrust::device, first, first + n, d_result.begin(), ::cuda::std::identity{});

...
public:
  using iterator = thrust::transform_iterator<head_flag_functor, cuda::counting_iterator<IndexType, IndexType>>;
  ...
  _CCCL_HOST_DEVICE head_flags(RandomAccessIterator first, RandomAccessIterator last, BinaryPredicate binary_pred = {})
      : m_begin(thrust::make_transform_iterator(
          cuda::counting_iterator<IndexType, IndexType>(0), head_flag_functor(first, binary_pred)))
      , m_count(last - first)
  {}

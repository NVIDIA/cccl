...
struct body
{
  ...
  void operator()(const ::tbb::blocked_range<Size>& r) const
  {
    ...
    using sum_type       = ::cuda::std::__accumulator_t<BinaryFunction, decltype(*my_first)>;
    result[interval_idx] = thrust::reduce(
      thrust::seq, my_first + 1, my_last, static_cast<sum_type>(thrust::raw_reference_cast(*my_first)), binary_op);
  }
};

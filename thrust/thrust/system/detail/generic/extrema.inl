  ...
  thrust::execution_policy<DerivedPolicy>& exec, ForwardIterator first, ForwardIterator last, BinaryPredicate comp)
{
  ...
  ::cuda::std::tuple<InputType, IndexType> result = thrust::reduce(
    exec,
    thrust::make_zip_iterator(first, cuda::counting_iterator<IndexType, IndexType>(0)),
    thrust::make_zip_iterator(first, cuda::counting_iterator<IndexType, IndexType>(0)) + (last - first),

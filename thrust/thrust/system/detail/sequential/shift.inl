...
template <typename DerivedPolicy, typename ForwardIterator>
__host__ __device__
  ForwardIterator shift_right(sequential::execution_policy<DerivedPolicy> &exec,
                             ForwardIterator first,
                             ForwardIterator last,
                             typename thrust::iterator_traits<ForwardIterator>::difference_type n)
{
  ...
  for (; trail != current; ++trail, ++lead) {
    if (lead == last) {
      thrust::system::detail::sequential::copy(exec, first, trail, current);
      return current;
    }
  }
  ...
  while (true) {
    trail = thrust::swap_ranges(exec, first, current, trail);

    // check whether we reached end
    ForwardIterator mid = first;
    for (auto i = n; i != 0; --i, ++lead, ++mid) {
      if (lead == last) {
        thrust::system::detail::sequential::copy(exec, first, mid, trail);
        return current;
      }
    }
  }

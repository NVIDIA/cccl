...
{
  explicit constexpr log_space_step(size_t const total_count)
    ...
    std::size_t) noexcept
  {
    ...
    std::size_t num_cores = static_cast<std::size_t>((std::max) (1LL, std::llround(pred_c)));
    ...
    std::size_t num_iterations)
  {
    ...
    chunk_size = (std::max) (chunk_size, (num_iterations + max_chunks - 1) / max_chunks);
...
template <>
struct hpx::execution::experimental::is_executor_parameters<thrust::system::hpx::detail::log_space_step>
    : std::true_type
{};

//===----------------------------------------------------------------------===//
//
// Part of the Thrust Parallel Algorithms Library
//
//===----------------------------------------------------------------------===//
...
state.exec(
    nvbench::exec_tag::gpu | nvbench::exec_tag::no_batch | nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
      // Create a policy using the NVBench allocator and the current launch stream
      auto policy = thrust::cuda::par(alloc).on(launch.get_stream());

      do_not_optimize(thrust::is_partitioned(
        policy, 
        dinput.begin(), 
        dinput.end(), 
        select_op_t{static_cast<T>(mismatch_point)}));
    });
}

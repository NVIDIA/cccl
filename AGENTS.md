...
* **Python CCCL packages** — `cuda.compute` parallel primitives and the aggregate `cuda-cccl` metapackage
...
not copy them beneath `python/cuda_cccl`.
...
* `tests/packaging` — Private wheel packaging behavior
...
├── python/cuda_cccl/      # cuda.compute implementation wheel
├── python/cuda_cccl_meta/ # Module-free aggregate metapackage

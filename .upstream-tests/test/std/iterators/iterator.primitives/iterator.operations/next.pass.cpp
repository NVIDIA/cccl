...
{
    assert(cuda::std::next(i, n) == x);

    It (*next)(It, typename cuda::std::iterator_traits<It>::difference_type) = cuda::std::next;
    assert(next(i, n) == x);
}

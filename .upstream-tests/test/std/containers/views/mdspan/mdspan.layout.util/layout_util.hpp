...
struct TestLayoutCompatCtors<cuda::std::tuple<
  Mapping,
  cuda::std::integer_sequence<size_t, DynamicSizes...>,
  Mapping2,
...
//TYPED_TEST(TestLayout{Left|Right}CompatCtors, compatible_assign_{1|2}) {
{
    ...
    // Assign
    {
        ...
        if constexpr ( cuda::std::is_convertible<typename TestFixture::mapping_type2, typename TestFixture::mapping_type1>::value )
        {
            t.map1 = t.map2;
        }
        else
...
using test_left_type_pair = cuda::std::tuple<
  typename cuda::std::layout_left::template mapping<E1>, S1,
  typename cuda::std::layout_left::template mapping<E2>, S2
>;

template <class E1, class S1, class E2, class S2>
using test_right_type_pair = cuda::std::tuple<
    typename cuda::std::layout_right::template mapping<E1>, S1,
    typename cuda::std::layout_right::template mapping<E2>, S2
>;
...
struct is_cons_avail< T1
                    , T2
                    , cuda::std::enable_if_t< cuda::std::is_same< decltype( T1{ cuda::std::declval<T2>() } )
                                                                , T1
                                                                >::value
                                            >
                    > : cuda::std::true_type {};
...
struct is_paren_op_avail< cuda::std::enable_if_t< cuda::std::is_same< decltype(cuda::std::declval<T>()(cuda::std::declval<Indicies>()...))

...
int main(int, char**)
{
  ...
  }
  {
    ...
    // expected-error@experimental/coroutine:* 1 {{{{(static_assert|static assertion)}} failed "coroutine_handle<promise_type>::from_address cannot be used with pointers to the coroutine's promise type; use 'from_promise' instead"}}
    H::from_address((const char*)nullptr); // expected-note {{requested here}}

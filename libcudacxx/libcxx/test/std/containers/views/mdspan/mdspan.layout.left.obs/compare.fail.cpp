...
int main(int, char**)
{
    ...
    // match but it will return false if the ranks are not consistent
    {
        ...
        constexpr std::layout_left::mapping<ext2d_t> m0{ e0 };
        constexpr std::layout_left::mapping<ext3d_t> m1{ e1 };

        static_assert( m0 == m1, "" ); // expected-error-re {{{{(static_assert|static assertion)}} failed due to requirement 'm0 == m1'}}
    }

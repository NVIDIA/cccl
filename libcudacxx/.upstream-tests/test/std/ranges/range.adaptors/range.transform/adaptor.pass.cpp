...
inline constexpr bool CanBePiped<V, T, cuda::std::void_t<decltype(cuda::std::declval<V>() | cuda::std::declval<T>())>> = true;
#endif

struct NonCopyableFunction {
  NonCopyableFunction(NonCopyableFunction const&) = delete;
  template <class T>
  __host__ __device__ constexpr T operator()(T x) const { return x; }
};
...
__host__ __device__ constexpr bool test() {
  ...
  {
    {
      using Result = cuda::std::ranges::transform_view<cuda::std::ranges::transform_view<MoveOnlyView, PlusOne>, TimesTwo>;
      decltype(auto) result = MoveOnlyView{buff} | cuda::std::views::transform(PlusOne{}) | cuda::std::views::transform(TimesTwo{});
      static_assert(cuda::std::same_as<decltype(result), Result>);
  ...
  {
    static_assert(cuda::std::is_same_v<decltype(cuda::std::ranges::views::transform), decltype(cuda::std::views::transform)>);
  }

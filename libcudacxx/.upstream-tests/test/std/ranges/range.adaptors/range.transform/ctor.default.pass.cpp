...
struct DefaultConstructibleFunction {
  int state_;
  __host__ __device__ constexpr DefaultConstructibleFunction() : state_(100) { }
  __host__ __device__ constexpr int operator()(int i) const { return i + state_; }
};
...
__host__ __device__ constexpr bool test() {
  ...
  static_assert(!cuda::std::is_default_constructible_v<cuda::std::ranges::transform_view<NoDefaultCtrView,            DefaultConstructibleFunction>>);
  static_assert(!cuda::std::is_default_constructible_v<cuda::std::ranges::transform_view<DefaultConstructibleView, NoDefaultFunction>>);
  static_assert(!cuda::std::is_default_constructible_v<cuda::std::ranges::transform_view<NoDefaultCtrView,            NoDefaultFunction>>);

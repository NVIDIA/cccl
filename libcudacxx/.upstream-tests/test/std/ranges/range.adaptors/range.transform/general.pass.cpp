...
template<class T, class F>
inline constexpr bool ValidTransformView<T, F, cuda::std::void_t<typename cuda::std::ranges::transform_view<T, F>>> = true;
#endif
...
__host__ __device__ auto toUpper(R range) {
  return cuda::std::ranges::transform_view(range, [](char c) { return cuda::std::toupper(c); });
}
#endif
...
struct NonConstView : cuda::std::ranges::view_base {
  __host__ __device__ explicit NonConstView(int *b, int *e) : b_(b), e_(e) {}
  __host__ __device__ const int *begin() { return b_; }  // deliberately non-const
  __host__ __device__ const int *end() { return e_; }    // deliberately non-const
  const int *b_;
  const int *e_;
};
...
int main(int, char**) {
#if defined(_LIBCUDACXX_HAS_VECTOR)
  {
    ...
    auto transformed = cuda::std::ranges::transform_view(vec, [](int x) { return x + 42; });
...

  {
    ...
    int a[] = {1, 2, 3, 4};
    auto transformed = NonConstView(a, a + 4) | cuda::std::views::transform([](int x) { return x + 42; });

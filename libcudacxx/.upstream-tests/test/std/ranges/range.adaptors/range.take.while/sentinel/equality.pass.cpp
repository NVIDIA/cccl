...
struct Sent {
  ...
  __host__ __device__ friend constexpr bool operator==(const Iter<Const>& i, const Sent& s) { return i.it_ == s.end_; }
#if TEST_STD_VER < 20
  __host__ __device__ friend constexpr bool operator==(const Sent& s, const Iter<Const>& i) { return i.it_ == s.end_; }
  __host__ __device__ friend constexpr bool operator!=(const Iter<Const>& i, const Sent& s) { return i.it_ != s.end_; }
  __host__ __device__ friend constexpr bool operator!=(const Sent& s, const Iter<Const>& i) { return i.it_ != s.end_; }
#endif
};
...
struct CrossComparableSent {
  ...
  template <bool C>
  __host__ __device__ friend constexpr bool operator==(const Iter<C>& i, const CrossComparableSent& s) { return i.it_ == s.end_; }
#if TEST_STD_VER < 20
  template <bool C>
  __host__ __device__ friend constexpr bool operator==(const CrossComparableSent& s, const Iter<C>& i) { return i.it_ == s.end_; }
  template <bool C>
  __host__ __device__ friend constexpr bool operator!=(const Iter<C>& i, const CrossComparableSent& s) { return i.it_ != s.end_; }
  template <bool C>
  __host__ __device__ friend constexpr bool operator!=(const CrossComparableSent& s, const Iter<C>& i) { return i.it_ != s.end_; }
...
struct Range : IntBufferViewBase {
  ...
  Range(T&& input) : IntBufferViewBase(cuda::std::forward<T>(input)) {}
#else // ^^^ TEST_COMPILER_NVRTC ^^^ / vvv !TEST_COMPILER_NVRTC vvv
  using IntBufferViewBase::IntBufferViewBase;
#endif // !TEST_COMPILER_NVRTC

  __host__ __device__ constexpr Iter<false> begin() { return Iter<false>{buffer_}; }
  __host__ __device__ constexpr Iter<true> begin() const { return Iter<true>{buffer_}; }
  __host__ __device__ constexpr St<false> end() { return St<false>{buffer_ + size_}; }
  __host__ __device__ constexpr St<true> end() const { return St<true>{buffer_ + size_}; }
};
...
template <class I, class S>
inline constexpr bool HasEqual<I, S,
  cuda::std::void_t<decltype(cuda::std::declval<const I>() == cuda::std::declval<const S>())>> = true;
#endif
...
__host__ __device__ constexpr bool test_invoke() {  // test cuda::std::invoke is used
  struct Data {
    bool b;
  };
...
#if defined(_LIBCUDACXX_ADDRESSOF) \
 && !defined(TEST_COMPILER_GCC)    \
 && !defined(TEST_COMPILER_NVCC_BELOW_11_3)
  static_assert(test_invoke());
#endif // defined(_LIBCUDACXX_ADDRESSOF) && !defined(TEST_COMPILER_GCC) && !defined(TEST_COMPILER_NVCC_BELOW_11_3)

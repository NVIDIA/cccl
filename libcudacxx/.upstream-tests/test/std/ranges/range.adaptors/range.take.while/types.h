...
struct BufferViewBase : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr BufferViewBase(T (&b)[N]) : buffer_(b), size_(N) {}

  template <cuda::std::size_t N>
  __host__ __device__ constexpr BufferViewBase(cuda::std::array<T, N>& arr) : buffer_(arr.data()), size_(N) {}
};
...
struct SimpleView : IntBufferViewBase {
  ...
  SimpleView(T&& input) : IntBufferViewBase(cuda::std::forward<T>(input)) {}
#else // ^^^ TEST_COMPILER_NVRTC ^^^ / vvv !TEST_COMPILER_NVRTC vvv
  using IntBufferViewBase::IntBufferViewBase;
#endif // !TEST_COMPILER_NVRTC
  __host__ __device__ constexpr int* begin() const { return buffer_; }
  __host__ __device__ constexpr int* end() const { return buffer_ + size_; }
};
...
struct ConstNotRange : IntBufferViewBase {
  ...
  template <class T>
  __host__ __device__ constexpr
  ConstNotRange(T&& input) : IntBufferViewBase(cuda::std::forward<T>(input)) {}
#else // ^^^ TEST_COMPILER_NVRTC ^^^ / vvv !TEST_COMPILER_NVRTC vvv
  using IntBufferViewBase::IntBufferViewBase;
#endif // !TEST_COMPILER_NVRTC
  __host__ __device__ constexpr int* begin() { return buffer_; }
  __host__ __device__ constexpr int* end() { return buffer_ + size_; }
};
...
struct NonSimple : IntBufferViewBase {
  ...
  template <class T>
  __host__ __device__ constexpr
  NonSimple(T&& input) : IntBufferViewBase(cuda::std::forward<T>(input)) {}
#else // ^^^ TEST_COMPILER_NVRTC ^^^ / vvv !TEST_COMPILER_NVRTC vvv
  using IntBufferViewBase::IntBufferViewBase;
#endif // !TEST_COMPILER_NVRTC

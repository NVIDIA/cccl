...
struct move_only_input_iter_with_arrow {
  ...
  __host__ __device__ constexpr move_only_input_iter_with_arrow(Base it) : it_(cuda::std::move(it)) {}
...
struct move_iter_sentinel {
  ...
  __host__ __device__ constexpr move_iter_sentinel(Base it) : it_(cuda::std::move(it)) {}
  __host__ __device__ constexpr friend bool operator==(const move_iter_sentinel& s, const move_only_input_iter_with_arrow<Base>& other) { return s.it_ == other.it_; }
#if TEST_STD_VER < 20
  __host__ __device__ constexpr friend bool operator==(const move_only_input_iter_with_arrow<Base>& other, const move_iter_sentinel& s) { return s.it_ == other.it_; }
  __host__ __device__ constexpr friend bool operator!=(const move_iter_sentinel& s, const move_only_input_iter_with_arrow<Base>& other) { return s.it_ != other.it_; }
  __host__ __device__ constexpr friend bool operator!=(const move_only_input_iter_with_arrow<Base>& other, const move_iter_sentinel& s) { return s.it_ != other.it_; }
#endif
};
...
struct MoveOnlyIterInner : BufferView<move_only_input_iter_with_arrow<Box*>, move_iter_sentinel<Box*>> {
  ...
  MoveOnlyIterInner(T&& arr)
    noexcept(noexcept(BufferView<move_only_input_iter_with_arrow<Box*>, move_iter_sentinel<Box*>>(cuda::std::declval<T>())))
  ...
  __host__ __device__ sentinel end() const { return sentinel{data_ + size_}; }
};
...
struct arrow_input_iter {
  ...
  __host__ __device__ constexpr arrow_input_iter(Base it) : it_(cuda::std::move(it)) {}
  ...
  __host__ __device__ constexpr cuda::std::iter_reference_t<Base> operator*() const { return *it_; }
  __host__ __device__ constexpr auto operator->() const { return it_; }

#if TEST_STD_VER > 17
  __host__ __device__ friend constexpr bool operator==(const arrow_input_iter& x, const arrow_input_iter& y) = default;
#else
  __host__ __device__ friend constexpr bool operator==(const arrow_input_iter& x, const arrow_input_iter& y) { return x.it_ == y.it_; }
  __host__ __device__ friend constexpr bool operator!=(const arrow_input_iter& x, const arrow_input_iter& y) { return x.it_ != y.it_; }
#endif
};

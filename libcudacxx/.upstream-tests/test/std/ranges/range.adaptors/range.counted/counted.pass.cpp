...
concept CountedInvocable = requires (Ts&&... ts) {
  cuda::std::views::counted(cuda::std::forward<Ts>(ts)...);
};
#else
template<class... Ts>
...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    static_assert(cuda::std::addressof(cuda::std::views::counted) == cuda::std::addressof(cuda::std::ranges::views::counted));
...

  {
    ...
    using Expected = cuda::std::ranges::subrange<cuda::std::counted_iterator<decltype(it)>, cuda::std::default_sentinel_t>;
    using ConstExpected = cuda::std::ranges::subrange<cuda::std::counted_iterator<decltype(cit)>, cuda::std::default_sentinel_t>;

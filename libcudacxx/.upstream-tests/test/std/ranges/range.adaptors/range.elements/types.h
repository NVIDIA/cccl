...
struct BufferView : cuda::std::ranges::view_base {
  ...
  template <cuda::std::size_t N>
  __host__ __device__ constexpr BufferView(cuda::std::array<T, N>& arr) : buffer_(arr.data()), size_(N) {}
};
...
struct Common : TupleBufferView {
  ...
  template <class T>
  __host__ __device__ constexpr Common(T&& input) : TupleBufferView(cuda::std::forward<T>(input)) {}
#else
  ...
  __host__ __device__ constexpr const cuda::std::tuple<int>* begin() const { return buffer_; }

  template<bool Simple2 = Simple, cuda::std::enable_if_t<!Simple2, int> = 0>
  __host__ __device__ constexpr cuda::std::tuple<int>* end() {
    return buffer_ + size_;
  }
  __host__ __device__ constexpr const cuda::std::tuple<int>* end() const { return buffer_ + size_; }
};
...
#if defined(TEST_COMPILER_NVRTC) // nvbug 3961621
  template <class T>
  __host__ __device__ constexpr NonCommon(T&& input) : TupleBufferView(cuda::std::forward<T>(input)) {}
#else
  using TupleBufferView::TupleBufferView;
#endif
...
struct IterBase {
  ...
  __host__ __device__ constexpr cuda::std::tuple<int> operator*() const { return cuda::std::tuple<int>(5); }

  __host__ __device__ constexpr Derived& operator++() { return *this; }
...
#else
  __host__ __device__ friend constexpr bool operator==(const IterBase&, const IterBase&) noexcept { return true; }
  __host__ __device__ friend constexpr bool operator!=(const IterBase&, const IterBase&) noexcept { return false; }
#endif
};

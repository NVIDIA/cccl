...
template <class T, class U>
inline constexpr bool CanSubscript<T, U, cuda::std::void_t<decltype(cuda::std::declval<T>()[cuda::std::declval<U>()])>> = true;
#endif
...
__host__ __device__ constexpr bool test() {
  ...
  {
    // value
    auto ev = cuda::std::views::iota(0, 5) | cuda::std::views::transform([](int i) { return cuda::std::tuple<int>{i}; }) |

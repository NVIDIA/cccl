...
struct ConvertibleIter : IterBase<ConvertibleIter<Const>> {
  ...
  __host__ __device__ constexpr ConvertibleIter(int ii) : i(ii) {}
  template <bool otherConst, cuda::std::enable_if_t<Const != otherConst, int> = 0>
  __host__ __device__ constexpr ConvertibleIter(ConvertibleIter<otherConst> it) : movedFromOtherConst(true), i(it.i) {}
};

template <class Iter, class ConstIter>
struct BasicView : cuda::std::ranges::view_base {
  __host__ __device__ Iter begin() { return Iter{}; }
  __host__ __device__ Iter end() { return Iter{}; }

  __host__ __device__ ConstIter begin() const { return ConstIter{}; }
  __host__ __device__ ConstIter end() const { return ConstIter{}; }
};
...
using NonConvertibleView = BasicView<forward_iterator<cuda::std::tuple<int>*>, bidirectional_iterator<cuda::std::tuple<int>*>>;

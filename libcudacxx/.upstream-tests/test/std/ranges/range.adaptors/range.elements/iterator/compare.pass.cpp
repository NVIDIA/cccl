...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    auto ev = Subrange{It{&ts[0]}, It{&ts[0] + 1}} | cuda::std::views::elements<0>;
...

  {
    ...
    using Iter     = cpp20_input_iterator<cuda::std::tuple<int>*>;
    using Sent     = sentinel_wrapper<Iter>;
    using Subrange = cuda::std::ranges::subrange<Iter, Sent>;
#ifndef TEST_HAS_NO_SPACESHIP_OPERATOR
    using R        = cuda::std::ranges::elements_view<Subrange, 0>;
    static_assert(!cuda::std::three_way_comparable<cuda::std::ranges::iterator_t<R>>);
#endif

    auto ev = Subrange{Iter{&ts[0]}, Sent{Iter{&ts[0] + 1}}} | cuda::std::views::elements<0>;
    auto it = ev.begin();

...
template <class V, size_t N>
inline constexpr bool HasElementsView<V, N, cuda::std::void_t<typename cuda::std::ranges::elements_view<V, N>>> = true;
#endif
...
using NonMovableGenerator =
    decltype(cuda::std::views::iota(0, 1) | cuda::std::views::transform(ToPair{}));

...
struct Range : TupleBufferView {
  ...
  template <class T>
  __host__ __device__ constexpr Range(T&& input) : TupleBufferView(cuda::std::forward<T>(input)) {}
#else
...
template <class I, class S>
inline constexpr bool HasEqual<I, S, cuda::std::void_t<decltype(cuda::std::declval<const I>() == cuda::std::declval<const S>())>> = true;
#endif

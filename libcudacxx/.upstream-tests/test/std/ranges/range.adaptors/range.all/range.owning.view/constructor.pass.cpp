...
struct DefaultConstructible {
  int i;
  __host__ __device__ constexpr explicit DefaultConstructible(int j = 42) noexcept(false) : i(j) {}
...
struct NotDefaultConstructible {
  int i;
  __host__ __device__ constexpr explicit NotDefaultConstructible(int j) noexcept(false) : i(j) {}
  __host__ __device__ int *begin() const;
  __host__ __device__ int *end() const;
};
...
struct MoveChecker {
  int i;
  __host__ __device__ constexpr explicit MoveChecker(int j) : i(j) {}
  __host__ __device__ constexpr MoveChecker(MoveChecker&& v) : i(cuda::std::exchange(v.i, -1)) {}

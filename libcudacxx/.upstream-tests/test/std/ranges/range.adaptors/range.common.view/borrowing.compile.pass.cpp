...
struct Uncommon : cuda::std::ranges::view_base {
    ...
    __host__ __device__ cuda::std::unreachable_sentinel_t end() const { return cuda::std::unreachable_sentinel; }
};

static_assert(!cuda::std::ranges::borrowed_range<cuda::std::ranges::common_view<Uncommon>>);
static_assert(!cuda::std::ranges::borrowed_range<cuda::std::ranges::common_view<cuda::std::ranges::owning_view<Uncommon>>>);

...
__host__ __device__ constexpr bool test() {
  ...
  {
    // test ID 26
    cuda::std::ranges::zip_view v{NonSimpleNonSizedRandomAccessView(buffer1), NonSimpleNonSizedRandomAccessView(buffer3)};
    static_assert(!cuda::std::ranges::common_range<decltype(v)>);
    assert(v.begin() + 3 == v.end());
    static_assert(!cuda::std::is_same_v<decltype(v.end()), decltype(cuda::std::as_const(v).end())>);
  }
  ...
  {
    // test ID 36
    cuda::std::ranges::zip_view v{NonSimpleNonCommonRandomAcessSized(buffer1), NonSimpleNonCommonRandomAcessSized(buffer2)};
    static_assert(cuda::std::ranges::common_range<decltype(v)>);
    assert(v.begin() + 1 == v.end());
    static_assert(!cuda::std::is_same_v<decltype(v.end()), decltype(cuda::std::as_const(v).end())>);
  }

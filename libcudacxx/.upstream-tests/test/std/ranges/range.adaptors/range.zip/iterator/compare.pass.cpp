...
struct LessThanIterator {
  ...
  __host__ __device__ constexpr LessThanIterator(int* it) : it_(it) {}
  ...
  __host__ __device__ constexpr int& operator[](difference_type n) const { return it_[n]; }
  ...
  __host__ __device__ constexpr LessThanIterator operator++(int) { return LessThanIterator(it_++); }
  __host__ __device__ constexpr LessThanIterator operator--(int) { return LessThanIterator(it_--); }
  ...
  __host__ __device__ constexpr friend difference_type operator-(LessThanIterator x, LessThanIterator y) { return x.it_ - y.it_; }

  __host__ __device__ constexpr friend bool operator==(LessThanIterator const& x, LessThanIterator const& y) { return x.it_ == y.it_; }
...
struct SmallerThanRange : IntBufferView {
  ...
  __host__ __device__ constexpr SmallerThanRange(int (&b)[N]) : IntBufferView(b) {}
#else
  using IntBufferView::IntBufferView;
#endif
  __host__ __device__ constexpr LessThanIterator begin() const { return {buffer_}; }
  __host__ __device__ constexpr LessThanIterator end() const { return {buffer_ + size_}; }
};
...
struct ForwardCommonView : IntBufferView {
#if defined(TEST_COMPILER_NVRTC) // nvbug 3961621
  template <cuda::std::size_t N>
  __host__ __device__ constexpr ForwardCommonView(int (&b)[N]) : IntBufferView(b) {}
#else
  using IntBufferView::IntBufferView;
#endif
  ...
  __host__ __device__ constexpr iterator end() const { return iterator(buffer_ + size_); }
};

...
struct ForwardView : cuda::std::ranges::view_base {
  __host__ __device__ forward_iterator<T*> begin() const { return forward_iterator<T*>{nullptr}; }
  __host__ __device__ sentinel_wrapper<forward_iterator<T*>> end() const { return sentinel_wrapper<forward_iterator<T*>>{}; }
};

template <class T>
struct InputView : cuda::std::ranges::view_base {
  __host__ __device__ cpp17_input_iterator<T*> begin() const { return cpp17_input_iterator<T*>{nullptr}; }
  __host__ __device__ sentinel_wrapper<cpp17_input_iterator<T*>> end() const { return sentinel_wrapper<cpp17_input_iterator<T*>>{}; }
};
...
#else
  __host__ __device__ friend constexpr bool operator==(const DiffTypeIter&, const DiffTypeIter&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(const DiffTypeIter&, const DiffTypeIter&) { return false; }
#endif
};

template <class T>
struct DiffTypeRange {
  __host__ __device__ DiffTypeIter<T> begin() const { return DiffTypeIter<T>{}; }
  __host__ __device__ DiffTypeIter<T> end() const { return DiffTypeIter<T>{}; }
};
...
struct ConstVeryDifferentRange {
  ...
  __host__ __device__ forward_iterator<double*> begin() const { return forward_iterator<double*>{}; }
  __host__ __device__ forward_iterator<double*> end() const { return forward_iterator<double*>{}; }
};
...
__host__ __device__ void test() {
...

  {
    ...
    decltype(auto) it = v.begin();
    using Iter = decltype(it);
    static_assert(cuda::std::is_same_v<Iter::difference_type, cuda::std::common_type_t<intptr_t, cuda::std::ptrdiff_t>>);
    unused(it);
  }

...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  {
    ...
    static_assert(cuda::std::is_same_v<decltype(cuda::std::ranges::iter_move(v.begin())), cuda::std::tuple<int&&, const double&&, long>>);

    auto it = v.begin();
    static_assert(noexcept(cuda::std::ranges::iter_move(it)));
  }

#if !defined(TEST_COMPILER_NVCC_BELOW_11_3) && !defined(TEST_COMPILER_GCC)
  {
    struct ToThrowing {
      __host__ __device__ constexpr ThrowingMove operator()(int) const noexcept { return ThrowingMove{}; }
    };

    // underlying iter_move may throw
    auto throwingMoveRange = cuda::std::views::iota(0, 2) | cuda::std::views::transform(ThrowingMove{});
    cuda::std::ranges::zip_view v(throwingMoveRange);
    auto it = v.begin();
    static_assert(!noexcept(cuda::std::ranges::iter_move(it)));
  }
#endif // !defined(TEST_COMPILER_NVCC_BELOW_11_3) && !defined(TEST_COMPILER_GCC)

...
static_assert(cuda::std::constructible_from<cuda::std::ranges::zip_view<SimpleCommon, SimpleCommon>, SimpleCommon, SimpleCommon>);
static_assert(
...
struct MoveAwareView : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr MoveAwareView(MoveAwareView&& other) : moves(other.moves + 1) { other.moves = 1; }
  __host__ __device__ constexpr MoveAwareView& operator=(MoveAwareView&& other) {
    moves = other.moves + 1;
    other.moves = 0;
    return *this;
  }
  __host__ __device__ constexpr const int* begin() const { return &moves; }
  __host__ __device__ constexpr const int* end() const { return &moves + 1; }
};

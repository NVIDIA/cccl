...
struct View : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr View(cuda::std::size_t s) : size_(s) {}
  __host__ __device__ constexpr auto begin() const { return buffer; }
  __host__ __device__ constexpr auto end() const { return buffer + size_; }
};

struct SizedNonConst : cuda::std::ranges::view_base {
  using iterator = forward_iterator<int*>;
  cuda::std::size_t size_ = 0;
  __host__ __device__ constexpr SizedNonConst(cuda::std::size_t s) : size_(s) {}
  __host__ __device__ constexpr auto begin() const { return iterator{buffer}; }
  __host__ __device__ constexpr auto end() const { return iterator{buffer + size_}; }
  __host__ __device__ constexpr cuda::std::size_t size() { return size_; }
};

struct StrangeSizeView : cuda::std::ranges::view_base {
  __host__ __device__ constexpr auto begin() const { return buffer; }
  __host__ __device__ constexpr auto end() const { return buffer + 8; }

  __host__ __device__ constexpr auto size() { return 5; }
  __host__ __device__ constexpr auto size() const { return 6; }
};

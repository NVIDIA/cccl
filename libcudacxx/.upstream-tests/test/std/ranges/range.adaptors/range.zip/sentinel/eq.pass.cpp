...
struct ComparableView :  IntBufferView {
  ...
  __host__ __device__ constexpr ComparableView(int (&b)[N]) : IntBufferView(b) {}
#else
...
struct ConstIncompatibleView : cuda::std::ranges::view_base {
  __host__ __device__ cpp17_input_iterator<int*> begin() { return cpp17_input_iterator<int*>{nullptr}; }
  __host__ __device__ forward_iterator<const int*> begin() const { return forward_iterator<const int*>{nullptr}; }
  __host__ __device__ sentinel_wrapper<cpp17_input_iterator<int*>> end() { return sentinel_wrapper<cpp17_input_iterator<int*>>{}; }
  __host__ __device__ sentinel_wrapper<forward_iterator<const int*>> end() const { return sentinel_wrapper<forward_iterator<const int*>>{}; }
};

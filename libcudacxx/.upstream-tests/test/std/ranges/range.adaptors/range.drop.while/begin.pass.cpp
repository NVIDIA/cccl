...
struct always {
  template<class... Args>
  __host__ __device__ constexpr bool operator()(Args&&...) const { return Ret; }
};

struct TrackingPred {
  __host__ __device__ constexpr explicit TrackingPred(bool* moved, bool* copied) : moved_(moved), copied_(copied) {}
  __host__ __device__ constexpr TrackingPred(TrackingPred const& other) : moved_(other.moved_), copied_(other.copied_) {
    *copied_ = true;
  }
  __host__ __device__ constexpr TrackingPred(TrackingPred&& other) : moved_(other.moved_), copied_(other.copied_) { *moved_ = true; }
...
__host__ __device__ constexpr void testOne() {
  ...
  // Make sure we do not make a copy of the predicate when we call begin()
  {
    ...
    auto range = make_subrange<Range, Iter, Sent>(buffer);
    cuda::std::ranges::drop_while_view<decltype(range), TrackingPred> dwv{cuda::std::move(range), TrackingPred(&moved, &copied)};
    moved                    = false;
    copied                   = false;
    auto it = dwv.begin();
    assert(!moved);
  ...
  {
    auto mutable_pred = [](int& i) mutable { return i < 3; };
    int buffer[] = {1, 2, 3, 2, 1};
    auto range   = make_subrange<Range, Iter, Sent>(buffer);
    ...
    if constexpr(cuda::std::indirect_unary_predicate<const decltype(mutable_pred), Iter>) {
      cuda::std::ranges::drop_while_view<decltype(range), decltype(mutable_pred)> dwv{cuda::std::move(range), mutable_pred};
      decltype(auto) it = dwv.begin();
      static_assert(cuda::std::same_as<decltype(it), Iter>);
      assert(base(it) == buffer + 2);
    }

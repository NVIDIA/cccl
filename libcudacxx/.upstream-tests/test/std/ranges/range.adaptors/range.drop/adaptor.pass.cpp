...
struct SizedView : cuda::std::ranges::view_base {
  int* begin_ = nullptr;
  int* end_ = nullptr;
  __host__ __device__ constexpr SizedView(int* begin, int* end) : begin_(begin), end_(end) {}

  __host__ __device__ constexpr auto begin() const { return forward_iterator<int*>(begin_); }
  __host__ __device__ constexpr auto end() const { return sized_sentinel<forward_iterator<int*>>(forward_iterator<int*>(end_)); }
};
...
struct SizedViewWithUnsizedSentinel : cuda::std::ranges::view_base {
  ...
  int* begin_ = nullptr;
  int* end_ = nullptr;
  __host__ __device__ constexpr SizedViewWithUnsizedSentinel(int* begin, int* end) : begin_(begin), end_(end) {}

  __host__ __device__ constexpr auto begin() const { return iterator(begin_); }
  __host__ __device__ constexpr auto end() const { return sentinel(iterator(end_)); }
  __host__ __device__ constexpr size_t size() const { return end_ - begin_; }
};
...
static_assert(!cuda::std::sized_sentinel_for<SizedViewWithUnsizedSentinel::sentinel, SizedViewWithUnsizedSentinel::iterator>);
...
struct Pred {
  __host__ __device__ constexpr int operator()(int i) noexcept { return i; }
};
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  ...
  // `views::drop(subrange, n)` returns a `subrange` when `subrange::StoreSize == true`.
  {
    ...
    using Subrange = cuda::std::ranges::subrange<View::iterator, View::sentinel, cuda::std::ranges::subrange_kind::sized>;
  ...
  // `views::drop(subrange, n)` doesn't return a `subrange` if it's not a random access range.
  {
    ...
    using Result = cuda::std::ranges::drop_view<cuda::std::ranges::subrange<forward_iterator<int*>,
  ...
  // Test that it's possible to call `cuda::std::views::drop` with any single argument as long as the resulting closure is

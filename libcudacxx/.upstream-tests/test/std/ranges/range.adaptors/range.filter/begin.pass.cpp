...
struct Range : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr explicit Range(int* b, int* e) : begin_(b), end_(e) { }
  __host__ __device__ constexpr Iterator begin() const { return Iterator(begin_); }
  __host__ __device__ constexpr Sentinel end() const { return Sentinel(Iterator(end_)); }
...
struct InputRange : cuda::std::ranges::view_base {
  ...
  using Sentinel = sentinel_wrapper<Iterator>;
  __host__ __device__ constexpr explicit InputRange(int* b, int* e) : begin_(b), end_(e) { }
  __host__ __device__ constexpr Iterator begin() const { return Iterator(begin_); }
  __host__ __device__ constexpr Sentinel end() const { return Sentinel(Iterator(end_)); }
...
struct TrackingPred : TrackInitialization {
  ...
  __host__ __device__ constexpr bool operator()(int i) const { return i % 2 == 0; }
};

...
// unsigned first).
//     const cuda::std::ranges::iota_view<int, int> io(cuda::std::numeric_limits<int>::min(), cuda::std::numeric_limits<int>::max());
//     assert(io.size() == (static_cast<unsigned>(cuda::std::numeric_limits<int>::max()) * 2) + 1);
  }

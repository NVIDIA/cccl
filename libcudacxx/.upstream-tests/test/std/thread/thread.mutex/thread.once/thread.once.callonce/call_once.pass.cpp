...
__host__ __device__ void test() {
...

    {
        ...
        cuda::std::call_once(*f, MoveOnly(), MoveOnly());
    }
    // check LWG2442: call_once() shouldn't DECAY_COPY()
    {
        ...
        cuda::std::call_once(*f, NonCopyable(), i);
    }
    // reference qualifiers on functions are a C++11 extension
    {
        ...
        RefQual rq;
        cuda::std::call_once(*f1, rq, *lv_called);
        assert(*lv_called == 1);
        cuda::std::call_once(*f2, cuda::std::move(rq), *rv_called);
        assert(*rv_called == 1);
    }
#endif // TEST_STD_VER >= 11
}

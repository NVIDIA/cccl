...
public:
    ...
    __host__ __device__ explicit constexpr ConvertibleTo(int *it) : it_(it) {}

    __host__ __device__ constexpr reference operator*() const {return *it_;}

    __host__ __device__ constexpr ConvertibleTo& operator++() {++it_; return *this;}
    __host__ __device__ constexpr ConvertibleTo operator++(int)
        {ConvertibleTo tmp(*this); ++(*this); return tmp;}

    __host__ __device__ constexpr operator T() const { return T(it_); }
};

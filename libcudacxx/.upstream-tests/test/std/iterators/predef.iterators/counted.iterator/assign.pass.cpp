...
public:
    ...
    __host__ __device__ friend constexpr int *base(const AssignableFromIter& i) {return i.it_;}

    AssignableFromIter() = default;
    __host__ __device__ explicit constexpr AssignableFromIter(int *it) : it_(it) {}
    __host__ __device__ constexpr AssignableFromIter(const forward_iterator<int*>& it) : it_(base(it)) {}
    ...
    __host__ __device__ constexpr AssignableFromIter& operator++() {++it_; return *this;}
...
struct InputOrOutputArchetype {
  ...
  __host__ __device__ int operator*() { return *ptr; }
  __host__ __device__ void operator++(int) { ++ptr; }
  __host__ __device__ InputOrOutputArchetype& operator++() { ++ptr; return *this; }
};

...
public:
  ...
  __host__ __device__ constexpr int *base() const {return it_;}

  HasNoexceptIterMove() = default;
  __host__ __device__ explicit constexpr HasNoexceptIterMove(int *it) : it_(it) {}

  __host__ __device__ constexpr reference operator*() const noexcept(IsNoexcept) { return *it_; }

  __host__ __device__ constexpr HasNoexceptIterMove& operator++() {++it_; return *this;}
...
__host__ __device__ constexpr bool test() {
  ...
  // Test noexceptness.
  {
    static_assert( noexcept(cuda::std::ranges::iter_move(cuda::std::declval<cuda::std::counted_iterator<HasNoexceptIterMove<true>>>())));
    static_assert(!noexcept(cuda::std::ranges::iter_move(cuda::std::declval<cuda::std::counted_iterator<HasNoexceptIterMove<false>>>())));
  }

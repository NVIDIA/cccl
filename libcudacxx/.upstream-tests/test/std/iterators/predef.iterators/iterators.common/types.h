...
public:
  ...
  __host__ __device__ friend constexpr It base(const simple_iterator& i) {return i.it_;}

  simple_iterator() = default;
  __host__ __device__ explicit constexpr simple_iterator(It it) : it_(it) {}

  __host__ __device__ constexpr reference operator*() const {return *it_;}

  __host__ __device__ constexpr simple_iterator& operator++() {++it_; return *this;}
...
public:
  ...
  __host__ __device__ friend constexpr It base(const value_iterator& i) {return i.it_;}

  value_iterator() = default;
  __host__ __device__ explicit constexpr value_iterator(It it) : it_(it) {}

  __host__ __device__ constexpr value_type operator*() const {return cuda::std::move(*it_);}

  __host__ __device__ constexpr value_iterator& operator++() {++it_; return *this;}
...
public:
  ...
  __host__ __device__ friend constexpr It base(const void_plus_plus_iterator& i) {return i.it_;}

  void_plus_plus_iterator() = default;
  __host__ __device__ explicit constexpr void_plus_plus_iterator(It it) : it_(it) {}

  __host__ __device__ constexpr value_type operator*() const {return cuda::std::move(*it_);}

  __host__ __device__ constexpr void_plus_plus_iterator& operator++() {++it_; return *this;}
...
public:
  ...
  struct hold {
    T value_;
    __host__ __device__ hold(T v) : value_(v) {}
  ...
  __host__ __device__ friend constexpr It base(const value_type_not_move_constructible_iterator& i) {return i.it_;}

  value_type_not_move_constructible_iterator() = default;
  __host__ __device__ explicit constexpr value_type_not_move_constructible_iterator(It it) : it_(it) {}

  __host__ __device__ constexpr underlying_value_type operator*() const {return cuda::std::move(*it_);}

  __host__ __device__ constexpr value_type_not_move_constructible_iterator& operator++() {++it_; return *this;}
  __host__ __device__ constexpr void operator++(int) {++(*this);}
};
...
public:
  ...
  __host__ __device__ friend constexpr It base(const comparable_iterator& i) {return i.it_;}

  comparable_iterator() = default;
  __host__ __device__ explicit constexpr comparable_iterator(It it) : it_(it) {}

  __host__ __device__ constexpr reference operator*() const {return *it_;}

  __host__ __device__ constexpr comparable_iterator& operator++() {++it_; return *this;}
...
struct sentinel_type {
  ...
  template<class U>
  __host__ __device__ friend constexpr bool operator==(const sentinel_type& lhs, const U& rhs) { return lhs.base_ == base(rhs); }
  template<class U>
  __host__ __device__ friend constexpr bool operator==(const U& lhs, const sentinel_type& rhs) { return base(lhs) == rhs.base_; }
#if TEST_STD_VER < 20
  template<class U>
  __host__ __device__ friend constexpr bool operator!=(const sentinel_type& lhs, const U& rhs) { return lhs.base_ != base(rhs); }
  template<class U>
...
struct sized_sentinel_type {
  ...
  template<class U>
  __host__ __device__ friend constexpr bool operator==(const sized_sentinel_type& lhs, const U& rhs) { return lhs.base_ == base(rhs); }
  template<class U>
  __host__ __device__ friend constexpr bool operator==(const U& lhs, const sized_sentinel_type& rhs) { return base(lhs) == rhs.base_; }
#if TEST_STD_VER < 20
  template<class U>
  __host__ __device__ friend constexpr bool operator!=(const sized_sentinel_type& lhs, const U& rhs) { return lhs.base_ != base(rhs); }
  template<class U>
  ...
  template<class U>
  __host__ __device__ friend constexpr auto operator-(const sized_sentinel_type& lhs, const U& rhs) { return lhs.base_ - base(rhs); }
  template<class U>
  __host__ __device__ friend constexpr auto operator-(const U& lhs, const sized_sentinel_type& rhs) { return base(lhs) - rhs.base_; }
};
...
public:
    ...
    __host__ __device__ friend constexpr It base(const assignable_iterator& i) {return i.it_;}

    assignable_iterator() = default;
    __host__ __device__ explicit constexpr assignable_iterator(It it) : it_(it) {}

    __host__ __device__ constexpr assignable_iterator(const forward_iterator<It>& it) : it_(base(it)) {}
    __host__ __device__ constexpr assignable_iterator(const sentinel_type<It>& it) : it_(base(it)) {}

    __host__ __device__ constexpr reference operator*() const {return *it_;}

    __host__ __device__ constexpr assignable_iterator& operator++() {++it_; return *this;}

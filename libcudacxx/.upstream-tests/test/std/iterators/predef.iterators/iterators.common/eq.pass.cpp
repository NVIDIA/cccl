...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    auto commonSent1 = cuda::std::common_iterator<decltype(iter1), sentinel_type<int*>>(sentinel_type<int*>{buffer + 8});

    const auto commonIter2 = cuda::std::common_iterator<decltype(iter1), sentinel_type<int*>>(iter1);
    const auto commonSent2 = cuda::std::common_iterator<decltype(iter1), sentinel_type<int*>>(sentinel_type<int*>{buffer + 8});
  ...
  }
  {
    ...
    const auto commonIter2 = cuda::std::common_iterator<decltype(iter2), sentinel_type<int*>>(iter2);
    const auto commonSent2 = cuda::std::common_iterator<decltype(iter2), sentinel_type<int*>>(sentinel_type<int*>{buffer + 8});

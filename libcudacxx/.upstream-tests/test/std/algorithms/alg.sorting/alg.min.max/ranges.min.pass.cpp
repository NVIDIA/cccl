...
#if TEST_STD_VER > 17
template <class T>
concept HasMinR = requires (T t) { cuda::std::ranges::min(cuda::std::declval<T>()); };
#else
template<class T, class = void>
constexpr bool HasMinR = false;

template<class T>
constexpr bool HasMinR<T, cuda::std::void_t<decltype(cuda::std::ranges::min(cuda::std::declval<T>()))>> = true;
#endif
...
constexpr bool HasMin2<T, U, cuda::std::void_t<decltype(cuda::std::ranges::min(cuda::std::declval<T>(), cuda::std::declval<U>()))>> = true;
#endif
...
__host__ __device__ constexpr void test_2_arguments() {
  ...
  // test projection
  assert(cuda::std::ranges::min(1, 2, cuda::std::ranges::less{}, [](int i){ return i == 1 ? 10 : i; }) == 2);

...
template<class T>
constexpr bool HasMinElement<T, cuda::std::void_t<decltype(cuda::std::ranges::min_element(cuda::std::declval<T>()))>> = true;
#endif

struct NoLessThanOp {};
struct NotTotallyOrdered {
  int i;
  __host__ __device__ bool operator<(const NotTotallyOrdered& o) const { return i < o.i; }
};
...
__host__ __device__ constexpr void test_projection() {
  int a[] = {7, 6, 9, 3, 5, 1, 2, 4};
  {
    int* ret = cuda::std::ranges::min_element(a, cuda::std::ranges::less{}, [](int i) { return i == 5 ? -100 : i; });
    assert(ret == a + 4);
    assert(*ret == 5);
  }
  {
    int* ret = cuda::std::ranges::min_element(a, cuda::std::less<int*>{}, [](int& i) { return &i; });
    assert(ret == a);
    assert(*ret == 7);
  }
}
...
struct Immobile {
  ...
  __host__ __device__ constexpr Immobile(int i_) : i(i_) {}
  Immobile(const Immobile&) = delete;
  Immobile(Immobile&&) = delete;

  __host__ __device__ bool operator==(const Immobile& lhs) const { return i == lhs.i; }
  __host__ __device__ bool operator!=(const Immobile& lhs) const { return i != lhs.i; }

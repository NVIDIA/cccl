...
inline constexpr bool HasFindIfIt<It, Sent,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if(cuda::std::declval<It>(), cuda::std::declval<Sent>(), cuda::std::declval<Predicate>()))>> = true;
#endif
...
template <class Pred>
inline constexpr bool HasFindIfPred<Pred,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if(static_cast<int*>(nullptr), static_cast<int*>(nullptr), cuda::std::declval<Pred>()))>> = true;
#endif
...
template <class R>
inline constexpr bool HasFindIfR<R,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if(cuda::std::declval<R>(), cuda::std::declval<Predicate>()))>> = true;
#endif
...
__host__ __device__ constexpr void test_iterators() {
  {
    int a[] = {1, 2, 3, 4};
    decltype(auto) ret = cuda::std::ranges::find_if(It(a), Sent(It(a + 4)), [](int x) mutable { return x == 4; });
  ...
  }
  {
    ...
    decltype(auto) ret = cuda::std::ranges::find_if(range, [](int x) mutable { return x == 4; });
    static_assert(cuda::std::same_as<decltype(ret), It>);
    assert(base(ret) == a + 3);
    assert(*ret == 4);
  }
...
struct NonConstComparable {
  __host__ __device__ friend constexpr bool operator==(const NonConstComparable&, const NonConstComparable&) { return false; }
  __host__ __device__ friend constexpr bool operator==(NonConstComparable&, NonConstComparable&) { return false; }
  __host__ __device__ friend constexpr bool operator==(const NonConstComparable&, NonConstComparable&) { return false; }
  __host__ __device__ friend constexpr bool operator==(NonConstComparable&, const NonConstComparable&) { return true; }
#if TEST_STD_VER < 20
  __host__ __device__ friend constexpr bool operator!=(const NonConstComparable&, const NonConstComparable&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(NonConstComparable&, NonConstComparable&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(const NonConstComparable&, NonConstComparable&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(NonConstComparable&, const NonConstComparable&) { return false; }
#endif
};
...
struct CheckStar {
  template <class T>
  __host__ __device__ constexpr bool operator()(T&& e) const { return e == NonConstComparable{}; }
};
...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    };
    {
      ...
      auto ret = cuda::std::ranges::find_if(a, a + 4, CountPredicate{predicate_count}, CountProjection{projection_count});
      assert(ret == a + 1);
      assert(*ret == 2);

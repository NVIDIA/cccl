...
inline constexpr bool HasFindIfNotIt<It, Sent,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if_not(cuda::std::declval<It>(), cuda::std::declval<Sent>(), cuda::std::declval<Predicate>()))>> = true;
#endif
...
template <class Pred>
inline constexpr bool HasFindIfNotPred<Pred,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if_not(static_cast<int*>(nullptr), static_cast<int*>(nullptr), cuda::std::declval<Pred>()))>> = true;
#endif
...
template <class R>
inline constexpr bool HasFindIfNotR<R,
  cuda::std::void_t<decltype(cuda::std::ranges::find_if_not(cuda::std::declval<R>(), cuda::std::declval<Predicate>()))>> = true;
#endif
...
__host__ __device__ constexpr void test_iterators() {
  {
    int a[] = {1, 2, 3, 4};
    decltype(auto) ret = cuda::std::ranges::find_if_not(It(a), Sent(It(a + 4)), [c = 0](int) mutable { return c++ <= 2; });
  ...
  }
  {
    ...
    decltype(auto) ret = cuda::std::ranges::find_if_not(range, [c = 0](int) mutable { return c++ <= 2; });
    static_assert(cuda::std::same_as<decltype(ret), It>);
    assert(base(ret) == a + 3);
    assert(*ret == 4);
  }
...
struct NonConstComparableLValue {
  __host__ __device__ friend constexpr bool operator==(const NonConstComparableLValue&, const NonConstComparableLValue&) { return false; }
  __host__ __device__ friend constexpr bool operator==(NonConstComparableLValue&, NonConstComparableLValue&) { return false; }
  __host__ __device__ friend constexpr bool operator==(const NonConstComparableLValue&, NonConstComparableLValue&) { return false; }
  __host__ __device__ friend constexpr bool operator==(NonConstComparableLValue&, const NonConstComparableLValue&) { return true; }
#if TEST_STD_VER < 20
  __host__ __device__ friend constexpr bool operator!=(const NonConstComparableLValue&, const NonConstComparableLValue&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(NonConstComparableLValue&, NonConstComparableLValue&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(const NonConstComparableLValue&, NonConstComparableLValue&) { return true; }
  __host__ __device__ friend constexpr bool operator!=(NonConstComparableLValue&, const NonConstComparableLValue&) { return false; }
#endif
};

struct AlwaysFalse {
  __host__ __device__ constexpr bool operator()(int) const { return false; }
};
struct AlwaysTrue {
  __host__ __device__ constexpr bool operator()(int) const { return true; }
};
struct CheckStar {
  template<class T>
  __host__ __device__ constexpr bool operator()(T&& e) const { return e != NonConstComparableLValue{}; }
};
...
__host__ __device__ constexpr bool test() {
  ...
  { // check that projections are used properly and that they are called with the iterator directly
    struct ToAddress {
      __host__ __device__ constexpr int* operator()(int& i) const { return &i; }
    };
    struct PointsToLast {
      int* a;
      __host__ __device__ constexpr bool operator()(int* i) const { return i != a + 3; }
    };
    {
      int a[] = {1, 2, 3, 4};
...

  {
    ...
    struct NotZero {
      __host__ __device__ constexpr bool operator()(int i) const { return i != 0; }
    };
    {
      struct S {
        int comp;
  ...
  { // count projection and predicate invocation count
    struct CountPredicate {
      int& predicate_count;
      __host__ __device__ constexpr bool operator()(int i) const { ++predicate_count; return i != 2; }
    };
    struct CountProjection {
      int& projection_count;
      __host__ __device__ constexpr int operator()(int i) const { ++projection_count; return i; }
    };

    {
      ...
      auto ret = cuda::std::ranges::find_if_not(a, a + 4, CountPredicate{predicate_count}, CountProjection{projection_count});
      assert(ret == a + 1);
      assert(*ret == 2);
...

  {
    ...
    struct ReturnBooleanTestable {
      __host__ __device__ constexpr BooleanTestable operator()(const int& i) const { return BooleanTestable{i != 3}; }
    };
    {
      int a[] = {1, 2, 3, 4};

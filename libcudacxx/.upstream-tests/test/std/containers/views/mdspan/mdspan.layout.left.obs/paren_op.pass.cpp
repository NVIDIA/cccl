...
int main(int, char**)
{
...

    {
        ...
        cuda::std::layout_left::mapping<cuda::std::extents<index_t,dyn,dyn>> m{e};

        assert( m(2,1) == 2*1 + 1*16 );
    }

    {
        cuda::std::extents<index_t,dyn,dyn,dyn> e{16, 32, 8};
        cuda::std::layout_left::mapping<cuda::std::extents<index_t,dyn,dyn,dyn>> m{e};

        assert( m(2,1,3) == 2*1 + 1*16 + 3*16*32 );
    }
    ...
    // Indices are of a type implicitly convertible to index_type
    {
        ...
        assert( m(my_int(2),my_int(1)) == 2*1 + 1*16 );
    }

...
bool tests()
{
    ...
    }
    {
        ...
        typename C::iterator i = array.begin();
        typename C::const_iterator j = array.cbegin();
#ifndef TEST_COMPILER_NVCC_BELOW_11_3 // seems there are different nullptr's
        assert(i == j);
#else // ^^^ !TEST_COMPILER_NVCC_BELOW_11_3 ^^^ / vvv TEST_COMPILER_NVCC_BELOW_11_3 vvv
        assert(i == nullptr);
        assert(j == nullptr);
...
#ifndef TEST_COMPILER_NVCC_BELOW_11_3 // old NVCC has issues comparing int* with const int*
            assert(ii1 == cii);
#else // ^^^ !TEST_COMPILER_NVCC_BELOW_11_3 ^^^ / vvv TEST_COMPILER_NVCC_BELOW_11_3 vvv
            assert(ii1 == nullptr);
            assert(cii == nullptr);

...
struct fake_rng
{
  ...
  [[nodiscard]] TEST_HOST_DEVICE_FUNC constexpr result_type operator()() noexcept
  {
    return __random_indices[__start++ % 5];
  }

  // Needed for uniform_int_distribution
  [[nodiscard]] TEST_HOST_DEVICE_FUNC static constexpr result_type min() noexcept
  {
    return 0;
  }

  [[nodiscard]] TEST_HOST_DEVICE_FUNC static constexpr result_type max() noexcept
  {
    return 5;
  }
...
struct fake_bijection
{
  ...
  TEST_HOST_DEVICE_FUNC constexpr fake_bijection(index_type, RNG&&) noexcept {}

  [[nodiscard]] TEST_HOST_DEVICE_FUNC constexpr index_type size() const noexcept(HasNothrowCallOperator)
  {
    return 5;
  }

  [[nodiscard]] TEST_HOST_DEVICE_FUNC constexpr index_type operator()(index_type n) const
    noexcept(HasNothrowCallOperator)
  {

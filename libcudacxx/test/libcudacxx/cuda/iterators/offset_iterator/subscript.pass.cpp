...
__host__ __device__ constexpr bool test()
{
  ...
  {
    const int offset = 5;
    const cuda::offset_iterator iter(random_access_iterator<int*>{buffer}, offset);
    assert(iter[2] = buffer[offset + 2]);
    static_assert(cuda::std::is_same_v<int&, decltype(iter[0])>);
  }
  ...
  {
    const int offset = 5;
    const cuda::offset_iterator iter(random_access_iterator<int*>{buffer}, random_access_iterator<const int*>{&offset});
    assert(iter[2] = buffer[offset + 2]);
    static_assert(cuda::std::is_same_v<int&, decltype(iter[0])>);
  }

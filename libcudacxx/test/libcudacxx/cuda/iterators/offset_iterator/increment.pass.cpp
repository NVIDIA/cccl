...
__host__ __device__ constexpr bool test()
{
  ...
  {
    const int offset[]    = {2, 4, 6};
    using offset_iterator = cuda::offset_iterator<random_access_iterator<int*>, random_access_iterator<const int*>>;
    offset_iterator iter(random_access_iterator<int*>{buffer}, random_access_iterator<const int*>{offset});
  ...
  {
    static_assert(PlusEnabled<cuda::offset_iterator<random_access_iterator<int*>>>);
    static_assert(!PlusEnabled<const cuda::offset_iterator<random_access_iterator<int*>>>);
  }

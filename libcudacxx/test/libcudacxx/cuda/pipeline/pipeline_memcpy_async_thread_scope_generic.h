...
TEST_HOST_DEVICE_FUNC cuda::pipeline<scope> get_pipeline(cuda::pipeline_shared_state<scope, PipelineStages>* pipe_state)
{
  NV_DISPATCH_TARGET(NV_IS_DEVICE,
                     (auto group = cooperative_groups::this_thread_block(); return make_pipeline(group, pipe_state);),
                     NV_IS_HOST,
                     (auto group = cuda::__single_thread_group{}; return make_pipeline(group, pipe_state);))
}
...
TEST_HOST_DEVICE_FUNC __noinline__ void test_fully_specialized()
{
...
TEST_HOST_DEVICE_FUNC __noinline__ void test_select_destination()
{

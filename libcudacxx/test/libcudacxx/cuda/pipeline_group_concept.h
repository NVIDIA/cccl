...
struct single_thread_test_group {
  ...
  __host__ __device__ T_size size() const { return 1; }
  __host__ __device__ T_thread_rank thread_rank() const { return 0; }
};
...
__host__ __device__ void test_fully_specialized() {
  ...
  auto pipe_partitioned_explicit_consumer =
      make_pipeline(group, &shared_state, cuda::pipeline_role::consumer);
  auto pipe_partitioned_explicit_producer =
      make_pipeline(group, &shared_state, cuda::pipeline_role::producer);
  ...
  memcpy_async(group, &dst[1], &src[1], cuda::aligned_size_t<1>(1),
               pipe_unified);

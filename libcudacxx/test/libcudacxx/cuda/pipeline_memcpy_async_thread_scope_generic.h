...
__host__ __device__ __noinline__ void test_fully_specialized() {
  ...
  typename DestSelector<T, constructor_initializer>::template offsetted<
      ...
      dest_sel;
  PipelineSelector<cuda::pipeline_shared_state<Scope, PipelineStages>,
                   constructor_initializer>
      pipe_state_sel;

  T* source = source_sel.construct(static_cast<T>(12));
  T* dest = dest_sel.construct(static_cast<T>(0));
  cuda::pipeline_shared_state<Scope, PipelineStages>* pipe_state =
  ...
  *source = 42;
  ...
  cuda::memcpy_async(static_cast<void*>(dest), static_cast<void*>(source),
                     sizeof(T), pipe);
  pipe.producer_commit();
  pipe.consumer_wait_until(cuda::std::chrono::system_clock::now() +
                           cuda::std::chrono::seconds(30));
...
__host__ __device__ __noinline__ void test_select_pipeline() {
  ...
  test_fully_specialized<Scope, T, SourceSelector, DestSelector,
                         local_memory_selector, stages_count>();
  NV_IF_TARGET(
      NV_IS_DEVICE,
      (test_fully_specialized<Scope, T, SourceSelector, DestSelector,
                              shared_memory_selector, stages_count>();
       test_fully_specialized<Scope, T, SourceSelector, DestSelector,
                              global_memory_selector, stages_count>();))
}
...
__host__ __device__ __noinline__ void test_select_destination() {
  ...
  NV_IF_TARGET(
      ...
      (test_select_pipeline<Scope, T, SourceSelector, shared_memory_selector>();
       test_select_pipeline<Scope, T, SourceSelector,

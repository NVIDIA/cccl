...
struct test_murmurhash3_x64_128
{
  ...
  TEST_FUNC constexpr cuda::std::array<cuda::std::uint64_t, 2> make_result(cuda::std::array<cuda::std::uint64_t, 2> arr)
  {
    return arr;
  }
...
TEST_FUNC void test()
{
  ...
  test_empty_span<cuda::hash_algorithm::murmurhash3_x64_128>(cuda::std::array<cuda::std::uint64_t, 2>{});
}

...
template <typename T>
__device__ void
arrive_on_device_copy(T* global_array, T* shared_array, unsigned copy_count,
...
__device__ bool arrive_on_test(char* global_buffer, size_t buffer_size) {
  ...
  for (unsigned copy_count = 2; copy_count <= max_copy_count;
...
__device__ void arrive_on_nvrtc(size_t buffer_size) {
  auto scramble_buffer = [](char* buffer, size_t buffer_size,
                            size_t base_value) {
    if (threadIdx.x == 0) {
...
void arrive_on_launch(char* global_buffer, size_t buffer_size,
                      ...
                      unsigned block_size) {
  *success_hptr = false;
  printf("arrive_on_kernel<%2zu><<<%u, %2u, %zu>>> ", sizeof(T), 1, block_size,
         buffer_size);
...
void arrive_on_run(char* global_buffer, size_t buffer_size, bool* success_dptr,
                   volatile bool* success_hptr, unsigned block_size) {
  arrive_on_launch<char>(global_buffer, buffer_size, success_dptr, success_hptr,
                         block_size);
  arrive_on_launch<short>(global_buffer, buffer_size, success_dptr,
                          success_hptr, block_size);
  arrive_on_launch<int>(global_buffer, buffer_size, success_dptr, success_hptr,
                        block_size);
  arrive_on_launch<int2>(global_buffer, buffer_size, success_dptr, success_hptr,
                         block_size);
  arrive_on_launch<int4>(global_buffer, buffer_size, success_dptr, success_hptr,
                         block_size);
}
...
void arrive_on() {
  ...
  cudaError_t result;
  CUDA_CALL(result, cudaHostAlloc(&success_hptr, sizeof(*success_hptr),
                                  cudaHostAllocMapped));
  CUDA_CALL(result,
            cudaHostGetDevicePointer(&success_dptr, (void*)success_hptr, 0));
  CUDA_CALL(result,
            cudaDeviceGetAttribute(&lanes_per_warp, cudaDevAttrWarpSize, 0));
  CUDA_CALL(result, cudaDeviceGetAttribute(
                        &max_shmem, cudaDevAttrMaxSharedMemoryPerBlock, 0));
  ...
  // 1 Thread
  {
    ...
    arrive_on_run(global_buffer, buffer_size, success_dptr, success_hptr,
                  block_size);
  }
...
int main(int argc, char** argv) {
  ...
  arrive_on_nvrtc(cuda_block_shmem_size -
                  sizeof(cuda::barrier<cuda::thread_scope_block>) -
                  sizeof(char*));

          ...
          if (__isShared(cuda::device::barrier_native_handle(b))) {
            asm volatile(
                ...
                : "r"((unsigned int)__cvta_generic_to_shared(
                      cuda::device::barrier_native_handle(b))),
                  "r"(transaction_count)
                : "memory");
          } else { __trap(); }),
      NV_ANY_TARGET,
...
__device__ void thread(cuda::barrier<cuda::thread_scope_block>& b,
                       int arrives_per_thread) {
...
__device__ void test() {
  NV_DISPATCH_TARGET(
      ...
      (
          ...
          barrier_t* bar_1 = sel_1.construct(blockDim.x); __syncthreads();
          ...
          barrier_t* bar_2 = sel_2.construct(2 * blockDim.x); __syncthreads();

...
tensor_len(cuda::std::array<T, num_dims> dims) {
  T len = 1;
  for (T d : dims) {
    len *= d;
  }
...
smem_lin_idx_to_gmem_lin_idx(int smem_lin_idx,
...
cp_tensor_global_to_shared(CUtensorMap* tensor_map,
                           cuda::std::array<uint32_t, num_dims> indices,
                           void* smem, barrier& bar) {

  switch (indices.size()) {
  case 1:
    cde::cp_async_bulk_tensor_1d_global_to_shared(smem, tensor_map, indices[0],
                                                  bar);
    break;
  case 2:
    cde::cp_async_bulk_tensor_2d_global_to_shared(smem, tensor_map, indices[0],
                                                  indices[1], bar);
    break;
  case 3:
    cde::cp_async_bulk_tensor_3d_global_to_shared(smem, tensor_map, indices[0],
                                                  indices[1], indices[2], bar);
    break;
  case 4:
    ...
    break;
  case 5:
    cde::cp_async_bulk_tensor_5d_global_to_shared(smem, tensor_map, indices[0],
                                                  indices[1], indices[2],
                                                  indices[3], indices[4], bar);
    break;
  default:
...
template <size_t num_dims>
__device__ inline void
cp_tensor_shared_to_global(CUtensorMap* tensor_map,
                           cuda::std::array<uint32_t, num_dims> indices,
                           void* smem) {
  ...
  case 1:
    ...
    break;
  case 2:
    cde::cp_async_bulk_tensor_2d_shared_to_global(tensor_map, indices[0],
                                                  indices[1], smem);
    break;
  case 3:
    cde::cp_async_bulk_tensor_3d_shared_to_global(tensor_map, indices[0],
                                                  indices[1], indices[2], smem);
    break;
  case 4:
    cde::cp_async_bulk_tensor_4d_shared_to_global(
        tensor_map, indices[0], indices[1], indices[2], indices[3], smem);
    break;
  case 5:
    cde::cp_async_bulk_tensor_5d_shared_to_global(tensor_map, indices[0],
                                                  indices[1], indices[2],
                                                  indices[3], indices[4], smem);
    break;
  default:
    assert(false && "Wrong number of dimensions.");
  }
...
__device__ void test(cuda::std::array<uint32_t, num_dims> smem_coord,
                     cuda::std::array<uint32_t, num_dims> smem_dims,
                     cuda::std::array<uint64_t, num_dims> gmem_dims,
                     int* gmem_tensor, int gmem_len) {
  CUtensorMap* global_tensor_map =
      reinterpret_cast<CUtensorMap*>(&global_fake_tensor_map);
  ...
  for (int i = threadIdx.x; i < smem_len; i += blockDim.x) {
    int gmem_lin_idx =
        smem_lin_idx_to_gmem_lin_idx(i, smem_coord, smem_dims, gmem_dims);
...
PFN_cuTensorMapEncodeTiled get_cuTensorMapEncodeTiled() {
  ...
  auto code = cudaGetDriverEntryPoint("cuTensorMapEncodeTiled", &driver_ptr,
...
void init_tensor_map(const T& gmem_tensor_symbol,
  ...
  // Copy it to device
  code = cudaMemcpyToSymbol(global_fake_tensor_map, &local_tensor_map,
                            sizeof(CUtensorMap));

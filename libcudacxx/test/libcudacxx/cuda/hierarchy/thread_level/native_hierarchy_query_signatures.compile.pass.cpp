...
__device__ void test_query_signatures(const Level& level)
{
  ...
  static_assert(cuda::std::is_same_v<unsigned, decltype(cuda::thread_level::count(level))>);
  ...
  static_assert(cuda::std::is_same_v<unsigned, decltype(cuda::thread_level::rank(level))>);

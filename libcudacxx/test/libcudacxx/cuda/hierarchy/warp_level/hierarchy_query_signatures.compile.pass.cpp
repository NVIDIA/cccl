...
TEST_DEVICE_FUNC void test_query_signatures(const Level& level, const Hierarchy& hier)
{
  ...
  static_assert(cuda::std::is_same_v<ProductType, decltype(cuda::warp_level::count(level, hier))>);
  ...
  static_assert(cuda::std::is_same_v<ProductType, decltype(cuda::warp_level::rank(level, hier))>);
...
TEST_DEVICE_FUNC void test_query_as_signatures(const Level& level, const Hierarchy& hier)
{
  ...
  static_assert(
    cuda::std::is_same_v<cuda::hierarchy_query_result<T>, decltype(cuda::warp_level::index_as<T>(level, hier))>);

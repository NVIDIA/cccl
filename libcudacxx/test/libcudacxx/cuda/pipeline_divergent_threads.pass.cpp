...
__global__ void with_staging(int* global_out, int const* global_in, size_t size,
  ...
  auto block = cooperative_groups::this_thread_block();
  assert(size ==
         batch_sz * grid.size()); // Assume input size fits batch_sz * grid_size

  // Two batches must fit in shared memory:
  constexpr int smem_size = stages_count * nthreads;
  __shared__ int shared[smem_size];
  size_t shared_offset[stages_count] = {0,
                                        block.size()}; // Offsets to each batch

  // Allocate shared storage for a two-stage cuda::pipeline:
  using pipeline_state =
      cuda::pipeline_shared_state<cuda::thread_scope::thread_scope_block,
                                  stages_count>;
  ...
  cuda::memcpy_async(block, shared + shared_offset[0],
                     global_in + block_batch(0), sizeof(int) * block.size(),
                     pipeline);
  ...
  for (size_t batch = 1; batch < batch_sz; ++batch) {
    ...
    cuda::memcpy_async(block, shared + shared_offset[copy_stage_idx],
                       global_in + global_copy_idx, sizeof(int) * block.size(),
                       pipeline);
    ...
    compute(global_out + global_compute_idx,
            shared + shared_offset[compute_stage_idx]);
...
int main(int argc, char** argv) {
  NV_IF_ELSE_TARGET(
      ...
      (
          ...
          constexpr size_t size = batch_size * nthreads; int* in, *out;

          if (cudaMallocManaged((void**)&in, sizeof(int) * size) !=
                  cudaSuccess ||
              cudaMallocManaged((void**)&out, sizeof(int) * size) !=
                  cudaSuccess) {
            printf("Setup failed\n");
            return -1;
          }
          ...
          int num_errors = 0; for (size_t i = 0; i < size; ++i) {
            if (out[i] != (int)i) {

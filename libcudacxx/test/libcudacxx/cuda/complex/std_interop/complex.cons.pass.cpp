...
__host__ __device__ _LIBCUDACXX_CONSTEXPR_STD_COMPLEX_ACCESS void test_construction()
{
  ...
  const ::cuda::std::complex<T> only_real{data.only_real};
  const ::cuda::std::complex<T> only_imag{data.only_imag};
  const ::cuda::std::complex<T> real_imag{data.real_imag};
...
__host__ __device__ _LIBCUDACXX_CONSTEXPR_STD_COMPLEX_ACCESS bool test()
{
  ...
  test_construction<float, double>();

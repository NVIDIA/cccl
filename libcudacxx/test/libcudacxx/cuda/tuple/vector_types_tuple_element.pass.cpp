...
__host__ __device__ TEST_CONSTEXPR_CXX14 void test() {
  assert((expected_type<VType, BaseType, Index>::value));
  assert((expected_type<const VType, const BaseType, Index>::value));
  assert((expected_type<volatile VType, volatile BaseType, Index>::value));
  assert((expected_type<const volatile VType, const volatile BaseType, Index>::value));
}

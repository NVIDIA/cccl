...
test_producer(T* dest, T* source, cuda::pipeline<PipelineScope>& pipe) {
  pipe.producer_acquire();
  cuda::memcpy_async(static_cast<void*>(&dest[0]),
                     static_cast<void*>(&source[0]), sizeof(T), pipe);
  pipe.producer_commit();

  pipe.producer_acquire();
  cuda::memcpy_async(static_cast<void*>(&dest[1]),
                     static_cast<void*>(&source[1]), sizeof(T), pipe);
  pipe.producer_commit();

  pipe.producer_acquire();
  cuda::memcpy_async(static_cast<void*>(&dest[2]),
                     static_cast<void*>(&source[2]), sizeof(T), pipe);
  pipe.producer_commit();
}

template <class T, cuda::thread_scope PipelineScope>
__device__ __noinline__ void
test_consumer(T* dest, T* source, cuda::pipeline<PipelineScope>& pipe) {
  pipe.consumer_wait();
  assert(source[0] == 12);
  assert(dest[0] == 12);
...
template <class T, template <typename, typename> class PipelineSelector,
          cuda::thread_scope PipelineScope, uint8_t PipelineStages>
__device__ __noinline__ void test_fully_specialized() {
  __shared__ T dest[3];
  __shared__ T* source;

  PipelineSelector<cuda::pipeline_shared_state<PipelineScope, PipelineStages>,
                   constructor_initializer>
      pipe_state_sel;
  cuda::pipeline_shared_state<PipelineScope, PipelineStages>* pipe_state =
      pipe_state_sel.construct();
...

  {
    ...
    const cuda::pipeline_role role = (group.thread_rank() % 2)
...

  {
    ...
    const cuda::pipeline_role role = (group.thread_rank() < 32)
                                         ? cuda::pipeline_role::producer
                                         : cuda::pipeline_role::consumer;
    cuda::pipeline<PipelineScope> pipe =
        make_pipeline(group, pipe_state, group.size() / 2);
...
int main(int argc, char** argv) {
  NV_IF_ELSE_TARGET(
      NV_IS_HOST, cuda_thread_count = 64;
      , (test_select_pipeline<uint8_t>(); test_select_pipeline<uint16_t>();
         test_select_pipeline<uint32_t>(); test_select_pipeline<uint64_t>();))

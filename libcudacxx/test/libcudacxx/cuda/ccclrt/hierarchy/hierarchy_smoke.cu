...
C2H_TEST("Different constructions", "[hierarchy]")
{
  ...
  static_assert(config.dims.has_level(cuda::block));
  static_assert(config.dims.has_level(cuda::cluster));
  static_assert(config.dims.has_level(cuda::grid));
  static_assert(!config.dims.has_level(cuda::thread));
  */
}
...
C2H_TEST("cuda::distribute", "[hierarchy]")
{
  ...
  CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::block, config.dims) == 256);
  CCCLRT_REQUIRE(cuda::block.count(cuda::grid, config.dims) == (numElements + threadsPerBlock - 1) / threadsPerBlock);
}

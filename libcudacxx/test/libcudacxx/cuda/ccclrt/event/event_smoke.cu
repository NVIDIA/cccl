...
void test_event_uses_explicit_device_when_current_device_differs()
{
  ...
  REQUIRE(explicit_device_stream.device() == explicit_device);
...
C2H_CCCLRT_TEST("can construct an event_ref from a cudaEvent_t", "[event]")
{
  ...
  ::cudaEvent_t ev;
  REQUIRE_CUDART(cudaEventCreate(&ev));
  ...
  REQUIRE(ref2.get() == ev);
  REQUIRE_CUDART(cudaEventDestroy(ev));
  // test an empty event_ref:
  cuda::event_ref ref3(::cudaEvent_t{});
  REQUIRE(ref3.get() == ::cudaEvent_t{});
  REQUIRE(!ref3);
}
...
C2H_CCCLRT_TEST("can copy construct an event_ref and compare for equality", "[event]")
{
  ...
  REQUIRE(ref2 == ref);
  REQUIRE(!(ref != ref2));
  REQUIRE((ref ? true : false)); // test contextual convertibility to bool
  REQUIRE(!!ref);
  REQUIRE(::cudaEvent_t{} != ref);
  REQUIRE_CUDART(cudaEventDestroy(ev));

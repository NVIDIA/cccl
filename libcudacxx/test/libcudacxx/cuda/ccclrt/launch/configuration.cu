...
C2H_TEST("Hierarchy construction in config", "[launch]")
{
  ...
  static_assert(cuda::block.count(cuda::grid, config.dims) == 2);

  auto config_larger = cuda::make_config(cuda::grid_dims<2>(), cuda::block_dims(256), cuda::cooperative_launch());
  CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::grid, config_larger.dims) == 512);

  auto config_no_options = cuda::make_config(cuda::grid_dims(2), cuda::block_dims<128>());
  CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::grid, config_no_options.dims) == 256);
...
C2H_TEST("Configuration combine", "[launch]")
{
  ...
  SECTION("Combine with no overlap")
  {
    ...
    CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::grid, combined.dims) == 512);
  }
  SECTION("Combine with overlap")
  {
    ...
    auto combined     = config_part1.combine(config_part2);
    CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::grid, combined.dims) == 2048);
    ...
    CCCLRT_REQUIRE(cuda::gpu_thread.count(cuda::grid, replaced_one_option.dims) == 2048);

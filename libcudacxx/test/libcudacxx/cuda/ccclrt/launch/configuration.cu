...
void test_launch_kernel_replacement(CUlaunchConfig &config, CUfunction kernel,
                                    void *args[]);
    ...
    for (j = 0; j < expectedConfig.numAttrs; ++j) {
      ...
      if (expectedAttr.id == actualAttr.id) {
        ...
        case CU_LAUNCH_ATTRIBUTE_CLUSTER_DIMENSION:
          CCCLRT_CHECK(expectedAttr.value.clusterDim.x ==
                       actualAttr.value.clusterDim.x);
          CCCLRT_CHECK(expectedAttr.value.clusterDim.y ==
                       actualAttr.value.clusterDim.y);
          CCCLRT_CHECK(expectedAttr.value.clusterDim.z ==
                       actualAttr.value.clusterDim.z);
          has_cluster = true;
          break;
        case CU_LAUNCH_ATTRIBUTE_COOPERATIVE:
          CCCLRT_CHECK(expectedAttr.value.cooperative ==
                       actualAttr.value.cooperative);
          break;
        case CU_LAUNCH_ATTRIBUTE_PRIORITY:
          CCCLRT_CHECK(expectedAttr.value.priority ==
                       actualAttr.value.priority);
          break;
        default:
          CCCLRT_CHECK(false);
          break;
...
auto make_test_dims(const dim3 &grid_dims, const dim3 &block_dims,
                    const dim3 &cluster_dims = dim3()) {
  if constexpr (HasCluster) {
    return cuda::make_hierarchy(cuda::grid_dims(grid_dims),
                                cuda::cluster_dims(cluster_dims),
...
auto configuration_test(::cuda::stream_ref stream, const dim3 &grid_dims,
  ...
  SECTION("Priority and dynamic smem") {
    ...
    auto config =
        cuda::make_config(dims, cuda::launch_priority(priority),
                          cuda::dynamic_shared_memory<int[num_ints]>());
  ...
  SECTION("Large dynamic smem") {
    ...
    auto config = cuda::make_config(
        dims, cuda::dynamic_shared_memory<S>(cuda::non_portable));
...
C2H_TEST("Launch configuration", "[launch]") {
  ...
  SECTION("No cluster") { configuration_test<false>(stream, 8, 64); }
  SECTION("With cluster") { configuration_test<true>(stream, 8, 32, 2); }
...
C2H_TEST("Hierarchy construction in config", "[launch]") {
  auto config =
      cuda::make_config(cuda::grid_dims<2>(), cuda::cooperative_launch());
  static_assert(config.dims.count(cuda::block) == 2);

  auto config_larger = cuda::make_config(
      cuda::grid_dims<2>(), cuda::block_dims(256), cuda::cooperative_launch());
  CCCLRT_REQUIRE(config_larger.dims.count(cuda::thread) == 512);

  auto config_no_options =
      cuda::make_config(cuda::grid_dims(2), cuda::block_dims<128>());
  CCCLRT_REQUIRE(config_no_options.dims.count(cuda::thread) == 256);

  [[maybe_unused]] auto config_no_dims =
      cuda::make_config(cuda::cooperative_launch());
  static_assert(cuda::std::is_same_v<decltype(config_no_dims.dims),
                                     cuda::__empty_hierarchy>);
}
...
C2H_TEST("Configuration combine", "[launch]") {
  ...
  SECTION("Combine with no overlap") {
    ...
    [[maybe_unused]] auto combined_other_way =
        config_part2.combine(config_part1);
    [[maybe_unused]] auto combined_with_empty =
        combined.combine(cuda::make_config());
    [[maybe_unused]] auto empty_with_combined =
        cuda::make_config().combine(combined);
    static_assert(
        cuda::std::is_same_v<decltype(combined),
                             decltype(cuda::make_config(
                                 grid, block, cuda::launch_priority(2)))>);
    static_assert(
        cuda::std::is_same_v<decltype(combined), decltype(combined_other_way)>);
    static_assert(cuda::std::is_same_v<decltype(combined),
                                       decltype(combined_with_empty)>);
    static_assert(cuda::std::is_same_v<decltype(combined),
                                       decltype(empty_with_combined)>);
  ...
  SECTION("Combine with overlap") {
    ...
    auto config_part2 =
        make_config(cuda::cluster_dims<256>, block, cuda::launch_priority(42));
    auto combined = config_part1.combine(config_part2);
    CCCLRT_REQUIRE(combined.dims.count(cuda::thread) == 2048);
    CCCLRT_REQUIRE(cuda::std::get<0>(combined.options).priority == 2);

    auto replaced_one_option =
        cuda::make_config(cuda::launch_priority(3)).combine(combined);
    CCCLRT_REQUIRE(replaced_one_option.dims.count(cuda::thread) == 2048);
    CCCLRT_REQUIRE(cuda::std::get<0>(replaced_one_option.options).priority ==
                   3);

    [[maybe_unused]] auto combined_with_extra_option =
        combined.combine(cuda::make_config(cuda::cooperative_launch()));
    static_assert(
        cuda::std::is_same_v<decltype(combined.dims),
                             decltype(combined_with_extra_option.dims)>);
    static_assert(
        cuda::std::tuple_size_v<decltype(combined_with_extra_option.options)> ==
        2);
  }
}

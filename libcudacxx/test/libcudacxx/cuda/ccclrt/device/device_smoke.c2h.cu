...
C2H_CCCLRT_TEST("Device attributes use the explicit device when current device differs", "[device][multi_gpu]")
{
...

  {
    ...
    REQUIRE(explicit_device.attribute(cuda::device_attributes::pci_bus_id) == expected_bus_id);
    REQUIRE(explicit_device.attribute(cuda::device_attributes::pci_device_id) == expected_device_id);
  }
}

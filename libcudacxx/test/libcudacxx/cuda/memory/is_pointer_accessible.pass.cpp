...
{
  assert(cuda::is_host_accessible_pointer(ptr) == is_host_accessible);
  assert(cuda::is_device_accessible_pointer(ptr) == is_device_accessible);
  ...
  {
    assert(cuda::is_host_accessible_pointer(ptr + 1) == is_host_accessible);
    assert(cuda::is_device_accessible_pointer(ptr + 1) == is_device_accessible);
...
bool test()
{
  [[maybe_unused]] int host_ptr1[] = {1, 2, 3, 4};
  [[maybe_unused]] auto host_ptr2  = new int[2];
  [[maybe_unused]] int* host_ptr3  = nullptr;
  cudaMallocHost(&host_ptr3, sizeof(int) * 2);

  [[maybe_unused]] int* device_ptr1 = nullptr;
  cudaMalloc(&device_ptr1, sizeof(int) * 2);

  [[maybe_unused]] int* managed_ptr1 = nullptr;
  ...
  test_accessible_pointer(host_ptr4, true, true, true); // memory space cannot be verified for global array
...
{
  assert(cuda::is_host_accessible_pointer(static_cast<const void*>(nullptr)));
  assert(cuda::is_device_accessible_pointer(static_cast<const void*>(nullptr)));
  assert(cuda::is_managed_pointer(static_cast<const void*>(nullptr)));
  return true;
}
...
int main(int, char**)
{
  ...
  NV_IF_TARGET(NV_IS_HOST, (assert(constexpr_test());))
  return 0;
}

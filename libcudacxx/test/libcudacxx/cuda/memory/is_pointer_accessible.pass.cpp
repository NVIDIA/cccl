...
bool test()
{
  ...
  cudaHostAllocMapped(&host_ptr4, sizeof(int) * 2, cudaHostAllocMapped);
...
{
  assert(cuda::is_host_accessible(static_cast<const void*>(nullptr)));
  assert(cuda::is_device_accessible(static_cast<const void*>(nullptr)));

...
__global__ void test_kernel()
{
  ...
  auto ptr = smem_value;
  assert(cuda::device::is_address_from(cuda::device::address_space::shared, ptr + 3));
  assert(cuda::device::is_address_from(cuda::device::address_space::shared, cuda::align_up(ptr + 3, 8)));
}

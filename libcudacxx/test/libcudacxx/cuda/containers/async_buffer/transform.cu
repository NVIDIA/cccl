...
C2H_TEST("DeviceTransform::Transform cuda::device_buffer",
         "[device][device_transform]") {
  using type = int;
  const int num_items = 1 << 24;

  cuda::stream stream{cuda::device_ref{0}};
  cuda::device_memory_pool_ref resource =
  ...
  thrust::sequence(thrust::cuda::par_nosync.on(stream.get()), a.begin(),
                   a.end());
  thrust::sequence(thrust::cuda::par_nosync.on(stream.get()), b.begin(),
                   b.end());

  cuda::device_buffer<type> result{stream, resource, num_items, cuda::no_init};

  cub::DeviceTransform::Transform(::cuda::std::make_tuple(a.begin(), b.begin()),
                                  result.begin(), num_items,
                                  ::cuda::std::plus<type>{});
  ...
  REQUIRE(cudaMemcpyAsync(a_h.data(), a.data(), num_items * sizeof(type),
                          cudaMemcpyDeviceToHost, stream.get()) == cudaSuccess);
  REQUIRE(cudaMemcpyAsync(b_h.data(), b.data(), num_items * sizeof(type),
                          cudaMemcpyDeviceToHost, stream.get()) == cudaSuccess);
  REQUIRE(cudaMemcpyAsync(result_h.data(), result.data(),
                          num_items * sizeof(type), cudaMemcpyDeviceToHost,
                          stream.get()) == cudaSuccess);
  stream.sync();

  // compute reference and verify
  thrust::host_vector<type> reference_h(num_items);
  std::transform(a_h.begin(), a_h.end(), b_h.begin(), reference_h.begin(),
                 std::plus<type>{});
...
C2H_CCCLRT_TEST("cuda::buffer launch transform", "[container][buffer]") {
  ...
  const cuda::device_buffer<int> b =
      cuda::make_buffer(stream, resource, a.size(), 1);

  cudax::launch(stream,
                cudax::make_config(cuda::grid_dims<1>, cuda::block_dims<32>),
                add_kernel{}, a, b);

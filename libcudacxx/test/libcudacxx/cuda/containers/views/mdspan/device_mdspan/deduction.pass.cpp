...
//    device_mdspan(CArray&)
//      -> device_mdspan<remove_all_extents_t<CArray>, extents<size_t, extent_v<CArray, 0>>>;
//
//  template<class Pointer>
//    requires(is_pointer_v<remove_reference_t<Pointer>>)
//    device_mdspan(Pointer&&)
//      -> device_mdspan<remove_pointer_t<remove_reference_t<Pointer>>, extents<size_t>>;
//
//  template<class ElementType, class... Integrals>
//    requires((is_convertible_v<Integrals, size_t> && ...) && sizeof...(Integrals) > 0)
//    explicit device_mdspan(ElementType*, Integrals...)
//      -> device_mdspan<ElementType, extents<size_t, maybe-static-ext<Integrals>...>>;
//
//  template<class ElementType, class OtherIndexType, size_t N>
//    device_mdspan(ElementType*, span<OtherIndexType, N>)
//      -> device_mdspan<ElementType, dextents<size_t, N>>;
//
//  template<class ElementType, class OtherIndexType, size_t N>
//    device_mdspan(ElementType*, const array<OtherIndexType, N>&)
//      -> device_mdspan<ElementType, dextents<size_t, N>>;
//
//  template<class ElementType, class IndexType, size_t... ExtentsPack>
//    device_mdspan(ElementType*, const extents<IndexType, ExtentsPack...>&)
//      -> device_mdspan<ElementType, extents<IndexType, ExtentsPack...>>;
//
...
//    device_mdspan(const typename AccessorType::data_handle_type&, const MappingType&,
//           const AccessorType&)
//      -> device_mdspan<typename AccessorType::element_type, typename MappingType::extents_type,
...
__host__ __device__ constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc)
{
  ...
  static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, map, acc)), MDS>);
  ...
  if constexpr (cuda::std::is_same<A, cuda::std::default_accessor<typename A::element_type>>::value)
  {
    ...
    static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, map)), MDS>);
    if constexpr (cuda::std::is_same<typename M::layout_type, cuda::std::layout_right>::value)
    {
      // deduction from pointer and extents
      static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, map.extents())), MDS>);
    }
  }
}
...
template <class H>
_CCCL_CONCEPT can_deduce_layout = _CCCL_REQUIRES_EXPR((H))((cuda::device_mdspan(cuda::std::declval<H>(), 10)));
...
__host__ __device__ constexpr bool test_no_layout_deduction_guides(const H& handle, const A&)
{
  ...
  static_assert(
    cuda::std::is_same_v<decltype(cuda::device_mdspan(handle)), cuda::device_mdspan<T, cuda::std::extents<size_t>>>);
  // deduction from pointer and integral like
  static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, 5, SizeTIntType(6))),
                                     cuda::device_mdspan<T, cuda::std::dextents<size_t, 2>>>);

  // P3029R1: deduction from `integral_constant`
  static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, cuda::std::integral_constant<size_t, 5>{})),
                                     cuda::device_mdspan<T, cuda::std::extents<size_t, 5>>>);
  static_assert(
    cuda::std::is_same_v<
      decltype(cuda::device_mdspan(handle, cuda::std::integral_constant<size_t, 5>{}, cuda::std::dynamic_extent)),
      cuda::device_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent>>>);
  static_assert(
    cuda::std::is_same_v<decltype(cuda::device_mdspan(handle,
                                                      cuda::std::integral_constant<size_t, 5>{},
                                                      cuda::std::dynamic_extent,
                                                      cuda::std::integral_constant<size_t, 7>{})),
                         cuda::device_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent, 7>>>);

  cuda::std::array<char, 3> exts;
  // deduction from pointer and array
  static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, exts)),
                                     cuda::device_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  // deduction from pointer and span
  static_assert(cuda::std::is_same_v<decltype(cuda::device_mdspan(handle, cuda::std::span(exts))),
                                     cuda::device_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  return true;
}

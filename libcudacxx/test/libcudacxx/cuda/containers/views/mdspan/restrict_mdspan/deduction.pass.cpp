...
__host__ __device__ constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc)
{
  ...
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, map, acc)), MDS>);
  ...
  if constexpr (cuda::std::is_same<A, cuda::std::default_accessor<typename A::element_type>>::value)
  {
    ...
    static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, map)), MDS>);
    if constexpr (cuda::std::is_same<typename M::layout_type, cuda::std::layout_right>::value)
    {
      // deduction from pointer and extents
      static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, map.extents())), MDS>);
    }
  }
}
...
template <class H>
_CCCL_CONCEPT can_deduce_layout = _CCCL_REQUIRES_EXPR((H))((cuda::restrict_mdspan(cuda::std::declval<H>(), 10)));
...
__host__ __device__ constexpr bool test_no_layout_deduction_guides(const H& handle, const A&)
{
  ...
  static_assert(
    cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle)), cuda::restrict_mdspan<T, cuda::std::extents<size_t>>>);
  // deduction from pointer and integral like
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, 5, SizeTIntType(6))),
                                     cuda::restrict_mdspan<T, cuda::std::dextents<size_t, 2>>>);

  // P3029R1: deduction from `integral_constant`
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, cuda::std::integral_constant<size_t, 5>{})),
                                     cuda::restrict_mdspan<T, cuda::std::extents<size_t, 5>>>);
  static_assert(
    cuda::std::is_same_v<
      decltype(cuda::restrict_mdspan(handle, cuda::std::integral_constant<size_t, 5>{}, cuda::std::dynamic_extent)),
      cuda::restrict_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent>>>);
  static_assert(
    cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle,
                                                        cuda::std::integral_constant<size_t, 5>{},
                                                        cuda::std::dynamic_extent,
                                                        cuda::std::integral_constant<size_t, 7>{})),
                         cuda::restrict_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent, 7>>>);

  cuda::std::array<char, 3> exts;
  // deduction from pointer and array
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, exts)),
                                     cuda::restrict_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  // deduction from pointer and span
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(handle, cuda::std::span(exts))),
                                     cuda::restrict_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  return true;
}
...
__host__ __device__ constexpr bool test()
{
  ...
  float a[12] = {};
  static_assert(cuda::std::is_same_v<decltype(cuda::restrict_mdspan(a)),
                                     cuda::restrict_mdspan<float, cuda::std::extents<size_t, 12>>>);
  unused(a);

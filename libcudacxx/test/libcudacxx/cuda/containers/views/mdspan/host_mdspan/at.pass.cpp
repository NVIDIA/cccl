...
template <class... Args>
constexpr bool check_operator_constraints(Args...)
{
  return false;
}
...
constexpr void assert_access(MDS mds, Args... args)
{
...
void iterate_invalid(MDS mds, Args... args)
{
  ...
  {
    const cuda::std::array<typename MDS::index_type, MDS::rank()> args_arr{
...
constexpr void test_iteration(Mapping m)
{
  ...
  using MDS = cuda::host_mdspan<int, typename Mapping::extents_type, typename Mapping::layout_type>;
  MDS mds(data.data(), m);
  iterate(mds);
}
  ...
  if constexpr (cuda::std::is_same<Layout, cuda::std::layout_left>::value)
  {
...

    {
      ...
      assert(!check_operator_constraints(
        cuda::host_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))), s));
    }
    {
      cuda::std::array idx{IntConfig<o, o, t, t>(0)};
      cuda::std::span s(idx);
...
constexpr void test_index_cast_happens()
{
  ...
  using mds_t     = cuda::host_mdspan<int, extents_t, layout_wrapping_integral<4>>;

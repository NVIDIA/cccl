...
//    host_mdspan(CArray&)
//      -> host_mdspan<remove_all_extents_t<CArray>, extents<size_t, extent_v<CArray, 0>>>;
//
//  template<class Pointer>
//    requires(is_pointer_v<remove_reference_t<Pointer>>)
//    host_mdspan(Pointer&&)
//      -> host_mdspan<remove_pointer_t<remove_reference_t<Pointer>>, extents<size_t>>;
//
//  template<class ElementType, class... Integrals>
//    requires((is_convertible_v<Integrals, size_t> && ...) && sizeof...(Integrals) > 0)
//    explicit host_mdspan(ElementType*, Integrals...)
//      -> host_mdspan<ElementType, extents<size_t, maybe-static-ext<Integrals>...>>;
//
//  template<class ElementType, class OtherIndexType, size_t N>
//    host_mdspan(ElementType*, span<OtherIndexType, N>)
//      -> host_mdspan<ElementType, dextents<size_t, N>>;
//
//  template<class ElementType, class OtherIndexType, size_t N>
//    host_mdspan(ElementType*, const array<OtherIndexType, N>&)
//      -> host_mdspan<ElementType, dextents<size_t, N>>;
//
//  template<class ElementType, class IndexType, size_t... ExtentsPack>
//    host_mdspan(ElementType*, const extents<IndexType, ExtentsPack...>&)
//      -> host_mdspan<ElementType, extents<IndexType, ExtentsPack...>>;
//
...
//    host_mdspan(const typename AccessorType::data_handle_type&, const MappingType&,
//           const AccessorType&)
//      -> host_mdspan<typename AccessorType::element_type, typename MappingType::extents_type,
...
__host__ __device__ constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc)
{
  ...
  static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, map, acc)), MDS>);
  ...
  if constexpr (cuda::std::is_same<A, cuda::std::default_accessor<typename A::element_type>>::value)
  {
    ...
    static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, map)), MDS>);
    if constexpr (cuda::std::is_same<typename M::layout_type, cuda::std::layout_right>::value)
    {
      // deduction from pointer and extents
      static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, map.extents())), MDS>);
    }
  }
}
...
template <class H>
_CCCL_CONCEPT can_deduce_layout = _CCCL_REQUIRES_EXPR((H))((cuda::host_mdspan(cuda::std::declval<H>(), 10)));
...
__host__ __device__ constexpr bool test_no_layout_deduction_guides(const H& handle, const A&)
{
  ...
  static_assert(
    cuda::std::is_same_v<decltype(cuda::host_mdspan(handle)), cuda::host_mdspan<T, cuda::std::extents<size_t>>>);
  // deduction from pointer and integral like
  static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, 5, SizeTIntType(6))),
                                     cuda::host_mdspan<T, cuda::std::dextents<size_t, 2>>>);

  // P3029R1: deduction from `integral_constant`
  static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, cuda::std::integral_constant<size_t, 5>{})),
                                     cuda::host_mdspan<T, cuda::std::extents<size_t, 5>>>);
  static_assert(
    cuda::std::is_same_v<
      decltype(cuda::host_mdspan(handle, cuda::std::integral_constant<size_t, 5>{}, cuda::std::dynamic_extent)),
      cuda::host_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent>>>);
  static_assert(
    cuda::std::is_same_v<decltype(cuda::host_mdspan(handle,
                                                    cuda::std::integral_constant<size_t, 5>{},
                                                    cuda::std::dynamic_extent,
                                                    cuda::std::integral_constant<size_t, 7>{})),
                         cuda::host_mdspan<T, cuda::std::extents<size_t, 5, cuda::std::dynamic_extent, 7>>>);

  cuda::std::array<char, 3> exts;
  // deduction from pointer and array
  static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, exts)),
                                     cuda::host_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  // deduction from pointer and span
  static_assert(cuda::std::is_same_v<decltype(cuda::host_mdspan(handle, cuda::std::span(exts))),
                                     cuda::host_mdspan<T, cuda::std::dextents<size_t, 3>>>);
  return true;
}
...
__host__ __device__ constexpr bool test()
{
  ...
  float a[12] = {};
  static_assert(
    cuda::std::is_same_v<decltype(cuda::host_mdspan(a)), cuda::host_mdspan<float, cuda::std::extents<size_t, 12>>>);
  unused(a);

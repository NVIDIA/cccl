...
TEST_FUNC void not_abstract_class()
{
  ...
  cuda::host_mdspan<AbstractClass, cuda::std::extents<int>> m;
  unused(m);
}
...
TEST_FUNC void not_array_type()
{
  ...
  cuda::host_mdspan<int[5], cuda::std::extents<int>> m;
  unused(m);
}
...
TEST_FUNC void element_type_mismatch()
{
  ...
  cuda::host_mdspan<int, cuda::std::extents<int>, cuda::std::layout_right, cuda::std::default_accessor<const int>> m;
  unused(m);
}

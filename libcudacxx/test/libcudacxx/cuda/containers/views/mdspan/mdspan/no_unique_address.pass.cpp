...
void test()
{
  // NOTE: we are not using CTAD because of nvbug5296090, which leads to a segfault with MSVC
  int data = 42;
  { // all dynamic
    using extents =
      cuda::std::extents<size_t, cuda::std::dynamic_extent, cuda::std::dynamic_extent, cuda::std::dynamic_extent>;
    cuda::std::mdspan<int, extents, Layout> mdspan{&data, typename Layout::mapping<extents>{extents{42, 1337, 7}}};
  ...
  { // middle static
    ...
    cuda::std::mdspan<int, extents, Layout> mdspan{&data, typename Layout::mapping<extents>{extents{42, 7}}};
    test(mdspan, sizeof(decltype(mdspan)), &data);
    test_kernel<<<1, 1>>>(mdspan, sizeof(decltype(mdspan)), &data);
  }
  ...
  { // middle dynamic
    ...
    cuda::std::mdspan<int, extents, Layout> mdspan{&data, typename Layout::mapping<extents>{extents{1337}}};
    test(mdspan, sizeof(decltype(mdspan)), &data);
    test_kernel<<<1, 1>>>(mdspan, sizeof(decltype(mdspan)), &data);
  }
  ...
  { // all dynamic
    ...
    cuda::std::mdspan<int, extents, Layout> mdspan{&data, typename Layout::mapping<extents>{extents{}}};
    test(mdspan, sizeof(decltype(mdspan)), &data);
    test_kernel<<<1, 1>>>(mdspan, sizeof(decltype(mdspan)), &data);
  }
}
...
void test()
{
  test<cuda::std::layout_left>();
  test<cuda::std::layout_right>();
}

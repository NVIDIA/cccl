...
template <class MDS, class IndexType>
void check_at_span_throws(MDS mds, cuda::std::array<IndexType, MDS::rank()>& indices)
{
  try
  {
    (void) mds.at(cuda::std::span<IndexType, MDS::rank()>(indices));
    assert(false);
  }
  catch (const std::out_of_range&)
  {
...
_CCCL_DEVICE __managed__ int index_cast_data[4]{};
...
TEST_FUNC void test_index_cast_happens()
{
  [[maybe_unused]] constexpr size_t D = cuda::std::dynamic_extent;
  using extents_t                     = cuda::std::extents<int, D, D>;
  using mds_t                         = cuda::managed_mdspan<int, extents_t, layout_wrapping_integral<4>>;

  mds_t mds(index_cast_data, construct_mapping(layout_wrapping_integral<4>(), extents_t(2, 2)));

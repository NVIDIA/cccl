...
template <class MDS>
TEST_FUNC constexpr auto& access(MDS mds, int64_t i0)
{
  return mds[i0];
}
...
#if _CCCL_HAS_MULTIARG_OPERATOR_BRACKETS()
template <class MDS>
TEST_FUNC constexpr auto& access(MDS mds)
{
  return mds[];
}
template <class MDS>
TEST_FUNC constexpr auto& access(MDS mds, int64_t i0, int64_t i1)
{
  return mds[i0, i1];
}
template <class MDS>
TEST_FUNC constexpr auto& access(MDS mds, int64_t i0, int64_t i1, int64_t i2)
{
  return mds[i0, i1, i2];
}
template <class MDS>
TEST_FUNC constexpr auto& access(MDS mds, int64_t i0, int64_t i1, int64_t i2, int64_t i3)
{
...
TEST_FUNC constexpr void assert_access(MDS mds, Arg arg)
{
...
{
  using MDS = cuda::managed_mdspan<int, typename Mapping::extents_type, typename Mapping::layout_type>;
...
TEST_FUNC constexpr void test_layout()
{
  ...
  static_assert(check_operator_constraints(
                  cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))), 0),
                "");
  static_assert(!check_operator_constraints(
                  cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))), 0, 0),
                "");

  // Check operator constraint for convertibility of arguments to index_type
  static_assert(check_operator_constraints(
                  cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))), IntType(0)),
                "");
  static_assert(
    !check_operator_constraints(
      cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<unsigned, D>(1))), IntType(0)),
    "");

  // Check operator constraint for no-throw-constructibility of index_type from arguments
  static_assert(
    !check_operator_constraints(
      cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<unsigned char, D>(1))), IntType(0)),
    "");
  ...
  static_assert(
    check_operator_constraints(
      cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<unsigned char, D, D>(1, 1))),
      int(0),
      size_t(0)),
    "");
  static_assert(check_operator_constraints(
                  cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D, D>(1, 1))),
                  unsigned(0),
                  IntType(0)),
                "");
  ...
  {
    static_assert(!check_operator_constraints(
                    cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))),
    ...
    static_assert(!check_operator_constraints(
                  ...
                  "");
    static_assert(
      check_operator_constraints(cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))),
...

    {
      ...
      assert(!check_operator_constraints(
        cuda::managed_mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))), s));
    }
    {
      cuda::std::array idx{IntConfig<o, o, t, t>(0)};
      cuda::std::span s(idx);

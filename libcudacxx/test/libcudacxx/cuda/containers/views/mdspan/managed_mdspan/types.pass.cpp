...
{
  using MDS = cuda::managed_mdspan<T, E, L, A>;

  static_assert(cuda::std::is_same_v<typename MDS::extents_type, E>);
  static_assert(cuda::std::is_same_v<typename MDS::layout_type, L>);
  static_assert(cuda::std::is_same_v<typename MDS::accessor_type, cuda::managed_accessor<A>>);
  ...
  static_assert(
    cuda::std::is_same_v<cuda::managed_mdspan<T, E>,
                         cuda::managed_mdspan<T, E, cuda::std::layout_right, cuda::std::default_accessor<T>>>);
  static_assert(
    cuda::std::is_same_v<cuda::managed_mdspan<T, E, L>, cuda::managed_mdspan<T, E, L, cuda::std::default_accessor<T>>>);
...
__host__ __device__ constexpr bool test()
{
  ...
  check_triviality<cuda::managed_mdspan<int, cuda::std::extents<int>>, false, true, true, true, true, true>();
  check_triviality<cuda::managed_mdspan<int, cuda::std::dextents<int, 1>>, false, true, true, true, true, true>();
  check_triviality<
    cuda::managed_mdspan<int, cuda::std::dextents<int, 1>, cuda::std::layout_right, checked_accessor<int>>,
    false,
    true,
    false,
    true,

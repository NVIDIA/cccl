...
__host__ __device__ void not_abstract_class()
{
  ...
  cuda::managed_mdspan<AbstractClass, cuda::std::extents<int>> m;
  unused(m);
}
...
__host__ __device__ void not_array_type()
{
  ...
  cuda::managed_mdspan<int[5], cuda::std::extents<int>> m;
  unused(m);
}
...
__host__ __device__ void element_type_mismatch()
{
  ...
  cuda::managed_mdspan<int, cuda::std::extents<int>, cuda::std::layout_right, cuda::std::default_accessor<const int>> m;
  unused(m);
}

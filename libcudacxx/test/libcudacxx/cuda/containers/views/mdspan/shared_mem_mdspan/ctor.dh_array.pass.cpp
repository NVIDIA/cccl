...
TEST_TILE_DEVICE_FUNC constexpr auto array_from_extents(const Extents& exts, cuda::std::index_sequence<Idxs...>)
{
...
TEST_TILE_DEVICE_FUNC constexpr void
...
TEST_TILE_DEVICE_FUNC constexpr cuda::std::array<typename MDS::index_type, MDS::rank_dynamic()>
...
TEST_TILE_DEVICE_FUNC constexpr void test_mdspan_ctor(const H& handle, const M& map, const A& acc)
{
  using MDS =
    cuda::shared_memory_mdspan<typename A::element_type, typename M::extents_type, typename M::layout_type, A>;
...
TEST_TILE_DEVICE_FUNC constexpr void test_mdspan_ctor(const H& handle, const M& map, const A& acc)
{
  ...
  static_assert(ac == cuda::std::is_default_constructible_v<A>);
  static_assert(
    !cuda::std::is_constructible_v<MDS, const H&, const cuda::std::array<typename MDS::index_type, MDS::rank()>&>);
}
...
TEST_TILE_DEVICE_FUNC void test()
{
  ...
  // sanity check
  static_assert(cuda::std::is_constructible_v<mds_t, float*, cuda::std::array<int, 3>>);
  static_assert(cuda::std::is_constructible_v<mds_t, float*, cuda::std::array<int, 2>>);
  // wrong size
  static_assert(!cuda::std::is_constructible_v<mds_t, float*, cuda::std::array<int, 1>>);
  static_assert(!cuda::std::is_constructible_v<mds_t, float*, cuda::std::array<int, 4>>);
  ...
  static_assert(!cuda::std::is_constructible_v<mds_t, float*, cuda::std::array<IntType, 2>>);
  ...
  static_assert(!cuda::std::is_constructible_v<mds_uchar_t, float*, cuda::std::array<IntType, 2>>);
  ...
  static_assert(!cuda::std::is_constructible_v<mds_int_t, float*, cuda::std::array<IntTypeNC, 2>>);
  ...
  static_assert(!cuda::std::is_constructible_v<mds_t, const float*, cuda::std::array<int, 2>>);
}

...
__device__ constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc)
{
  ...
  static_assert(cuda::std::is_nothrow_move_constructible_v<MDS>);
  static_assert(cuda::std::is_nothrow_move_assignable_v<MDS>);
  static_assert(cuda::std::is_nothrow_swappable_v<MDS>);

...
__device__ constexpr void mixin_layout(const H& handle, const A& acc)
{
  ...
  static_assert(cuda::std::is_trivially_assignable_v<
                ...
                const typename cuda::std::layout_left::template mapping<cuda::std::extents<int>>&>);
  ...
  static_assert(!cuda::std::is_trivially_assignable_v<
                ...
                const typename layout_wrapping_integral<4>::template mapping<cuda::std::extents<int>>&>);

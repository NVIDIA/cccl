...
//  constexpr mapping(const extents_type& e, span<OtherIndexType, rank_> s, offset_type offset = 0) noexcept;
//
// Constraints:
...
__host__ __device__ constexpr bool test()
{
  ...
  {
    cuda::std::array<int, 1> s{1};
    test_construction(cuda::std::extents<unsigned, D>(7), cuda::std::span<int, 1>(s), 5);
  }
  {
    cuda::std::array<int, 2> s{3, 30};
    test_construction(cuda::std::extents<unsigned, 7, 8>(), cuda::std::span<int, 2>(s), 10);
  }

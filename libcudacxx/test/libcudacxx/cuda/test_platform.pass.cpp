...
__host__ __device__ void test() {
  ...
  NV_DISPATCH_TARGET(
      ...
      NV_PROVIDES_SM_35,
      (static_assert(arch_val >= 350, "cuda arch expected 350");), NV_IS_HOST,
  ...
  NV_DISPATCH_TARGET(
      NV_IS_HOST, (), NV_IS_DEVICE,
      (static_assert(NV_TARGET_MINIMUM_SM_INTEGER == (__CUDA_ARCH__ / 10),
                     "arch mismatch");
...
__host__ __device__ void test() {
  ...
  NV_IF_TARGET(NV_IS_HOST, invoke_count += 1;, invoke_count += 1;
               invoke_count += threadIdx.x;)

  assert(invoke_count == 1);
  invoke_count = 0;

  NV_IF_TARGET(NV_IS_DEVICE, invoke_count += 1; invoke_count += threadIdx.x;
               , invoke_count += 1;)

  assert(invoke_count == 1);
  invoke_count = 0;

  NV_DISPATCH_TARGET(NV_NO_TARGET, invoke_count += 0;
  ...
  NV_DISPATCH_TARGET(
      NV_IS_HOST, (), NV_IS_DEVICE,
      (static_assert(NV_TARGET_MINIMUM_SM_INTEGER ==
                         (__CUDA_MINIMUM_ARCH__ / 10),
                     "arch mismatch");
       static_assert(nv::target::detail::toint(NV_TARGET_MINIMUM_SM_SELECTOR) ==
                         (__CUDA_MINIMUM_ARCH__ / 10),
                     "arch mismatch");))
}

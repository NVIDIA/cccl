...
static_assert(false, "Insufficient CUDA Compute Capability: "
                     "cuda::device::memcpy_async_tx is not available.");
  ...
  NV_DISPATCH_TARGET(
      ...
      (
          ...
          for (int i = threadIdx.x; i < 2048;
               i += blockDim.x) { gmem_x[i] = i; } __syncthreads();

          barrier_t::arrival_token token; if (threadIdx.x == 0) {
            auto fulfillment = cuda::device::memcpy_async_tx(
                smem_x, gmem_x, cuda::aligned_size_t<16>(sizeof(smem_x)), *b);
          ...
          for (int i = threadIdx.x; i < 1024;
               i += blockDim.x) { assert(smem_x[i] == i); }));
  return 0;
}

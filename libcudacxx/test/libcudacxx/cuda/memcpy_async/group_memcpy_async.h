...
struct storage {
  ...
  __host__ __device__ friend bool operator==(const storage& lhs,
                                             const storage& rhs) {
    for (cuda::std::size_t i = 0; i < size; ++i) {
...
template <class T, template <typename, typename> class SourceSelector,
          ...
          cuda::thread_scope BarrierScope, typename... CompletionF>
__device__ __noinline__ void test_fully_specialized() {
  SourceSelector<T, constructor_initializer> source_sel;
  typename DestSelector<T, constructor_initializer>::template offsetted<
      decltype(source_sel)::shared_offset>
      dest_sel;
  BarrierSelector<cuda::barrier<BarrierScope, CompletionF...>,
  ...
  cuda::memcpy_async(cg::this_thread_block(), dest, source, sizeof(T), *bar);
  ...
  cuda::memcpy_async(cg::this_thread_block(), static_cast<void*>(dest),
                     static_cast<void*>(source), sizeof(T), *bar);
...
__host__ __device__ __noinline__ void test_select_scope() {
  test_fully_specialized<T, SourceSelector, DestSelector, BarrierSelector,
  ...
  test_fully_specialized<T, SourceSelector, DestSelector, BarrierSelector,
                         cuda::thread_scope_block, completion>();
  test_fully_specialized<T, SourceSelector, DestSelector, BarrierSelector,
...
__host__ __device__ __noinline__ void test_select_barrier() {
  NV_IF_TARGET(NV_IS_DEVICE, (test_select_scope<T, SourceSelector, DestSelector,
                                                shared_memory_selector>();
                              test_select_scope<T, SourceSelector, DestSelector,

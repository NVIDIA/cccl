...
struct int_generator {
  __host__ __device__ auto operator()() const { return Value; }
};

struct nullopt_generator {
  __host__ __device__ auto operator()() const { return cuda::std::nullopt; }
};
...
using testers = tester_list<
    assign_tester<nullopt_generator>, assign_tester<int_generator<123> >,
    assign_tester<nullopt_generator>, swap_tester<int_generator<17> >,
    reset_tester, assign_tester<int_generator<31> >,
    swap_tester<nullopt_generator> >;

struct non_trivial {
  int i;

  __host__ __device__ non_trivial(int i) : i(i) {}

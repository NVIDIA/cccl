...
using aw_aw_pw1 =
    performer_list<barrier_parity_wait<false>, barrier_arrive_and_wait,
                   barrier_arrive_and_wait, async_tester_fence, clear_token>;

using aw_aw_pw2 =
    performer_list<barrier_parity_wait<true>, barrier_arrive_and_wait,
                   barrier_arrive_and_wait, async_tester_fence, clear_token>;

void kernel_invoker() {
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, aw_aw_pw1>(
      2);
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, aw_aw_pw2>(
      2);
}

...
static_assert(sizeof(cuda::barrier<cuda::thread_scope_system, void (*)()>) ==
                  4 * 64,
              "");
...
struct barrier_and_token_with_completion {
  struct completion_t {
    ...
    __host__ __device__ void operator()() const {
      ...
      NV_IF_ELSE_TARGET(NV_IS_HOST, completed_from_host = true;
...
using a_aw_w = performer_list<barrier_arrive, barrier_arrive_and_wait,
                              barrier_wait, async_tester_fence, clear_token>;

using aw_aw = performer_list<barrier_arrive_and_wait, barrier_arrive_and_wait>;

using a_w_aw =
    performer_list<barrier_arrive, barrier_wait, barrier_arrive_and_wait,
                   async_tester_fence, clear_token>;

using a_w_a_w = performer_list<barrier_arrive, barrier_wait, barrier_arrive,
                               barrier_wait, async_tester_fence, clear_token>;

using completion_performers_a =
    performer_list<clear_token, barrier_arrive, barrier_arrive_and_wait,
                   async_tester_fence, validate_completion_result,
                   barrier_wait>;
...
using cuda_barrier_system =
    cuda::barrier<cuda::thread_scope_system, Completion>;

void kernel_invoker() {
  validate_not_movable<barrier_and_token<cuda::std::barrier<> >, a_aw_w>(2);
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, a_aw_w>(2);

  validate_not_movable<barrier_and_token<cuda::std::barrier<> >, aw_aw>(2);
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, aw_aw>(2);

  validate_not_movable<barrier_and_token<cuda::std::barrier<> >, a_w_aw>(2);
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, a_w_aw>(2);

  validate_not_movable<barrier_and_token<cuda::std::barrier<> >, a_w_a_w>(2);
  validate_not_movable<
      barrier_and_token<cuda::barrier<cuda::thread_scope_system> >, a_w_a_w>(2);

  validate_not_movable<barrier_and_token_with_completion<cuda::std::barrier>,
                       completion_performers_a>(2);
  validate_not_movable<barrier_and_token_with_completion<cuda_barrier_system>,
                       completion_performers_a>(2);

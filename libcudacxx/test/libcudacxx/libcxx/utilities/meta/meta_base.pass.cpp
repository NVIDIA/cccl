...
__host__ __device__ void test_or() {
  ...
  static_assert(
      cuda::std::_Or<False, False, False, False, True, BOOM<2> >::value, "");
}

__host__ __device__ void test_combined() {
  static_assert(
      cuda::std::_And<True, cuda::std::_Or<False, True, BOOM<4> > >::value, "");
  static_assert(
      cuda::std::_And<True, cuda::std::_Or<False, True, BOOM<4> > >::value, "");
...
using FuncCallable =
    decltype(cuda::std::declval<T>().func(cuda::std::declval<U>()));
template <class T>
...
__host__ __device__ void test_is_valid_trait() {
  ...
  static_assert(cuda::std::_IsValidExpansion<HasFooType, MemberTest2>::value,
                "");
  static_assert(
      cuda::std::_IsValidExpansion<FuncCallable, MemberTest, int>::value, "");
  static_assert(
      !cuda::std::_IsValidExpansion<FuncCallable, MemberTest, void*>::value,
      "");
}

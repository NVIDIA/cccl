...
TEST_FUNC TEST_CONSTEXPR_CXX20 bool test()
{
  ...
  using cuda::std::__compressed_box_move_construct_available;
  ...
  { // trivial empty type
    ...
    static_assert(__compressed_box_move_construct_available<T> == __smf_availability::__trivial);

    box<T> input{};
    box<T> b{cuda::std::move(input)};
    assert(b.__get<0>() == 42);
    static_assert(cuda::std::is_nothrow_move_constructible_v<box<T>>);
  }
  ...
  { // copy constructible but not copyable
    ...
    static_assert(__compressed_box_move_construct_available<T> == __smf_availability::__available);
    test<T>(1337);
  }

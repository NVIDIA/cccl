...
__global__ void fake_main_kernel(int* ret) { *ret = fake_main(0, NULL); }
...
// cudaLaunchKernelEx is supported from 11.8 onwards
#if __CUDACC_VER_MAJOR__ >= 11 && __CUDACC_VER_MINOR__ >= 8
  {
    ...
    attribute[0].val.clusterDim.x =
        cuda_cluster_size; // Cluster size in X-dimension
  ...
  CUDA_CALL(err, cudaDeviceSynchronize());
  CUDA_CALL(err,
            cudaMemcpy(&ret, cuda_ret, sizeof(int), cudaMemcpyDeviceToHost));

...
struct Test {
  ...
  __host__ __device__ TEST_CONSTEXPR_CXX14 void operator()() {
    ...
    assert(
        !cuda::std::equal(Iter1(a), Iter1(a + s), Iter2(a), Iter2(a + s - 1)));
...
struct TestNarrowingEqualTo {
  ...
  __host__ __device__ TEST_CONSTEXPR_CXX14 void operator()() {
    UnderlyingType a[] = {UnderlyingType(0x1000), UnderlyingType(0x1001),
                          UnderlyingType(0x1002), UnderlyingType(0x1003),
                          UnderlyingType(0x1004)};
    UnderlyingType b[] = {UnderlyingType(0x1600), UnderlyingType(0x1601),
                          UnderlyingType(0x1602), UnderlyingType(0x1603),
...
struct AddressCompare {
  ...
  operator==(const AddressCompare& lhs, const AddressCompare& rhs) {
    return &lhs == &rhs;
  }
...
public:
  ...
  operator==(const trivially_equality_comparable& other) const noexcept {
    return i_ == other.i_;
  };
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
  types::for_each(types::cpp17_input_iterator_list<int*>(),
                  TestIter2<int, types::cpp17_input_iterator_list<int*> >());
  types::for_each(types::cpp17_input_iterator_list<char*>(),
                  TestIter2<char, types::cpp17_input_iterator_list<char*> >());
  types::for_each(
      types::cpp17_input_iterator_list<AddressCompare*>(),
      TestIter2<AddressCompare,
                types::cpp17_input_iterator_list<AddressCompare*> >());
...
int main(int, char**) {
  ...
  types::for_each(
      types::as_pointers<types::cv_qualified_versions<int> >(),
      TestIter2<int,
                types::as_pointers<types::cv_qualified_versions<int> > >());
  types::for_each(
      types::as_pointers<types::cv_qualified_versions<char> >(),
      TestIter2<char,
                types::as_pointers<types::cv_qualified_versions<char> > >());

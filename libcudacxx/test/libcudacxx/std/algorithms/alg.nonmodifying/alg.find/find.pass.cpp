...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
  ...
  { // first element matches
    Iter iter = cuda::std::find(Iter(cuda::std::begin(arr)),
                                Iter(cuda::std::end(arr)), Comparable(2));
    assert(*iter == 2);
  ...
  { // if multiple elements match, return the first match
    ...
    Iter iter =
        cuda::std::find(Iter(cuda::std::begin(arr_multiple)),
                        Iter(cuda::std::end(arr_multiple)), Comparable(4));
    assert(*iter == 4);
  ...
  { // some element matches
    Iter iter = cuda::std::find(Iter(cuda::std::begin(arr)),
                                Iter(cuda::std::end(arr)), Comparable(6));
    assert(*iter == 6);
  ...
  { // last element matches
    Iter iter = cuda::std::find(Iter(cuda::std::begin(arr)),
                                Iter(cuda::std::end(arr)), Comparable(8));
    assert(*iter == 8);
  ...
  { // if no element matches, last is returned
    Iter iter = cuda::std::find(Iter(cuda::std::begin(arr)),
                                Iter(cuda::std::end(arr)), Comparable(10));

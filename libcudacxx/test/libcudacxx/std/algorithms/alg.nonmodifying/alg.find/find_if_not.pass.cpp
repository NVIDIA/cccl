...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
    ...
    { // first element matches
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr)), Iter(cuda::std::end(arr)), Comparable(2));
      assert(*iter == 2);
      assert(base(iter) == arr);
    }

    { // range is empty; return last
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr)), Iter(cuda::std::begin(arr)), Comparable(2));
      assert(base(iter) == arr);
    }

    { // if multiple elements match, return the first match
      constexpr int arr_multiple[] = {2, 4, 4, 8};
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr_multiple)), Iter(cuda::std::end(arr_multiple)), Comparable(4));
      assert(*iter == 4);
      assert(base(iter) == arr_multiple + 1);
    }

    { // some element matches
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr)), Iter(cuda::std::end(arr)), Comparable(6));
      assert(*iter == 6);
      assert(base(iter) == arr + 2);
    }

    { // last element matches
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr)), Iter(cuda::std::end(arr)), Comparable(8));
      assert(*iter == 8);
      assert(base(iter) == arr + 3);
    }

    { // if no element matches, last is returned
      Iter iter = cuda::std::find_if_not(Iter(cuda::std::begin(arr)), Iter(cuda::std::end(arr)), Comparable(10));

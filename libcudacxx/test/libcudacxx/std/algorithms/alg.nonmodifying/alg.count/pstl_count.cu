...
void test_count(const Policy& policy)
{
  { // empty should not access anything
    const auto res = cuda::std::count(policy, static_cast<int*>(nullptr), static_cast<int*>(nullptr), 42);
    CHECK(res == 0);
  }
  ...
  { // convertible type
    const auto res =
      cuda::std::count(policy, cuda::counting_iterator{0}, cuda::counting_iterator{size}, static_cast<short>(42));
    CHECK(res == 1);
  }
}

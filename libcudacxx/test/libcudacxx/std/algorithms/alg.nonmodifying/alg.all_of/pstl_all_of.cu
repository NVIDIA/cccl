...
void test_all_of(const Policy& policy)
{
  { // empty should not access anything
    const auto res =
      cuda::std::all_of(policy, static_cast<int*>(nullptr), static_cast<int*>(nullptr), less_than_val{size + 1});
    CHECK(res);
  }

  { // same type
    const auto res = cuda::std::all_of(
      policy, cuda::counting_iterator{short{0}}, cuda::counting_iterator{size}, less_than_val<short>{size + 1});
    CHECK(res);
  }
  ...
  { // convertible pred
    const auto res = cuda::std::all_of(
      policy, cuda::counting_iterator{short{0}}, cuda::counting_iterator{size}, less_than_val<int>{size + 1});
    CHECK(res);
  }

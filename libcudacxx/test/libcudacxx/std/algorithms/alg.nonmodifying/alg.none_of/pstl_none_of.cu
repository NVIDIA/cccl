...
C2H_TEST("cuda::std::none_of", "[parallel algorithm]")
{
  SECTION("with default stream")
  {
    ...
    { // true case
      const auto res =
        cuda::std::none_of(policy, cuda::counting_iterator{size_t{0}}, cuda::counting_iterator{size}, less_than_val{0});
      CHECK(res);
    }

    { // false case
      const auto res =
        cuda::std::none_of(policy, cuda::counting_iterator{size_t{0}}, cuda::counting_iterator{size}, less_than_val{1});
      CHECK(!res);
    }
  }

...
C2H_TEST("cuda::std::copy_if", "[parallel algorithm]")
{
  ...
  SECTION("with default stream")
  {
    ...
    { // With non-contiguous iterator
      cuda::std::copy_if(policy, cuda::counting_iterator{0}, cuda::counting_iterator{size}, output.begin(), is_even{});
  ...
  SECTION("with provided stream")
  {
    ...
    { // With contiguous iterator
      cuda::std::copy_if(policy, input.begin(), input.end(), output.begin(), is_even{});
      CHECK(thrust::equal(output.begin(), output.end(), cuda::strided_iterator{cuda::counting_iterator{0}, 2}));
    }
  }

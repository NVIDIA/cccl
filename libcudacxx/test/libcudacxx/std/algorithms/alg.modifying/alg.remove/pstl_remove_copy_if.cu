                         ...
                         thrust::device_vector<int>& output)
{
  ...
  { // With matching predicate (copy odd elements)
    ...
    const auto res = cuda::std::remove_copy_if(policy, input.begin(), input.end(), output.begin(), is_even{});
  ...
  { // With conversion for predicate
    thrust::sequence(input.begin(), input.end(), 0);
    const auto res =
      cuda::std::remove_copy_if(policy, input.begin(), input.end(), output.begin(), is_even<cuda::std::ptrdiff_t>{});
    CHECK(thrust::equal(output.begin(), res, cuda::strided_iterator{cuda::counting_iterator{1}, 2}));
    CHECK(cuda::std::distance(output.begin(), res) == size / 2);
  }
}
...
C2H_TEST("cuda::std::remove_copy_if", "[parallel algorithm]")
{
  ...
  SECTION("with default stream")
  {
    ...
    test_remove_copy_if(policy, input, output);
  }

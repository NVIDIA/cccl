...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
  {
    ...
    assert(!cuda::std::is_partitioned(
        cpp17_input_iterator<const int*>(cuda::std::begin(ia)),
        cpp17_input_iterator<const int*>(cuda::std::end(ia)), pred));
    assert(static_cast<cuda::std::ptrdiff_t>(pred.count()) <=
           cuda::std::distance(cuda::std::begin(ia), cuda::std::end(ia)));
  }
  {
  ...
  }
  {
    ...
    assert(cuda::std::is_partitioned(
        cpp17_input_iterator<const int*>(cuda::std::begin(ia)),
        cpp17_input_iterator<const int*>(cuda::std::begin(ia)), pred));
    assert(static_cast<cuda::std::ptrdiff_t>(pred.count()) <=

...
TEST_FUNC constexpr void test(const int (&input_data)[num_elements])
{
  ...
  Iter i = cuda::std::max_element(first, last, cuda::std::greater<int>());
  if (first != last)
  {
    for (Iter j = first; j != last; ++j)
    {
      assert(!cuda::std::greater<int>()(*i, *j));
    }
  }
  else
  {
...
TEST_FUNC constexpr void test_eq(Iter first, Iter last, Pred p)
{
  assert(first == cuda::std::max_element(first, last, p));
}

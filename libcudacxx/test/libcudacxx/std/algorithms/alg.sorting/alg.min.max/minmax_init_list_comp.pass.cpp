...
test_all_equal(cuda::std::initializer_list<int> il) {
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
  assert((cuda::std::minmax({1, 2, 3}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({1, 3, 2}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({2, 1, 3}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({2, 3, 1}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({3, 1, 2}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({3, 2, 1}, cuda::std::greater<int>()) ==
          cuda::std::pair<int, int>(3, 1)));
  assert((cuda::std::minmax({1, 2, 3}, all_equal{}) ==
          cuda::std::pair<int, int>(1, 3)));

  binary_counting_predicate<cuda::std::greater<int>, int, int> pred(
      (cuda::std::greater<int>()));
  assert((cuda::std::minmax({1, 2, 2, 3, 3, 3, 5, 5, 5, 5, 5, 3}, pred) ==
          cuda::std::pair<int, int>(5, 1)));

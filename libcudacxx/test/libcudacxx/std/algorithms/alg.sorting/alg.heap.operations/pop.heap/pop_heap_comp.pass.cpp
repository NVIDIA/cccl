...
__host__ __device__ TEST_CONSTEXPR_CXX14 void test()
{
...

    {
        ...
        cuda::std::pop_heap(Iter(input), Iter(input + 5), cuda::std::greater<T>()); assert(input[4] == 1);
        cuda::std::pop_heap(Iter(input), Iter(input + 4), cuda::std::greater<T>()); assert(input[3] == 2);
        cuda::std::pop_heap(Iter(input), Iter(input + 3), cuda::std::greater<T>()); assert(input[2] == 3);
        cuda::std::pop_heap(Iter(input), Iter(input + 2), cuda::std::greater<T>()); assert(input[1] == 4);
        cuda::std::pop_heap(Iter(input), Iter(input + 1), cuda::std::greater<T>()); assert(input[0] == 5);
    }
}

...
__host__ __device__ TEST_CONSTEXPR_CXX14 void test4() {
  ...
  const T b[] = {22, 32, 43, 42, 52};
  {
    T result[20] = {};
    T expected[] = {33, 41};
    OutIter end = cuda::std::set_intersection(Iter1(a), Iter1(a + 4), Iter2(b),
                                              Iter2(b + 5), OutIter(result));
    assert(cuda::std::lexicographical_compare(result, base(end), expected,
                                              expected + 2, T::less) == 0);
  ...
  }
  {
    T result[20] = {};
    T expected[] = {32, 42};
    OutIter end = cuda::std::set_intersection(Iter1(b), Iter1(b + 5), Iter2(a),
                                              Iter2(a + 4), OutIter(result));
    assert(cuda::std::lexicographical_compare(result, base(end), expected,
                                              expected + 2, T::less) == 0);
    for (const T* it = base(end); it != result + 20; ++it) {
      assert(it->value == 0);

...
__host__ __device__ TEST_CONSTEXPR_CXX14 void test() {
  ...
  for (int n = 0; n < 15; ++n) {
    for (int m = 0; m < 15; ++m) {
      OutIter it = cuda::std::partial_sort_copy(
          Iter(orig), Iter(orig + n), OutIter(work), OutIter(work + m),
          cuda::std::greater<T>());
      if (n <= m) {
...

  {
    int input[] = {3, 4, 2, 5, 1};
    T output[] = {0, 0, 0};
    cuda::std::partial_sort_copy(Iter(input), Iter(input + 5), OutIter(output),
                                 OutIter(output + 3), cuda::std::greater<T>());
    assert(output[0] == 5);
    assert(output[1] == 4);
    assert(output[2] == 3);
  }
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool test() {
  ...
  test<MoveOnly, random_access_iterator<int*>,
       random_access_iterator<MoveOnly*> >();

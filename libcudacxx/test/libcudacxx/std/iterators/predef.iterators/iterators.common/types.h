...
struct sentinel_throws_on_convert
{
  ...
  template <class U>
  friend constexpr bool operator==(const sentinel_throws_on_convert& lhs, const U& rhs)
  {
    return lhs.base_ == base(rhs);
  }
  template <class U>
  friend constexpr bool operator==(const U& lhs, const sentinel_throws_on_convert& rhs)
  {
    return base(lhs) == rhs.base_;
  }
#  if TEST_STD_VER < 2020
...
#  endif // TEST_STD_VER < 2020
  ...
  [[noreturn]] operator sentinel_type<int*>() const
  {
    throw 42;
  }
};
...
public:
  ...
  friend constexpr It base(const maybe_valueless_iterator& i)
  {
    return i.it_;
  }

  maybe_valueless_iterator() = default;
  explicit constexpr maybe_valueless_iterator(It it)
      : it_(it)
  {}

  maybe_valueless_iterator(const forward_iterator<It>& it)
      : it_(base(it))
  {}
  ...
  constexpr maybe_valueless_iterator& operator++()
  {
    ++it_;
    return *this;
  }

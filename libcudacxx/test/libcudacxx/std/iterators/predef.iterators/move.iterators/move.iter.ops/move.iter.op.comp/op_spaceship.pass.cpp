...
static_assert(!HasSpaceship<cuda::std::move_iterator<int*>,
                            cuda::std::move_iterator<char*> >);
static_assert(HasSpaceship<cuda::std::move_iterator<int*>,
                           cuda::std::move_iterator<int*> >);
static_assert(HasSpaceship<cuda::std::move_iterator<int*>,
                           cuda::std::move_iterator<const int*> >);
static_assert(HasSpaceship<cuda::std::move_iterator<const int*>,
                           cuda::std::move_iterator<const int*> >);
static_assert(
    !HasSpaceship<cuda::std::move_iterator<forward_iterator<int*> >,
                  cuda::std::move_iterator<forward_iterator<int*> > >);
static_assert(
    !HasSpaceship<cuda::std::move_iterator<random_access_iterator<int*> >,
                  cuda::std::move_iterator<random_access_iterator<int*> > >);
static_assert(
    !HasSpaceship<cuda::std::move_iterator<contiguous_iterator<int*> >,
                  cuda::std::move_iterator<contiguous_iterator<int*> > >);
static_assert(HasSpaceship<
...
__host__ __device__ void test_spaceshippable_but_not_three_way_comparable() {
  struct A {
    ...
    __host__ __device__ cuda::std::strong_ordering operator<= >
        (const A&) const;
  };
  struct B {
    using value_type = int;
  ...
  static_assert(
      HasEquals<cuda::std::move_iterator<A>, cuda::std::move_iterator<B> >);
  static_assert(
      !HasSpaceship<cuda::std::move_iterator<A>, cuda::std::move_iterator<B> >);
}
...
__host__ __device__ constexpr void test_two() {
  ...
  const cuda::std::move_iterator<It> i2 =
      cuda::std::move_iterator<It>(It(a + 2));
  const cuda::std::move_iterator<Jt> j1 = cuda::std::move_iterator<Jt>(Jt(a));
  const cuda::std::move_iterator<Jt> j2 =
      cuda::std::move_iterator<Jt>(Jt(a + 2));
...
__host__ __device__ constexpr bool test() {
  ...
  test_two<three_way_contiguous_iterator<int*>,
           three_way_contiguous_iterator<int*> >();
  return true;
}

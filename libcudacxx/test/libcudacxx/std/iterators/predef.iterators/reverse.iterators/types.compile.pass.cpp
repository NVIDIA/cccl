...
{
    TEST_HOST_DEVICE void test() { (void)this->current; }
};
...
TEST_HOST_DEVICE constexpr void test_iter_category() {
...
struct BarIter {
  ...
  TEST_HOST_DEVICE BarIter& operator++();
  TEST_HOST_DEVICE BarIter& operator--();
  TEST_HOST_DEVICE BarIter operator++(int);
  TEST_HOST_DEVICE BarIter operator--(int);
};
template <>

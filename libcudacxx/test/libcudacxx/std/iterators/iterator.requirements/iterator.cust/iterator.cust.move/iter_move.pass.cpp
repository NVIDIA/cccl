...
__host__ __device__ constexpr void unqualified_lookup_move(It first_, It last_,
                                                           Out result_first_,
                                                           Out result_last_) {
  auto first = ::check_unqualified_lookup::unqualified_lookup_wrapper<It>{
      cuda::std::move(first_)};
  auto last = ::check_unqualified_lookup::unqualified_lookup_wrapper<It>{
      cuda::std::move(last_)};
  auto result_first =
      ::check_unqualified_lookup::unqualified_lookup_wrapper<It>{
          cuda::std::move(result_first_)};
  auto result_last = ::check_unqualified_lookup::unqualified_lookup_wrapper<It>{
      cuda::std::move(result_last_)};
  ...
  for (; first != last && result_first != result_last;
       (void)++first, ++result_first) {
...
rvalue_move(It first_, It last_, Out result_first_, Out result_last_) {
  ...
  static_assert(
      noexcept(cuda::std::ranges::iter_move(cuda::std::move(first))),
...
struct WithADL {
  ...
  __host__ __device__ friend constexpr int iter_move(WithADL&&)
      noexcept(NoExcept) {
    return 0;
  }
};
...
__host__ __device__ constexpr bool test() {
  ...
  unqualified_lookup_move(cuda::std::begin(v1), cuda::std::end(v1),
                          cuda::std::begin(v2), cuda::std::end(v2));
  assert(all_of(cuda::std::cbegin(v1), cuda::std::cend(v1),
                move_counter_is(reset)));
  assert(
      all_of(cuda::std::cbegin(v2), cuda::std::cend(v2), move_counter_is(1)));

  move_tracker v3[half_size];
  unqualified_lookup_move(cuda::std::begin(v1) + half_size, cuda::std::end(v1),
                          cuda::std::begin(v3), cuda::std::end(v3));
  assert(all_of(cuda::std::cbegin(v1), cuda::std::cend(v1),
                move_counter_is(reset)));
  assert(
      all_of(cuda::std::cbegin(v3), cuda::std::cend(v3), move_counter_is(1)));

  move_tracker v4[half_size];
  unqualified_lookup_move(cuda::std::begin(v3), cuda::std::end(v3),
                          cuda::std::begin(v4), cuda::std::end(v4));
  assert(all_of(cuda::std::cbegin(v3), cuda::std::cend(v3),
                move_counter_is(reset)));
  assert(
      all_of(cuda::std::cbegin(v4), cuda::std::cend(v4), move_counter_is(2)));

  lvalue_move(cuda::std::begin(v2), cuda::std::end(v2),
              cuda::std::begin(v1) + half_size, cuda::std::end(v1));
  assert(all_of(cuda::std::cbegin(v2), cuda::std::cend(v2),
                move_counter_is(reset)));
  assert(all_of(cuda::std::cbegin(v1) + half_size, cuda::std::cend(v1),
                move_counter_is(2)));

  lvalue_move(cuda::std::begin(v4), cuda::std::end(v4), cuda::std::begin(v1),
              cuda::std::end(v1));
  assert(all_of(cuda::std::cbegin(v4), cuda::std::cend(v4),
                move_counter_is(reset)));
  assert(all_of(cuda::std::cbegin(v1), cuda::std::cbegin(v1) + half_size,
                move_counter_is(3)));

  rvalue_move(cuda::std::begin(v1), cuda::std::end(v1), cuda::std::begin(v2),
              cuda::std::end(v2));
  assert(all_of(cuda::std::cbegin(v1), cuda::std::cbegin(v1) + half_size,
                move_counter_is(reset)));
  assert(
      all_of(cuda::std::cbegin(v2), cuda::std::cend(v2), move_counter_is(4)));

  rvalue_move(cuda::std::begin(v1) + half_size, cuda::std::end(v1),
              cuda::std::begin(v3), cuda::std::end(v3));
  assert(all_of(cuda::std::cbegin(v1), cuda::std::cend(v1),
                move_counter_is(reset)));
  assert(
      all_of(cuda::std::cbegin(v3), cuda::std::cend(v3), move_counter_is(3)));

  auto unscoped = check_unqualified_lookup::unscoped_enum::a;
  assert(cuda::std::ranges::iter_move(unscoped) ==
         check_unqualified_lookup::unscoped_enum::a);
#ifndef TEST_COMPILER_ICC
  ...
  static_assert(noexcept(
      cuda::std::ranges::iter_move(cuda::std::declval<WithADL<true> >())));
  static_assert(noexcept(
      cuda::std::ranges::iter_move(cuda::std::declval<WithoutADL<true> >())));
// old GCC seems to fall over the chaining of the noexcept clauses here
#if (!defined(TEST_COMPILER_GCC) || __GNUC__ >= 9)
  static_assert(!noexcept(
      cuda::std::ranges::iter_move(cuda::std::declval<WithADL<false> >())));
  static_assert(!noexcept(
      cuda::std::ranges::iter_move(cuda::std::declval<WithoutADL<false> >())));
#endif
#endif // TEST_COMPILER_ICC
...
static_assert(
    !cuda::std::is_invocable_v<IterMoveT, int*, int*>); // too many arguments

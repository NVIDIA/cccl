...
public:
  ...
  TEST_FUNC constexpr explicit unqualified_lookup_wrapper(I i) noexcept
  ...
  TEST_FUNC constexpr unqualified_lookup_wrapper& operator++() noexcept
  {
    ++base_;
    return *this;
  }
  ...
  TEST_FUNC constexpr bool operator==(unqualified_lookup_wrapper const& other) const noexcept
  {
  ...
  TEST_FUNC friend constexpr decltype(auto) iter_move(unqualified_lookup_wrapper& i) noexcept(false)
  {
...
TEST_FUNC constexpr unscoped_enum iter_move(unscoped_enum& e) noexcept(false)
{
  return e;
}
...
TEST_FUNC constexpr scoped_enum* iter_move(scoped_enum&) noexcept
{
  return nullptr;
}
...
TEST_FUNC constexpr int iter_move(some_union& u) noexcept(false)
{
  return u.x;
}
} // namespace check_unqualified_lookup
...
public:
  ...
  TEST_FUNC constexpr move_tracker(move_tracker&& other) noexcept
      : moves_{other.moves_ + 1}
  {
    other.moves_ = 0;
  }
  TEST_FUNC constexpr move_tracker& operator=(move_tracker&& other) noexcept
  {
    moves_       = other.moves_ + 1;
    other.moves_ = 0;
    return *this;
  ...
  TEST_FUNC constexpr int moves() const noexcept
  {
    return moves_;
  }

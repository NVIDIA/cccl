...
struct cuda::std::pointer_traits<to_address_wrong_return_type> {
  ...
  to_address(to_address_wrong_return_type const&);
};
...
static_assert(
    cuda::std::random_access_iterator<template_and_no_element_type<int> >);
static_assert(
    cuda::std::contiguous_iterator<template_and_no_element_type<int> >);
...
template <>
struct cuda::std::pointer_traits<
    no_operator_arrow</*DisableArrow=*/true, /*DisableToAddress=*/false> > {
  __host__ __device__ static constexpr int*
  to_address(const no_operator_arrow<true, false>&);
};

static_assert(
    cuda::std::contiguous_iterator<
        no_operator_arrow</*DisableArrow=*/false, /*DisableToAddress=*/true> >);
static_assert(
    !cuda::std::contiguous_iterator<
        no_operator_arrow</*DisableArrow=*/true, /*DisableToAddress=*/true> >);
static_assert(
    cuda::std::contiguous_iterator<
        no_operator_arrow</*DisableArrow=*/true, /*DisableToAddress=*/false> >);

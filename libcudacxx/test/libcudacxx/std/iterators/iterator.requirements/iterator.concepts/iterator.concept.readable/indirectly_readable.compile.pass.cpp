...
template <class In>
TEST_FUNC constexpr bool check_indirectly_readable()
{
...
struct indirection_mismatch
{
  ...
  TEST_FUNC float& operator*() const;
};
static_assert(
...
struct iter_move_mismatch
{
  ...
  TEST_FUNC friend float& iter_move(iter_move_mismatch&);
};
...
struct indirection_and_iter_move_mismatch
{
  ...
  TEST_FUNC friend unsigned long long& iter_move(indirection_and_iter_move_mismatch&);
};
...
{
  using value_type = int;
  TEST_FUNC iter_ref1& operator*() const;
};
...
struct unrelated_iter_ref_rvalue_and_iter_rvalue_ref_rvalue
{
  ...
  TEST_FUNC iter_ref2& operator*() const;
  TEST_FUNC friend iter_rvalue_ref&& iter_move(unrelated_iter_ref_rvalue_and_iter_rvalue_ref_rvalue);
};
static_assert(!check_indirectly_readable<unrelated_iter_ref_rvalue_and_iter_rvalue_ref_rvalue>());

struct iter_ref3
{
  TEST_FUNC operator iter_rvalue_ref() const;
};
namespace cuda::std
{
...
struct different_reference_types_with_common_reference
{
  ...
  TEST_FUNC iter_ref3& operator*() const;
  TEST_FUNC friend iter_rvalue_ref&& iter_move(different_reference_types_with_common_reference);
};
...
struct different_reference_types_without_common_reference_to_const
{
  ...
  TEST_FUNC iter_ref4& operator*() const;
  TEST_FUNC friend iter_rvalue_ref&& iter_move(different_reference_types_without_common_reference_to_const);
};
...
{
  using value_type = void;
  TEST_FUNC legacy_output_iterator& operator*();
};

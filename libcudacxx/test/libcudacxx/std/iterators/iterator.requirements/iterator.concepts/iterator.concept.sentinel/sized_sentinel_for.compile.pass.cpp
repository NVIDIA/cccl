...
struct int_sized_sentinel {
  ...
  TEST_HOST_DEVICE friend bool operator==(int_sized_sentinel, int*);
#else
  TEST_HOST_DEVICE friend bool operator==(int_sized_sentinel, int*);
  TEST_HOST_DEVICE friend bool operator==(int*,int_sized_sentinel);
  TEST_HOST_DEVICE friend bool operator!=(int_sized_sentinel, int*);
  TEST_HOST_DEVICE friend bool operator!=(int*,int_sized_sentinel);
#endif
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(int_sized_sentinel, int*);
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(int*, int_sized_sentinel);
};
...
#if TEST_STD_VER > 2017
  TEST_HOST_DEVICE bool operator==(cuda::std::input_or_output_iterator auto) const { return true; };
#else
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_HOST_DEVICE bool operator==(const It&) const { return true; };
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_HOST_DEVICE bool operator!=(const It&) const { return false; };
#endif
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(no_default_ctor, int*);
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(int*, no_default_ctor);
};
...
struct not_copyable {
  ...
  TEST_HOST_DEVICE bool operator!=(const It&) const { return false; };
#endif
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(not_copyable, int*);
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(int*, not_copyable);
};
static_assert(!cuda::std::sized_sentinel_for<not_copyable, int*>);

struct double_sized_sentinel {
  TEST_HOST_DEVICE friend bool operator==(double_sized_sentinel, double*);
  TEST_HOST_DEVICE friend int operator-(double_sized_sentinel, double*);
  TEST_HOST_DEVICE friend int operator-(double*, double_sized_sentinel);
};
template <>
...
struct only_one_sub_op {
  ...
  TEST_HOST_DEVICE friend bool operator==(only_one_sub_op, It);
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int>>
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(only_one_sub_op, It);
};
static_assert(!cuda::std::sized_sentinel_for<only_one_sub_op, int*>);

struct wrong_return_type {
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int>>
  TEST_HOST_DEVICE friend bool operator==(wrong_return_type, It);
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int>>
  TEST_HOST_DEVICE friend cuda::std::ptrdiff_t operator-(wrong_return_type, It);
  template<class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int>>
  TEST_HOST_DEVICE friend void operator-(It, wrong_return_type);
};

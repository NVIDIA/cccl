...
struct short_container {
  __host__ __device__ uint16_t size() const { return 60000; } // not noexcept
};
...
__host__ __device__ void test_container(C& c) {
  ...
  assert(cuda::std::ssize(c) ==
         static_cast<decltype(cuda::std::ssize(c))>(c.size()));
}
...
test_const_container(const cuda::std::initializer_list<T>& c) {
  LIBCPP_ASSERT_NOEXCEPT(
      cuda::std::ssize(c)); // our cuda::std::ssize is conditionally noexcept
  static_assert(cuda::std::is_signed_v<decltype(cuda::std::ssize(c))>, "");
  assert(cuda::std::ssize(c) ==
         static_cast<decltype(cuda::std::ssize(c))>(c.size()));
}
...
int main(int, char**) {
  ...
  static_assert(
      cuda::std::numeric_limits<decltype(cuda::std::ssize(sc))>::max() > 60000,
      "");
  static_assert(
      cuda::std::numeric_limits<
          cuda::std::make_signed_t<decltype(cuda::std::size(sc))> >::max() <
          60000,
      "");

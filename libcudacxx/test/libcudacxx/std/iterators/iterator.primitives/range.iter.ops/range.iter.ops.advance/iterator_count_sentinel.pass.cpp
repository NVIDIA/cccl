...
check_forward(int* first, int* last, cuda::std::iter_difference_t<It> n,
              int* expected) {
...
__host__ __device__ constexpr void check_forward_sized_sentinel(
    int* first, int* last, cuda::std::iter_difference_t<It> n, int* expected) {
  using Difference = cuda::std::iter_difference_t<It>;
...
template <typename It>
__host__ __device__ constexpr void
check_backward(int* first, int* last, cuda::std::iter_difference_t<It> n,
               int* expected) {
  static_assert(cuda::std::random_access_iterator<It>,
                "This test doesn't support non random access iterators");
  using Difference = cuda::std::iter_difference_t<It>;
  Difference const M =
      (expected - last); // expected travel distance (which is negative)
...
struct iota_iterator {
  ...
  operator==(const iota_iterator& other) const {
    return x == other.x;
  }
  __host__ __device__ constexpr bool
  operator!=(const iota_iterator& other) const {
...
__host__ __device__ constexpr bool test() {
  ...
  for (int size = 0; size != 10; ++size) {
    for (int n = 0; n != 20; ++n) {

      {
        ...
        check_forward<false, cpp17_input_iterator<int*> >(range, range + size,
                                                          n, expected);
        check_forward<false, cpp20_input_iterator<int*> >(range, range + size,
                                                          n, expected);
        check_forward<true, forward_iterator<int*> >(range, range + size, n,
                                                     expected);
        check_forward<true, bidirectional_iterator<int*> >(range, range + size,
                                                           n, expected);
        check_forward<true, random_access_iterator<int*> >(range, range + size,
                                                           n, expected);
        check_forward<true, contiguous_iterator<int*> >(range, range + size, n,
                                                        expected);
        check_forward<true, int*>(range, range + size, n, expected);

        check_forward_sized_sentinel<cpp17_input_iterator<int*> >(
            range, range + size, n, expected);
        check_forward_sized_sentinel<cpp20_input_iterator<int*> >(
            range, range + size, n, expected);
        check_forward_sized_sentinel<forward_iterator<int*> >(
            range, range + size, n, expected);
        check_forward_sized_sentinel<bidirectional_iterator<int*> >(
            range, range + size, n, expected);
        check_forward_sized_sentinel<random_access_iterator<int*> >(
            range, range + size, n, expected);
        check_forward_sized_sentinel<contiguous_iterator<int*> >(
            range, range + size, n, expected);
...

      {
        ...
        check_backward<random_access_iterator<int*> >(range, range + size, -n,
                                                      expected);
        check_backward<contiguous_iterator<int*> >(range, range + size, -n,
                                                   expected);
  ...
  // Regression-test that INT_MIN doesn't cause any undefined behavior
  {
    ...
    assert(cuda::std::ranges::advance(i, INT_MIN, iota_iterator{-2}) ==
           INT_MIN + 3);
    ...
    i = iota_iterator{+1};
    assert(cuda::std::ranges::advance(i, INT_MIN, iota_iterator{INT_MIN + 1}) ==
           0);

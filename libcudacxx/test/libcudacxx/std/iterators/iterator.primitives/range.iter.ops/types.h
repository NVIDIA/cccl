...
public:
  ...
  TEST_FUNC constexpr explicit distance_apriori_sentinel(cuda::std::ptrdiff_t const count)
      : count_(count)
  {}

  template <class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_FUNC friend constexpr bool operator==(distance_apriori_sentinel const, It const&)
  {
  ...
  template <class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_FUNC friend constexpr bool operator==(It const&, distance_apriori_sentinel const)
  {
    assert(false && "difference op should take precedence");
    return false;
  }
  ...
  template <class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_FUNC friend constexpr cuda::std::ptrdiff_t operator-(It const&, distance_apriori_sentinel const y)
  {
    return -y.count_;
  }

  template <class It, cuda::std::enable_if_t<cuda::std::input_or_output_iterator<It>, int> = 0>
  TEST_FUNC friend constexpr cuda::std::ptrdiff_t operator-(distance_apriori_sentinel const x, It const&)
  {
    return x.count_;
  }
...
public:
  ...
  TEST_FUNC constexpr explicit assignable_sentinel(const It& it)
      : base_(base(it))
  {}
  TEST_FUNC constexpr operator It() const
  {
    return It(base_);
  }
  TEST_FUNC friend constexpr bool operator==(const assignable_sentinel& s, const It& other)
  {
    return s.base_ == base(other);
  }
#if TEST_STD_VER < 2020
  TEST_FUNC friend constexpr bool operator==(const It& other, const assignable_sentinel& s)
  {
    return s.base_ == base(other);
  }
  TEST_FUNC friend constexpr bool operator!=(const assignable_sentinel& s, const It& other)
...
#endif
  TEST_FUNC friend constexpr It base(const assignable_sentinel& s)
  {
    return It(s.base_);
  }

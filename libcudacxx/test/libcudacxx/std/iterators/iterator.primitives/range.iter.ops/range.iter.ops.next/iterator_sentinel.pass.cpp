...
__host__ __device__ constexpr bool test() {
  ...
  for (int n = 0; n != 10; ++n) {
    ...
    check_assignable<bidirectional_iterator<int*> >(range, range + n,
                                                    range + n);
    check_assignable<random_access_iterator<int*> >(range, range + n,
                                                    range + n);
    check_assignable<contiguous_iterator<int*> >(range, range + n, range + n);
    check_assignable<int*>(range, range + n, range + n);

    check_sized_sentinel<cpp17_input_iterator<int*> >(range, range + n,
                                                      range + n);
    check_sized_sentinel<cpp20_input_iterator<int*> >(range, range + n,
                                                      range + n);
    check_sized_sentinel<forward_iterator<int*> >(range, range + n, range + n);
    check_sized_sentinel<bidirectional_iterator<int*> >(range, range + n,
                                                        range + n);
    check_sized_sentinel<random_access_iterator<int*> >(range, range + n,
                                                        range + n);
    check_sized_sentinel<contiguous_iterator<int*> >(range, range + n,
                                                     range + n);

...
TEST_FUNC constexpr void test_unsized()
{
...
template <class It, class Sent>
TEST_FUNC constexpr void test_sized()
{
...
struct StrideCounter
{
  ...
  TEST_FUNC explicit StrideCounter();
  TEST_FUNC constexpr explicit StrideCounter(int* it, int* inc)
      : it_(it)
      , inc_(inc)
  {}
  TEST_FUNC constexpr auto& operator++()
  {
    ++it_;
    *inc_ += 1;
    return *this;
  }
  TEST_FUNC StrideCounter operator++(int);
  TEST_FUNC int& operator*() const;
  TEST_FUNC bool operator==(StrideCounter) const;
  TEST_FUNC bool operator!=(StrideCounter) const;
};
...
struct SizedStrideCounter
{
  ...
  int* minus_;
  using value_type = int;
  TEST_FUNC explicit SizedStrideCounter();
  TEST_FUNC constexpr explicit SizedStrideCounter(int* it, int* minus)
      : it_(it)
      , minus_(minus)
  {}
  TEST_FUNC SizedStrideCounter& operator++();
  TEST_FUNC SizedStrideCounter operator++(int);
  TEST_FUNC int& operator*() const;
  TEST_FUNC bool operator==(SizedStrideCounter) const;
  TEST_FUNC bool operator!=(SizedStrideCounter) const;
  TEST_FUNC constexpr int operator-(SizedStrideCounter rhs) const
  {
    *minus_ += 1;
...
TEST_FUNC constexpr void test_stride_counting()
{
  {
    int a[] = {1, 2, 3};
    int inc = 0;

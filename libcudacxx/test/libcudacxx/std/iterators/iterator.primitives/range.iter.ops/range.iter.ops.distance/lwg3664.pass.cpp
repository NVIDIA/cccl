...
{
  It p_;
  TEST_FUNC friend constexpr bool operator==(EvilSentinel s, It p)
  {
    return s.p_ == p;
  }
#if TEST_STD_VER < 2020
  TEST_FUNC friend constexpr bool operator==(It p, EvilSentinel s)
  {
    return s.p_ == p;
  }
  TEST_FUNC friend constexpr bool operator!=(EvilSentinel s, It p)
...
#endif
  TEST_FUNC friend constexpr auto operator-(EvilSentinel s, It p)
  {
    return s.p_ - p;
  }
  TEST_FUNC friend constexpr auto operator-(It p, EvilSentinel s)
  {
    return p - s.p_;
  }
// Older clang confuses the all deleted overloads
#if !TEST_COMPILER(CLANG, <, 10)
  TEST_FUNC friend constexpr void operator-(EvilSentinel s, int (&)[3])       = delete;
  TEST_FUNC friend constexpr void operator-(EvilSentinel s, const int (&)[3]) = delete;

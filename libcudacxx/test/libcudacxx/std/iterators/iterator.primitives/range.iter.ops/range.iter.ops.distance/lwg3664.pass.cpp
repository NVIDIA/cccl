...
// Older clang confuses the all deleted overloads
  ...
  __host__ __device__ friend constexpr void operator-(EvilSentinel s,
                                                      int (&)[3]) = delete;
  __host__ __device__ friend constexpr void
  operator-(EvilSentinel s, const int (&)[3]) = delete;
...
static_assert(
    cuda::std::sized_sentinel_for<EvilSentinel<const int*>, const int*>);
...
__host__ __device__ constexpr bool test() {
  ...
  }
  {
    ...
    assert(cuda::std::ranges::distance(cuda::std::move(a),
                                       EvilSentinel<int*>{a + 3}) == 3);
  }
  {
    const int a[] = {1, 2, 3};
    assert(cuda::std::ranges::distance(a, EvilSentinel<const int*>{a + 3}) ==
           3);
    assert(cuda::std::ranges::distance(a, EvilSentinel<const int*>{a}) == 0);
    assert(cuda::std::ranges::distance(a + 3, EvilSentinel<const int*>{a}) ==
           -3);
    assert(cuda::std::ranges::distance(cuda::std::move(a),
                                       EvilSentinel<const int*>{a + 3}) == 3);
    static_assert(
        !cuda::std::is_invocable_v<decltype(cuda::std::ranges::distance),
                                   const int(&)[3], EvilSentinel<int*> >);
    static_assert(
        !cuda::std::is_invocable_v<decltype(cuda::std::ranges::distance),
                                   const int(&&)[3], EvilSentinel<int*> >);
  }

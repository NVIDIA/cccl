...
__host__ __device__ TEST_CONSTEXPR_CXX14 void check_advance(It it, Distance n,
                                                            It result) {
  static_assert(
      cuda::std::is_same<decltype(cuda::std::advance(it, n)), void>::value, "");
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool tests() {
  ...
  // Check with iterator_traits::difference_type
  {
    ...
    check_advance<Distance>(cpp17_input_iterator<const char*>(s), 10,
                            cpp17_input_iterator<const char*>(s + 10));
    check_advance<Distance>(forward_iterator<const char*>(s), 10,
  ...
  // Also check with other distance types
  {
    ...
    check_advance<Distance>(forward_iterator<const char*>(s), 10,
                            forward_iterator<const char*>(s + 10));
    check_advance<Distance>(bidirectional_iterator<const char*>(s), 10,
                            bidirectional_iterator<const char*>(s + 10));
    check_advance<Distance>(random_access_iterator<const char*>(s), 10,
                            random_access_iterator<const char*>(s + 10));
  }
  ...
  // Check with unsigned distance types to catch signedness-change issues
  {
    ...
    check_advance<Distance>(cpp17_input_iterator<const char*>(s), 10u,
                            cpp17_input_iterator<const char*>(s + 10));
    check_advance<Distance>(forward_iterator<const char*>(s), 10u,
                            forward_iterator<const char*>(s + 10));
    check_advance<Distance>(bidirectional_iterator<const char*>(s), 10u,
                            bidirectional_iterator<const char*>(s + 10));
    check_advance<Distance>(random_access_iterator<const char*>(s), 10u,
                            random_access_iterator<const char*>(s + 10));
  }

...
check_prev_n(It it, typename cuda::std::iterator_traits<It>::difference_type n,
             It result) {
  static_assert(cuda::std::is_same<decltype(cuda::std::prev(it, n)), It>::value,
                "");
  assert(cuda::std::prev(it, n) == result);

  It (*prev_ptr)(It, typename cuda::std::iterator_traits<It>::difference_type) =
      cuda::std::prev;
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool tests() {
  ...
  check_prev_n(forward_iterator<const char*>(s), -10,
               forward_iterator<const char*>(s + 10));
  check_prev_n(bidirectional_iterator<const char*>(s + 10), 10,
               bidirectional_iterator<const char*>(s));
  check_prev_n(bidirectional_iterator<const char*>(s), -10,
               bidirectional_iterator<const char*>(s + 10));
  check_prev_n(random_access_iterator<const char*>(s + 10), 10,
               random_access_iterator<const char*>(s));
  check_prev_n(random_access_iterator<const char*>(s), -10,
               random_access_iterator<const char*>(s + 10));
  check_prev_n(s + 10, 10, s);

  check_prev_1(bidirectional_iterator<const char*>(s + 1),
               bidirectional_iterator<const char*>(s));
  check_prev_1(random_access_iterator<const char*>(s + 1),
               random_access_iterator<const char*>(s));
  check_prev_1(s + 1, s);

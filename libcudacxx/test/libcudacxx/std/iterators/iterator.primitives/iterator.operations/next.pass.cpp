...
check_next_n(It it, typename cuda::std::iterator_traits<It>::difference_type n,
             It result) {
  static_assert(cuda::std::is_same<decltype(cuda::std::next(it, n)), It>::value,
                "");
  assert(cuda::std::next(it, n) == result);

  It (*next_ptr)(It, typename cuda::std::iterator_traits<It>::difference_type) =
      cuda::std::next;
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool tests() {
  ...
  check_next_n(cpp17_input_iterator<const char*>(s), 10,
  ...
  check_next_1(cpp17_input_iterator<const char*>(s),

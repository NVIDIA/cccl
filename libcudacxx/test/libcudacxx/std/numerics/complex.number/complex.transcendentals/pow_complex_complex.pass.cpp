...
template <class T>
__host__ __device__ void test(const cuda::std::complex<T>& a,
                              const cuda::std::complex<T>& b,
...
__host__ __device__ void test() {
  test(cuda::std::complex<T>(2, 3), cuda::std::complex<T>(2, 0),
       cuda::std::complex<T>(-5, 12));
}
  ...
  for (unsigned i = 0; i < N; ++i) {
    for (unsigned j = 0; j < N; ++j) {
      ...
      cuda::std::complex<T> z =
          cuda::std::exp(testcases[j] * log(testcases[i]));

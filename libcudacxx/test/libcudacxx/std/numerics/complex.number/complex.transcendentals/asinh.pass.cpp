...
__host__ __device__ void test_edges() {
  ...
  for (unsigned i = 0; i < N; ++i) {
    cuda::std::complex<T> r = asinh(testcases[i]);
    if (testcases[i].real() == T(0) && testcases[i].imag() == T(0)) {
      ...
      assert(cuda::std::signbit(r.imag()) ==
             cuda::std::signbit(testcases[i].imag()));
               ...
               cuda::std::abs(testcases[i].imag()) == T(1)) {
      assert(r.real() == T(0));
      assert(cuda::std::signbit(testcases[i].imag()) ==
             cuda::std::signbit(r.imag()));
      ...
      assert(cuda::std::signbit(testcases[i].real()) ==
             cuda::std::signbit(r.real()));
      if (cuda::std::signbit(testcases[i].imag()))
        is_about(r.imag(), -pi / T(2));
      else
        is_about(r.imag(), pi / T(2));

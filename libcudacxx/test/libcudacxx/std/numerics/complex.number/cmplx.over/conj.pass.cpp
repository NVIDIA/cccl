...
test(T x,
     typename cuda::std::enable_if<cuda::std::is_integral<T>::value>::type* =
         0) {
  static_assert((cuda::std::is_same<decltype(cuda::std::conj(x)),
...
template <class T>
__host__ __device__ void
test(T x, typename cuda::std::enable_if<
              cuda::std::is_floating_point<T>::value>::type* = 0) {
  static_assert((cuda::std::is_same<decltype(cuda::std::conj(x)),
                                    cuda::std::complex<T> >::value),
                "");

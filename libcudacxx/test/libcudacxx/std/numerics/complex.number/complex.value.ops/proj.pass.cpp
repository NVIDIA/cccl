...
template <class T>
__host__ __device__ void test(const cuda::std::complex<T>& z,
...
__host__ __device__ void test_edges() {
  ...
  for (unsigned i = 0; i < N; ++i) {
    ...
    case non_zero:
      ...
      assert(cuda::std::signbit(real(r)) ==
             cuda::std::signbit(real(testcases[i])));
      assert(cuda::std::signbit(imag(r)) ==
             cuda::std::signbit(imag(testcases[i])));
      break;
    case inf:

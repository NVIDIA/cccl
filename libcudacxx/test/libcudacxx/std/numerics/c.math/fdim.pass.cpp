...
__host__ __device__ void test(T val)
{
  ...
  assert(eq(cuda::std::fdim(T(5.0), T(2.0)), T(3.0)));
  ...
  {
    assert(eq(cuda::std::fdim(-T(2.0), -T(5.0)), T(3.0)));
    assert(eq(cuda::std::fdim(-T(5.0), -T(2.0)), T(0.0)));
  }
  ...
  if constexpr (cuda::std::is_same_v<T, float>)
  {
    ...
    assert(eq(cuda::std::fdimf(T(5.0), T(2.0)), T(3.0)));

    // negative values arer clamped to 0
    assert(eq(cuda::std::fdimf(T(), val), T()));

    assert(eq(cuda::std::fdimf(-T(2.0), -T(5.0)), T(3.0)));
    assert(eq(cuda::std::fdimf(-T(5.0), -T(2.0)), T(0.0)));
  ...
  if constexpr (cuda::std::is_same_v<T, long double>)
  {
    ...
    assert(eq(cuda::std::fdiml(T(5.0), T(2.0)), T(3.0)));

    // negative values arer clamped to 0
    assert(eq(cuda::std::fdiml(T(), val), T()));

    assert(eq(cuda::std::fdiml(-T(2.0), -T(5.0)), T(3.0)));
    assert(eq(cuda::std::fdiml(-T(5.0), -T(2.0)), T(0.0)));

...
{
  using result_t = cuda::std::make_unsigned_t<T>;
  result_t lhs_u = static_cast<result_t>(lhs);
  result_t rhs_u = static_cast<result_t>(rhs);
...
TEST_FUNC constexpr void test_values(cuda::std::array<T, N> lhs_values, cuda::std::array<T, N> rhs_values)
{
  ...
  using result_vec_t = simd::basic_vec<cuda::std::make_unsigned_t<T>, simd::fixed_size<N>>;
  vec_t lhs(lhs_values);
  vec_t rhs(rhs_values);

  static_assert(cuda::std::is_same_v<decltype(cuda::simd::abs_diff(lhs, rhs)), result_vec_t>);

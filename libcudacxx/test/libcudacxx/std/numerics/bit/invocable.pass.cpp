...
struct i_rotr {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::rotr(x, 1U));
};
...
struct i_countr_zero {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::countr_zero(x));
};
...
struct i_countr_one {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::countr_one(x));
};
...
struct i_countl_zero {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::countl_zero(x));
};
...
struct i_countl_one {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::countl_one(x));
};
...
struct i_has_single_bit {
  template <typename T>
  __host__ __device__ constexpr auto operator()(T x) const
      -> decltype(cuda::std::has_single_bit(x));
};

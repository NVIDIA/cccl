...
int main(int, char**)
{
  ...
  static_assert(cuda::counting_semaphore<cuda::thread_scope_cluster>::max() > 0);
  static_assert(cuda::counting_semaphore<cuda::thread_scope_cluster, 1>::max() >= 1);
  static_assert(
    cuda::counting_semaphore<cuda::thread_scope_cluster, cuda::std::numeric_limits<int>::max()>::max() >= 1);
  static_assert(
    cuda::counting_semaphore<cuda::thread_scope_cluster, cuda::std::numeric_limits<unsigned>::max()>::max() >= 1);
  static_assert(
    cuda::counting_semaphore<cuda::thread_scope_cluster, cuda::std::numeric_limits<ptrdiff_t>::max()>::max() >= 1);
  static_assert(cuda::counting_semaphore<cuda::thread_scope_cluster, 1>::max()
                == cuda::binary_semaphore<cuda::thread_scope_cluster>::max());

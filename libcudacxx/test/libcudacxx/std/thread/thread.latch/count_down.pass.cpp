...
template <typename Latch, template <typename, typename> typename Selector,
...
__host__ __device__ void test() {
  ...
  execute_on_main_thread([&] { l->count_down(); });

  auto count_downer = LAMBDA() { l->count_down(); };

  auto awaiter = LAMBDA() { l->wait(); };

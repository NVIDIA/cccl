...
TEST_HOST_DEVICE_FUNC void test(bool add_delay = false)
{
  ...
  auto awaiter = LAMBDA()
  {
    ...
    while (b->try_wait_until(cuda::std::move(*tok), until_time) == false)
    {
    }
  };
  auto arriver = LAMBDA()
  ...
  execute_on_main_thread([&] {
    ...
    const auto until_time = cuda::std::chrono::system_clock::now() + delay;
    while (b->try_wait_until(cuda::std::move(tok2), until_time) == false)
    {
    }
  });
}

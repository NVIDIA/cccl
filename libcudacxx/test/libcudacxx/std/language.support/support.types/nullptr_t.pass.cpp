...
__host__ __device__ void test_conversions() {
  {
    ...
    assert(p == nullptr);
    (void)
        p; // GCC spuriously claims that p is unused when T is nullptr_t, probably due to optimizations?
  }
  {
    T p = nullptr;
    assert(p == nullptr);
...
template <class T>
struct has_less<
    T, typename Voider<decltype(cuda::std::declval<T>() < nullptr)>::type>
    : cuda::std::true_type {};
...
__host__ __device__ void test_comparisons() {
  ...
  assert(!(nullptr != p));
  (void)
      p; // GCC spuriously claims that p is unused, probably due to optimizations?
}
...
#if defined(TEST_COMPILER_CLANG) && !defined(TEST_COMPILER_NVCC) &&            \
    !defined(TEST_COMPILER_CLANG_CUDA)
  {
    bool b = nullptr;
    assert(!b);
  }

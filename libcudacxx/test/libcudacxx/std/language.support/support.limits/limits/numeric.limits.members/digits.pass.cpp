...
__host__ __device__ void test() {
  static_assert(cuda::std::numeric_limits<T>::digits == expected,
                "digits test 1");
  static_assert(cuda::std::numeric_limits<const T>::digits == expected,
                "digits test 2");
  static_assert(cuda::std::numeric_limits<volatile T>::digits == expected,
                "digits test 3");
  static_assert(cuda::std::numeric_limits<const volatile T>::digits == expected,
                "digits test 4");
}
...
int main(int, char**) {
  ...
  test<wchar_t, cuda::std::numeric_limits<wchar_t>::is_signed
                    ? sizeof(wchar_t) * 8 - 1
                    : sizeof(wchar_t) * 8>();

...
//     Integral
//     atomic_fetch_min_explicit(volatile atomic<Integral>* obj, Integral op, memory_order m);
//
// template <class Integral>
//     Integral
//     atomic_fetch_min_explicit(atomic<Integral>* obj, Integral op, memory_order m);
//
// template <class T>
//     T*
//     atomic_fetch_min_explicit(volatile atomic<T*>* obj, T* op, memory_order m);
//
// template <class T>
//     T*
//     atomic_fetch_min_explicit(atomic<T*>* obj, T* op, memory_order m);
...
struct TestFn
{
  TEST_HOST_DEVICE_FUNC void operator()() const
  {
    // op less than the stored value: replaced, old value returned
    {
      ...
      assert(cuda::std::atomic_fetch_min_explicit(&t, T(1), cuda::std::memory_order_seq_cst) == T(3));
      assert(t == T(1));
    }
    {
      using A = cuda::std::atomic<T>;
    ...
    // op not less than the stored value: unchanged, old value returned
    {
      ...
      assert(cuda::std::atomic_fetch_min_explicit(&t, T(3), cuda::std::memory_order_relaxed) == T(1));
      assert(t == T(1));
    }
    {
      using A = cuda::std::atomic<T>;
...
TEST_HOST_DEVICE_FUNC void testp()
{

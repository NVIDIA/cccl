...
TEST_FUNC void testp()
{
  using X = typename cuda::std::remove_pointer<T>::type;
  {
    ...
    assert(cuda::std::atomic_fetch_max_explicit(&t, T(5 * sizeof(X)), cuda::std::memory_order_seq_cst)
           == T(2 * sizeof(X)));
  ...
  }
  {
    ...
    assert(cuda::std::atomic_fetch_max_explicit(&t, T(2 * sizeof(X)), cuda::std::memory_order_seq_cst)
           == T(5 * sizeof(X)));
    assert(t == T(5 * sizeof(X)));
  }
}

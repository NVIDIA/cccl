...
//     Integral
//     atomic_fetch_max(volatile atomic<Integral>* obj, Integral op);
//
// template <class Integral>
//     Integral
//     atomic_fetch_max(atomic<Integral>* obj, Integral op);
//
// template <class T>
//     T*
//     atomic_fetch_max(volatile atomic<T*>* obj, T* op);
//
// template <class T>
//     T*
//     atomic_fetch_max(atomic<T*>* obj, T* op);
...
struct TestFn
{
  TEST_FUNC void operator()() const
  {
    // op greater than the stored value: replaced, old value returned
    {
      ...
      assert(cuda::std::atomic_fetch_max(&t, T(3)) == T(1));
      assert(t == T(3));
    }
    {
      using A = cuda::std::atomic<T>;
    ...
    // op not greater than the stored value: unchanged, old value returned
    {
      ...
      assert(cuda::std::atomic_fetch_max(&t, T(1)) == T(3));
      assert(t == T(3));
    }
    {
      using A = cuda::std::atomic<T>;
...
TEST_FUNC void testp()
{
  using X = typename cuda::std::remove_pointer<T>::type;
  {
    ...
    assert(cuda::std::atomic_fetch_max(&t, T(5 * sizeof(X))) == T(2 * sizeof(X)));
    assert(t == T(5 * sizeof(X)));
  }
  {
    using A = cuda::std::atomic<T>;
    Selector<volatile A, constructor_initializer> sel;
    ...
    assert(cuda::std::atomic_fetch_max(&t, T(2 * sizeof(X))) == T(5 * sizeof(X)));
    assert(t == T(5 * sizeof(X)));
  }
}

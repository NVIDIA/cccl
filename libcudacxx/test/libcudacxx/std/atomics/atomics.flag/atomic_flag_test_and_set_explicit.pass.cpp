...
__host__ __device__ void test() {
  {
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_relaxed) == 0);
...
#ifndef TEST_COMPILER_ICC
  {
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_consume) == 0);
    assert(f.test_and_set() == 1);
  }
#endif
  {
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_acquire) == 0);
    assert(f.test_and_set() == 1);
  }
  {
    Selector<cuda::std::atomic_flag, default_initializer> sel;
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_release) == 0);
    assert(f.test_and_set() == 1);
  }
  {
    Selector<cuda::std::atomic_flag, default_initializer> sel;
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_acq_rel) == 0);
    assert(f.test_and_set() == 1);
  }
  {
    Selector<cuda::std::atomic_flag, default_initializer> sel;
    ...
    f.clear();
    assert(atomic_flag_test_and_set_explicit(
               &f, cuda::std::memory_order_seq_cst) == 0);
    assert(f.test_and_set() == 1);
  }
  {

...
__host__ __device__ __noinline__ void do_test() {
  {
    ...
    obj.store(true);
    x = true;
    assert(cmpxchg_weak_loop(obj, x, false, cuda::std::memory_order_seq_cst,
    ...
    assert(obj == false);
    assert(x == true);
    assert(obj.compare_exchange_strong(
               x, true, cuda::std::memory_order_seq_cst) == false);
    assert(obj == false);
    assert(x == false);
    x = true;
    obj.store(true);
    assert(
        obj.compare_exchange_strong(x, false, cuda::std::memory_order_seq_cst,
                                    cuda::std::memory_order_seq_cst) == true);
    assert(obj == false);
    assert(x == true);
  ...
  NV_DISPATCH_TARGET(
      ...
      (typedef Atomic<Scope> A;
       ...
       A& zero = *new (storage) A(); assert(zero == false); zero.~A();),
      NV_PROVIDES_SM_70,
      (typedef Atomic<Scope> A;
       TEST_ALIGNAS_TYPE(A) char storage[sizeof(A)] = {1};
       A& zero = *new (storage) A(); assert(zero == false); zero.~A();))

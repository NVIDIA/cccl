...
static_assert(
    !cuda::std::is_invocable_v<cuda::std::ranges::not_equal_to,
                               NotEqualityComparable, NotEqualityComparable>);
#if !defined(TEST_COMPILER_MSVC) ||                                            \
    TEST_STD_VER > 2017 // MSVC considers implict conversions in C++17
static_assert(
    !cuda::std::is_invocable_v<cuda::std::ranges::not_equal_to, int, MoveOnly>);
...
static_assert(requires {
  typename cuda::std::ranges::not_equal_to::is_transparent;
});
#else
...
struct PtrAndNotEqOperator {
  ...
  __host__ __device__ friend constexpr bool operator!=(PtrAndNotEqOperator,
                                                       PtrAndNotEqOperator) {
    return true;
  }
};
...
__host__ __device__ constexpr bool test() {
  ...
  assert(cuda::std::ranges::equal_to()(PtrAndNotEqOperator(),
                                       PtrAndNotEqOperator()));

...
struct TestClass {
  __host__ __device__ explicit TestClass(int x) : data(x) {}

  __host__ __device__ int& operator()(NonCopyable&&) & { return data; }
  ...
  __host__ __device__ int const volatile& operator()(NonCopyable&&) const
      volatile& {
    return data;
  }
...
struct DerivedFromTestClass : public TestClass {
  __host__ __device__ explicit DerivedFromTestClass(int x) : TestClass(x) {}
};
...
__host__ __device__ void test_b12(Functor&& f) {
  ...
  typedef decltype(cuda::std::invoke(func_ptr, cuda::std::forward<Functor>(f),
                                     cuda::std::move(arg))) DeducedReturnType;
  static_assert((cuda::std::is_same<DeducedReturnType, Expect>::value), "");

  // Check that result_of_t matches Expect.
  typedef typename cuda::std::result_of<
      ClassFunc && (Functor&&, NonCopyable&&)>::type ResultOfReturnType;
  static_assert((cuda::std::is_same<ResultOfReturnType, Expect>::value), "");

  // Run invoke and check the return value.
  DeducedReturnType ret = cuda::std::invoke(
      func_ptr, cuda::std::forward<Functor>(f), cuda::std::move(arg));
  assert(ret == 42);
}
...
__host__ __device__ void test_b5(Functor&& f) {
  ...
  // Check that the deduced return type of invoke is what is expected.
  typedef decltype(cuda::std::invoke(cuda::std::forward<Functor>(f),
                                     cuda::std::move(arg))) DeducedReturnType;
  static_assert((cuda::std::is_same<DeducedReturnType, Expect>::value), "");
...
__host__ __device__ void bullet_one_two_tests() {
  {
    ...
    test_b12<int const && (NonCopyable&&) const&&, int const&&>(
        cuda::std::move(cl));
    test_b12<int volatile && (NonCopyable&&) volatile&&, int volatile&&>(
        cuda::std::move(cl));
    test_b12<int const volatile && (NonCopyable&&) const volatile&&,
             int const volatile&&>(cuda::std::move(cl));
  }
  {
...
__host__ __device__ void bullet_three_four_tests() {
  ...
  // uncomment when reenabling reference_wrapper
  {
    ...
    test_b34<int const volatile&>(
        cuda::std::reference_wrapper<Fn const volatile>(cl));
  }
  {
    typedef DerivedFromTestClass Fn;
    Fn cl(42);

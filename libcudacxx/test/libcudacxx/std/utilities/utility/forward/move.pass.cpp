...
__host__ __device__ constexpr bool test_constexpr_move() {
  int y = 42;
  const int cy = y;
  return cuda::std::move(y) == 42 && cuda::std::move(cy) == 42 &&
...
int main(int, char**) {
  { // Test return type and noexcept.
    static_assert(
        cuda::std::is_same<decltype(cuda::std::move(global_var)), int&&>::value,
        "");
    ASSERT_NOEXCEPT(cuda::std::move(global_var));
    static_assert(
        cuda::std::is_same<decltype(cuda::std::move(global_reference)),
                           const int&&>::value,
        "");
    ASSERT_NOEXCEPT(cuda::std::move(global_reference));
    static_assert(
        cuda::std::is_same<decltype(cuda::std::move(42)), int&&>::value, "");
    ASSERT_NOEXCEPT(cuda::std::move(42));
    static_assert(
        cuda::std::is_same<decltype(cuda::std::move(get<const int&&>())),
                           const int&&>::value,
        "");

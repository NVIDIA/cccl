...
static_assert(cuda::std::__pretty_nameof_v<a_free_function>().find("a_free_function") != -1, "");
static_assert(cuda::std::__pretty_nameof_v<&a_free_function>().find("a_free_function") != -1, ""); // pointer form
static_assert(
  cuda::std::__pretty_nameof_v<a_namespace::a_function_in_a_namespace>().find("a_function_in_a_namespace") != -1, "");
static_assert(cuda::std::__pretty_nameof_v<a_free_function>()
                != cuda::std::__pretty_nameof_v<a_namespace::a_function_in_a_namespace>(),
              "");
...
int main(int, char**)
{
  ...
  NV_IF_TARGET(
    ...
    (assert(cuda::std::__pretty_nameof_v<42>() == cuda::std::__string_view("42"));
     ...
     assert(cuda::std::__pretty_nameof_v<true>() != cuda::std::__pretty_nameof_v<false>());
     assert(cuda::std::__pretty_nameof_v<a_free_function>().find("a_free_function") != -1);
     assert(cuda::std::__pretty_nameof_v<&a_free_function>().find("a_free_function") != -1);
     assert(cuda::std::__pretty_nameof_v<a_namespace::a_function_in_a_namespace>().find("a_function_in_a_namespace")
            != -1);))

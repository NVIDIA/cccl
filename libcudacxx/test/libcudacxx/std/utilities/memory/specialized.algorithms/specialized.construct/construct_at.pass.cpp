...
struct WithSpecialMoveAssignment {
  ...
  WithSpecialMoveAssignment&
  operator=(const WithSpecialMoveAssignment&) = default;
  __host__ __device__ constexpr WithSpecialMoveAssignment&
  operator=(WithSpecialMoveAssignment&&) noexcept {
...
static_assert(
    cuda::std::is_trivially_constructible_v<WithSpecialMoveAssignment>);
...
__host__ __device__ constexpr bool test() {
  ...
  // ensure that we can construct trivially constructible types with a nefarious move assignment
  {
    ...
    WithSpecialMoveAssignment* res =
        cuda::std::construct_at(&with_special_move_assignment);
...
template <class... Args, class = decltype(cuda::std::construct_at(
...
int main(int, char**) {
  test();
    ...
    !defined(TEST_COMPILER_MSVC_2017) && !defined(TEST_COMPILER_MSVC_2019)
  static_assert(test());
#endif
  return 0;
}

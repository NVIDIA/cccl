...
struct LVal {
  __host__ __device__ constexpr int operator()(TestError&) { return 42; }
...
struct CLVal {
  int operator()(TestError&) = delete;
  __host__ __device__ constexpr int operator()(const TestError&) { return 42; }
  int operator()(TestError&&) = delete;
  int operator()(const TestError&&) = delete;
};
...
struct CRefQual {
  int operator()(TestError) & = delete;
  __host__ __device__ constexpr int operator()(TestError) const& { return 42; }
  int operator()(TestError) && = delete;
  int operator()(TestError) const&& = delete;
};
...
__host__ __device__ constexpr void test_val_types() {
  const cuda::std::expected<void, TestError> previous_value{
      cuda::std::in_place};
  const cuda::std::expected<void, TestError> expected_error{cuda::std::unexpect,
                                                            42};
  ...
  // Test & overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform_error(LVal{})),
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform_error(l)),
                       cuda::std::expected<void, int>);
    }
  ...
  // Test const& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform_error(CLVal{})),
                       cuda::std::expected<void, int>);
    }
  ...
  // Test && overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(cuda::std::move(i).transform_error(RVal{})),
                       cuda::std::expected<void, int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform_error(RVRefQual{})),
                       cuda::std::expected<void, int>);
    }
  ...
  // Test const&& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(cuda::std::move(i).transform_error(CRVal{})),
                       cuda::std::expected<void, int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform_error(cuda::std::move(l))),
                       cuda::std::expected<void, int>);
    }
...
struct NonConst {
  __host__ __device__ constexpr int non_const() { return 1; }
};
...
struct NoCopy {
  ...
  __host__ __device__ constexpr int operator()(const NoCopy&&) { return 42; }
};

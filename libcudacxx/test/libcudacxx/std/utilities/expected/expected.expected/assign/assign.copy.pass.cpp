...
static_assert(!cuda::std::is_copy_assignable_v<
                  cuda::std::expected<NotCopyAssignable, int> >,
              "");

// !is_copy_constructible_v<T>
static_assert(!cuda::std::is_copy_assignable_v<
                  cuda::std::expected<NotCopyConstructible, int> >,
              "");

// !is_copy_assignable_v<E>
static_assert(!cuda::std::is_copy_assignable_v<
                  cuda::std::expected<int, NotCopyAssignable> >,
              "");

// !is_copy_constructible_v<E>
static_assert(!cuda::std::is_copy_assignable_v<
                  cuda::std::expected<int, NotCopyConstructible> >,
              "");

// !is_nothrow_move_constructible_v<T> && is_nothrow_move_constructible_v<E>
static_assert(
    cuda::std::is_copy_assignable_v<cuda::std::expected<MoveMayThrow, int> >,
    "");

// is_nothrow_move_constructible_v<T> && !is_nothrow_move_constructible_v<E>
static_assert(
    cuda::std::is_copy_assignable_v<cuda::std::expected<int, MoveMayThrow> >,
    "");
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  // If this->has_value() && rhs.has_value() is true, equivalent to val = *rhs.
  {
    ...
    const cuda::std::expected<Traced, int> e2(cuda::std::in_place, newState,
                                              10);
    decltype(auto) x = (e1 = e2);
    static_assert(
        cuda::std::same_as<decltype(x), cuda::std::expected<Traced, int>&>, "");
    assert(&x == &e1);
  ...
  //
  {
    ...
    Traced::state newState{};
    cuda::std::expected<TracedNoexcept, Traced> e1(cuda::std::in_place,
                                                   oldState, 5);
    const cuda::std::expected<TracedNoexcept, Traced> e2(cuda::std::unexpect,
                                                         newState, 10);

    decltype(auto) x = (e1 = e2);
    static_assert(
        cuda::std::same_as<decltype(x),
                           cuda::std::expected<TracedNoexcept, Traced>&>,
        "");
    assert(&x == &e1);
  ...
  //
  {
    ...
    cuda::std::expected<Traced, TracedNoexcept> e1(cuda::std::in_place,
                                                   oldState, 5);
    const cuda::std::expected<Traced, TracedNoexcept> e2(cuda::std::unexpect,
                                                         newState, 10);

    decltype(auto) x = (e1 = e2);
    static_assert(
        cuda::std::same_as<decltype(x),
                           cuda::std::expected<Traced, TracedNoexcept>&>,
        "");
    assert(&x == &e1);
  ...
  //
  {
    TracedNoexcept::state oldState{};
    Traced::state newState{};
    cuda::std::expected<Traced, TracedNoexcept> e1(cuda::std::unexpect,
                                                   oldState, 5);
    const cuda::std::expected<Traced, TracedNoexcept> e2(cuda::std::in_place,
                                                         newState, 10);
  ...
  //
  {
    Traced::state oldState{};
    TracedNoexcept::state newState{};
    cuda::std::expected<TracedNoexcept, Traced> e1(cuda::std::unexpect,
                                                   oldState, 5);
    const cuda::std::expected<TracedNoexcept, Traced> e2(cuda::std::in_place,
                                                         newState, 10);
  ...
  // Otherwise, equivalent to unex = rhs.error().
  {
    ...
    const cuda::std::expected<int, Traced> e2(cuda::std::unexpect, newState,
                                              10);
    decltype(auto) x = (e1 = e2);
    static_assert(
...
__host__ __device__ void testException() {
  ...
  struct ThrowOnCopyMoveMayThrow {
    ...
    ThrowOnCopyMoveMayThrow&
    operator=(const ThrowOnCopyMoveMayThrow&) = default;
    ThrowOnCopyMoveMayThrow(ThrowOnCopyMoveMayThrow&&) noexcept(false) {}
  };

  // assign value throw on copy
  {
    cuda::std::expected<ThrowOnCopyMoveMayThrow, int> e1(cuda::std::unexpect,
                                                         5);
    const cuda::std::expected<ThrowOnCopyMoveMayThrow, int> e2(
        cuda::std::in_place);
    try {
      e1 = e2;
      assert(false);
  ...
  // assign error throw on copy
  {
    ...
    const cuda::std::expected<int, ThrowOnCopyMoveMayThrow> e2(
        cuda::std::unexpect);
    try {
      e1 = e2;
      assert(false);

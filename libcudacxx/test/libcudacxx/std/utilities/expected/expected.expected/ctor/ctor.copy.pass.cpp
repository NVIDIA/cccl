...
#else
  __host__ __device__ friend constexpr bool
  operator==(const CopyableNonTrivial& lhs,
             const CopyableNonTrivial& rhs) noexcept {
    return lhs.i == rhs.i;
  }
  __host__ __device__ friend constexpr bool
  operator!=(const CopyableNonTrivial& lhs,
...
// - is_copy_constructible_v<E> is true.
static_assert(
    cuda::std::is_copy_constructible_v<cuda::std::expected<int, int> >, "");
static_assert(cuda::std::is_copy_constructible_v<
                  cuda::std::expected<CopyableNonTrivial, int> >,
              "");
static_assert(cuda::std::is_copy_constructible_v<
                  cuda::std::expected<int, CopyableNonTrivial> >,
              "");
static_assert(cuda::std::is_copy_constructible_v<
                  cuda::std::expected<CopyableNonTrivial, CopyableNonTrivial> >,
              "");
static_assert(
    !cuda::std::is_copy_constructible_v<cuda::std::expected<NonCopyable, int> >,
    "");
static_assert(
    !cuda::std::is_copy_constructible_v<cuda::std::expected<int, NonCopyable> >,
    "");
static_assert(!cuda::std::is_copy_constructible_v<
                  cuda::std::expected<NonCopyable, NonCopyable> >,
              "");

// Test: This constructor is trivial if
// - is_trivially_copy_constructible_v<T> is true and
// - is_trivially_copy_constructible_v<E> is true.
static_assert(cuda::std::is_trivially_copy_constructible_v<
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  ...
  {
    const cuda::std::expected<int, CopyableNonTrivial> e1(cuda::std::unexpect,
                                                          5);
    auto e2 = e1;
    assert(!e2.has_value());
    assert(e2.error().i == 5);

...
struct foo {};
static_assert(!cuda::std::is_constructible_v<cuda::std::expected<foo, int>,
                                             cuda::std::in_place_t, int>,
              "");
...
static_assert(!ImplicitlyConstructible<cuda::std::expected<int, int>,
                                       cuda::std::in_place_t>,
              "");
static_assert(!ImplicitlyConstructible<cuda::std::expected<int, int>,
                                       cuda::std::in_place_t, int>,
...
__host__ __device__ constexpr bool test() {
  ...
  // multi args
  {
    cuda::std::expected<cuda::std::tuple<int, short, MoveOnly>, int> e(
        cuda::std::in_place, 1, short{2}, MoveOnly(3));
    assert(e.has_value());
    assert((e.value() ==

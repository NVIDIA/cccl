...
int main(int, char**) {
  ...
  // this builtin has different diagnostic messages than the fallback implementation.
    ...
    !defined(_LIBCUDACXX_TESTING_FALLBACK_MAKE_INTEGER_SEQUENCE)
  MakeSeqT
      i; // expected-error@*:* {{integer sequences must have non-negative sequence length}}
#else
  MakeSeqT
      i; // expected-error-re@*:* {{{{(static_assert|static assertion)}} failed{{.*}}cuda::std::make_integer_sequence must have a non-negative sequence length}}
#endif

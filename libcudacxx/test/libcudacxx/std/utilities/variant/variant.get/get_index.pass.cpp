...
__host__ __device__ void test_rvalue_get() {
  ...
  }
  {
    using V = cuda::std::variant<int, const long>;
    V v(42l);
    ASSERT_SAME_TYPE(decltype(cuda::std::get<1>(cuda::std::move(v))),
                     const long&&);
...
// FIXME: Remove these once reference support is reinstated
  ...
  }
  {
    ...
    int x = 42;
    V v(x);
    ASSERT_SAME_TYPE(decltype(cuda::std::get<0>(cuda::std::move(v))),
                     const int&);
    assert(&cuda::std::get<0>(cuda::std::move(v)) == &x);
  }
  {
...
__host__ __device__ void test_throws_for_all_value_categories() {
  ...
  auto test = [](auto idx, auto&& v) {
    ...
    try {
      TEST_IGNORE_NODISCARD cuda::std::get<Idx::value>(

...
int main(int, char**) {
  ...
  { // operator/(const year_month& ym, const weekday_last& wdl) (and switched)
    ...
    ASSERT_SAME_TYPE(year_month_weekday_last,
                     decltype(Feb2018 / weekday_last{Tuesday}));
  ...
  { // operator/(const year& y, const month_weekday_last& mwdl) (and switched)
    ...
    ASSERT_SAME_TYPE(year_month_weekday_last,
                     decltype(year{2018} / FebLastTues));
    ASSERT_NOEXCEPT(FebLastTues / year{2018});
    ASSERT_SAME_TYPE(year_month_weekday_last,
                     decltype(FebLastTues / year{2018}));
    ...
    for (int i = 1000; i < 1010; ++i)
      for (unsigned j = 1; j <= 12; ++j)
        for (unsigned k = 0; k <= 6; ++k) {
          ...
          month m{j};
          weekday wd{k};
          year_month_weekday_last ymwdl1 =
              y / month_weekday_last{m, weekday_last{wd}};
          year_month_weekday_last ymwdl2 =
              month_weekday_last{m, weekday_last{wd}} / y;
    ...
    for (int i = 1000; i < 1010; ++i)
      for (unsigned j = 1; j <= 12; ++j)
        for (unsigned k = 0; k <= 6; ++k) {
          ...
          weekday wd{k};
          year_month_weekday_last ymwdl1 =
              i / month_weekday_last{m, weekday_last{wd}};
          year_month_weekday_last ymwdl2 =
              month_weekday_last{m, weekday_last{wd}} / i;
          assert(ymwdl1.year() == y);
          assert(ymwdl2.year() == y);
          assert(ymwdl1.month() == m);
          assert(ymwdl2.month() == m);

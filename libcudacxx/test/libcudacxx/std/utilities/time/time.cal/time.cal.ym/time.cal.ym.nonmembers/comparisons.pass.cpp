...
int main(int, char**) {
  ...
  static_assert(testComparisons(year_month{year{1234}, January},
                                year_month{year{1234}, January}, true, false),
                "");

  static_assert(testComparisons(year_month{year{1234}, January},
                                year_month{year{1234}, February}, false, true),
                "");

  static_assert(testComparisons(year_month{year{1234}, January},
                                year_month{year{1235}, January}, false, true),
                "");
  ...
  for (unsigned i = 1; i < 12; ++i)
    for (unsigned j = 1; j < 12; ++j)
      assert(
          (testComparisons(year_month{year{1234}, month{i}},
                           year_month{year{1234}, month{j}}, i == j, i < j)));

  //  same month, different years
  for (int i = 1000; i < 20; ++i)
    for (int j = 1000; j < 20; ++j)
      assert((testComparisons(year_month{year{i}, January},
                              year_month{year{j}, January}, i == j, i < j)));

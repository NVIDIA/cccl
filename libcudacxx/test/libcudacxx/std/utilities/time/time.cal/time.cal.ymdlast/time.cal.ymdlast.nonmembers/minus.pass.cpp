...
testConstexprYears(cuda::std::chrono::year_month_day_last ymdl) {
  cuda::std::chrono::year_month_day_last ym1 =
      ymdl - cuda::std::chrono::years{10};
  return ym1.year() ==
             cuda::std::chrono::year{static_cast<int>(ymdl.year()) - 10} &&
         ym1.month() == ymdl.month();
}

__host__ __device__ constexpr bool
testConstexprMonths(cuda::std::chrono::year_month_day_last ymdl) {
  cuda::std::chrono::year_month_day_last ym1 =
      ymdl - cuda::std::chrono::months{6};
  return ym1.year() == ymdl.year() &&
         ym1.month() ==
             cuda::std::chrono::month{static_cast<unsigned>(ymdl.month()) - 6};
}
...
int main(int, char**) {
  ...
  { // year_month_day_last - years
    ASSERT_NOEXCEPT(cuda::std::declval<year_month_day_last>() -
                    cuda::std::declval<years>());
    ASSERT_SAME_TYPE(year_month_day_last,
                     decltype(cuda::std::declval<year_month_day_last>() -
                              cuda::std::declval<years>()));

    static_assert(testConstexprYears(year_month_day_last{
                      year{1234}, month_day_last{December}}),
                  "");
  ...
  { // year_month_day_last - months
    ...
    static_assert(testConstexprMonths(year_month_day_last{
                      year{1234}, month_day_last{December}}),
                  "");
    //  TODO test wrapping
    year_month_day_last ym{year{1234}, month_day_last{December}};

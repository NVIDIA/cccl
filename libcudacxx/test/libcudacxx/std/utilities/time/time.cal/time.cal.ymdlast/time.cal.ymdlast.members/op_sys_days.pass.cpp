...
int main(int, char**) {
  ...
  ASSERT_NOEXCEPT(
      static_cast<sys_days>(cuda::std::declval<const year_month_day_last>()));
  ASSERT_SAME_TYPE(sys_days,
                   decltype(static_cast<sys_days>(
                       cuda::std::declval<const year_month_day_last>())));

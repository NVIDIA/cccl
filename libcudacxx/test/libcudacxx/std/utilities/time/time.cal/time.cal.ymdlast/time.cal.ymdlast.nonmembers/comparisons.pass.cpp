...
int main(int, char**) {
  ...
  static_assert(
      testComparisons(year_month_day_last{year{1234}, month_day_last{January}},
  ...
  static_assert(
      testComparisons(year_month_day_last{year{1234}, month_day_last{February}},
                      year_month_day_last{year{1235}, month_day_last{January}},
                      false, true),
      "");
  ...
  for (unsigned i = 1; i < 12; ++i)
    for (unsigned j = 1; j < 12; ++j)
      assert((testComparisons(
          ...
          year_month_day_last{year{1234}, month_day_last{month{j}}}, i == j,
          i < j)));
  ...
  for (int i = 1000; i < 20; ++i)
    for (int j = 1000; j < 20; ++j)
      assert((
          testComparisons(year_month_day_last{year{i}, month_day_last{January}},

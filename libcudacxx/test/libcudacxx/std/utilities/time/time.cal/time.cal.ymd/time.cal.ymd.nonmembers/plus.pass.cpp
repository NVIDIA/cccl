...
testConstexprYears(cuda::std::chrono::year_month_day ym) {
...
__host__ __device__ constexpr bool
testConstexprMonths(cuda::std::chrono::year_month_day ym) {
...
int main(int, char**) {
  ...
  { // year_month_day + months
    ASSERT_NOEXCEPT(cuda::std::declval<year_month_day>() +
                    cuda::std::declval<months>());
    ASSERT_NOEXCEPT(cuda::std::declval<months>() +
                    cuda::std::declval<year_month_day>());

    ASSERT_SAME_TYPE(year_month_day,
                     decltype(cuda::std::declval<year_month_day>() +
                              cuda::std::declval<months>()));
    ASSERT_SAME_TYPE(year_month_day,
    ...
    static_assert(
        testConstexprMonths(year_month_day{year{1}, month{1}, day{1}}), "");

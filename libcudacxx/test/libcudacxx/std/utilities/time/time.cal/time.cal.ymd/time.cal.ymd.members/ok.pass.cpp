...
int main(int, char**) {
  ...
  ASSERT_NOEXCEPT(cuda::std::declval<const year_month_day>().ok());
  ASSERT_SAME_TYPE(bool,
                   decltype(cuda::std::declval<const year_month_day>().ok()));

  static_assert(!year_month_day{year{-32768}, month{}, day{}}.ok(),
                ""); // All three bad

  static_assert(!year_month_day{year{-32768}, January, day{1}}.ok(),
                ""); // Bad year
  static_assert(!year_month_day{year{2019}, month{}, day{1}}.ok(),
                ""); // Bad month
  static_assert(!year_month_day{year{2019}, January, day{}}.ok(),
                ""); // Bad day

  static_assert(!year_month_day{year{-32768}, month{}, day{1}}.ok(),
                ""); // Bad year & month
  static_assert(!year_month_day{year{2019}, month{}, day{}}.ok(),
                ""); // Bad month & day
  static_assert(!year_month_day{year{-32768}, January, day{}}.ok(),
                ""); // Bad year & day
  ...
  static_assert(
      !year_month_day{year{2019}, cuda::std::chrono::February, day{29}}.ok(),
      ""); // Not a leap year
  static_assert(
      year_month_day{year{2020}, cuda::std::chrono::February, day{29}}.ok(),
      ""); // Ok; 2020 is a leap year

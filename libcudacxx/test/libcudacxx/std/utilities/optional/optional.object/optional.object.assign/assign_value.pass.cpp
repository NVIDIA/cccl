...
struct ThrowAssign {
  ...
  ThrowAssign() = default;
  __host__ __device__ ThrowAssign(int) { TEST_THROW(42); }
  __host__ __device__ ThrowAssign& operator=(int) {
    TEST_THROW(42);
    return *this;
  }
  __host__ __device__ ~ThrowAssign() { ++dtor_called(); }
};
...
struct MismatchType {
  ...
  __host__ __device__ MismatchType& operator=(int) { return *this; }
  __host__ __device__ MismatchType& operator=(int*) { return *this; }
...
__host__ __device__ void test_sfinae() {
  ...
  assert_assignable<FromOptionalType, cuda::std::optional<FromOptionalType>&,
                    false>();
}
...
__host__ __device__ void test_throws() {
...
int main(int, char**) {
  ...
  {
    optional<cuda::std::unique_ptr<int> > opt(
        cuda::std::unique_ptr<int>(new int(2)));
    opt = cuda::std::unique_ptr<int>(new int(3));
    assert(static_cast<bool>(opt) == true);
    assert(**opt == 3);
  }

...
int main(int, char**) {
  ...
  }
  {
    ...
    static_assert(
        !cuda::std::is_nothrow_move_assignable<optional<ThrowsMove> >::value,
        "");
    struct ThrowsMoveAssign {
      ...
      __host__ __device__ ThrowsMoveAssign&
      operator=(ThrowsMoveAssign const&) noexcept {
        return *this;
      }
      __host__ __device__ ThrowsMoveAssign& operator=(ThrowsMoveAssign&&)
          noexcept(false) {
        return *this;
      }
    };
    static_assert(!cuda::std::is_nothrow_move_assignable<
                      optional<ThrowsMoveAssign> >::value,
                  "");
    ...
    static_assert(
        cuda::std::is_nothrow_move_assignable<optional<NoThrowMove> >::value,
        "");
  }
  return 0;
}

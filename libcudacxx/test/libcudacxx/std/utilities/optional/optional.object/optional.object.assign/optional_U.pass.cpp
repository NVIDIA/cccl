...
struct Y2 {
  Y2() = default;
  Y2(const int&) = delete;
  __host__ __device__ Y2& operator=(const int&) { return *this; }
};
...
struct AssignableFrom {
  ...
  __host__ __device__ explicit AssignableFrom(T) { ++type_constructed(); }
  __host__ __device__ AssignableFrom& operator=(T) {
    ++type_assigned();
    return *this;
  }

  __host__ __device__ AssignableFrom(int) { ++int_constructed(); }
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  ...
  class StateTracker {
  public:
    __host__ __device__ constexpr StateTracker(state_t& s) : state_(&s) {
  ...
  {
    auto state =
        cuda::std::array<state_t, 2>{state_t::inactive, state_t::inactive};

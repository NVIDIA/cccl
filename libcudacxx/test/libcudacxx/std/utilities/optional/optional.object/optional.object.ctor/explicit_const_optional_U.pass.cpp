...
test(const optional<U>& rhs, bool is_going_to_throw = false) {
  static_assert(
      !(cuda::std::is_convertible<const optional<U>&, optional<T> >::value),
      "");
...
public:
  __host__ __device__ constexpr explicit X(int i) : i_(i) {}
  __host__ __device__ constexpr X(const X& x) : i_(x.i_) {}
  __host__ __device__ TEST_CONSTEXPR_CXX20 ~X() { i_ = 0; }
...
public:
  __host__ __device__ constexpr explicit Y(int i) : i_(i) {}

...
public:
  __host__ __device__ X() : i_(0) {}
  __host__ __device__ X(int i) : i_(i) {}
  __host__ __device__ X(int i, int j) : i_(i), j_(j) {}
...
int main(int, char**) {
  {
    static_assert(
        !cuda::std::is_constructible<X,
                                     cuda::std::initializer_list<int>&>::value,
        "");
    static_assert(
        !cuda::std::is_constructible<optional<X>,
                                     cuda::std::initializer_list<int>&>::value,
        "");
  }
#ifdef _LIBCUDACXX_HAS_VECTOR
  ...
  {
    static_assert(
        cuda::std::is_constructible<optional<Y>,
                                    cuda::std::initializer_list<int>&>::value,
        "");
    ...
    struct test_constexpr_ctor : public optional<Y> {
      ...
      test_constexpr_ctor(in_place_t, cuda::std::initializer_list<int> i)

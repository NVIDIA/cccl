...
__host__ __device__ constexpr bool implies(bool p, bool q) { return !p || q; }
...
struct SpecialMemberTest {
  ...
  static_assert(implies(cuda::std::is_trivially_copy_constructible_v<T>,
                ...
                "optional<T> is trivially copy constructible if T is trivially "
                "copy constructible.");

  static_assert(implies(cuda::std::is_trivially_move_constructible_v<T>,
                        cuda::std::is_trivially_move_constructible_v<O>),
                "optional<T> is trivially move constructible if T is trivially "
                "move constructible");
...
struct TrivialMoveNonTrivialCopy {
  ...
  __host__ __device__ TrivialMoveNonTrivialCopy&
...
struct TrivialCopyNonTrivialMove {
  ...
  operator=(const TrivialCopyNonTrivialMove&) = default;
  __host__ __device__ TrivialCopyNonTrivialMove&

...
public:
  ...
  __host__ __device__ X(int i) : i_(i) {}
  X(X&& x) = default;
  X& operator=(X&&) = default;
  __host__ __device__ ~X() { ++dtor_called(); }
...
public:
  STATIC_MEMBER_VAR(dtor_called, unsigned);
  __host__ __device__ Y(int i) : i_(i) {}
  Y(Y&&) = default;
  __host__ __device__ ~Y() { ++dtor_called(); }
...
public:
  ...
  __host__ __device__ Z(Z&&) { TEST_THROW(7); }

  __host__ __device__ friend constexpr bool operator==(const Z& x, const Z& y) {
    return x.i_ == y.i_;
  }
  __host__ __device__ friend void swap(Z&, Z&) { TEST_THROW(6); }
};
...
public:
  __host__ __device__ constexpr W(int i) : i_(i) {}

  __host__ __device__ friend constexpr bool operator==(const W& x, const W& y) {
    return x.i_ == y.i_;
  }
  __host__ __device__ friend TEST_CONSTEXPR_CXX14 void swap(W& x,
                                                            W& y) noexcept {
    cuda::std::swap(x.i_, y.i_);
  }
};

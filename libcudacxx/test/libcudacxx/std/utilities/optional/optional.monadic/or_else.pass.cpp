...
_LIBCUDACXX_CONCEPT_FRAGMENT(
    HasOrElse, requires(Opt&& opt, F&& f)(cuda::std::forward<Opt>(opt).or_else(
...
static_assert(
    has_or_else<cuda::std::optional<int>&, decltype(return_optional<int>)>, "");
static_assert(
    has_or_else<cuda::std::optional<int>&&, decltype(return_optional<int>)>,
    "");
static_assert(!has_or_else<cuda::std::optional<MoveOnly>&,
...
static_assert(!has_or_else<cuda::std::optional<NonMovable>&,
...
// And it's not because the tested expressions are valid; no, they fail to compile, but something in the sfinae machinery used by has_or_else
...
struct has_or_else_war<
    T, F,
    decltype(cuda::std::declval<T>().or_else(cuda::std::declval<F>()), void())>
    : cuda::std::true_type {};

static_assert(
    !has_or_else_war<cuda::std::optional<int>&, decltype(take_int)>::value, "");
static_assert(!has_or_else_war<cuda::std::optional<int>&,
                               decltype(take_int_return_void)>::value,
              "");
...
static_assert(
    !has_or_else<cuda::std::optional<int>&, decltype(take_int_return_void)>,
    "");
...
__host__ __device__ TEST_CONSTEXPR_CXX17 bool test() {
  {
    ...
    assert(opt.or_else([] { return cuda::std::optional<int>{0}; }) == 0);
    opt = 1;
    opt.or_else([] {

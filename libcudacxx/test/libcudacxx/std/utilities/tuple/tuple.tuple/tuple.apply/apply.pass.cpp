...
__host__ __device__ constexpr int constexpr_sum_fn() { return 0; }
...
struct CallInfo {
  ...
  __host__ __device__ CallInfo(CallQuals q, Args&&... xargs)
      : quals(q), arg_types(&makeArgumentID<Args&&...>()),
        args(cuda::std::forward<Args>(xargs)...) {}
};

template <class... Args>
__host__ __device__ inline CallInfo<
    decltype(cuda::std::forward_as_tuple(cuda::std::declval<Args>()...))>
...
__host__ __device__ void check_apply_quals_and_types(Tuple&& t) {
  ...
  {
    auto ret =
        cuda::std::apply(cuda::std::move(obj), cuda::std::forward<Tuple>(t));
  ...
  {
    auto ret =
        cuda::std::apply(cuda::std::move(cobj), cuda::std::forward<Tuple>(t));
...
__host__ __device__ void test_call_quals_and_arg_types() {
  ...
  check_apply_quals_and_types<int&&, int const&, unsigned&&>(
      cuda::std::move(t));
  check_apply_quals_and_types<int const&&, int const&, unsigned&&>(
      cuda::std::move(ct));
}
...
struct TestNoexceptCallable {
  template <class... Args>
  __host__ __device__ NothrowMoveable operator()(Args...) const
      noexcept(IsNoexcept) {
    return {};
  }
};
...
__host__ __device__ int f(index<1>) { return 0; }

__host__ __device__ int& f(index<2>) { return static_cast<int&>(my_int); }
...
__host__ __device__ int&& f(index<6>) { return static_cast<int&&>(my_int); }
...
__host__ __device__ int* f(index<10>) { return static_cast<int*>(&my_int); }

...
struct make_function<Tp,
                     cuda::std::integer_sequence<cuda::std::size_t, Idx...> > {
  using type = bool (*)(always_t<Tp, Idx>...);
};

template <class Tp, cuda::std::size_t Size>
using make_function_t =
    typename make_function<Tp, cuda::std::make_index_sequence<Size> >::type;
...
template <class Tp, cuda::std::size_t Size>
using make_tuple_t =
    typename make_tuple_imp<Tp, cuda::std::make_index_sequence<Size> >::type;

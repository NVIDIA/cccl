...
struct ConstexprConstructibleFromTuple {
  ...
  ConstexprConstructibleFromTuple(Args&&... xargs)
...
struct ConstructibleFromTuple<Tuple<Types...> > {
  ...
  __host__ __device__ explicit ConstructibleFromTuple(Args&&... xargs)
      : args(xargs...), arg_types(&makeArgumentID<Args&&...>()) {}
...
__host__ __device__ constexpr bool do_constexpr_test(Tuple&& tup) {
  ...
  return cuda::std::make_from_tuple<Tp>(cuda::std::forward<Tuple>(tup)).args ==
         tup;
}

// Needed by do_forwarding_test() since it compares pairs of different types.
template <class T1, class T2, class U1, class U2>
__host__ __device__ inline bool operator==(const cuda::std::pair<T1, T2>& lhs,
                                           const cuda::std::pair<U1, U2>& rhs) {
...
__host__ __device__ bool do_forwarding_test(Tuple&& tup) {
  ...
  const Tp value =
      cuda::std::make_from_tuple<Tp>(cuda::std::forward<Tuple>(tup));
...
__host__ __device__ void test_perfect_forwarding() {
  ...
  }
  {
    ...
    assert((do_forwarding_test<int&, const char*&&, unsigned&&>(
        cuda::std::move(tup))));
    assert((do_forwarding_test<int&, const char* const&&, unsigned&&>(
        cuda::std::move(ctup))));
  }
  // test with pair<T, U>
  {
    ...
    assert(
        (do_forwarding_test<int&, const char* const&&>(cuda::std::move(ctup))));
  }
  // test with array<T, I>
  {
    ...
    assert((do_forwarding_test<int const&&, int const&&, int const&&>(
        cuda::std::move(ctup))));
  }
}
...
__host__ __device__ void test_noexcept() {
  ...
  };
  {
    ...
    LIBCPP_ASSERT_NOEXCEPT(
        cuda::std::make_from_tuple<TestType>(cuda::std::move(tup)));
  }
  {

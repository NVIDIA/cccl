...
struct ExplicitTwo {
  ...
  template <class T, class = typename cuda::std::enable_if<
                         !cuda::std::is_same<T, ExplicitTwo>::value>::type>
...
int main(int, char**) {
  ...
  }
  {
    static_assert(cuda::std::is_convertible<ExplicitTwo&&, ExplicitTwo>::value,
                  "");
    static_assert(
        cuda::std::is_convertible<cuda::std::tuple<ExplicitTwo&&>&&,
                                  const cuda::std::tuple<ExplicitTwo>&>::value,
        "");

    ExplicitTwo e;
    cuda::std::tuple<ExplicitTwo> t =
        cuda::std::tuple<ExplicitTwo&&>(cuda::std::move(e));
    ((void)t);
  }
  return 0;
}

...
template <class Tp>
using uncvref_t = typename cuda::std::remove_cv<
...
struct ConstructibleFromTupleAndInt {
  ...
  __host__ __device__
  ConstructibleFromTupleAndInt(ConstructibleFromTupleAndInt const&)
      : state(Copied) {}
  __host__ __device__
  ConstructibleFromTupleAndInt(ConstructibleFromTupleAndInt&&)
      : state(Moved) {}

  template <class Tuple,
            class = typename cuda::std::enable_if<IsTuple<Tuple>::value>::type>
...
struct ConvertibleFromTupleAndInt {
  ...
  __host__ __device__
  ConvertibleFromTupleAndInt(ConvertibleFromTupleAndInt const&)
      : state(Copied) {}

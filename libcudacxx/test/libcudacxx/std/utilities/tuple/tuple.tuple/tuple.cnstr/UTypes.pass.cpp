...
__host__ __device__ void test_default_constructible_extension_sfinae() {
  {
    ...
    static_assert(
        cuda::std::is_constructible<Tuple, MoveOnly, NoDefault>::value, "");
  }
  {
    typedef cuda::std::tuple<MoveOnly, MoveOnly, NoDefault> Tuple;

    static_assert(
        !cuda::std::is_constructible<Tuple, MoveOnly, MoveOnly>::value, "");

    static_assert(cuda::std::is_constructible<Tuple, MoveOnly, MoveOnly,
                                              NoDefault>::value,
                  "");
  }
  {
    ...
    static_assert(!cuda::std::is_constructible<NestedTuple, MoveOnly, MoveOnly,
                                               MoveOnly, MoveOnly>::value,
                  "");

    static_assert(cuda::std::is_constructible<NestedTuple, MoveOnly, Tuple,
                                              MoveOnly, MoveOnly>::value,
                  "");
  }
  // testing extensions
...
int main(int, char**) {
  ...
  {
    cuda::std::tuple<MoveOnly, MoveOnly, MoveOnly> t(MoveOnly(0), MoveOnly(1),
                                                     MoveOnly(2));
    assert(cuda::std::get<0>(t) == 0);
    assert(cuda::std::get<1>(t) == 1);

...
__host__ __device__ void test_uses_allocator_sfinae_evaluation() {
  using BadDefault = DefaultCtorBlowsUp<>;
  {
    ...
    static_assert(
        !cuda::std::is_constructible<Tuple, cuda::std::allocator_arg_t, A1<int>,
                                     MoveOnly>::value,
        "");

    static_assert(
        cuda::std::is_constructible<Tuple, cuda::std::allocator_arg_t, A1<int>,
                                    MoveOnly, MoveOnly, BadDefault>::value,
        "");
  }
  {
    ...
    static_assert(
        !cuda::std::is_constructible<Tuple, cuda::std::allocator_arg_t, A1<int>,
                                     MoveOnly, MoveOnly>::value,
        "");

    static_assert(cuda::std::is_constructible<Tuple, cuda::std::allocator_arg_t,
                                              A1<int>, MoveOnly, MoveOnly,
                                              BadDefault, BadDefault>::value,
                  "");
  }
}
...
int main(int, char**) {
  ...
  /*
    ...
    */
  {
    cuda::std::tuple<MoveOnly> t(cuda::std::allocator_arg, A1<int>(),
                                 MoveOnly(0));
  ...
  {
    cuda::std::tuple<MoveOnly, MoveOnly, MoveOnly> t(
        cuda::std::allocator_arg, A1<int>(), MoveOnly(0), 1, 2);
    assert(cuda::std::get<0>(t) == 0);
    assert(cuda::std::get<1>(t) == 1);
  ...
  {
    using T = DefaultCtorBlowsUp<>;
    cuda::std::tuple<T, T, T> t(cuda::std::allocator_arg, A1<int>(), T(1), T(2),
                                T(3));

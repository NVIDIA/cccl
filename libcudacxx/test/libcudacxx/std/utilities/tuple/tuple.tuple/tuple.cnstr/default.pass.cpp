...
int main(int, char**) {
    ...
    */
  {
    // See bug #21157.
    static_assert(
        !cuda::std::is_default_constructible<cuda::std::tuple<NoDefault> >(),
        "");
    static_assert(!cuda::std::is_default_constructible<
                      cuda::std::tuple<DefaultOnly, NoDefault> >(),
                  "");
    static_assert(!cuda::std::is_default_constructible<
                      cuda::std::tuple<NoDefault, DefaultOnly, NoDefault> >(),
                  "");
  }
  {
    static_assert(noexcept(cuda::std::tuple<NoExceptDefault>()), "");
    static_assert(
        noexcept(cuda::std::tuple<NoExceptDefault, NoExceptDefault>()), "");

#ifndef TEST_COMPILER_BROKEN_SMF_NOEXCEPT
    static_assert(
        !noexcept(cuda::std::tuple<ThrowingDefault, NoExceptDefault>()), "");
    static_assert(
        !noexcept(cuda::std::tuple<NoExceptDefault, ThrowingDefault>()), "");
    static_assert(
        !noexcept(cuda::std::tuple<ThrowingDefault, ThrowingDefault>()), "");
  ...
  }
  {
    ...
    static_assert(!cuda::std::is_default_constructible<
                      cuda::std::tuple<Derived, int> >::value,
                  "");
  }

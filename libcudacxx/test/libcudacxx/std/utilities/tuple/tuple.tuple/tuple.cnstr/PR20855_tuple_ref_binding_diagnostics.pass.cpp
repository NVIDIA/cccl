...
template <class Tp>
struct ConvertsTo {
  using RawTp = typename cuda::std::remove_cv<
      typename cuda::std::remove_reference<Tp>::type>::type;

  __host__ __device__ operator Tp() const { return static_cast<Tp>(value); }
...
static_assert(cuda::std::is_constructible<
                  int&, cuda::std::reference_wrapper<int> >::value,
              "");
static_assert(cuda::std::is_constructible<
                  int const&, cuda::std::reference_wrapper<int> >::value,
              "");
...
F(typename CannotDeduce<cuda::std::tuple<Args...> >::type const&) {}
...
__host__ __device__ void compile_tests() {
  { F<int, int const&>(cuda::std::make_tuple(42, 42)); }
  {
    ...
    cuda::std::tuple<int, int const&> t(
        cuda::std::make_tuple<const int&, const int&>(42, 42));
  }
  // cuda::std::string not supported
  /*
  {

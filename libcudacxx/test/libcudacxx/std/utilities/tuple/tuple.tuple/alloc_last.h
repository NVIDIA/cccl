...
struct alloc_last {
  ...
  __host__ __device__ alloc_last(const A1<int>& a) : data_(0) {
    assert(a.id() == 5);
    allocator_constructed() = true;
  }

  __host__ __device__ alloc_last(int d, const A1<int>& a) : data_(d) {
    assert(a.id() == 5);
    allocator_constructed() = true;
  }
  ...
  __host__ __device__ friend bool operator==(const alloc_last& x,
                                             const alloc_last& y) {

...
int main(int, char**) {
  ...
  {
    cuda::std::tuple<int, MoveOnly> t =
        cuda::std::tuple_cat(cuda::std::pair<int, MoveOnly>(2, 1));
...

  {
    ...
    cuda::std::tuple<int*, MoveOnly> t2(nullptr, 4);
    cuda::std::tuple<MoveOnly, MoveOnly, int*, MoveOnly> t3 =
        cuda::std::tuple_cat(cuda::std::tuple<>(), cuda::std::move(t1),
                             cuda::std::move(t2));
    assert(cuda::std::get<0>(t3) == 1);
    assert(cuda::std::get<1>(t3) == 2);
    assert(cuda::std::get<2>(t3) == nullptr);
  ...
  }
  {
    ...
    cuda::std::tuple<int*, MoveOnly> t2(nullptr, 4);
    cuda::std::tuple<MoveOnly, MoveOnly, int*, MoveOnly> t3 =
        cuda::std::tuple_cat(cuda::std::move(t1), cuda::std::tuple<>(),
                             cuda::std::move(t2));
    assert(cuda::std::get<0>(t3) == 1);
    assert(cuda::std::get<1>(t3) == 2);
    assert(cuda::std::get<2>(t3) == nullptr);
  ...
  }
  {
    // See bug #19616.
    auto t1 =
        cuda::std::tuple_cat(cuda::std::make_tuple(cuda::std::make_tuple(1)),
                             cuda::std::make_tuple());
    ...
    assert(t2 == cuda::std::make_tuple(cuda::std::make_tuple(1),
                                       cuda::std::make_tuple(2)));
  }
  {
    int x = 101;
    cuda::std::tuple<int, const int, int&, const int&, int&&> t(
        42, 101, x, x, cuda::std::move(x));
    const auto& ct = t;
    cuda::std::tuple<int, const int, int&, const int&> t2(42, 101, x, x);
    const auto& ct2 = t2;

    auto r =
        cuda::std::tuple_cat(cuda::std::move(t), cuda::std::move(ct), t2, ct2);

    ASSERT_SAME_TYPE(
        decltype(r),
        cuda::std::tuple<int, const int, int&, const int&, int&&, int,
                         const int, int&, const int&, int&&, int, const int,

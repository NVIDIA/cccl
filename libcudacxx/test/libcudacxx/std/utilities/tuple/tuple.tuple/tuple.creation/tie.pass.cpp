...
__host__ __device__ constexpr bool test_tie_constexpr() {
  {
    ...
    double f = 1.1;
    using ExpectT =
        cuda::std::tuple<int&, decltype(cuda::std::ignore)&, double&>;
...
int main(int, char**) {
  NV_IF_TARGET(NV_IS_HOST, ({
                 ...
                 // cuda::std::string s;
                 const char* s;
                 cuda::std::tie(i, cuda::std::ignore, s) =
                     cuda::std::make_tuple(42, 3.14, _s);
                 assert(i == 42);
                 assert(s == _s);
               }))
#if TEST_STD_VER > 2011
  {
    ...
    constexpr cuda::std::tuple<const int&, const double&> t =
        cuda::std::tie(i, f);

...
static_assert(cuda::std::is_assignable_v<const cuda::std::tuple<int&>&, cuda::std::tuple<long&>&&>);
static_assert(!cuda::std::is_assignable_v<const cuda::std::tuple<int&, int&>&, cuda::std::tuple<long&>&&>);
static_assert(!cuda::std::is_assignable_v<const cuda::std::tuple<int&>&, cuda::std::tuple<long&, long&>&&>);

// (is_assignable_v<const Types&, UTypes&&> && ...) is true
static_assert(cuda::std::is_assignable_v<const cuda::std::tuple<AssignableFrom<ConstMoveAssign>>&,
                                         cuda::std::tuple<ConstMoveAssign>&&>);

static_assert(cuda::std::is_assignable_v<const cuda::std::tuple<AssignableFrom<ConstMoveAssign>, ConstMoveAssign>&,
                                         cuda::std::tuple<ConstMoveAssign, ConstMoveAssign>&&>);

static_assert(
  !cuda::std::is_assignable_v<const cuda::std::tuple<AssignableFrom<ConstMoveAssign>, AssignableFrom<MoveAssign>>&,
                              cuda::std::tuple<ConstMoveAssign, MoveAssign>&&>);
...
TEST_FUNC constexpr bool test()
{
  // reference types
  {
    ...
    long j1 = 3;
    long j2 = 4;
    cuda::std::tuple<int&, int&> t1{i1, i2};
  ...
  // user defined const copy assignment
  {
    ...
    const cuda::std::tuple<AssignableFrom<ConstMoveAssign>> t2{2};
    t2 = cuda::std::move(t1);

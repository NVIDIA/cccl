...
struct S {
  ...
  __host__ __device__ double const volatile& operator()(char, int&) const
      volatile;
};
...
template <class T>
struct HasType<T, typename Voider<typename T::type>::type>
    : cuda::std::true_type {};
...
int main(int, char**) {
  ...
  { // functor object
    ...
    test_result_of<S const volatile&(unsigned char, int&),
  ...
  { // pointer to member function
    ...
    test_result_of<PMS2(const cuda::std::reference_wrapper<S>&, int, int),
                   int&>();
    test_result_of<PMS2(cuda::std::reference_wrapper<SD>, int, int), int&>();
    test_result_of<PMS2(const cuda::std::reference_wrapper<SD>&, int, int),
                   int&>();
  ...
  { // extended lambda
    NV_IF_TARGET(
        ...
        (test_lambda<int>([] __host__ __device__() -> int { return 42; });
         test_lambda<double>(
             [] __host__ __device__() -> double { return 42.0; });

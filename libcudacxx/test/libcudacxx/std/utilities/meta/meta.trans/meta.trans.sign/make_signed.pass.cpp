...
int main(int, char**) {
  ...
  test_make_signed<wchar_t, cuda::std::conditional<sizeof(wchar_t) == 4, int,
                                                   short>::type>();
  test_make_signed<const wchar_t,
                   cuda::std::conditional<sizeof(wchar_t) == 4, const int,
                                          const short>::type>();
  test_make_signed<
      const Enum, cuda::std::conditional<sizeof(Enum) == sizeof(int), const int,
                                         const signed char>::type>();
  test_make_signed<BigEnum, cuda::std::conditional<sizeof(long) == 4, long long,
                                                   long>::type>();

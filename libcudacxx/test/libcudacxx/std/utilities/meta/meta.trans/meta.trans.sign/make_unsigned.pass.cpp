...
enum BigEnum : unsigned long long // MSVC's ABI doesn't follow the Standard
{
  ...
  big = 0xFFFFFFFFFFFFFFFFULL
    ...
    !defined(_LIBCUDACXX_HAS_NO_STRONG_ENUMS)
enum HugeEnum : __int128_t { hugezero };
#endif
...
int main(int, char**) {
  ...
  test_make_unsigned<wchar_t,
                     cuda::std::conditional<sizeof(wchar_t) == 4, unsigned int,
                                            unsigned short>::type>();
  test_make_unsigned<
      const wchar_t,
      cuda::std::conditional<sizeof(wchar_t) == 4, const unsigned int,
                             const unsigned short>::type>();
  test_make_unsigned<
      const Enum,
      cuda::std::conditional<sizeof(Enum) == sizeof(int), const unsigned int,
                             const unsigned char>::type>();
  test_make_unsigned<
      BigEnum, cuda::std::conditional<sizeof(long) == 4, unsigned long long,
                                      unsigned long>::type>();

...
static_assert(!cuda::std::is_invocable_v<decltype(cuda::std::ranges::ssize),
                                         const SizeMember&>);
...
__host__ __device__ constexpr bool test() {
  ...
  ASSERT_SAME_TYPE(decltype(cuda::std::ranges::ssize(SizeMember())),
                   SignedSizeT);

  assert(cuda::std::ranges::ssize(SizeFunction()) == 42);
  ASSERT_SAME_TYPE(decltype(cuda::std::ranges::ssize(SizeFunction())),
                   SignedSizeT);

  assert(cuda::std::ranges::ssize(SizeFunctionSigned()) == 42);
  ASSERT_SAME_TYPE(decltype(cuda::std::ranges::ssize(SizeFunctionSigned())),
                   cuda::std::ptrdiff_t);

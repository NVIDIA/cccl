...
static_assert(!cuda::std::ranges::empty(
    static_cast<const Incomplete (&&)[42]>(array_of_incomplete)));
...
static_assert(
    !cuda::std::is_invocable_v<RangeEmptyT, const InputRangeWithoutSize&>);
...
inline constexpr bool
    cuda::std::ranges::disable_sized_range<DisabledSizeRangeWithBeginEnd> =
        true;
static_assert(
    cuda::std::ranges::contiguous_range<DisabledSizeRangeWithBeginEnd>);

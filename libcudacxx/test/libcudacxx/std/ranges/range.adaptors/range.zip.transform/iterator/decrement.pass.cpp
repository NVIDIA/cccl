...
template <class Iter>
_CCCL_CONCEPT canDecrement = _CCCL_REQUIRES_EXPR((Iter), Iter it)(--it) || _CCCL_REQUIRES_EXPR((Iter), Iter it)(it--);
...
TEST_FUNC constexpr bool test()
{
  ...
  {
    // all random access
    cuda::std::ranges::zip_transform_view v(MakeTuple{}, a, b, cuda::std::views::iota(0, 5));
    auto it    = v.end();
    ...
    assert(*it == cuda::std::tuple(3, 4.3, 2));

    auto original                               = it;
    cuda::std::same_as<Iter> decltype(auto) it2 = it--;
    assert(original == it2);
    assert(*it == cuda::std::tuple(2, 3.2, 1));
  }

  {
    ...
    cuda::std::ranges::zip_transform_view v(MakeTuple{}, BidiCommonView{buffer}, cuda::std::views::iota(0, 5));
    auto it    = v.begin();
    using Iter = decltype(it);
    static_assert(canDecrement<Iter>);
...

  {
    // non bidi
    int buffer[3] = {4, 5, 6};
    cuda::std::ranges::zip_transform_view v(MakeTuple{}, a, InputCommonView{buffer});
...

  {
    ...
    assert(*it == cuda::std::tuple(6));

    auto original                               = it;
    cuda::std::same_as<Iter> decltype(auto) it2 = it--;
    assert(original == it2);
    assert(*it == cuda::std::tuple(5));
  }
...

  {
    ...
    assert(*it == cuda::std::tuple(1, 1, 2.0));

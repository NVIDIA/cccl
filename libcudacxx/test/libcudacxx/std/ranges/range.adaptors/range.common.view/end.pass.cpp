...
TEST_FUNC constexpr bool test()
{
  ...
  // const version
  {
    ...
    decltype(auto) end = common.end(); // Note this should NOT be the sentinel type.
    static_assert(cuda::std::same_as<decltype(end), RandomAccessIter>);
    static_assert(cuda::std::same_as<decltype(end), cuda::std::ranges::iterator_t<const SizedRandomAccessView>>);
    assert(base(end) == buf + 8);
  }
  ...
  // const versions
  {
    ...
    decltype(auto) end = common.end();
    static_assert(cuda::std::same_as<decltype(end), CommonForwardIter>);
    using ConstCommonForwardIter = cuda::std::common_iterator<cuda::std::ranges::iterator_t<const SizedForwardView>,
                                                              cuda::std::ranges::sentinel_t<const SizedForwardView>>;
    static_assert(cuda::std::same_as<decltype(end), ConstCommonForwardIter>);
    assert(end == CommonForwardIter(cuda::std::ranges::end(view)));
    assert(end == ConstCommonForwardIter(cuda::std::ranges::end(cuda::std::as_const(view))));
  }
  {
    CopyableView view{buf, buf + 8};

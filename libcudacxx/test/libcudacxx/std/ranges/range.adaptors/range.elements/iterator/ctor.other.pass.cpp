...
struct ConvertibleIter : IterBase<ConvertibleIter<Const>>
{
  ...
  TEST_FUNC constexpr ConvertibleIter(int ii)
      : i(ii)
  {}
  template <bool otherConst, cuda::std::enable_if_t<Const != otherConst, int> = 0>
  TEST_FUNC constexpr ConvertibleIter(ConvertibleIter<otherConst> it)
...
struct BasicView : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC ConstIter begin() const
  {
    return ConstIter{};
  }
  TEST_FUNC ConstIter end() const
  {
    return ConstIter{};
  }
};

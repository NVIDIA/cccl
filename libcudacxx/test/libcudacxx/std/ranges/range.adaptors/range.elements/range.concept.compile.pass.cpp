...
template <size_t N>
using size_t_constant = cuda::std::integral_constant<size_t, N>;

template <class View, class Np>
_CCCL_CONCEPT HasElementsView2 =
  _CCCL_REQUIRES_EXPR((View, Np))(typename(typename cuda::std::ranges::elements_view<View, Np::value>));

template <class View, size_t N>
_CCCL_CONCEPT HasElementsView = HasElementsView2<View, cuda::std::integral_constant<size_t, N>>;
...
// !tuple-like
static_assert(!cuda::std::__tuple_like<int>);
...
struct NonMovable
{
  TEST_FUNC constexpr NonMovable(int) {}
...
struct ToPair
{
  TEST_FUNC constexpr auto operator()(int) const noexcept
  {

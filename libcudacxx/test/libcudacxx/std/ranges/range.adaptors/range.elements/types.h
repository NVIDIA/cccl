...
struct BufferView : cuda::std::ranges::view_base
{
  ...
  template <cuda::std::size_t N>
  TEST_FUNC constexpr BufferView(cuda::std::array<T, N>& arr)
      : buffer_(arr.data())
      , size_(N)
  {}
};
...
#  define DELEGATE_TUPLEBUFFERVIEW(Derived)             \
    template <class T>                                  \
    TEST_FUNC constexpr Derived(T&& input)              \
...
struct Common : TupleBufferView
{
  ...
  TEST_FUNC constexpr cuda::std::tuple<int>* begin()
  {
    return buffer_;
  }
  TEST_FUNC constexpr const cuda::std::tuple<int>* begin() const
  {
    return buffer_;
  }

  template <bool Simple2 = Simple, cuda::std::enable_if_t<!Simple2, int> = 0>
  TEST_FUNC constexpr cuda::std::tuple<int>* end()
  {
    return buffer_ + size_;
  }
  TEST_FUNC constexpr const cuda::std::tuple<int>* end() const
  {
    return buffer_ + size_;
  }
};
...
struct NonCommon : TupleBufferView
{
  ...
  template <bool Simple2 = Simple, cuda::std::enable_if_t<!Simple2, int> = 0>
  TEST_FUNC constexpr sentinel_wrapper<cuda::std::tuple<int>*> end()
  {
    return sentinel_wrapper<cuda::std::tuple<int>*>(buffer_ + size_);
  }
  TEST_FUNC constexpr sentinel_wrapper<const cuda::std::tuple<int>*> end() const
  {
...
struct IterBase
{
  ...
  TEST_FUNC constexpr cuda::std::tuple<int> operator*() const
  {
    return cuda::std::tuple<int>(5);
  }

  TEST_FUNC constexpr Derived& operator++()
  {
    return *this;
  }
  ...
  TEST_FUNC friend constexpr bool operator==(const IterBase&, const IterBase&) = default;
#else // ^^^ C++20 ^^^ / vvv C++17 vvv
  TEST_FUNC friend constexpr bool operator==(const IterBase&, const IterBase&) noexcept
  {
    return true;
  }
  TEST_FUNC friend constexpr bool operator!=(const IterBase&, const IterBase&) noexcept

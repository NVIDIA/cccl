...
{
  TEST_FUNC [[nodiscard]] T* begin() const
  {
    return nullptr;
  }
  TEST_FUNC [[nodiscard]] T* end() const
  {
    return nullptr;
  }
};
...
TEST_FUNC constexpr bool test()
{
  ...
  // Test views::elements<0> | views::elements<0>
  {
    ...
    using Result = cuda::std::ranges::elements_view<
      ...
      0>;
    auto const partial =
      cuda::std::views::elements<0> | cuda::std::views::elements<0>; // NOLINT(misc-redundant-expression)
  ...
  // Test views::keys | views::keys
  {
    ...
    using Result = cuda::std::ranges::elements_view<
      cuda::std::ranges::
        elements_view<cuda::std::ranges::ref_view<cuda::std::pair<cuda::std::tuple<int>, cuda::std::tuple<int>>[3]>, 0>,
      0>;
    auto const partial    = cuda::std::views::keys | cuda::std::views::keys; // NOLINT(misc-redundant-expression)
    decltype(auto) result = nested | partial;
    static_assert(cuda::std::same_as<decltype(result), Result>);
    auto expected = {1, 3, 5};
    assert(equal(result, expected));
  ...
  // Test views::values | views::values
  {
    ...
    using Result = cuda::std::ranges::elements_view<
      cuda::std::ranges::elements_view<
        ...
        1>,
      1>;
    auto const partial    = cuda::std::views::values | cuda::std::views::values; // NOLINT(misc-redundant-expression)
    decltype(auto) result = nested | partial;
    static_assert(cuda::std::same_as<decltype(result), Result>);
    auto expected = {3, 6, 9};
    assert(equal(result, expected));

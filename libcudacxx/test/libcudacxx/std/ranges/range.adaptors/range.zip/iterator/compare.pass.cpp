...
struct SmallerThanRange : IntBufferView
{
  ...
  TEST_FUNC constexpr SmallerThanRange(int (&b)[N])
      : IntBufferView(b)
  {}
#else
  using IntBufferView::IntBufferView;
#endif
  TEST_FUNC constexpr LessThanIterator begin() const
  {
    return {buffer_};
  }
  TEST_FUNC constexpr LessThanIterator end() const
  {
...
struct ForwardCommonView : IntBufferView
{
#if TEST_COMPILER(NVRTC) // nvbug 3961621
  template <cuda::std::size_t N>
  TEST_FUNC constexpr ForwardCommonView(int (&b)[N])
      : IntBufferView(b)
  {}
#else
  using IntBufferView::IntBufferView;
  ...
  TEST_FUNC constexpr iterator end() const
  {

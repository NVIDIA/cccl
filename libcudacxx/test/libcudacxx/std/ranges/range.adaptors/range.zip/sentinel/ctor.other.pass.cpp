...
struct convertible_sentinel_wrapper
{
  ...
  TEST_FUNC constexpr friend bool operator==(const T& other, convertible_sentinel_wrapper const& self)
  {
    return self.it_ == other;
  }
  TEST_FUNC constexpr friend bool operator!=(convertible_sentinel_wrapper const& self, const T& other)
...
struct NonSimpleNonCommonConvertibleView : IntBufferView
{
  ...
  TEST_FUNC constexpr NonSimpleNonCommonConvertibleView(int (&b)[N])
      : IntBufferView(b)
  {}
#else
...
{
  test();
#if !TEST_COMPILER(MSVC) // conversion from "int *" to "const int *" is invalid in constant-expression
  static_assert(test());

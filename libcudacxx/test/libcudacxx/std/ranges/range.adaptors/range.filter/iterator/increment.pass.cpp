...
TEST_FUNC constexpr void test()
{
  ...
  // Increment an iterator when it won't find another satisfied value after begin()
  {
    ...
    FilterView view =
      make_filter_view<FilterView, View, Iterator, Sentinel>(array.data(), array.data() + array.size(), EqualTo{1});
  ...
  if constexpr (!IsConst)
  {
    ...
    auto view = cuda::std::ranges::filter_view(cuda::std::move(v), pred);
    auto it   = view.begin();
    assert(base(it.base()) == array.data() + 1);
    ++it;
    assert(base(it.base()) == array.data() + 3);
  ...
  // (we should be passing it to ranges::find_if using cuda::std::ref)
  {
    ...
    View v{Iterator(array.data()), Sentinel(Iterator(array.data() + array.size()))};
    auto view = cuda::std::ranges::filter_view(cuda::std::move(v), TrackingPred(&moved, &copied));
    moved     = false;
    copied    = false;
    auto it   = view.begin();
    ++it;

...
struct diff_type_iter
{
  ...
  TEST_FUNC V& operator*() const;
  TEST_FUNC diff_type_iter& operator++();
  TEST_FUNC void operator++(int);
#if TEST_STD_VER >= 2020
  TEST_FUNC friend constexpr bool operator==(diff_type_iter, diff_type_iter) = default;
#else // ^^^ C++20 ^^^ / vvv C++17 vvv
  TEST_FUNC friend constexpr bool operator==(const diff_type_iter&, const diff_type_iter&)
  {
    return true;
  }
  TEST_FUNC friend constexpr bool operator!=(const diff_type_iter&, const diff_type_iter&)
...
{
  TEST_FUNC diff_type_iter<T, V> begin() const
  {
    return diff_type_iter<T, V>{};
  }
  TEST_FUNC diff_type_iter<T, V> end() const
  {
    return diff_type_iter<T, V>{};
  }
};
...
TEST_FUNC void test()
{
...

  {
    ...
    static_assert(!cuda::std::is_same_v<InnerValue, cuda::std::remove_cvref_t<InnerReference>>);

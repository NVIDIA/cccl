...
struct move_only_input_iter_with_arrow
{
  ...
  TEST_FUNC constexpr move_only_input_iter_with_arrow(Base it)
  ...
  TEST_FUNC constexpr move_only_input_iter_with_arrow& operator++()
  {
    ++it_;
    return *this;
  }
...
struct move_iter_sentinel
{
  ...
  TEST_FUNC constexpr move_iter_sentinel(Base it)
      : it_(cuda::std::move(it))
  {}
  TEST_FUNC constexpr friend bool
...
struct MoveOnlyIterInner : BufferView<move_only_input_iter_with_arrow<Box*>, move_iter_sentinel<Box*>>
{
  ...
  template <class T>
  TEST_FUNC constexpr MoveOnlyIterInner(T&& arr) noexcept(
    noexcept(BufferView<move_only_input_iter_with_arrow<Box*>, move_iter_sentinel<Box*>>(cuda::std::declval<T>())))
      : BufferView<move_only_input_iter_with_arrow<Box*>, move_iter_sentinel<Box*>>(cuda::std::forward<T>(arr))
  {}
...
struct arrow_input_iter
{
  ...
  TEST_FUNC constexpr arrow_input_iter(Base it)
      : it_(cuda::std::move(it))
  {}

  TEST_FUNC constexpr arrow_input_iter& operator++()
  {
    ++it_;
    return *this;
  }
  ...
  TEST_FUNC friend constexpr bool operator==(const arrow_input_iter& x, const arrow_input_iter& y) = default;
#else // ^^^ C++20 ^^^ / vvv C++17 vvv
  TEST_FUNC friend constexpr bool operator==(const arrow_input_iter& x, const arrow_input_iter& y)
  {
    return x.it_ == y.it_;
  }
  TEST_FUNC friend constexpr bool operator!=(const arrow_input_iter& x, const arrow_input_iter& y)

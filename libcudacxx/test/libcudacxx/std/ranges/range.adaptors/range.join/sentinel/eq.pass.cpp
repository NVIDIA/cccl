...
struct ConstComparableSentinel
{
  ...
  TEST_FUNC constexpr explicit ConstComparableSentinel(const Iter& it)
      : iter_(it)
  {}

  TEST_FUNC constexpr friend bool operator==(const Iterator& i, const ConstComparableSentinel& s)
  {
    return base(i) == base(s.iter_);
  }
#if TEST_STD_VER <= 2017
  TEST_FUNC constexpr friend bool operator==(const ConstComparableSentinel& s, const Iterator& i)
  {
    return base(i) == base(s.iter_);
  }
  TEST_FUNC constexpr friend bool operator!=(const Iterator& i, const ConstComparableSentinel& s)
  ...
  TEST_FUNC constexpr friend bool operator==(const ConstIterator& i, const ConstComparableSentinel& s)
  {
    return base(i) == base(s.iter_);
  }
#if TEST_STD_VER <= 2017
  TEST_FUNC constexpr friend bool operator==(const ConstComparableSentinel& s, const ConstIterator& i)
  {
    return base(i) == base(s.iter_);
  }
  TEST_FUNC constexpr friend bool operator!=(const ConstIterator& i, const ConstComparableSentinel& s)
...
struct ConstComparableView : BufferView<BufferView<int*>*>
{
  ...
  template <class T>
  TEST_FUNC constexpr ConstComparableView(T&& arr) noexcept(
    noexcept(BufferView<BufferView<int*>*>(cuda::std::declval<T>())))
      : BufferView<BufferView<int*>*>(cuda::std::forward<T>(arr))
  {}
#else

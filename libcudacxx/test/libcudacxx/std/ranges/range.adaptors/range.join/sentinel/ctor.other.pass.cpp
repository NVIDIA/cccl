...
struct ConstConvertibleView : BufferView<BufferView<int*>*>
{
  ...
  ConstConvertibleView() noexcept = default;

  template <class T>
  TEST_FUNC constexpr ConstConvertibleView(T&& arr) noexcept(
...
static_assert(!cuda::std::ranges::common_range<ConstConvertibleView>);
static_assert(cuda::std::convertible_to<cuda::std::ranges::sentinel_t<ConstConvertibleView>,
                                        cuda::std::ranges::sentinel_t<ConstConvertibleView const>>);

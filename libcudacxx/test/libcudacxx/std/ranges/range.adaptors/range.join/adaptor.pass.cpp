...
struct MoveOnlyOuter : SimpleForwardCommonOuter<ForwardCommonInner>
{
  ...
  template <class T>
  TEST_FUNC constexpr MoveOnlyOuter(T&& arr) noexcept(
    noexcept(SimpleForwardCommonOuter<ForwardCommonInner>(cuda::std::declval<T>())))
      : SimpleForwardCommonOuter<ForwardCommonInner>(cuda::std::forward<T>(arr))
  {}
#else
...
{
  int i;
  TEST_FUNC constexpr Foo(int ii)
      : i(ii)
  {}
};
...
TEST_FUNC constexpr bool test()
{
...
#if !TEST_COMPILER(MSVC) // MSVC cannot parse this
  {
    ...
    static_assert(cuda::std::is_same_v<cuda::std::ranges::range_reference_t<decltype(jv)>, Foo(&)[3]>);

    auto jv2 = cuda::std::views::join(jv);
    static_assert(cuda::std::is_same_v<cuda::std::ranges::range_reference_t<decltype(jv2)>, Foo&>);
...

  {
    ...
    static_assert(cuda::std::is_same_v<cuda::std::ranges::range_reference_t<decltype(jv)>, Foo&>);
...
#  if !TEST_CUDA_COMPILER(NVCC) && !TEST_COMPILER(NVRTC) // access to expired storage
  static_assert(test());

...
TEST_GLOBAL_VARIABLE int globalBuffer[4][4] = {
  {1111, 2222, 3333, 4444},
  {555, 666, 777, 888},
  {99, 1010, 1111, 1212},
  {13, 14, 15, 16},
...
struct ChildViewBase : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC constexpr ChildViewBase(int* ptr = globalBuffer[0])
      : ptr_(ptr)
  {}
  ...
  TEST_FUNC constexpr const_sentinel end() const
  {
...
TEST_GLOBAL_VARIABLE ChildView globalChildren[4] = {
...
struct ParentView : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC constexpr ParentView(T* ptr, unsigned size = 4)
      : ptr_(ptr)
      , size_(size)
  {}
  template <class T2 = T, cuda::std::enable_if_t<cuda::std::same_as<ChildView, T2>, int> = 0>
  TEST_FUNC constexpr ParentView(ChildView* ptr = globalChildren, unsigned size = 4)
      : ptr_(ptr)
      , size_(size)
  {}
...
template <class T>
TEST_FUNC ParentView(T*) -> ParentView<T>;
...
struct CopyableChild : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC constexpr CopyableChild(int* ptr = globalBuffer[0], unsigned size = 4) noexcept
      : ptr_(ptr)
      , size_(size)
  {}
...
struct CopyableParentTemplate : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC constexpr CopyableParentTemplate(CopyableChild* ptr)
      : ptr_(ptr)
  {}
...
struct InputValueIter
{
  ...
  TEST_FUNC constexpr InputValueIter(T* ptr)
      : ptr_(ptr)
  {}
  ...
  TEST_FUNC constexpr InputValueIter& operator++()
  {
    ++ptr_;
    return *this;
  }

  TEST_FUNC constexpr T* operator->()
  {
    return ptr_;
  }

#if TEST_STD_VER >= 2020
  TEST_FUNC constexpr friend bool operator==(const InputValueIter&, const InputValueIter&) = default;
#else // ^^^ C++20 ^^^ / vvv C++17 vvv
  TEST_FUNC constexpr friend bool operator==(const InputValueIter& lhs, const InputValueIter& rhs)
  {
...
struct ValueView : cuda::std::ranges::view_base
{
  ...
  TEST_FUNC constexpr ValueView(T* ptr)
      : ptr_(ptr)
  {}

  TEST_FUNC constexpr ValueView(ValueView&& other)
      : ptr_(other.ptr_)
  {
    other.ptr_.ptr_ = nullptr;
  }

  TEST_FUNC constexpr ValueView& operator=(ValueView&& other)
  {
    ptr_       = other.ptr_;
  ...
  TEST_FUNC constexpr InputValueIter<T> begin() const
  {
    return ptr_;
  }
...
{
#if TEST_COMPILER(MSVC) // MSVC cannot handle iter_value_t here
  ...
  TEST_FUNC constexpr BufferView(T (&b)[N])
      ...
      , size_(N)
  {}
  TEST_FUNC constexpr BufferView(T* p, cuda::std::size_t s)
      : data_(p)
      , size_(s)
  {}
...
struct copying_iterator
{
  ...
  TEST_FUNC constexpr copying_iterator() noexcept(cuda::std::is_nothrow_default_constructible_v<It2>)
  {}
  TEST_FUNC constexpr copying_iterator(It it)
  ...
  TEST_FUNC constexpr copying_iterator& operator++()
  {
    ++it_;
    return *this;
  }
  ...
  template <class It2 = It, cuda::std::enable_if_t<cuda::std::forward_iterator<It2>, int> = 0>
  TEST_FUNC constexpr copying_iterator operator++(int)
  {
...
#if TEST_STD_VER >= 2020
  TEST_FUNC friend constexpr bool operator==(copying_iterator const&, copying_iterator const&) = default;
#else // ^^^ C++20 ^^^ / vvv C++17 vvv
  TEST_FUNC constexpr friend bool operator==(const copying_iterator& lhs, const copying_iterator& rhs)
  {
...
struct InnerRValue : Outer
{
  ...
  TEST_FUNC constexpr InnerRValue(_Args&&... __args) noexcept(noexcept(Outer(cuda::std::declval<_Args>()...)))
      : Outer(cuda::std::forward<_Args>(__args)...)
  {}
...
struct move_swap_aware_iter
{
  ...
  TEST_FUNC constexpr move_swap_aware_iter& operator++()
  {
    ++i_;
    return *this;
  }
  ...
  TEST_FUNC constexpr friend rvalue_reference iter_move(const move_swap_aware_iter& x) noexcept
  {
    ++(*x.iter_move_called);
    return rvalue_reference{cuda::std::move(*x.i_), cuda::std::move(*x.i_)};
  }

  TEST_FUNC TEST_CONSTEXPR_CXX20 friend void
...
struct IterMoveSwapAwareView : BufferView<int*>
{
  ...
  template <class T>
  TEST_FUNC constexpr IterMoveSwapAwareView(T&& arr) noexcept(noexcept(BufferView<int*>(cuda::std::declval<T>())))
      : BufferView<int*>(cuda::std::forward<T>(arr))
  {}
#else // ^^^ TEST_COMPILER(NVRTC) ^^^ / vvv !TEST_COMPILER(NVRTC) vvv
...
public:
  ...
  TEST_FUNC constexpr StashingIterator()
      : letter_('a')
  {}

  TEST_FUNC constexpr StashingIterator& operator++()
  {
    str_ += letter_;
    ++letter_;
    return *this;
  ...
  TEST_FUNC constexpr bool operator==(cuda::std::default_sentinel_t) const
  {
    return letter_ > 'z';
  }
#  if TEST_STD_VER <= 2017
...
{
public:
  TEST_FUNC constexpr StashingIterator begin()
  {
    return {};
  }
  TEST_FUNC constexpr cuda::std::default_sentinel_t end()
  {
    return {};
  }

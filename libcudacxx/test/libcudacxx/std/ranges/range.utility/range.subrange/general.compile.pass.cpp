...
concept ValidSubrangeKind = requires {
  typename cuda::std::ranges::subrange<Args..., K>;
};

template <class... Args>
concept ValidSubrange = requires {
  typename cuda::std::ranges::subrange<Args...>;
};
#else
...
template <class It>
constexpr bool
    ValidSubrange1<It, cuda::std::void_t<cuda::std::ranges::subrange<It> > > =
        true;
...
constexpr bool ValidSubrange2<
    It, Sent, cuda::std::void_t<cuda::std::ranges::subrange<It, Sent> > > =
    true;
...
template <cuda::std::ranges::subrange_kind Kind, class It, class Sent>
constexpr bool ValidSubrangeKind<
    Kind, It, Sent,
    cuda::std::void_t<cuda::std::ranges::subrange<It, Sent, Kind> > > = true;
#endif

static_assert(ValidSubrange<forward_iterator<int*> >);
static_assert(ValidSubrange<forward_iterator<int*>, forward_iterator<int*> >);
static_assert(
    ValidSubrangeKind<cuda::std::ranges::subrange_kind::unsized,
                      forward_iterator<int*>, forward_iterator<int*> >);
static_assert(
    ValidSubrangeKind<cuda::std::ranges::subrange_kind::sized,
                      forward_iterator<int*>, forward_iterator<int*> >);
// Wrong sentinel type.
...
// Must be sized.
static_assert(
    !ValidSubrangeKind<cuda::std::ranges::subrange_kind::unsized, int*, int*>);
static_assert(
    ValidSubrangeKind<cuda::std::ranges::subrange_kind::sized, int*, int*>);
// Wrong sentinel type.

...
template <size_t I, class S>
constexpr bool HasGet<
    I, S,
    cuda::std::void_t<decltype(cuda::std::get<I>(cuda::std::declval<S>()))> > =
    true;
#endif
...
__host__ __device__ constexpr bool test() {
  {
    ...
    ASSERT_SAME_TYPE(decltype(cuda::std::get<1>(static_cast<const R&>(r))),
                     Sent);
    ASSERT_SAME_TYPE(decltype(cuda::std::get<0>(static_cast<const R&&>(r))),
                     It);
    ASSERT_SAME_TYPE(decltype(cuda::std::get<1>(static_cast<const R&&>(r))),
                     Sent);
    ...
    assert(base(base(cuda::std::get<1>(cuda::std::move(r)))) ==
           a + 3); // copy from Sent
  }
  {
    using It = int*;

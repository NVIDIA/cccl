...
template <class T>
concept ValidViewInterfaceType = requires {
  typename cuda::std::ranges::view_interface<T>;
};
#else
...
template <class T>
constexpr bool ValidViewInterfaceType<
    T, cuda::std::void_t<cuda::std::ranges::view_interface<T> > > = true;
#endif
...
__host__ __device__ constexpr cuda::std::ptrdiff_t
operator-(const ForwardIter& x, const ForwardIter& y) {
...
struct BoolConvertibleComparison
    : cuda::std::ranges::view_interface<BoolConvertibleComparison> {
  struct ResultType {
    bool value;
...
#if TEST_STD_VER > 2017
template <class T>
concept EmptyInvocable = requires(T const& obj) {
  obj.empty();
};

template <class T>
concept BoolOpInvocable = requires(T const& obj) {
  bool(obj);
};
#else
template <class T, class = void>
...
template <class T>
constexpr bool EmptyInvocable<
    T, cuda::std::void_t<decltype(cuda::std::declval<T const&>().empty())> > =
    true;

template <class T, class = void>
constexpr bool BoolOpInvocable = false;
template <class T>
constexpr bool BoolOpInvocable<
    T, cuda::std::void_t<decltype(bool(cuda::std::declval<T const&>()))> > =
    true;
#endif
...
__host__ __device__ constexpr bool testEmpty() {
  ...
  assert(!cuda::std::ranges::empty(
      static_cast<ForwardRange const&>(forwardRange)));
  ...
  assert(!emptyTrue
              .cuda::std::ranges::view_interface<EmptyIsTrue>::operator bool());
  ...
  // old GCC seems to fall over the noexcept clauses here
    ...
    (!defined(TEST_COMPILER_MSVC)) && (!defined(TEST_COMPILER_ICC))
  ...
  assert(!cuda::std::ranges::empty(
      static_cast<const BoolConvertibleComparison&>(boolConv)));
...
#if TESTS_STD_VER > 17
template <class T>
concept DataInvocable = requires(T const& obj) {
  obj.data();
};
#else
template <class T, class = void>
...
template <class T>
constexpr bool DataInvocable<
    T, cuda::std::void_t<decltype(cuda::std::declval<T const&>().data())> > =
    true;
#endif
...
__host__ __device__ constexpr bool testData() {
  ...
  assert(cuda::std::ranges::data(static_cast<ContRange const&>(contiguous)) ==
         contiguous.buff);

  DataIsNull dataNull{};
  assert(dataNull.data() == nullptr);
  assert(static_cast<DataIsNull const&>(dataNull).data() == nullptr);
  assert(dataNull.cuda::std::ranges::view_interface<DataIsNull>::data() ==
         dataNull.buff);

  assert(cuda::std::ranges::data(dataNull) == nullptr);
  assert(cuda::std::ranges::data(static_cast<DataIsNull const&>(dataNull)) ==
         nullptr);
...
#if TEST_STD_VER > 2017
template <class T>
concept SizeInvocable = requires(T const& obj) {
  obj.size();
};
#else
template <class T, class = void>
...
template <class T>
constexpr bool SizeInvocable<
    T, cuda::std::void_t<decltype(cuda::std::declval<T const&>().size())> > =
    true;
#endif
...
__host__ __device__ constexpr bool testSize() {
  ...
  // Test the test.
  static_assert(cuda::std::same_as<decltype(cuda::std::declval<ForwardIter>() -
                                            cuda::std::declval<ForwardIter>()),
                                   cuda::std::ptrdiff_t>);
  using UnsignedSize = cuda::std::make_unsigned_t<cuda::std::ptrdiff_t>;
  using SignedSize =
      cuda::std::common_type_t<cuda::std::ptrdiff_t,
                               cuda::std::make_signed_t<UnsignedSize> >;
  ...
  static_assert(cuda::std::same_as<decltype(cuda::std::ranges::size(
                                       cuda::std::declval<ForwardRange>())),
                                   UnsignedSize>);
  static_assert(cuda::std::same_as<decltype(cuda::std::ranges::ssize(
                                       cuda::std::declval<ForwardRange>())),
                                   SignedSize>);

  assert(cuda::std::ranges::size(
             static_cast<ForwardRange const&>(forwardRange)) == 8);
  static_assert(
      cuda::std::same_as<decltype(cuda::std::ranges::size(
                             cuda::std::declval<ForwardRange const>())),
                         UnsignedSize>);
  static_assert(
      cuda::std::same_as<decltype(cuda::std::ranges::ssize(
                             cuda::std::declval<ForwardRange const>())),
                         SignedSize>);
...
#if TEST_STD_VER > 2017
template <class T>
concept SubscriptInvocable = requires(T const& obj, size_t n) {
  obj[n];
};
#else
template <class T, class = void>
...
template <class T>
constexpr bool SubscriptInvocable<
    T, cuda::std::void_t<decltype(
           cuda::std::declval<T const&>()[cuda::std::declval<size_t>()])> > =
    true;
#endif
...
#if TEST_STD_VER > 2017
template <class T>
concept FrontInvocable = requires(T const& obj) {
  obj.front();
};

template <class T>
concept BackInvocable = requires(T const& obj) {
  obj.back();
};
#else
template <class T, class = void>
...
template <class T>
constexpr bool FrontInvocable<
    T, cuda::std::void_t<decltype(cuda::std::declval<T const&>().front())> > =
    true;
template <class T, class = void>

constexpr bool BackInvocable = false;
template <class T>
constexpr bool BackInvocable<
    T, cuda::std::void_t<decltype(cuda::std::declval<T const&>().back())> > =
    true;
#endif

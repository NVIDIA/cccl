...
struct TestLayoutCtors<cuda::std::tuple<
    Mapping, cuda::std::integer_sequence<size_t, DynamicSizes...> > > {
...
template <class Mapping, size_t... DynamicSizes, class Mapping2,
          size_t... DynamicSizes2>
struct TestLayoutCompatCtors<cuda::std::tuple<
    Mapping, cuda::std::integer_sequence<size_t, DynamicSizes...>, Mapping2,
    cuda::std::integer_sequence<size_t, DynamicSizes2...> > > {
  using mapping_type1 = Mapping;
  using mapping_type2 = Mapping2;
  using extents_type1 = cuda::std::remove_reference_t<decltype(
      cuda::std::declval<mapping_type1>().extents())>;
  using extents_type2 = cuda::std::remove_reference_t<decltype(
      cuda::std::declval<mapping_type2>().extents())>;
...
//TYPED_TEST(TestLayout{Left|Right}CompatCtors, compatible_assign_{1|2}) {
{
  ...
  // Assign
  {
    ...
    if constexpr (cuda::std::is_convertible<
...
using test_left_type_pair =
    cuda::std::tuple<typename cuda::std::layout_left::template mapping<E1>, S1,
                     typename cuda::std::layout_left::template mapping<E2>, S2>;

template <class E1, class S1, class E2, class S2>
using test_right_type_pair =
    cuda::std::tuple<typename cuda::std::layout_right::template mapping<E1>, S1,
...
struct is_cons_avail<T1, T2,
                     cuda::std::enable_if_t<cuda::std::is_same<
                         decltype(T1{cuda::std::declval<T2>()}), T1>::value> >
    : cuda::std::true_type {};
...
struct is_paren_op_avail<
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(cuda::std::declval<T>()(cuda::std::declval<Indicies>()...)),
        typename T::index_type>::value>,
    T, Indicies...> : cuda::std::true_type {};

template <class T, class... Indicies>
constexpr bool is_paren_op_avail_v =
    is_paren_op_avail<void, T, Indicies...>::value;
...
struct is_stride_avail<T, RankType,
                       cuda::std::enable_if_t<cuda::std::is_same<
                           decltype(cuda::std::declval<T>().stride(
                               cuda::std::declval<RankType>())),
                           typename T::index_type>::value> >
    : cuda::std::true_type {};

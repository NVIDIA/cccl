...
TEST_FUNC constexpr void test_layout()
{
  ...
  static_assert(check_operator_constraints(
    ...
    size_t(0)));
  static_assert(check_operator_constraints(
    cuda::std::mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D, D>(1, 1))), unsigned(0), IntType(0)));
  ...
  {
    static_assert(!check_operator_constraints(
      cuda::std::mdspan(data, construct_mapping(Layout(), cuda::std::extents<int, D>(1))),
...
TEST_FUNC constexpr void test_index_cast_happens()
{
  ...
  using mds_t     = cuda::std::mdspan<int, extents_t, layout_wrapping_integral<4>>;

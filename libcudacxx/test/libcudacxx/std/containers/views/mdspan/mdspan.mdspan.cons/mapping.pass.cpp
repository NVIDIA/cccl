...
struct is_mapping_cons_avail<
    T, DataHandleType, MappingType,
    cuda::std::enable_if_t<
        cuda::std::is_same<decltype(T{cuda::std::declval<DataHandleType>(),
                                      cuda::std::declval<MappingType>()}),
                           T>::value> > : cuda::std::true_type {};

template <class T, class DataHandleType, class MappingType>
constexpr bool is_mapping_cons_avail_v =
    is_mapping_cons_avail<T, DataHandleType, MappingType>::value;
...
int main(int, char**) {
  ...
  // mapping and accessor
  {
    ...
    cuda::std::mdspan<data_t, ext_t, cuda::std::layout_left> m{d.data(), map,
                                                               a};

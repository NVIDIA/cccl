...
struct is_extents_cons_avail<
    T, DataHandleType, ExtentsType,
    cuda::std::enable_if_t<
        cuda::std::is_same<decltype(T{cuda::std::declval<DataHandleType>(),
                                      cuda::std::declval<ExtentsType>()}),
                           T>::value> > : cuda::std::true_type {};

template <class T, class DataHandleType, class ExtentsType>
constexpr bool is_extents_cons_avail_v =
    is_extents_cons_avail<T, DataHandleType, ExtentsType>::value;
...
int main(int, char**) {
  ...
  {
    using ext_t = cuda::std::extents<int, 16, 16>;
    using mdspan_t = cuda::std::mdspan<int, ext_t, cuda::std::layout_right,
                                       Foo::my_accessor<int> >;

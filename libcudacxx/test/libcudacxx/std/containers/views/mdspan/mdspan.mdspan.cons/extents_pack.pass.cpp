...
struct is_param_pack_cons_avail<
    cuda::std::enable_if_t<
        cuda::std::is_same<decltype(T{cuda::std::declval<DataHandleT>(),
...
constexpr bool is_param_pack_cons_avail_v =
    is_param_pack_cons_avail<void, T, DataHandleT, SizeTypes...>::value;

int main(int, char**) {
  {
    using index_t = int;
    cuda::std::array<int, 1> d{42};
    cuda::std::mdspan<int, cuda::std::extents<index_t, dyn, dyn> > m{d.data(),
                                                                     64, 128};
...

  {
    using index_t = int;
    cuda::std::array<int, 1> d{42};
    cuda::std::mdspan<int, cuda::std::extents<index_t, dyn, dyn>,
...

  {
    ...
    static_assert(is_param_pack_cons_avail_v<mdspan_t, decltype(d.data()),
                                             other_index_t, other_index_t> ==
                      true,
                  "");
  }
  ...
  // Constraint: (is_convertible_v<OtherIndexTypes, index_type> && ...) is true
  {
    ...
    static_assert(is_param_pack_cons_avail_v<mdspan_t, int*, other_index_t,
                                             other_index_t> == false,
                  "");
  }
  ...
  {
    using mdspan_t = cuda::std::mdspan<int, cuda::std::extents<int, 16>,
  ...
  {
    using mdspan_t =
        cuda::std::mdspan<int, cuda::std::extents<int, 16>,
                          cuda::std::layout_right, Foo::my_accessor<int> >;

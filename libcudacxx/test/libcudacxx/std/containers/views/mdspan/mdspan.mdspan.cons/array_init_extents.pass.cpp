...
struct is_array_cons_avail<
    T, DataHandleT, SizeType, N,
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(T{cuda::std::declval<DataHandleT>(),
                   cuda::std::declval<cuda::std::array<SizeType, N> >()}),
        T>::value> > : cuda::std::true_type {};

template <class T, class DataHandleT, class SizeType, size_t N>
constexpr bool is_array_cons_avail_v =
    is_array_cons_avail<T, DataHandleT, SizeType, N>::value;

int main(int, char**) {
  // extents from cuda::std::array
  {
    cuda::std::array<int, 1> d{42};
    cuda::std::mdspan<int, cuda::std::extents<int, dyn, dyn> > m{
        d.data(), cuda::std::array<int, 2>{64, 128}};
  ...
  {
    using mdspan_t =
        cuda::std::mdspan<const int, cuda::std::extents<int, dyn, dyn> >;

    cuda::std::array<int, 1> d{42};
    const int* const ptr = d.data();

    static_assert(
        is_array_cons_avail_v<mdspan_t, decltype(ptr), int, 2> == true, "");
  ...
  // Constraint: (is_convertible_v<OtherIndexTypes, index_type> && ...) is true
  {
    ...
    static_assert(
        is_array_cons_avail_v<mdspan_t, int*, other_index_t, 2> == false, "");
  }

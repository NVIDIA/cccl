...
constexpr bool is_span_cons_avail_v =
    is_span_cons_avail<T, DataHandleT, SizeType, N>::value;
...
int main(int, char**) {
  // extents from cuda::std::span
  {
    ...
    static_assert(
        is_span_cons_avail_v<mdspan_t, int*, other_index_t, 2> == true, "");
  ...
  // Constraint: (is_convertible_v<OtherIndexTypes, index_type> && ...) is true
  {
    ...
    static_assert(
        is_span_cons_avail_v<mdspan_t, int*, other_index_t, 2> == false, "");
  }
  ...
  // Constraint: N == rank() || N == rank_dynamic() is true
  {
    ...
    static_assert(
        is_span_cons_avail_v<mdspan_t, int*, other_index_t, 1> == false, "");
  }

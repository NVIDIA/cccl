...
struct is_bracket_op_avail<cuda::std::enable_if_t<cuda::std::is_same<
                               decltype(cuda::std::declval<T>()(
                                   cuda::std::declval<OtherIndexTypes>()...)),
                               typename T::accessor_type::reference>::value>,
                           T, OtherIndexTypes...> : cuda::std::true_type {};

template <class T, class... OtherIndexTypes>
constexpr bool is_bracket_op_avail_v =
    is_bracket_op_avail<void, T, OtherIndexTypes...>::value;
...
struct is_bracket_op_array_avail<
    ...
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(cuda::std::declval<T>()(
            cuda::std::declval<cuda::std::array<OtherIndexType, N> >())),
        typename T::accessor_type::reference>::value> > : cuda::std::true_type {
};

template <class T, class OtherIndexType, size_t N>
constexpr bool is_bracket_op_array_avail_v =
    is_bracket_op_array_avail<T, OtherIndexType, N>::value;
...
struct is_bracket_op_span_avail<
    ...
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(cuda::std::declval<T>()(
            cuda::std::declval<cuda::std::span<OtherIndexType, N> >())),
        typename T::accessor_type::reference>::value> > : cuda::std::true_type {
};

template <class T, class OtherIndexType, size_t N>
constexpr bool is_bracket_op_span_avail_v =
    is_bracket_op_span_avail<T, OtherIndexType, N>::value;
...
int main(int, char**) {
  {
    ...
    // span of indices
    assert(m(cuda::std::span<const int, 2>{cuda::std::array<int, 2>{0, 0}}) ==
           42);
    assert(m(cuda::std::span<const int, 2>{cuda::std::array<int, 2>{0, 1}}) ==
           43);
    assert(m(cuda::std::span<const int, 2>{cuda::std::array<int, 2>{1, 0}}) ==
           44);
    assert(m(cuda::std::span<const int, 2>{cuda::std::array<int, 2>{1, 1}}) ==
           45);
  ...
  // Constraint: rank consistency
  {
    ...
    using index_t = int;
    using mdspan_t =
        cuda::std::mdspan<element_t, cuda::std::extents<index_t, dyn> >;
  ...
  // Constraint: convertibility
  {
    ...
    static_assert(
        is_bracket_op_avail_v<mdspan_t, my_int_non_convertible> == false, "");

    static_assert(
        is_bracket_op_array_avail_v<mdspan_t, my_int_non_convertible, 1> ==
            false,
        "");

    static_assert(
        is_bracket_op_span_avail_v<mdspan_t, my_int_non_convertible, 1> ==
            false,
        "");
  }
...
#ifndef TEST_COMPILER_BROKEN_SMF_NOEXCEPT
  {
    ...
    static_assert(
        is_bracket_op_avail_v<mdspan_t, my_int_non_nothrow_constructible> ==
            false,
        "");

    static_assert(
        is_bracket_op_array_avail_v<mdspan_t, my_int_non_nothrow_constructible,
                                    1> == false,
        "");

    static_assert(
        is_bracket_op_span_avail_v<mdspan_t, my_int_non_nothrow_constructible,
                                   1> == false,
        "");
  }

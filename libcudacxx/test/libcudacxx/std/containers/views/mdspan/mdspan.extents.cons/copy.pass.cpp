...
struct is_copy_cons_avail<
    ...
    cuda::std::enable_if_t<
        cuda::std::is_same<decltype(T1{cuda::std::declval<T2>()}), T1>::value> >
    : cuda::std::true_type {};
...
int main(int, char**) {
  ...
  static_assert(is_copy_cons_avail_v<cuda::std::extents<int, 2>,
                                     cuda::std::extents<int, 2> > == true,
                "");

  // Constraint: rank consistency
  static_assert(is_copy_cons_avail_v<cuda::std::extents<int, 2, 2>,
                                     cuda::std::extents<int, 2> > == false,
                "");

  // Constraint: extents consistency
  static_assert(is_copy_cons_avail_v<cuda::std::extents<int, 1>,
                                     cuda::std::extents<int, 2> > == false,
                "");

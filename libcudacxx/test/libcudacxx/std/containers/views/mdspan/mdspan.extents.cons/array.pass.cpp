...
struct is_array_cons_avail<
    ...
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(T{cuda::std::declval<cuda::std::array<IndexType, N> >()}),
        T>::value> > : cuda::std::true_type {};

template <class T, class IndexType, size_t N>
constexpr bool is_array_cons_avail_v =
    is_array_cons_avail<T, IndexType, N>::value;
...
int main(int, char**) {
  ...
  static_assert(
      is_array_cons_avail_v<cuda::std::dextents<int, 2>, int, 2> == true, "");

  static_assert(is_array_cons_avail_v<cuda::std::dextents<int, 2>, my_int, 2> ==
                    true,
                "");
  ...
  static_assert(
      is_array_cons_avail_v<cuda::std::dextents<int, 1>, int, 2> == false, "");

  // Constraint: convertibility
  static_assert(is_array_cons_avail_v<cuda::std::dextents<my_int, 1>,
                                      my_int_non_convertible, 1> == false,
                "");

  // Constraint: nonthrow-constructibility
#ifndef TEST_COMPILER_BROKEN_SMF_NOEXCEPT
  static_assert(is_array_cons_avail_v<cuda::std::dextents<int, 1>,
                                      my_int_non_nothrow_constructible, 1> ==
                    false,
                "");

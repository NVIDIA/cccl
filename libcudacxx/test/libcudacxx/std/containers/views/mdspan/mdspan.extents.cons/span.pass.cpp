...
struct is_span_cons_avail<
    ...
    cuda::std::enable_if_t<cuda::std::is_same<
        decltype(T{cuda::std::declval<cuda::std::span<IndexType, N> >()}),
        T>::value> > : cuda::std::true_type {};

template <class T, class IndexType, size_t N>
constexpr bool is_span_cons_avail_v =
    is_span_cons_avail<T, IndexType, N>::value;
...
int main(int, char**) {
  ...
  static_assert(
      is_span_cons_avail_v<cuda::std::dextents<int, 2>, int, 2> == true, "");

  static_assert(
      is_span_cons_avail_v<cuda::std::dextents<int, 2>, my_int, 2> == true, "");

#if !defined(TEST_COMPILER_CUDACC_BELOW_11_3)
  // Constraint: rank consistency
  static_assert(
      is_span_cons_avail_v<cuda::std::dextents<int, 1>, int, 2> == false, "");

  // Constraint: convertibility
  static_assert(is_span_cons_avail_v<cuda::std::dextents<int, 1>,

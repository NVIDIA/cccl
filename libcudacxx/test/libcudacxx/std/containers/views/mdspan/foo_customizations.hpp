...
struct foo_accessor {
  ...
  foo_accessor(cuda::std::default_accessor<OtherElementType>) noexcept {
    flag = nullptr;
  }

  template <class OtherElementType>
  __MDSPAN_INLINE_FUNCTION constexpr
  foo_accessor(foo_accessor<OtherElementType> other) noexcept {
    flag = other.flag;
  }

  _LIBCUDACXX_HOST_DEVICE constexpr reference access(data_handle_type p,
                                                     size_t i) const noexcept {
    return p.data[i];
  }

  _LIBCUDACXX_HOST_DEVICE constexpr data_handle_type
  offset(data_handle_type p, size_t i) const noexcept {
    return data_handle_type(p.data + i);
  }
  int* flag;

  _LIBCUDACXX_HOST_DEVICE friend constexpr void swap(foo_accessor& x,
                                                     foo_accessor& y) {
    x.flag[0] = 99;
    y.flag[0] = 77;
...
private:
  static_assert(cuda::std::__detail::__is_extents_v<extents_type>,
                "layout_foo::mapping must be instantiated with a "
                "specialization of cuda::std::extents.");
  static_assert(extents_type::rank() < 3,
                "layout_foo only supports 0D, 1D and 2D");
...
public:
  ...
  __MDSPAN_INLINE_FUNCTION_DEFAULTED constexpr
  ...
  __MDSPAN_TEMPLATE_REQUIRES(
      class OtherExtents,
      /* requires */ (_LIBCUDACXX_TRAIT(cuda::std::is_constructible,
                                        extents_type, OtherExtents)))
  __MDSPAN_CONDITIONAL_EXPLICIT(
      (!cuda::std::is_convertible<
          OtherExtents, extents_type>::value)) // needs two () due to comma
  __MDSPAN_INLINE_FUNCTION constexpr mapping(mapping<OtherExtents> const& other)
  ...
  __MDSPAN_CONDITIONAL_EXPLICIT(
      (!cuda::std::is_convertible<
          OtherExtents, extents_type>::value)) // needs two () due to comma
  __MDSPAN_INLINE_FUNCTION constexpr
  mapping(cuda::std::layout_right::mapping<OtherExtents> const& other)
      noexcept // NOLINT(google-explicit-constructor)
      : __extents(other.extents()) {}
  ...
  __MDSPAN_CONDITIONAL_EXPLICIT(
      (!cuda::std::is_convertible<
          OtherExtents, extents_type>::value)) // needs two () due to comma
  __MDSPAN_INLINE_FUNCTION constexpr
  mapping(cuda::std::layout_left::mapping<OtherExtents> const& other)
      noexcept // NOLINT(google-explicit-constructor)
      : __extents(other.extents()) {}
  ...
  __MDSPAN_INLINE_FUNCTION constexpr
  mapping(cuda::std::layout_stride::mapping<OtherExtents> const&
    ...
    NV_IF_TARGET(
        NV_IS_HOST,
        (size_t stride = 1; for (rank_type r = __extents.rank(); r > 0; r--) {
  ...
  __MDSPAN_INLINE_FUNCTION constexpr index_type
  operator()(Indx0 idx0) const noexcept {
    return static_cast<index_type>(idx0);
  }

  template <class Indx0, class Indx1>
  __MDSPAN_INLINE_FUNCTION constexpr index_type
  operator()(Indx0 idx0, Indx1 idx1) const noexcept {
  ...
  __get_stride(cuda::std::extents<SizeType, E...>,
               cuda::std::integer_sequence<size_t, Idx...>) const {
    return __MDSPAN_FOLD_TIMES_RIGHT(
        (Idx > N ? __extents.template __extent<Idx>() : 1), 1);
  }
  template <size_t N>
  _LIBCUDACXX_HOST_DEVICE constexpr index_type __stride() const noexcept {
    return __get_stride<N>(__extents,

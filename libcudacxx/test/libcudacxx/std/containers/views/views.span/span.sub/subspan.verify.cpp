...
TEST_FUNC void f()
{
  ...
  [[maybe_unused]] auto s1 = sp.subspan<5>(); // expected-error

  //  Count too large templatized
  [[maybe_unused]] auto s2 = sp.subspan<0, 5>(); // expected-error

  //  Offset + Count too large templatized
  [[maybe_unused]] auto s3 = sp.subspan<2, 3>(); // expected-error

  //  Offset + Count overflow templatized
  [[maybe_unused]] auto s4 = sp.subspan<3, cuda::std::size_t(-2)>(); // expected-error
}

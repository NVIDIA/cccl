...
__host__ __device__ void checkCV() {
  ...
  //  dynamic -> dynamic
  {
    cuda::std::span<const int> s1{
        sp}; // a cuda::std::span<const          int> pointing at int.
    cuda::std::span<volatile int> s2{
        sp}; // a cuda::std::span<      volatile int> pointing at int.
    cuda::std::span<const volatile int> s3{
        sp}; // a cuda::std::span<const volatile int> pointing at int.
    cuda::std::span<const volatile int> s4{
        vsp}; // a cuda::std::span<const volatile int> pointing at volatile int.
    assert(s1.size() + s2.size() + s3.size() + s4.size() == 0);
  }

  //  static -> static
  {
    cuda::std::span<const int, 0> s1{
        sp0}; // a cuda::std::span<const          int> pointing at int.
    cuda::std::span<volatile int, 0> s2{
        sp0}; // a cuda::std::span<      volatile int> pointing at int.
    cuda::std::span<const volatile int, 0> s3{
        sp0}; // a cuda::std::span<const volatile int> pointing at int.
    cuda::std::span<const volatile int, 0> s4{
        vsp0}; // a cuda::std::span<const volatile int> pointing at volatile int.
    assert(s1.size() + s2.size() + s3.size() + s4.size() == 0);
  }
...
__host__ __device__ constexpr bool testConstexprSpan() {
  ...
  return s0.data() == nullptr && s0.size() == 0 && s1.data() == nullptr &&
         s1.size() == 0 && s2.data() == nullptr && s2.size() == 0;
}

...
int main(int, char**)
{
  ...
  {
    cuda::std::span<int> s{1, 2, 3}; // expected-error {{no matching constructor for initialization of
                                     // 'cuda::std::span<int>'}}
  }

  // Non-const element type with dynamic extent (double): fails because is_const_v<double> is false
  {
    cuda::std::span<double> s{1.0, 2.0}; // expected-error {{no matching constructor for initialization of
                                         // 'cuda::std::span<double>'}}
  }

  // Non-const element type with static extent: fails because is_const_v<int> is false
  {
    cuda::std::span<int, 3> s{1, 2, 3}; // expected-error {{no matching constructor for initialization of
                                        // 'cuda::std::span<int, 3>'}}
  }

  // Narrowing: int to bool: fails because same_as<int, bool> is false
  {
    cuda::std::span<const bool> s{1, 0, 1}; // expected-error {{no matching constructor for initialization of
                                            // 'cuda::std::span<const bool>'}}
  }

  // Narrowing: int to short: fails because same_as<int, short> is false
  {
    cuda::std::span<const short> s{1, 2, 3}; // expected-error {{no matching constructor for initialization of
                                             // 'cuda::std::span<const short>'}}
  }

  // Narrowing: double to int: fails because same_as<double, int> is false
  {
    cuda::std::span<const int> s{1.0, 2.0}; // expected-error {{no matching constructor for initialization of
                                            // 'cuda::std::span<const int>'}}
  }

  // Narrowing: int to bool with static extent: fails because same_as<int, bool> is false
  {
    cuda::std::span<const bool, 3> s{1, 0, 1}; // expected-error {{no matching constructor for initialization of
                                               // 'cuda::std::span<const bool, 3>'}}
  }

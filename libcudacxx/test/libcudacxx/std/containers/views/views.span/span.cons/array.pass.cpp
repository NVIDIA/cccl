...
__host__ __device__ void checkCV() {
  //  Types the same (dynamic sized)
  {
    ...
    cuda::std::span<const int> s2{
        carr}; // a cuda::std::span<const          int> pointing at const int.
    cuda::std::span<volatile int> s3{
        varr}; // a cuda::std::span<      volatile int> pointing at volatile int.
  ...
  //  types different (dynamic sized)
  {
    ...
    cuda::std::span<const volatile int> s5{
        carr}; // a cuda::std::span<const volatile int> pointing at const int.
    cuda::std::span<const volatile int> s6{
        varr}; // a cuda::std::span<const volatile int> pointing at volatile int.
  ...
  //  types different (static sized)
  {
    ...
    cuda::std::span<const volatile int, 3> s4{
        arr}; // a cuda::std::span<const volatile int> pointing at int.
    cuda::std::span<const volatile int, 3> s5{
        carr}; // a cuda::std::span<const volatile int> pointing at const int.
    cuda::std::span<const volatile int, 3> s6{
        varr}; // a cuda::std::span<const volatile int> pointing at volatile int.
    assert(s1.size() + s2.size() + s3.size() + s4.size() + s5.size() +
               s6.size() ==
           18);

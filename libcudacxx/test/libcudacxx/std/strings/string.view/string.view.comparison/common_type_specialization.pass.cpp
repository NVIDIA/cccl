...
struct cuda::std::char_traits<char_wrapper>
{
  ...
  TEST_FUNC static bool eq(char_wrapper lhs, char_wrapper rhs)
  {
    return lhs.c == rhs.c;
  }

  TEST_FUNC static cuda::std::size_t length(const char_wrapper* a)
  {
    static_assert(sizeof(char_wrapper) == 1, "strlen requires this");
    return cuda::std::strlen(reinterpret_cast<const char*>(a));
  }

  TEST_FUNC static int compare(const char_wrapper* lhs, const char_wrapper* rhs, cuda::std::size_t count)
  {
...
{
  WrappedSV sv;
  TEST_FUNC convertible_to_string_view(const char_wrapper* a)
      : sv(a)
  {}
  TEST_FUNC operator WrappedSV() const
  {
    return sv;
  }
};

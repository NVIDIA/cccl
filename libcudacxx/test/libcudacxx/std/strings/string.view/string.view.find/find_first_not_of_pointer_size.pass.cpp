...
test_find_first_not_of(const SV& sv, const typename SV::value_type* str, typename SV::size_type x)
{
...
__host__ __device__ constexpr void test_find_first_not_of()
{
  ...
  static_assert(cuda::std::is_same_v<SizeT, decltype(SV{}.find_first_not_of(cuda::std::declval<const CharT*>()))>);
  static_assert(

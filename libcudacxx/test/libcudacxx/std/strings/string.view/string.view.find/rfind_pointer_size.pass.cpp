...
template <class SV>
__host__ __device__ constexpr void test_rfind(const SV& sv, const typename SV::value_type* str, typename SV::size_type x)
{
  assert(sv.rfind(str) == x);
  if (x != SV::npos)
  {
    typename SV::size_type pos = sv.size();
    typename SV::size_type n   = SV::traits_type::length(str);
...
test_rfind(const SV& sv, const typename SV::value_type* str, typename SV::size_type pos, typename SV::size_type x)
{
...
__host__ __device__ constexpr void test_rfind()
{
  ...
  static_assert(cuda::std::is_same_v<SizeT, decltype(SV{}.rfind(cuda::std::declval<const CharT*>()))>);
  static_assert(cuda::std::is_same_v<SizeT, decltype(SV{}.rfind(cuda::std::declval<const CharT*>(), SizeT{}))>);

  static_assert(noexcept(SV{}.rfind(cuda::std::declval<const CharT*>())));
  static_assert(noexcept(SV{}.rfind(cuda::std::declval<const CharT*>(), SizeT{})));

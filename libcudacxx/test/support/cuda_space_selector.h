...
struct malloc_memory_provider {
  ...
  static const constexpr cuda::std::size_t shared_offset =
      prefix_size + sizeof(T*);
...
public:
  ...
  __host__ __device__ ~malloc_memory_provider() {
    execute_on_main_thread([&] { free((void*)get_pointer()); });
  }
};
...
struct local_memory_provider {
  ...
  __host__ __device__ T* get() { return reinterpret_cast<T*>(&buffer); }
};
...
public:
  ...
  using offsetted =
      memory_selector<T, Provider, Initializer, SharedOffset + SharedOffset_>;

  static const constexpr cuda::std::size_t prefix_size =
      Provider<T, SharedOffset>::prefix_size;
  static const constexpr cuda::std::size_t shared_offset =
      Provider<T, SharedOffset>::shared_offset;
  ...
  __host__ __device__ ~memory_selector() {
    execute_on_main_thread([&] { ptr->~T(); });
  }
};

template <typename T, typename Initializer = constructor_initializer>
using local_memory_selector =
    memory_selector<T, local_memory_provider, Initializer>;

template <typename T, typename Initializer = constructor_initializer>
using shared_memory_selector =
    memory_selector<T, device_shared_memory_provider, Initializer>;

template <typename T, typename Initializer = constructor_initializer>
using global_memory_selector =
    memory_selector<T, malloc_memory_provider, Initializer>;

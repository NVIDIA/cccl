...
struct IsSmallObject
    : public std::integral_constant<
          ...
          sizeof(T) <= (sizeof(void*) * 3) &&
              std::alignment_of<void*>::value % std::alignment_of<T>::value ==
                  0 &&
...
// 'value'.
template <class Type>
_LIBCUDACXX_AVAILABILITY_THROW_BAD_ANY_CAST void modifyValue(std::any& a,
                                                             int value) {
  using namespace std;
...
struct small_type {
  ...
  explicit small_type(int val = 0) : value(val) { ++count; }
  explicit small_type(int, int val, int) : value(val) { ++count; }
  small_type(std::initializer_list<int> il) : value(*il.begin()) { ++count; }
...
struct large_type {
  ...
  large_type(std::initializer_list<int> il) : value(*il.begin()) { ++count; }
...
struct small_throws_on_copy {
  ...
  explicit small_throws_on_copy(int val = 0) : value(val) { ++count; }
  explicit small_throws_on_copy(int, int val, int) : value(val) { ++count; }
  small_throws_on_copy(small_throws_on_copy const&) { throwMyAnyExpression(); }
...
struct large_throws_on_copy {
  ...
  large_throws_on_copy(large_throws_on_copy const&) { throwMyAnyExpression(); }
...
struct throws_on_move {
  ...
  throws_on_move(throws_on_move&&) { throwMyAnyExpression(); }
...
struct small_tracked_t {
  small_tracked_t() : arg_types(&makeArgumentID<>()) {}
  ...
  template <class... Args>
  explicit small_tracked_t(Args&&...) : arg_types(&makeArgumentID<Args...>()) {}
  template <class... Args>
...
struct large_tracked_t {
  large_tracked_t() : arg_types(&makeArgumentID<>()) { dummy[0] = 42; }
  ...
  template <class... Args>
  explicit large_tracked_t(Args&&...) : arg_types(&makeArgumentID<Args...>()) {}
  template <class... Args>

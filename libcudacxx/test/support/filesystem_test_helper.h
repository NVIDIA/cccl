...
static const fs::path TestFileList[] = {
    makePath("empty_file"), makePath("non_empty_file"), makePath("dir1/file1"),
    makePath("dir1/file2")};
const std::size_t TestFileListSize = sizeof(TestFileList) / sizeof(fs::path);

static const fs::path TestDirList[] = {makePath("dir1"), makePath("dir1/dir2"),
...
const std::size_t DirIterationListSize =
    sizeof(DirIterationList) / sizeof(fs::path);
...
private:
  ...
  template <class T>
  static inline std::string fs_make_cmd(std::string const& cmd_name,
                                        T const& arg) {
  ...
  static bool checkDynamicTestRoot() {
    ...
    if (!fs_root) {
      std::printf(
          "ERROR: LIBCXX_FILESYSTEM_DYNAMIC_TEST_ROOT must be a defined "
          "environment variable when running the test.\n");
      std::abort();
    }
    if (std::string(fs_root) != LIBCXX_FILESYSTEM_DYNAMIC_TEST_ROOT) {
      std::printf(
          "ERROR: LIBCXX_FILESYSTEM_DYNAMIC_TEST_ROOT environment variable"

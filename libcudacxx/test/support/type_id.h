...
struct TypeID {
  TEST_HOST_DEVICE friend bool operator==(TypeID const& LHS, TypeID const& RHS)
...
private:
  TEST_HOST_DEVICE explicit constexpr TypeID(const char* xid) : m_id(xid) {}

  TypeID(const TypeID&) = delete;
  TypeID& operator=(TypeID const&) = delete;

  const char* const m_id;
  template <class T> TEST_HOST_DEVICE friend TypeID const& makeTypeIDImp();
};

// makeTypeID - Return the TypeID for the specified type 'T'.
template <class T>
TEST_HOST_DEVICE inline TypeID const& makeTypeIDImp() {
  #ifdef __CUDA_ARCH__
...
template <class T>
TEST_HOST_DEVICE inline  TypeID const& makeTypeID() {
...
// of arguments.
template <class ...Args>
TEST_HOST_DEVICE inline  TypeID const& makeArgumentID() {

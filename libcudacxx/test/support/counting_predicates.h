...
public:
  ...
  __host__ __device__ TEST_CONSTEXPR_CXX14 void reset() { count_ = 0; }
...
public:
  ...
  binary_counting_predicate(Predicate p)
      : p_(p), count_(0) {}

  __host__ __device__ TEST_CONSTEXPR_CXX14 bool operator()(const Arg1& a1,
                                                           const Arg2& a2) {
    ++count_;
    return p_(a1, a2);
  }
...
public:
  ...
  __host__ __device__ constexpr counting_predicate(Predicate pred, int& count)
      : pred_(cuda::std::move(pred)), count_(&count) {}

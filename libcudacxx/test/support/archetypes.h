...
struct TestBase {
  ...
  template <bool Dummy = true,
            typename cuda::std::enable_if<Dummy && Explicit, bool>::type = true>
  __host__ __device__ explicit TestBase(int x) noexcept : value(x) {
    ++alive();
    ++constructed();
    ++value_constructed();
  }
  template <bool Dummy = true, typename cuda::std::enable_if<Dummy && !Explicit,
                                                             bool>::type = true>
  ...
  template <bool Dummy = true,
            typename cuda::std::enable_if<Dummy && Explicit, bool>::type = true>
  __host__ __device__ explicit TestBase(std::initializer_list<int>& il,
                                        int = 0) noexcept
...
struct ValueBase {
  ...
  template <bool Dummy = true,
            typename cuda::std::enable_if<Dummy && Explicit, bool>::type = true>
  __host__ __device__ explicit constexpr
  ValueBase(std::initializer_list<int>& il, int = 0)
      : value(static_cast<int>(il.size())) {}
  template <bool Dummy = true, typename cuda::std::enable_if<Dummy && !Explicit,
                                                             bool>::type = true>
  __host__ __device__ constexpr ValueBase(std::initializer_list<int>& il,
                                          int = 0)
      : value(static_cast<int>(il.size())) {}
  __host__ __device__ TEST_CONSTEXPR_CXX14 ValueBase&
  operator=(int xvalue) noexcept {
    value = xvalue;
    return *this;
  }
...
protected:
  ...
  __host__ __device__ constexpr static int check_value(int& val,
                                                       int val_cp = 0) {
#if TEST_STD_VER < 2014
    return val_cp = val, val = -1,
           (val_cp == -1 || val_cp == 999 ? (TEST_THROW(42), 0) : val_cp);
#else
    assert(val != -1);
    assert(val != 999);
    val_cp = val;
  ...
  __host__ __device__ constexpr ValueBase(ValueBase&& o) noexcept
      : value(check_value(o.value)) {}
  __host__ __device__ TEST_CONSTEXPR_CXX14 ValueBase&
  operator=(ValueBase const& o) noexcept {
    assert(o.value != -1);
    assert(o.value != -999);
    value = o.value;
    return *this;
...
struct TrivialValueBase {
  ...
  template <bool Dummy = true,
            typename cuda::std::enable_if<Dummy && Explicit, bool>::type = true>
  __host__ __device__ explicit constexpr
  TrivialValueBase(std::initializer_list<int>& il, int = 0)
      : value(static_cast<int>(il.size())) {}
  template <bool Dummy = true, typename cuda::std::enable_if<Dummy && !Explicit,
                                                             bool>::type = true>
  __host__ __device__ constexpr TrivialValueBase(std::initializer_list<int>& il,
                                                 int = 0)
      : value(static_cast<int>(il.size())) {}
  int value;
#ifndef TEST_WORKAROUND_C1XX_BROKEN_ZA_CTOR_CHECK
...
template <class Tp>
__host__ __device__ constexpr bool operator==(Tp const& L,
                                              Tp const& R) noexcept {

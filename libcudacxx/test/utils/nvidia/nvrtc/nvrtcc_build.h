...
GpuProg nvrtc_build_prog(const std::string& testCu, const ArchConfig& config,
  ...
  std::for_each(argList.begin(), argList.end(),
                [&](const auto& it) { optList.emplace_back(it.c_str()); });
  ...
  nvrtcProgram prog;
  NVRTC_SAFE_CALL(
      nvrtcCreateProgram(&prog, testCu.c_str(), "test.cu", 0, NULL, NULL));

  nvrtcResult compile_result =
      nvrtcCompileProgram(prog, optList.size(), optList.data());

...
constexpr auto make_greedy_handler = [](char const* match) {
  return ArgPair{std::regex(match), [](const std::smatch&) { return GREEDY; }};
};
...
ArgPair argHandlers[] = {
    ...
    {// If an input is a .exe file, search for other builds and run those
     ...
     [](const std::smatch& match) {
       execute = true;
       executionConfig =
           load_execution_config_from_file(match[1].str() + ".build.yml");
...
int main(int argc, char** argv) {
  ...
  if (!skipOutput) {
    ...
    for (const auto& build : buildList) {
      ...
      std::string gpuCodeFile =
          outputTemplate + "." + archString(build) + ".gpu";

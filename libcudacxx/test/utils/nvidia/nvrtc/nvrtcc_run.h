...
load_execution_config_from_file(const std::string& file) {
  std::vector<std::string> builds;
...
static void load_and_run_gpu_code(const std::string inputFile,
                                  const RunConfig& rc) {
  std::ifstream istr(inputFile, std::ios::binary);
  assert(!!istr);

  std::vector<char> code(std::istreambuf_iterator<char>{istr},
  ...
  CUDA_SAFE_CALL(cuLaunchKernel(kernel, 1, 1, 1, rc.threadCount, 1, 1,
                                rc.shmemSize, NULL, NULL, 0));

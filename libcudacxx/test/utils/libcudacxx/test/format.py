...
class LibcxxTestFormat(object):
    ...
    def _execute(self, test, lit_config):
        ...
        if is_verify_test and not test_cxx.isVerifySupported():
            return (
                lit.Test.UNSUPPORTED,
                ".verify.cpp tests are unsupported with this compiler",
            )
    ...
    def _evaluate_verify_test(self, test, test_cxx, parsers):
        ...
        if test_cxx.type == "clang":
            test_cxx.verify_flags += ["-fsyntax-only", "-ferror-limit=0"]

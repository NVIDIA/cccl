...
extern "C" __global__ void test_less_f16_4(const __half* lhs, const __half* rhs, bool* out)
{
  ...
  const auto result = lhs_vec < rhs_vec;
...
; SMXX-LABEL: {{[[:space:]]*}}Function : test_less_f16_4
; SM80: {{.*HSETP2.*}}
; SM80: {{.*HSETP2.*}}
; SM90: {{.*HSETP2.*}}
; SM90: {{.*HSETP2.*}}

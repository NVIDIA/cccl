...
static void basic(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  const auto count = cuda::std::count(cuda::execution::__cub_par_unseq, in.begin(), in.end(), 42);
  ...
  state.exec(nvbench::exec_tag::gpu | nvbench::exec_tag::no_batch | nvbench::exec_tag::sync,
             [&](nvbench::launch& launch) {
               cuda::std::remove_copy(
                 policy.with_stream(launch.get_stream().get_stream()), in.begin(), in.end(), out.begin(), 42);
             });
}

...
static void range_iter(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  state.exec(nvbench::exec_tag::gpu | nvbench::exec_tag::no_batch | nvbench::exec_tag::sync,
             [&](nvbench::launch& launch) {
               (void) cuda::std::equal(policy, dinput.begin(), dinput.end(), cuda::constant_iterator<T>{0});
             });
}
...
static void range_range(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  state.exec(
    nvbench::exec_tag::gpu | nvbench::exec_tag::no_batch | nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
      (void) cuda::std::equal(
        policy, dinput.begin(), dinput.end(), cuda::constant_iterator<T>{0}, cuda::constant_iterator<T>{0, elements});
    });
}

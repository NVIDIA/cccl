...
static inline _CCCL_DEVICE void __cuda_atomic_exchange(
  _Type* __ptr, _Type& __dst, _Type __op, __atomic_operand_tag<__atomic_operand_type::_{0},{1}>, _Scope, _Memorder)
{{
  asm volatile("atom.exch.%3%4.{0}{1} %0,[%1],%2;"
                : "={2}"(__dst)
                : "l"(__ptr), "{2}"(__op), "C"(_Scope::value), "C"(_Memorder::value)
                : "memory");
}}
)XXX";

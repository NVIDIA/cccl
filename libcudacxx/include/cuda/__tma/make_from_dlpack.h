...
// 5 dimensions max -> 4 strides, the first stride is implicit stride = 1
using __tma_strides_array_t = ::cuda::std::array<::cuda::std::uint64_t, 4>;
  ...
  TmaInterleaveLayout __interleave_layout) noexcept
{
  ...
  const auto __data_type_size = ::cuda::__to_cutensor_map(__data_type);
  ...
  for (int i = 1; i < __rank; ++i)
  {
    ...
    const auto __tensor_size                                   = __tensor_sizes[(__rank - 1) - (i - 1)];
    _CCCL_ASSERT(__strides[i] >= __tensor_size && __strides[i] <= __max_allowed_stride_bytes / __data_type_size,
...
_CCCL_HOST_API inline void __check_elem_strides(::cuda::std::span<int, _ElemStrideSize> __elem_strides,
                                                const __tma_sizes_array_t& __tensor_sizes) noexcept
{
  ...
  TmaOOBfill __oobfill                    = TmaOOBfill::none) noexcept
{
  ...
  ::cuda::__check_elem_strides(__elem_strides, __tensor_sizes);
  ::cuda::__check_box_sizes(__box_sizes, __tensor_sizes, __interleave_layout, __swizzle, __data_type);
  ::cuTensorMapEncodeTiled(
    ...
    __raw_swizzle,
    __raw_l2_fetch_size,
    __raw_oobfill); // replace with ::cuda::__driver::__tensorMapEncodeTiled
  return __tensor_map;
}

  ...
  switch (__data_type)
  {
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT16:
      _CCCL_THROW_INVALID_ARG_IF(
        __num_items > __max_size_t / 2, "Number of items must be less than or equal to 2^64 / 2");
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT32:
      _CCCL_THROW_INVALID_ARG_IF(
        __num_items > __max_size_t / 4, "Number of items must be less than or equal to 2^64 / 4");
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT64:
      _CCCL_THROW_INVALID_ARG_IF(
        __num_items > __max_size_t / 8, "Number of items must be less than or equal to 2^64 / 8");
      return __num_items * 8;
#  if _CCCL_CTK_AT_LEAST(12, 8)
    case ::CU_TENSOR_MAP_DATA_TYPE_16U4_ALIGN8B:
      _CCCL_THROW_INVALID_ARG_IF(__num_items % 2 != 0, "Number of items must be a multiple of 2 for U4x16");
      return __num_items / 2;
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
    default:
      _CCCL_UNREACHABLE();
...
#  if _CCCL_CTK_AT_LEAST(12, 8)
    ...
    {
      _CCCL_THROW_INVALID_ARG_IF(true, "tma_swizzle::bytes128_atom* are not supported with compute capability 9");
    }
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
    if (__tensor.dtype.code == ::kDLUInt && __tensor.dtype.bits == 4 && __tensor.dtype.lanes == 16)
    {
      _CCCL_THROW_INVALID_ARG_IF(true, "U4x16 is not supported with compute capability 9");
    }
  }
}
...
__get_tensor_map_data_type(const ::DLTensor& __tensor, tma_oob_fill __oobfill)
{
  ...
  {
    case ::kDLInt:
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "Int data type must be 1 lane");
      _CCCL_THROW_INVALID_ARG_IF(
        __tensor.dtype.bits != 32 && __tensor.dtype.bits != 64, "Int data type must be 32 or 64 bits");
      _CCCL_THROW_INVALID_ARG_IF(
        __oobfill != tma_oob_fill::none, "tma_oob_fill::nan is only supported for floating-point data types");
    ...
    case ::kDLUInt: {
      ...
      {
        case 4: {
          _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 16, "uint4 data type must be 16 lanes");
#  if _CCCL_CTK_AT_LEAST(12, 8)
          return ::CU_TENSOR_MAP_DATA_TYPE_16U4_ALIGN8B;
#  else
          _CCCL_THROW_INVALID_ARG_IF(true, "U4x16 is not supported for compute capability 9");
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
        }
        case 8:
          _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "uint8 data type must be 1 lane");
          return ::CU_TENSOR_MAP_DATA_TYPE_UINT8;
        case 16:
          _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "uint16 data type must be 1 lane");
          return ::CU_TENSOR_MAP_DATA_TYPE_UINT16;
        case 32:
          _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "uint32 data type must be 1 lane");
          return ::CU_TENSOR_MAP_DATA_TYPE_UINT32;
        case 64:
          _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "uint64 data type must be 1 lane");
          return ::CU_TENSOR_MAP_DATA_TYPE_UINT64;
        default:
          _CCCL_THROW_INVALID_ARG_IF(true, "UInt data type must be 4, 8, 16, 32, or 64 bits");
      }
    }
    case ::kDLFloat:
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "Float data type must be 1 lane");
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.bits != 16 && __tensor.dtype.bits != 32 && __tensor.dtype.bits != 64,
    ...
    case ::kDLBfloat:
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 1, "Bfloat16 data type must be 1 lane");
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.bits != 16, "Bfloat16 data type must be 16 bits");
    ...
    case ::kDLFloat4_e2m1fn:
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.lanes != 16, "Float4_e2m1fn data type must be 16 lanes");
      _CCCL_THROW_INVALID_ARG_IF(__tensor.dtype.bits != 4, "Float4_e2m1fn data type must be 4 bits");
#  if _CCCL_CTK_AT_LEAST(12, 8)
      return ::CU_TENSOR_MAP_DATA_TYPE_16U4_ALIGN8B;
#  else
      _CCCL_THROW_INVALID_ARG_IF(true, "U4x16 (Float4_e2m1fn) is not supported for compute capability 9");
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
    default:
      _CCCL_THROW_INVALID_ARG_IF(true, "Unsupported data type");
  }
  _CCCL_UNREACHABLE();
}
...
__get_tensor_map_rank(const ::DLTensor& __tensor, tma_interleave_layout __interleave_layout)
{
  ...
  _CCCL_THROW_INVALID_ARG_IF(__rank <= 0 || __rank > __max_rank, "tensor.ndim (rank) must be between 1 and 5");
  if (__interleave_layout != tma_interleave_layout::none)
  {
    _CCCL_THROW_INVALID_ARG_IF(
      __rank <= 3, "tensor.ndim (rank) must be greater than or equal to 3 for interleaved layout");
  }
  return __rank;
}
...
__get_tensor_address(const ::DLTensor& __tensor, tma_interleave_layout __interleave_layout)
{
  ...
  _CCCL_THROW_INVALID_ARG_IF(
    !::cuda::is_aligned(__address, __alignment), "tensor.data (address) is not sufficiently aligned");
  _CCCL_THROW_INVALID_ARG_IF(
    !::cuda::is_device_accessible(__address, __tensor.device.device_id), "Address is not a valid GPU global address");
...
__get_tensor_sizes(const ::DLTensor& __tensor, int __rank, ::CUtensorMapDataType __data_type)
{
  ...
  _CCCL_THROW_INVALID_ARG_IF(__tensor.shape == nullptr, "__tensor.shape is null");
#  if _CCCL_CTK_AT_LEAST(12, 8)
  if (__data_type == ::CU_TENSOR_MAP_DATA_TYPE_16U4_ALIGN8B)
  {
    _CCCL_THROW_INVALID_ARG_IF(
      __tensor_sizes[__rank - 1] % 2 != 0,
...
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
  for (int __i = 0; __i < __rank; ++__i)
  {
    ...
    _CCCL_THROW_INVALID_ARG_IF(__tensor_sizes[__i] <= 0 || __tensor_sizes[__i] > __max_allowed_size,
  ...
  tma_interleave_layout __interleave_layout)
{
  ...
  if (__input_strides == nullptr)
  {
    for (int __i = 0; __i < __rank - 1; ++__i)
    {
      ...
      _CCCL_THROW_INVALID_ARG_IF(
        __stride_bytes % __alignment != 0, "Stride in bytes is not a multiple of the alignment (32 or 16)");
      _CCCL_THROW_INVALID_ARG_IF(
        __stride_bytes >= __max_allowed_stride_bytes, "Stride in bytes is greater than or equal to 2^40");
  ...
  for (int __i = __rank - 2; __i >= 0; --__i)
  {
    ...
    // TODO(fbusato): check mul overflow
    _CCCL_THROW_INVALID_ARG_IF(
      __input_strides[__i] != 0 && (__input_strides[__i] < __input_sizes[__i + 1] * __next_stride),
    ...
    _CCCL_THROW_INVALID_ARG_IF(
      __input_stride_bytes % __alignment != 0, "Stride in bytes is not a multiple of the alignment (32 or 16)");
    _CCCL_THROW_INVALID_ARG_IF(
      __input_stride_bytes >= __max_allowed_stride_bytes, "Stride in bytes is greater than or equal to 2^40");
  ...
  int __device_id)
{
  ...
  _CCCL_THROW_INVALID_ARG_IF(__box_sizes.size() != static_cast<size_t>(__rank), "Box sizes size mismatch");
  ...
  for (int __i = 0; __i < __rank; ++__i)
  {
    ...
    _CCCL_THROW_INVALID_ARG_IF(__box_size <= 0 || __box_size > __max_box_size,
...
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
  ...
  _CCCL_THROW_INVALID_ARG_IF(
    ::cuda::__to_cutensor_map_size(__total_size, __data_type) > static_cast<size_t>(__max_shmem),
  ...
  tma_interleave_layout __interleave_layout)
{
  using ::cuda::std::size_t;
  using ::cuda::std::uint64_t;
  _CCCL_THROW_INVALID_ARG_IF(__elem_strides.size() != static_cast<size_t>(__rank), "Elem strides size mismatch");
  ...
  for (int __i = __init_index; __i < __rank; ++__i)
  {
    ...
    _CCCL_THROW_INVALID_ARG_IF(
      __elem_stride <= 0 || __elem_stride > __max_elem_stride,
...
_CCCL_HOST_API inline void __check_swizzle(tma_interleave_layout __interleave_layout, tma_swizzle __swizzle)
{
  ...
  {
    _CCCL_THROW_INVALID_ARG_IF(
      __swizzle != tma_swizzle::bytes32, "tma_interleave_layout::bytes32 requires tma_swizzle::bytes32");
  }
}

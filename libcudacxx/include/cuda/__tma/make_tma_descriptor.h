  ...
  switch (__data_type)
  {
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT16:
      _CCCL_THROW_IF(__num_items > __max_size_t / 2,
                     ...
                     "Number of items must be less than or equal to 2^64 / 2");
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT32:
      _CCCL_THROW_IF(__num_items > __max_size_t / 4,
                     ::std::invalid_argument,
                     "Number of items must be less than or equal to 2^64 / 4");
    ...
    case ::CU_TENSOR_MAP_DATA_TYPE_FLOAT64:
      _CCCL_THROW_IF(__num_items > __max_size_t / 8,
                     ::std::invalid_argument,
                     "Number of items must be less than or equal to 2^64 / 8");
      return __num_items * 8;
#  if _CCCL_CTK_AT_LEAST(12, 8)
    case ::CU_TENSOR_MAP_DATA_TYPE_16U4_ALIGN8B:
      _CCCL_THROW_IF(__num_items % 2 != 0, ::std::invalid_argument, "Number of items must be a multiple of 2 for U4x16");
      return __num_items / 2;
#  endif // _CCCL_CTK_AT_LEAST(12, 8)
    default:
      _CCCL_UNREACHABLE();
  ...
  switch (__type)
  {
    case ::kDLInt:
      ...
      _CCCL_THROW_IF(__tensor.dtype.bits != 32 && __tensor.dtype.bits != 64,
                     ::std::invalid_argument,
                     "Int data type must be 32 or 64 bits");
...
__get_tensor_map_rank(const ::DLTensor& __tensor, tma_interleave_layout __interleave_layout)
{
  ...
  _CCCL_THROW_IF(
    __rank <= 0 || __rank > __max_rank, ::std::invalid_argument, "tensor.ndim (rank) must be between 1 and 5");
...
__get_tensor_address(const ::DLTensor& __tensor, tma_interleave_layout __interleave_layout)
{
  ...
  _CCCL_THROW_IF(!::cuda::is_aligned(__address, __alignment),
                 ::std::invalid_argument,
                 "tensor.data (address) is not sufficiently aligned");
  ...
  if (__input_strides == nullptr)
  {
    for (int __i = 0; __i < __rank - 1; ++__i)
    {
      ...
      _CCCL_THROW_IF(__stride_bytes >= __max_allowed_stride_bytes,
                     ::std::invalid_argument,
                     "Stride in bytes is greater than or equal to 2^40");
  ...
  tma_interleave_layout __interleave_layout)
{
  ...
  using ::cuda::std::uint64_t;
  _CCCL_THROW_IF(
    __elem_strides.size() != static_cast<size_t>(__rank), ::std::invalid_argument, "Elem strides size mismatch");
...
_CCCL_HOST_API inline void __check_swizzle(tma_interleave_layout __interleave_layout, tma_swizzle __swizzle)
{
  if (__interleave_layout == tma_interleave_layout::bytes32)
  {
    _CCCL_THROW_IF(__swizzle != tma_swizzle::bytes32,
                   ::std::invalid_argument,
                   "tma_interleave_layout::bytes32 requires tma_swizzle::bytes32");
  }
}

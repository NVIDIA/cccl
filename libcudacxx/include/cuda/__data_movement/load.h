...
#  define _CCCL_LOAD_ADD_PREFETCH(LOAD_BEHAVIOR, EVICT_POLICY, PREFETCH)          \
    if constexpr ((PREFETCH) == no_prefetch)                                      \
    {                                                                             \
...
_CCCL_NODISCARD _CCCL_DEVICE _CCCL_FORCEINLINE _Tp __load_sm70(
  const _Tp* __ptr,
...
_LIBCUDACXX_HIDE_FROM_ABI void __eviction_policy_and_prefetch_require_SM_70_or_higher();
...
load(const _Tp* __ptr,
     ...
     __eviction_policy_t<_Ep> __eviction_policy = eviction_normal,
     __prefetch_spatial_t<_Pp> __prefetch       = prefetch_spatial_none) noexcept
{
  ...
  else
  {
    ...
    NV_IF_ELSE_TARGET(NV_PROVIDES_SM_70,
                      (return __load_sm70(__ptr, __load_behavior, __eviction_policy, __prefetch);),

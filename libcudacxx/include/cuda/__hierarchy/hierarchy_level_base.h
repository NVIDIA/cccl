...
__hierarchy_extents_mul(const ::cuda::std::extents<_LhsIndex, _LhsExts...>& __lhs,
                        const ::cuda::std::extents<_RhsIndex, _RhsExts...>& __rhs) noexcept
{
  using _Lhs         = ::cuda::std::extents<_LhsIndex, _LhsExts...>;
  using _Rhs         = ::cuda::std::extents<_RhsIndex, _RhsExts...>;
  using _ResultIndex = ::cuda::std::common_type_t<_LhsIndex, _RhsIndex>;

  constexpr auto __result_rank = ::cuda::std::max(_Lhs::rank(), _Rhs::rank());
  using _Result                = decltype(::cuda::__hierarchy_static_extents_mul<_ResultIndex, _Lhs, _Rhs>(
  ...
  for (::cuda::std::size_t __i = 0; __i < __result_rank; ++__i)
  {
    ...
    {
      __ret[__i] = static_cast<_ResultIndex>((__i < _Lhs::rank()) ? __lhs.extent(__i) : 1)
                 * static_cast<_ResultIndex>((__i < _Rhs::rank()) ? __rhs.extent(__i) : 1);
    }
    else
    {
...
struct hierarchy_level_base
{
  ...
  template <class _InLevel>
  using __default_1d_query_type = unsigned;
  ...
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto dims(const _InLevel& __level, const _Hierarchy& __hier) noexcept
  {
  ...
  static_dims(const _InLevel& __level, const _Hierarchy& __hier) noexcept
  {
  ...
  _CCCL_REQUIRES(__is_hierarchy_level_v<_InLevel> _CCCL_AND __is_hierarchy_v<_Hierarchy>)
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto
  extents(const _InLevel& __level, const _Hierarchy& __hier) noexcept
  {
  ...
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto
  dims_as(const _InLevel& __level, const _Hierarchy& __hier) noexcept
  {
  ...
  _CCCL_REQUIRES(::cuda::std::__cccl_is_integer_v<_Tp> _CCCL_AND __is_hierarchy_level_v<_InLevel> _CCCL_AND
                   __is_hierarchy_v<_Hierarchy>)
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto
  extents_as(const _InLevel& __in_level, const _Hierarchy& __hier) noexcept
  {
    ...
    typename decltype(__hier.extents(_Level{}, _NextLevel{}))::_Dims __curr_exts =
      __hier.extents(_Level{}, _NextLevel{});
    if constexpr (!::cuda::std::is_same_v<_NextLevel, _InLevel>)
    {
      auto __next_exts = _NextLevel::template extents_as<_Tp>(__in_level, __hier);
  ...
  _CCCL_REQUIRES(::cuda::std::__cccl_is_integer_v<_Tp> _CCCL_AND __is_hierarchy_level_v<_InLevel> _CCCL_AND
                   __is_hierarchy_v<_Hierarchy>)
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto
  count_as(const _InLevel& __level, const _Hierarchy& __hier) noexcept
  {
  ...
  {
    return __rank_as_impl<_Tp>(__level, __hier);
  }
...
private:
  ...
  [[nodiscard]] _CCCL_DEVICE_API static constexpr auto __dims_as_impl(const _Args&... __args) noexcept
  {
  ...
  template <class _Tp, class... _Args>
  [[nodiscard]] _CCCL_DEVICE_API static constexpr _Tp __count_as_impl(const _Args&... __args) noexcept
  {
  ...
  template <class _Tp, class... _Args>
  [[nodiscard]] _CCCL_DEVICE_API static constexpr ::cuda::std::size_t __rank_as_impl(const _Args&... __args) noexcept
  {
    const auto __exts = _Level::template extents_as<_Tp>(__args...);
    const auto __idcs = _Level::template index_as<_Tp>(__args...);

    _Tp __ret = 0;
    for (::cuda::std::size_t __i = __exts.rank(); __i > 0; --__i)
    {
      _Tp __inc = __idcs[__i - 1];
      for (::cuda::std::size_t __j = __i - 1; __j > 0; --__j)
      {
        __inc *= __exts.extent(__j - 1);
      }
      __ret += __inc;
    }
    return __ret;

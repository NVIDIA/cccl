...
template <class _IndexType = _CUDA_VSTD::uint64_t, class _Bijection = __feistel_bijection>
class random_bijection
{
private:
  static_assert(_CUDA_VSTD::is_integral_v<_IndexType>, "_IndexType must be an integral type");
  static_assert(_CUDA_VSTD::is_integral_v<typename _Bijection::index_type>,
                "_Bijection::index_type must be an integral type");
  static_assert(_CUDA_VSTD::is_convertible_v<_IndexType, typename _Bijection::index_type>,
...
public:
  ...
  _CCCL_API constexpr random_bijection(_IndexType __num_elements, _RNG&& __gen) noexcept
      : __bijection(__num_elements, _CUDA_VSTD::forward<_RNG>(__gen))

...
_CCCL_NODISCARD _CCCL_HIDE_FROM_ABI _CCCL_DEVICE ShuffleResult<_Tp> warp_shuffle(
  _Tp __data, int __src_lane, uint32_t __lane_mask = 0xFFFFFFFF, _CUDA_VSTD::integral_constant<int, _Width> = {})
{
  ...
  else
  {
    ...
    auto __clamp_segmask   = ::cuda::ptx::get_sreg_laneid() | ((__warp_size - _Width) << 8);
    bool __pred;
...
#pragma unroll
    ...
    _Tp __result;
    ::memcpy(&__result, __array, sizeof(_Tp));
    return ShuffleResult{__result, __pred};
  }
}

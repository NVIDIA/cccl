...
[[nodiscard]] _CCCL_API constexpr conditional_t<is_integral_v<_Tp>, double, _Tp> fmax(_Tp __x, _Tp __y) noexcept
{
  ...
  if constexpr (is_same_v<_Tp, __half>)
  {
    NV_IF_ELSE_TARGET(NV_IS_DEVICE,
                      ...
                      (return ::__float2half(_CUDA_VSTD::fmax(::__half2float(__x), ::__half2float(__y)));))
  }
  else
    ...
    if constexpr (is_same_v<_Tp, __nv_bfloat16>)
  {
    NV_IF_ELSE_TARGET(NV_IS_DEVICE,
                      (return ::__hmax(__x, __y);),
                      (return ::__float2bfloat16(_CUDA_VSTD::fmax(::__bfloat162float(__x), ::__bfloat162float(__y)));))
  }
  else
...
[[nodiscard]] _CCCL_API constexpr conditional_t<is_integral_v<_Tp>, double, _Tp> fmin(_Tp __x, _Tp __y) noexcept
{
  ...
  if constexpr (is_same_v<_Tp, __half>)
  {
    NV_IF_ELSE_TARGET(NV_IS_DEVICE,
                      ...
                      (return ::__float2half(_CUDA_VSTD::fmin(::__half2float(__x), ::__half2float(__y)));))
  }
  else
#endif // _CCCL_HAS_NVFP16()
#if _CCCL_HAS_NVBF16()
    if constexpr (is_same_v<_Tp, __nv_bfloat16>)
  {
    NV_IF_ELSE_TARGET(NV_IS_DEVICE,
                      (return ::__hmin(__x, __y);),
                      (return ::__float2bfloat16(_CUDA_VSTD::fmin(::__bfloat162float(__x), ::__bfloat162float(__y)));))
  }
  else
#endif // _CCCL_HAS_NVBF16()
    if constexpr (!is_integral_v<_Tp>)

...
public:
#  if _CCCL_STD_VER >= 2020
  _LIBCUDACXX_HIDE_FROM_ABI single_view()
    requires default_initializable<_Tp>
  = default;
#  else // ^^^ C++20 ^^^ / vvv C++17 vvv
  _LIBCUDACXX_TEMPLATE(class _Tp2 = _Tp)
  _LIBCUDACXX_REQUIRES(default_initializable<_Tp2>)
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr single_view() noexcept(
    is_nothrow_default_constructible_v<_Tp>)
      : view_interface<single_view<_Tp>>()
      , __value_(){};
#  endif // _CCCL_STD_VER <= 2017

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit single_view(const _Tp& __t) noexcept(
    is_nothrow_copy_constructible_v<_Tp>)
      : view_interface<single_view<_Tp>>()
      , __value_(in_place, __t)
  {}

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit single_view(_Tp&& __t) noexcept(
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit single_view(
    in_place_t, _Args&&... __args) noexcept(is_nothrow_constructible_v<_Tp, _Args...>)
      : view_interface<single_view<_Tp>>()
      , __value_{in_place, _CUDA_VSTD::forward<_Args>(__args)...}
  {}

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp* begin() noexcept
  {
    return data();
  }

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp* begin() const noexcept
  {
    return data();
  }

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp* end() noexcept
  {
    return data() + 1;
  }

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp* end() const noexcept
  {
    return data() + 1;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp* data() noexcept
  {
    return __value_.operator->();
  }

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp* data() const noexcept
  {
    return __value_.operator->();
  }
};
...
struct __fn : __range_adaptor_closure<__fn>
{
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Range&& __range) const

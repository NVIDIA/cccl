...
public:
#  if _CCCL_STD_VER >= 2020
  _LIBCUDACXX_HIDE_FROM_ABI common_view()
    requires default_initializable<_View>
  = default;
#  else // ^^^ C++20 ^^^ / vvv C++17 vvv
  _LIBCUDACXX_TEMPLATE(class _View2 = _View)
  _LIBCUDACXX_REQUIRES(default_initializable<_View2>)
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr common_view() noexcept(
    is_nothrow_default_constructible_v<_View2>)
      : view_interface<common_view<_View>>()
  {}
#  endif // _CCCL_STD_VER <= 2017

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit common_view(_View __v) noexcept(
...
template <class _View>
_LIBCUDACXX_INLINE_VAR constexpr bool enable_borrowed_range<common_view<_View>> = enable_borrowed_range<_View>;
...
struct __fn : __range_adaptor_closure<__fn>
{
  ...
  operator()(_Range&& __range) const noexcept(noexcept(_CUDA_VIEWS::all(_CUDA_VSTD::forward<_Range>(__range))))
    -> all_t<_Range>
  {
  ...
  _LIBCUDACXX_REQUIRES((!common_range<_Range>) )
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr auto
  operator()(_Range&& __range) const noexcept(noexcept(common_view{_CUDA_VSTD::forward<_Range>(__range)}))
    -> common_view<all_t<_Range>>
  {

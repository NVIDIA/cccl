...
public:
  class __iterator
  {
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit __iterator(const _Tp* __value, _IndexT __bound_sentinel = _IndexT())
        : __value_(__value)
  ...
  public:
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp& operator*() const noexcept
    {
      return *__value_;
    }

    _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator& operator++()
    {
      ++__current_;
      return *this;
    }

    _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator operator++(int)
    {
      auto __tmp = *this;
      ++*this;
      return __tmp;
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator& operator--()
    {
      ...
      {
        _LIBCUDACXX_ASSERT(__current_ > 0, "The value of bound must be greater than or equal to 0");
      }
      --__current_;
      return *this;
    }
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator& operator+=(difference_type __n)
    {
      if constexpr (!same_as<_Bound, unreachable_sentinel_t>)
      {
        _LIBCUDACXX_ASSERT(__current_ + __n >= 0, "The value of bound must be greater than or equal to 0");
      }
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp& operator[](difference_type __n) const noexcept
    {
      return *(*this + __n);
    }

    _LIBCUDACXX_INLINE_VISIBILITY friend constexpr bool operator==(const __iterator& __x, const __iterator& __y)
    {
...
#  ifndef _LIBCUDACXX_HAS_NO_SPACESHIP_OPERATOR
    _LIBCUDACXX_INLINE_VISIBILITY friend constexpr auto operator<=>(const __iterator& __x, const __iterator& __y)
    {
      return __x.__current_ <=> __y.__current_;
    }
#  else
...
#  endif // !_LIBCUDACXX_HAS_NO_SPACESHIP_OPERATOR
    ...
    _LIBCUDACXX_INLINE_VISIBILITY friend constexpr __iterator operator+(__iterator __i, difference_type __n)
    {
      __i += __n;
      return __i;
    }

    _LIBCUDACXX_INLINE_VISIBILITY friend constexpr __iterator operator+(difference_type __n, __iterator __i)
    {
      __i += __n;
      return __i;
    }
    ...
    _LIBCUDACXX_INLINE_VISIBILITY friend constexpr difference_type
...
#  else // ^^^ C++20 ^^^ / vvv C++20 vvv
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr repeat_view() noexcept(is_nothrow_default_constructible_v<_Tp2>) {}
#  endif // _CCCL_STD_VER <= 2017

  _LIBCUDACXX_TEMPLATE(class _Tp2 = _Tp)
  _LIBCUDACXX_REQUIRES((!same_as<_Tp2, repeat_view>) _LIBCUDACXX_AND copy_constructible<_Tp2>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit repeat_view(const _Tp2& __value, _Bound __bound_sentinel = _Bound())
      : __value_(in_place, __value)
      , __bound_(__bound_sentinel)
  {
    if constexpr (!same_as<_Bound, unreachable_sentinel_t>)
    {
      _LIBCUDACXX_ASSERT(__bound_ >= 0, "The value of bound must be greater than or equal to 0");
    }
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit repeat_view(_Tp&& __value, _Bound __bound_sentinel = _Bound())
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit repeat_view(
      ...
      , __bound_(_CUDA_VSTD::make_from_tuple<_Bound>(_CUDA_VSTD::move(__bound_args)))
  {
    if constexpr (!same_as<_Bound, unreachable_sentinel_t>)
    {
      _LIBCUDACXX_ASSERT(__bound_ >= 0,
                         "The behavior is undefined if Bound is not unreachable_sentinel_t and bound is negative");
    }
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator begin() const
  {
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __iterator end() const
  {
  ...
  _LIBCUDACXX_TEMPLATE(class _Bound2 = _Bound)
  _LIBCUDACXX_REQUIRES(same_as<_Bound2, unreachable_sentinel_t>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr unreachable_sentinel_t end() const noexcept
  {
  ...
  _LIBCUDACXX_TEMPLATE(class _Bound2 = _Bound)
  _LIBCUDACXX_REQUIRES((!same_as<_Bound2, unreachable_sentinel_t>) )
  _LIBCUDACXX_INLINE_VISIBILITY constexpr auto size() const
  {
...
struct __fn {
  template <class _Tp>
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Tp&& __value) const
  ...
  template <class _Tp, class _Bound>
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Tp&& __value, _Bound&& __bound_sentinel) const
...
{
_LIBCUDACXX_CPO_ACCESSIBILITY auto repeat = __repeat::__fn{};
} // namespace __cpo
_LIBCUDACXX_END_NAMESPACE_VIEWS
...
template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_repeat_specialization = false;

template <class _Tp, class _Bound>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_repeat_specialization<repeat_view<_Tp, _Bound>> = true;

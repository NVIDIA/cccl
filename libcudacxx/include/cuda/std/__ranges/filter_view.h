...
// It seems nvcc has a bug where the noexcept specification is incorrectly set
    ...
    _CCCL_API constexpr __iterator(filter_view& __parent, iterator_t<_View> __current)
        : __current_(::cuda::std::move(__current))
        , __parent_(::cuda::std::addressof(__parent))
    {}

    _CCCL_API constexpr iterator_t<_View> const& base() const& noexcept
    {
      return __current_;
    }
    _CCCL_API constexpr iterator_t<_View> base() &&
    {
      return ::cuda::std::move(__current_);
    }

    _CCCL_API constexpr range_reference_t<_View> operator*() const
    {
      return *__current_;
    }

    _CCCL_TEMPLATE(class _View2 = _View)
    _CCCL_REQUIRES(__has_arrow<iterator_t<_View2>>&& copyable<iterator_t<_View2>>)
    ...
    _CCCL_API friend constexpr auto operator==(__iterator const& __x, __iterator const& __y)
      _CCCL_TRAILING_REQUIRES(bool)(equality_comparable<iterator_t<_View2>>)
    {
    ...
    template <class _View2 = _View>
    _CCCL_API friend constexpr range_rvalue_reference_t<_View2>
    iter_move(__iterator const& __it) noexcept(noexcept(::cuda::std::ranges::iter_move(__it.__current_)))
    {
    ...
    iter_swap(__iterator const& __x, __iterator const& __y) noexcept(__noexcept_swappable<iterator_t<_View2>>)
      _CCCL_TRAILING_REQUIRES(void)(indirectly_swappable<iterator_t<_View2>>)
    {
  ...
  public:
    ...
    _CCCL_API constexpr sentinel_t<_View> base() const
    {
      return __end_;
    }

    _CCCL_API friend constexpr bool operator==(__iterator const& __x, __sentinel const& __y)
    {
      return __x.__current_ == __y.__end_;
    }
#if _CCCL_STD_VER <= 2017
    _CCCL_API friend constexpr bool operator==(__sentinel const& __x, __iterator const& __y)
    {
...
#else // ^^^ _CCCL_HAS_CONCEPTS() ^^^ / vvv !_CCCL_HAS_CONCEPTS() vvv
  _CCCL_TEMPLATE(class _View2 = _View)
  _CCCL_REQUIRES(default_initializable<_View2>&& default_initializable<_Pred>)
  ...
  _CCCL_API constexpr filter_view(_View __base, _Pred __pred)
      : view_interface<filter_view<_View, _Pred>>()
  ...
  _CCCL_API constexpr _Pred const& pred() const
  {
    return *__pred_;
  }

  _CCCL_API constexpr __iterator begin()
  {
...
_CCCL_HOST_DEVICE filter_view(_Range&&, _Pred) -> filter_view<::cuda::std::ranges::views::all_t<_Range>, _Pred>;
...
struct __fn
{
  ...
  [[nodiscard]] _CCCL_API constexpr auto operator()(_Range&& __range, _Pred&& __pred) const
    noexcept(noexcept(/**/ filter_view(::cuda::std::forward<_Range>(__range), ::cuda::std::forward<_Pred>(__pred))))
      -> filter_view<all_t<_Range>, remove_cvref_t<_Pred>>
  {
    return /*-----------*/ filter_view(::cuda::std::forward<_Range>(__range), ::cuda::std::forward<_Pred>(__pred));
  }

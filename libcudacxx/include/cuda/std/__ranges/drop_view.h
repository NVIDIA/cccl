...
#  else // ^^^ C++20 ^^^ / vvv C++17 vvv
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr drop_view() noexcept(
    is_nothrow_default_constructible_v<_View2>)
      : view_interface<drop_view<_View>>()
  {}
#  endif // _CCCL_STD_VER <= 2017

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr drop_view(
    _View __base, range_difference_t<_View> __count) noexcept(is_nothrow_move_constructible_v<_View>)
      : view_interface<drop_view<_View>>()
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr _View
  base() const& noexcept(is_nothrow_copy_constructible_v<_View2>)
  {
    return __base_;
  }

  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr _View
  base() && noexcept(is_nothrow_move_constructible_v<_View>)
  {
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY static constexpr auto __size(_Self& __self)
  {
...
template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool enable_borrowed_range<drop_view<_Tp>> = enable_borrowed_range<_Tp>;
...
template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_empty_view = false;

template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_empty_view<empty_view<_Tp>> = true;

template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_passthrough_specialization = false;

template <class _Tp, size_t _Extent>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_passthrough_specialization<span<_Tp, _Extent>> = true;

#  if defined(_LIBCUDACXX_HAS_STRING_VIEW)
template <class _CharT, class _Traits>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_passthrough_specialization<basic_string_view<_CharT, _Traits>> = true;
#  endif

template <class _Np, class _Bound>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_passthrough_specialization<iota_view<_Np, _Bound>> = true;

template <class _Iter, class _Sent, _CUDA_VRANGES::subrange_kind _Kind>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_passthrough_specialization<_CUDA_VRANGES::subrange<_Iter, _Sent, _Kind>> =
  !_CUDA_VRANGES::subrange<_Iter, _Sent, _Kind>::_StoreSize;

template <class _Tp>
_LIBCUDACXX_INLINE_VAR constexpr bool __is_subrange_specialization_with_store_size = false;
...
struct __fn
{
  ...
  operator()(_Range&& __range, _Np&&) const
  ...
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Range&& __range, _Np&& __n) const
  ...
  _LIBCUDACXX_REQUIRES(convertible_to<_Np, range_difference_t<_Range>> _LIBCUDACXX_AND
                         __is_repeat_specialization<_RawRange> _LIBCUDACXX_AND (!sized_range<_RawRange>))
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Range&& __range, _Np&&) const
  ...
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr auto
  operator()(_Range&& __range, _Np&& __n) const
  ...
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Np&& __n) const

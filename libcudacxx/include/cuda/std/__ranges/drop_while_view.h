...
public:
#  if _CCCL_STD_VER >= 2020
  _LIBCUDACXX_HIDE_FROM_ABI drop_while_view()
...
#  else
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr drop_while_view() noexcept(
...
#  endif // _CCCL_STD_VER <= 2017
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Pred& pred() const
  {
    return *__pred_;
  }
...
_LIBCUDACXX_INLINE_VAR constexpr bool enable_borrowed_range<drop_while_view<_View, _Pred>> =
...
{
  template <class _Range, class _Pred>
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Range&& __range, _Pred&& __pred) const
  ...
  _CCCL_NODISCARD _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Pred&& __pred) const

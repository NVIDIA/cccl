...
public:
  ...
  private:
    ...
    using _OuterType                    = _If<_OuterPresent, _Outer, ::cuda::std::ranges::__empty_cache>;
    _CCCL_NO_UNIQUE_ADDRESS _OuterType __outer_{};
    ...
    _CCCL_API constexpr void __satisfy()
    {
      for (; __get_outer() != ::cuda::std::ranges::end(__parent_->__base_); ++__get_outer())
      {
        ...
        __inner_       = ::cuda::std::ranges::begin(__inner);
        if (*__inner_ != ::cuda::std::ranges::end(__inner))
        {
          return;
        }
      }
  ...
  public:
    _CCCL_API constexpr _Outer& __get_outer() noexcept
    {
    ...
    _CCCL_API constexpr const _Outer& __get_outer() const noexcept
    {
      if constexpr (forward_range<_Base>)
      {
        return __outer_;
    ...
    _CCCL_API constexpr explicit __iterator(_Parent& __parent)
    ...
    _CCCL_TEMPLATE(class _Base2 = _Base)
    _CCCL_REQUIRES(forward_range<_Base2>)
    _CCCL_API constexpr __iterator(_Parent& __parent, _Outer __outer)
  ...
  public:
    ...
    _CCCL_TEMPLATE(class _Inner2 = _Inner)
    ...
    _CCCL_API constexpr __iterator& operator--()
    {
      if (__outer_ == ::cuda::std::ranges::end(__parent_->__base_))
      {
        ...
        __inner_     = ::cuda::std::ranges::end(*__outer);
    ...
    _CCCL_API friend constexpr auto operator==(const __iterator& __x, const __iterator& __y)
      _CCCL_TRAILING_REQUIRES(bool)(__ref_is_glvalue2&& equality_comparable<iterator_t<_Base>>&&
  ...
  public:
    ...
    _CCCL_API constexpr explicit __sentinel(_Parent& __parent)
  ...
  _CCCL_API constexpr auto begin()
  {
    if constexpr (forward_range<_View>)
    {
      ...
      return __iterator<__use_const>{*this, ::cuda::std::ranges::begin(__base_)};
    }
    else
    {
  ...
  _CCCL_API constexpr auto end()
  {
    ...
    {
      return __iterator<__simple_view<_View>>{*this, ::cuda::std::ranges::end(__base_)};
    }
    else
    {

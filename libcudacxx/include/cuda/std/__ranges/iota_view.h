...
template <class _Iter>
_CCCL_CONCEPT_FRAGMENT(__decrementable_,
                       requires(_Iter __i)(requires(incrementable<_Iter>),
                                           requires(same_as<decltype(--__i), _Iter&>),
                                           requires(same_as<decltype(__i--), _Iter>)));

template <class _Iter>
_CCCL_CONCEPT __decrementable = _CCCL_FRAGMENT(__decrementable_, _Iter);
...
template <class _Iter>
_CCCL_CONCEPT __advanceable = _CCCL_FRAGMENT(__advanceable_, _Iter);
...
public:
  struct __iterator : public __iota_iterator_category<_Start>
  {
    ...
    _CCCL_TEMPLATE(class _Start2 = _Start)
    _CCCL_REQUIRES((!incrementable<_Start2>) )
...
public:
  ...
  _CCCL_TEMPLATE(class _BoundSentinel2 = _BoundSentinel)
  _CCCL_REQUIRES((!same_as<_Start, _BoundSentinel2> && !same_as<_Start, unreachable_sentinel_t>) )
...
{
  template <class _Start>
  _LIBCUDACXX_HIDE_FROM_ABI constexpr auto operator()(_Start&& __start) const

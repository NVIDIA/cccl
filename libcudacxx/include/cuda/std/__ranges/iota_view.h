...
public:
  struct __iterator : public __iota_iterator_category<_Start>
  {
    ...
    _LIBCUDACXX_HIDE_FROM_ABI constexpr explicit __iterator(_Start __value) noexcept(
    ...
    _CCCL_TEMPLATE(class _Start2 = _Start)
    _CCCL_REQUIRES(__advanceable<_Start2>)
#if !_CCCL_COMPILER(CLANG) || _CCCL_CUDA_COMPILER(NVCC, >=, 12, 8) // clang fails to apply _LIBCUDACXX_HIDE_FROM_ABI
    [[nodiscard]]

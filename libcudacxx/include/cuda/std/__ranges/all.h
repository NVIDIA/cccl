...
template <class _Tp>
_CCCL_CONCEPT_FRAGMENT(__can_ref_view_, requires(_Tp&& __t)((_CUDA_VRANGES::ref_view{_CUDA_VSTD::forward<_Tp>(__t)})));

template <class _Tp>
_CCCL_CONCEPT __can_ref_view = _CCCL_FRAGMENT(__can_ref_view_, _Tp);

template <class _Tp>
_CCCL_CONCEPT_FRAGMENT(__to_ref_view_,
                       requires()(requires(!_CUDA_VRANGES::view<decay_t<_Tp>>), requires(__can_ref_view<_Tp>)));

template <class _Tp>
_CCCL_CONCEPT __to_ref_view = _CCCL_FRAGMENT(__to_ref_view_, _Tp);

template <class _Tp>
_CCCL_CONCEPT_FRAGMENT(__to_owning_view_,
                       requires(_Tp&& __t)(requires(!_CUDA_VRANGES::view<decay_t<_Tp>>),
                                           requires(!__can_ref_view<_Tp>),
                                           (_CUDA_VRANGES::owning_view{_CUDA_VSTD::forward<_Tp>(__t)})));

template <class _Tp>
_CCCL_CONCEPT __to_owning_view = _CCCL_FRAGMENT(__to_owning_view_, _Tp);

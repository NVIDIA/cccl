...
public:
  ...
  public:
    ...
    operator==(const _Iter<_Const>& __lhs, const __sentinel& __rhs)
    {
      return __lhs.count() == 0 || __lhs.base() == __rhs.__end_;
    }
#if _CCCL_STD_VER <= 2017
    [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool
    operator==(const __sentinel& __lhs, const _Iter<_Const>& __rhs)
    {
...
{
  using type = _CCCL_NODEBUG_ALIAS _CUDA_VRANGES::subrange<_Iter>;
};
...
struct __fn
{
  ...
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI constexpr auto operator()(_Range&& __rng, _Np&& __n) const

...
struct __fn
{
  ...
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY static constexpr auto __go(
    _It __it,
    iter_difference_t<_It> __count) noexcept(noexcept(span(_CUDA_VSTD::to_address(__it), static_cast<size_t>(__count))))
  ...
  _LIBCUDACXX_REQUIRES((!contiguous_iterator<_It>) _LIBCUDACXX_AND(!random_access_iterator<_It>))
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY static constexpr auto
  __go(_It __it, iter_difference_t<_It> __count) noexcept(
  ...
  operator()(_It&& __it, _Diff&& __count) const

...
template <class _View, class _Fn>
_CCCL_CONCEPT __transform_view_constraints = _CCCL_FRAGMENT(__transform_view_constraints_, _View, _Fn);
...
public:
  ...
  _CCCL_TEMPLATE(class _View2 = _View)
  _CCCL_REQUIRES((range<const _View2> && !common_range<const _View2>)
                   _CCCL_AND __regular_invocable_with_range_ref<const _Fn&, const _View2>)
  _LIBCUDACXX_HIDE_FROM_ABI constexpr __sentinel<true> end() const
  {
    return __sentinel<true>(_CUDA_VRANGES::end(__base_));
  }
  _CCCL_TEMPLATE(class _View2 = _View)
  _CCCL_REQUIRES(common_range<const _View2> _CCCL_AND __regular_invocable_with_range_ref<const _Fn&, const _View2>)

...
struct __wrapper
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit __wrapper(__forward_tag, _Args&&... __args)
      : __t_(_CUDA_VSTD::forward<_Args>(__args)...)
  {}

  template <class _Fn>
  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit __wrapper(__from_tag, const _Fn& __fun)
      : __t_(__fun())
  {}
...
struct __npc_destruct_base
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __npc_destruct_base() noexcept {}

  template <class... _Args>
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __npc_destruct_base(__forward_tag, _Args&&... __args) noexcept(
    is_nothrow_constructible_v<_Tp, _Args...>)
      : __val_(__forward_tag{}, _CUDA_VSTD::forward<_Args>(__args)...)
      , __engaged_(true)
  {}

  _LIBCUDACXX_INLINE_VISIBILITY _CCCL_CONSTEXPR_CXX20 ~__npc_destruct_base() noexcept
  {
    if (__engaged_)
    {
      __val_.~_Tp();
  ...
  _LIBCUDACXX_INLINE_VISIBILITY _CCCL_CONSTEXPR_CXX20 void __reset() noexcept
  {
    if (__engaged_)
    {
      __val_.~_Tp();
...
struct __npc_destruct_base<_Tp, enable_if_t<is_trivially_destructible_v<_Tp>>>
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr void __reset() noexcept
  {
    if (__engaged_)
    {
      __engaged_ = false;
...
public:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __non_propagating_cache(const __non_propagating_cache&) noexcept
      : __base()
  {}

  _LIBCUDACXX_INLINE_VISIBILITY constexpr __non_propagating_cache(__non_propagating_cache&& __other) noexcept
      : __base()
  {
    __other.__reset();
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr __non_propagating_cache&
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __non_propagating_cache& operator=(__non_propagating_cache&& __other) noexcept
  {
    this->__reset();
    __other.__reset();
    return *this;
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp& operator*() noexcept
  {
    return this->__val_.__t_;
  }
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp const& operator*() const noexcept
  {
    return this->__val_.__t_;
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr bool __has_value() const noexcept
  {

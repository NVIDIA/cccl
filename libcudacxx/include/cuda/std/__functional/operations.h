...
struct _CCCL_TYPE_VISIBILITY_DEFAULT negate : __unary_function<_Tp, _Tp>
{
  using __result_type = _Tp; // used by valarray
  _CCCL_EXEC_CHECK_DISABLE
  constexpr _CCCL_API _Tp operator()(const _Tp& __x) const
  {
    return -__x;
  }
};
...
struct _CCCL_TYPE_VISIBILITY_DEFAULT negate<void>
{
  _CCCL_EXEC_CHECK_DISABLE
  template <class _Tp>
  constexpr _CCCL_API auto operator()(_Tp&& __x) const noexcept(noexcept(-_CUDA_VSTD::forward<_Tp>(__x)))
...
struct _CCCL_TYPE_VISIBILITY_DEFAULT bit_or : __binary_function<_Tp, _Tp, _Tp>
{
  using __result_type = _Tp; // used by valarray
  _CCCL_EXEC_CHECK_DISABLE
  constexpr _CCCL_API _Tp operator()(const _Tp& __x, const _Tp& __y) const
  {
    return __x | __y;
  }
};

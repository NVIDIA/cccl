...
private:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY
  static constexpr auto __magnitude_geq(_Iter_difference __a, _Iter_difference __b) noexcept {
    return __a == 0 ? __b == 0 :
                      __a > 0  ? __a >= __b :
                                 __a <= __b;
  };
...
public:
  ...
  _LIBCUDACXX_TEMPLATE(class _Ip, class _Sp)
    (requires input_or_output_iterator<_Ip> _LIBCUDACXX_AND
              sentinel_for<_Sp, _Ip>)
  _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY
  constexpr void operator()(_Ip& __i, _Sp __bound_sentinel) const {
  ...
  constexpr iter_difference_t<_Ip> operator()(_Ip& __i, iter_difference_t<_Ip> __n, _Sp __bound_sentinel) const {

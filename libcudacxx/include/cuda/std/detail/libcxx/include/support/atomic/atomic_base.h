...
_LIBCUDACXX_HIDE_FROM_ABI constexpr int __cxx_atomic_order_to_int(memory_order __order) {
...
_LIBCUDACXX_HIDE_FROM_ABI constexpr int __cxx_atomic_failure_order_to_int(memory_order __order) {
  // Avoid switch statement to make this a constexpr.
  return __order == memory_order_relaxed ? __ATOMIC_RELAXED:
         (__order == memory_order_acquire ? __ATOMIC_ACQUIRE:

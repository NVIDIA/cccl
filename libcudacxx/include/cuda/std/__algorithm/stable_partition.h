...
_LIBCUDACXX_HIDE_FROM_ABI _ForwardIterator __stable_partition_impl(
  ...
  forward_iterator_tag __fit)
{
  ...
  _ForwardIterator __first_false =
    _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(__first, __m, __pred, __len2, __p, __fit);
  // TTTFFFFF??????????
  // f  ff   m         l
  ...
  // f  ff   m  m1     l
  __second_false =
    _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(__m1, __last, __pred, __len_half, __p, __fit);
__second_half_done:
  // TTTFFFFFTTTTTFFFFF
  // f  ff   m    sf   l
  return _CUDA_VSTD::__rotate<_AlgPolicy>(__first_false, __m, __second_false).first;
  // TTTTTTTTFFFFFFFFFF
  //         |
}
...
__stable_partition_impl(_ForwardIterator __first, _ForwardIterator __last, _Predicate __pred, forward_iterator_tag)
{
  ...
  {
    // TODO: Remove the use of _CUDA_VSTD::get_temporary_buffer
    __p = _CUDA_VSTD::get_temporary_buffer<value_type>(__len);
    __h.reset(__p.first);
  }
  return _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(
    _CUDA_VSTD::move(__first), _CUDA_VSTD::move(__last), __pred, __len, __p, forward_iterator_tag());
}

template <class _AlgPolicy, class _Predicate, class _BidirectionalIterator, class _Distance, class _Pair>
_LIBCUDACXX_HIDE_FROM_ABI _BidirectionalIterator __stable_partition_impl(
  ...
  bidirectional_iterator_tag __bit)
{
  ...
  // f   m1  m        l
  __first_false =
    _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(__first, __m1, __pred, __len_half, __p, __bit);
__first_half_done:
  // TTTFFFFF?????????T
  // f  ff   m        l
  // recurse on [__m, __last], except increase __m until *(__m) is false, *__last know to be true
  ...
  // f  ff   m  m1    l
  __second_false =
    _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(__m1, __last, __pred, __len_half, __p, __bit);
__second_half_done:
  // TTTFFFFFTTTTTFFFFF
  // f  ff   m    sf  l
  return _CUDA_VSTD::__rotate<_AlgPolicy>(__first_false, __m, __second_false).first;
  ...
  _BidirectionalIterator __first, _BidirectionalIterator __last, _Predicate __pred, bidirectional_iterator_tag)
{
  ...
  return _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, _Predicate&>(
    _CUDA_VSTD::move(__first), _CUDA_VSTD::move(__last), __pred, __len, __p, bidirectional_iterator_tag());
}

template <class _AlgPolicy, class _Predicate, class _ForwardIterator, class _IterCategory>
_LIBCUDACXX_HIDE_FROM_ABI _ForwardIterator __stable_partition(
  _ForwardIterator __first, _ForwardIterator __last, _Predicate&& __pred, _IterCategory __iter_category)
{
  return _CUDA_VSTD::__stable_partition_impl<_AlgPolicy, remove_cvref_t<_Predicate>&>(
...
stable_partition(_ForwardIterator __first, _ForwardIterator __last, _Predicate __pred)
{
  ...
  return _CUDA_VSTD::__stable_partition<_ClassicAlgPolicy, _Predicate&>(

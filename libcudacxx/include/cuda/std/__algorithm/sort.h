...
_LIBCUDACXX_HIDE_FROM_ABI _Number __log2i(_Number __n)
{
  ...
  {
    return sizeof(uint32_t) * CHAR_BIT - 1 - _CUDA_VSTD::__cccl_clz(static_cast<uint32_t>(__n));
  }
  if (sizeof(__n) <= sizeof(uint64_t))
  {
    return sizeof(uint64_t) * CHAR_BIT - 1 - _CUDA_VSTD::__cccl_clz(static_cast<uint64_t>(__n));
  }

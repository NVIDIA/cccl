...
public:
  ...
  template <class _T1>
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI bool operator()(const _T1& __x)
  {
    return !__p_(__x);
  }

  template <class _T1, class _T2>
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI bool operator()(const _T1& __x, const _T2& __y)
  {
    return __p_(__y, __x);
  }
};
...
_LIBCUDACXX_HIDE_FROM_ABI void __half_inplace_merge(
  ...
  _Compare&& __comp)
{
  for (; __first1 != __last1; ++__result)
  {
    if (__first2 == __last2)
    {
      _CUDA_VSTD::__move<_AlgPolicy>(__first1, __last1, __result);
      return;
    }
...
_LIBCUDACXX_HIDE_FROM_ABI void __buffered_inplace_merge(
  ...
  typename iterator_traits<_BidirectionalIterator>::value_type* __buff)
{
  ...
  if (__len1 <= __len2)
  {
    ...
    _CUDA_VSTD::__half_inplace_merge<_AlgPolicy>(__buff, __p, __middle, __last, __first, __comp);
  }
  else
  {
    value_type* __p = __buff;
    ...
    _CUDA_VSTD::__half_inplace_merge<_AlgPolicy>(
      _Rv(__p), _Rv(__buff), _RBi(__middle), _RBi(__first), _RBi(__last), _Inverted(__comp));
  }
}

template <class _AlgPolicy, class _Compare, class _BidirectionalIterator>
_LIBCUDACXX_HIDE_FROM_ABI void __inplace_merge(
  _BidirectionalIterator __first,
  _BidirectionalIterator __middle,
  _BidirectionalIterator __last,
  _Compare&& __comp,
  ...
  ptrdiff_t __buff_size)
{
  ...
  while (true)
  {
    ...
    {
      return _CUDA_VSTD::__buffered_inplace_merge<_AlgPolicy>(__first, __middle, __last, __comp, __len1, __len2, __buff);
    }
    ...
    { // __len >= 1, __len2 >= 2
      ...
      __m1    = _CUDA_VSTD::__upper_bound<_AlgPolicy>(__first, __middle, *__m2, __comp, _CUDA_VSTD::__identity());
    ...
    else
    {
      ...
      __m2    = _CUDA_VSTD::lower_bound(__middle, __last, *__m1, __comp);
    ...
    __middle = _CUDA_VSTD::__rotate<_AlgPolicy>(__m1, __middle, __m2).first;
    // __len12 and __len21 now have swapped meanings
    // merge smaller range with recursive call and larger with tail recursion elimination
    if (__len11 + __len21 < __len12 + __len22)
    {
      _CUDA_VSTD::__inplace_merge<_AlgPolicy>(__first, __m1, __middle, __comp, __len11, __len21, __buff, __buff_size);
      __first  = __middle;
      __middle = __m2;
      __len1   = __len12;
      __len2   = __len22;
    ...
    {
      _CUDA_VSTD::__inplace_merge<_AlgPolicy>(__middle, __m2, __last, __comp, __len12, __len22, __buff, __buff_size);
      __last   = __middle;
      __middle = __m1;
      __len1   = __len11;
      __len2   = __len21;
  ...
  _BidirectionalIterator __first, _BidirectionalIterator __middle, _BidirectionalIterator __last, _Compare&& __comp)
{
  ...
  return _CUDA_VSTD::__inplace_merge<_AlgPolicy>(
    _CUDA_VSTD::move(__first),
    _CUDA_VSTD::move(__middle),
...
_LIBCUDACXX_HIDE_FROM_ABI void inplace_merge(
  _BidirectionalIterator __first, _BidirectionalIterator __middle, _BidirectionalIterator __last, _Compare __comp)
{
  _CUDA_VSTD::__inplace_merge<_ClassicAlgPolicy>(
    _CUDA_VSTD::move(__first),
    _CUDA_VSTD::move(__middle),
    _CUDA_VSTD::move(__last),
...
{
  _CUDA_VSTD::inplace_merge(_CUDA_VSTD::move(__first), _CUDA_VSTD::move(__middle), _CUDA_VSTD::move(__last), __less{});
}

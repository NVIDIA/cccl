...
struct __fn
{
  ...
  [[nodiscard]] _CCCL_API constexpr _Ip operator()(_Ip __first, _Sp __last, _Pred __pred, _Proj __proj = {}) const
  {
    return ::cuda::std::ranges::find_if(
      ::cuda::std::move(__first), ::cuda::std::move(__last), _Not_pred{__pred}, ::cuda::std::move(__proj));
  }
  ...
  {
    return ::cuda::std::ranges::find_if(::cuda::std::forward<_Rp>(__r), _Not_pred{__pred}, ::cuda::std::move(__proj));
  }
};

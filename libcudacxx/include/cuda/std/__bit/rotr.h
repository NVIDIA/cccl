...
#ifndef _CUDA_STD___BIT_ROTR_H
#define _CUDA_STD___BIT_ROTR_H
...
#if _CCCL_HAS_BUILTIN(__builtin_rotateright8)
#  define _CCCL_BUILTIN_ROTATERIGHT8(...) __builtin_rotateright8(__VA_ARGS__)
#endif // has __builtin_rotateright8

#if _CCCL_HAS_BUILTIN(__builtin_rotateright16)
#  define _CCCL_BUILTIN_ROTATERIGHT16(...) __builtin_rotateright16(__VA_ARGS__)
#endif // has __builtin_rotateright16

#if _CCCL_HAS_BUILTIN(__builtin_rotateright32)
#  define _CCCL_BUILTIN_ROTATERIGHT32(...) __builtin_rotateright32(__VA_ARGS__)
#endif // has __builtin_rotateright32

#if _CCCL_HAS_BUILTIN(__builtin_rotateright64)
#  define _CCCL_BUILTIN_ROTATERIGHT64(...) __builtin_rotateright64(__VA_ARGS__)
#endif // has __builtin_rotateright64

_CCCL_BEGIN_NAMESPACE_CUDA_STD

template <class _Tp>
[[nodiscard]] _CCCL_API constexpr _Tp __rotr_impl_generic(const _Tp __v, const int __cnt) noexcept
{
...
template <class _Tp>
[[nodiscard]] _CCCL_HOST_API _Tp __rotr_impl_host(const _Tp __v, const int __cnt) noexcept
{
...
#  elif _CCCL_COMPILER(MSVC)
    return ::_rotr8(__v, static_cast<unsigned char>(__cnt));
#  else // ^^^ use builtins ^^^ / vvv fallback vvv
    return ::cuda::std::__rotr_impl_generic(__v, __cnt);
#  endif // ^^^ fallback ^^^
  }
...
#  elif _CCCL_COMPILER(MSVC)
    return ::_rotr16(__v, static_cast<unsigned char>(__cnt));
#  else // ^^^ use builtins ^^^ / vvv fallback vvv
    return ::cuda::std::__rotr_impl_generic(__v, __cnt);
#  endif // ^^^ fallback ^^^
  }
...
template <class _Tp>
[[nodiscard]] _CCCL_DEVICE_API _Tp __rotr_impl_device(const _Tp __v, const int __cnt) noexcept
{
  ...
  {
    return ::__funnelshift_r(__v, __v, __cnt);
  }
  else if constexpr (sizeof(_Tp) == sizeof(uint64_t))
  {
...
[[nodiscard]] _CCCL_API constexpr _Tp rotr(const _Tp __v, const int __cnt) noexcept
{
  ...
  {
    NV_IF_ELSE_TARGET(NV_IS_HOST, ({ return ::cuda::std::__rotr_impl_host(__v, __cnt); }), ({
                        return ::cuda::std::__rotr_impl_device(__v, __cnt);
                      }))
  }

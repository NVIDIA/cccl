...
#    if _CCCL_DEVICE_COMPILATION() // Only NVCC and NVRTC 12.8+ on architectures at least SM100 supports float128 on
                                   // device
#      if (_CCCL_CUDA_COMPILER(NVCC, >=, 12, 8) || _CCCL_CUDA_COMPILER(NVRTC, >=, 12, 8)) \
        && _CCCL_PTX_ARCH() >= 1000 /*DEVICE CODE*/

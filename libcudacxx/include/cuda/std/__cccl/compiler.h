...
// When compiling with nvcc and gcc as the host compiler, gcc reports unknown pragma for #pragma unroll
#if _CCCL_CUDA_COMPILER(NVCC) && _CCCL_COMPILER(GCC)
#  undef _CCCL_PRAGMA_UNROLL
#  define _CCCL_PRAGMA_UNROLL(_N)
#endif // _CCCL_CUDA_COMPILER(NVCC) && _CCCL_COMPILER(GCC)

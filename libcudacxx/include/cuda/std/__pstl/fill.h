...
struct __fill_constant_value
{
  ...
  template <class _Up>
  [[nodiscard]] _CCCL_DEVICE_API _CCCL_FORCEINLINE constexpr const _Tp& operator()(const _Up&) const
...
fill([[maybe_unused]] const _Policy& __policy, _InputIterator __first, _InputIterator __last, const _Tp& __value)
{
  ...
  if constexpr (::cuda::std::execution::__pstl_can_dispatch<decltype(__dispatch)>)
  {
    ...
    (void) __dispatch(
      __policy,
      ::cuda::counting_iterator{static_cast<iter_difference_t<_InputIterator>>(0)},
      ::cuda::counting_iterator{__count},
      ::cuda::std::move(__first),
      __fill_constant_value{__value});
    return;
  }
  else
  {

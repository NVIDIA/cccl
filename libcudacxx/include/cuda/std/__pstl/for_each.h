...
_CCCL_HOST_API void for_each(const _Policy& __pol, _Iter __first, _Iter __last, _Fn __func)
{
  ...
  {
    (void) __dispatch(
      __pol, ::cuda::std::move(__first), ::cuda::std::distance(__first, __last), ::cuda::std::move(__func));
  }
  else
  {

...
[[nodiscard]] _CCCL_HOST_API constexpr size_t __temporary_storage_num_bytes(const size_t __num_bytes_storage) noexcept
{
  ...
  // This might waste some space, e.g 254 bytes for short, but given the memory available on modern devices this is fine
  constexpr size_t __padding = sizeof(_ResultType) % ::cuda::mr::default_cuda_malloc_alignment;
  return sizeof(_ResultType) + __padding + __num_bytes_storage;
}

template <class _ResultType>
[[nodiscard]] _CCCL_HOST_API constexpr size_t __temporary_storage_min_alignment() noexcept
{
...
struct __temporary_storage
{
  ...
  _CCCL_HOST_API __temporary_storage(::cuda::stream_ref __stream, _Resource& __resource, size_t __num_bytes)
      ...
      , __num_bytes_allocated_(::cuda::std::execution::__temporary_storage_num_bytes<_ResultType>(__num_bytes))
      , __res_(static_cast<_ResultType*>(__resource_.allocate(
          __stream_, __num_bytes_allocated_, ::cuda::std::execution::__temporary_storage_min_alignment<_ResultType>())))
  {}
  ...
  _CCCL_HOST_API ~__temporary_storage()
  {
    __resource_.deallocate(
      __stream_,
      __res_,
      __num_bytes_allocated_,
      ::cuda::std::execution::__temporary_storage_min_alignment<_ResultType>());
  }
  ...
  [[nodiscard]] _CCCL_HOST_API auto __get_result_iter() noexcept
  {
    ...
    {
      return ::cuda::std::assume_aligned<::cuda::mr::default_cuda_malloc_alignment>(__res_);
    }
    else
    {
  ...
  [[nodiscard]] _CCCL_HOST_API void* __get_temp_storage() noexcept
  {
    ...
    unsigned char* __storage   = static_cast<unsigned char*>(static_cast<void*>(__res_)) + __offset;
    return static_cast<void*>(::cuda::std::assume_aligned<::cuda::mr::default_cuda_malloc_alignment>(__storage));
  }
};

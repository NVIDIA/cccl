...
struct __count_if_compare_eq
{
  ...
  [[nodiscard]] _CCCL_API _CCCL_FORCEINLINE constexpr int operator()(const _Up& __rhs) const
    noexcept(is_nothrow_invocable_v<const _UnaryPredicate&, const _Up&>)
  {
    return static_cast<bool>(::cuda::std::invoke(__pred_, __rhs)) ? 1 : 0;
  }
};
...
count_if([[maybe_unused]] const _Policy& __policy, _Iter __first, _Iter __last, _UnaryPredicate __pred)
{
  ...
  if constexpr (::cuda::std::execution::__pstl_can_dispatch<decltype(__dispatch)>)
  {
    return __dispatch(
      ...
      ::cuda::transform_iterator{::cuda::std::move(__last), __count_if_compare_eq{__pred}}, // pred is unused in __last

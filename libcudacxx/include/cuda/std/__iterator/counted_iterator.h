...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr counted_iterator(const counted_iterator<_I2>& __other) noexcept(
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr counted_iterator&
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr iter_difference_t<_Iter> count() const noexcept
  {
    return __count_;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr counted_iterator& operator++()
  {
  ...
  _LIBCUDACXX_HIDE_FROM_ABI decltype(auto) operator++(int)
  {
    _CCCL_ASSERT(__count_ > 0, "Iterator already at or past end.");
    --__count_;
  ...
  _CCCL_TEMPLATE(class _I2 = _Iter)
  _CCCL_REQUIRES(forward_iterator<_I2>)
  _LIBCUDACXX_HIDE_FROM_ABI constexpr counted_iterator operator++(int)
  {
    _CCCL_ASSERT(__count_ > 0, "Iterator already at or past end.");
  ...
  _CCCL_TEMPLATE(class _I2 = _Iter)
  _CCCL_REQUIRES(random_access_iterator<_I2>)
  _LIBCUDACXX_HIDE_FROM_ABI constexpr counted_iterator& operator+=(iter_difference_t<_I2> __n)
  {
  ...
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr iter_difference_t<_I2>
  ...
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr iter_difference_t<_Iter>
  ...
  _CCCL_TEMPLATE(class _I2 = _Iter)
  _CCCL_REQUIRES(random_access_iterator<_I2>)
  _LIBCUDACXX_HIDE_FROM_ABI constexpr decltype(auto) operator[](iter_difference_t<_I2> __n) const
  {
  ...
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool operator==(const counted_iterator& __lhs, default_sentinel_t)
  {
    return __lhs.__count_ == 0;
  }

#if _CCCL_STD_VER <= 2017
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool operator==(default_sentinel_t, const counted_iterator& __lhs)
  {
    return __lhs.__count_ == 0;
  }
  ...
  _CCCL_TEMPLATE(class _I2)
  _CCCL_REQUIRES(common_with<_I2, _Iter>)
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr strong_ordering
  operator<=>(const counted_iterator& __lhs, const counted_iterator<_I2>& __rhs)
  {
    return __lhs.__count_ <=> __rhs.__count_;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool operator<(const counted_iterator& __lhs, const counted_iterator& __rhs)
  {
    return __lhs.__count_ < __rhs.__count_;
  }
...
[[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI constexpr iter_rvalue_reference_t<_Iter> iter_move(
...
struct pointer_traits<counted_iterator<_Iter>, enable_if_t<contiguous_iterator<_Iter>>>
{
  ...
  _LIBCUDACXX_HIDE_FROM_ABI static constexpr auto to_address(const pointer __iter) noexcept
  {

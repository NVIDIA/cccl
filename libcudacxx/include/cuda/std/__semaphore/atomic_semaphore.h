...
class __atomic_semaphore
{
  ...
  [[nodiscard]] _CCCL_API bool __fetch_sub_if_slow(ptrdiff_t __old)
  {
    while (__old != 0)
    {
  ...
  [[nodiscard]] _CCCL_API bool __fetch_sub_if()
  {
  ...
  [[nodiscard]] _CCCL_API bool __acquire_slow_timed(chrono::nanoseconds const& __rel_time)
  {
...
public:
  ...
  _CCCL_API void release(ptrdiff_t __update = 1)
  {
  ...
  [[nodiscard]] _CCCL_API bool try_acquire() noexcept
  {
    return __fetch_sub_if();
  }

  template <class Clock, class Duration>
  [[nodiscard]] _CCCL_API bool try_acquire_until(chrono::time_point<Clock, Duration> const& __abs_time)
  {
    if (try_acquire())
    {
      return true;
  ...
  [[nodiscard]] _CCCL_API bool try_acquire_for(chrono::duration<Rep, Period> const& __rel_time)
  {
    if (try_acquire())
    {
      return true;
...
public:
  ...
  _CCCL_API void release([[maybe_unused]] ptrdiff_t __update = 1)
  {

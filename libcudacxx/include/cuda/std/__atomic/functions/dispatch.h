...
template <class _Sco>
_CCCL_DEVICE_API void __atomic_thread_fence_cuda(int __order, _Sco __scope)
{
  __cuda_atomic_thread_fence(__cuda_atomic_ptx_backend{}, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
[[nodiscard]] _CCCL_DEVICE_API _Type __atomic_load_n_cuda(const _Type* __ptr, int __order, _Sco __scope)
{
  return __cuda_atomic_load_dispatch(__cuda_atomic_ptx_backend{}, __ptr, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
[[nodiscard]] _CCCL_DEVICE_API _Type __atomic_load_n_cuda(const volatile _Type* __ptr, int __order, _Sco __scope)
{
  return __cuda_atomic_load_dispatch(__cuda_atomic_ptx_backend{}, __ptr, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
_CCCL_DEVICE_API void __atomic_store_n_cuda(_Type* __ptr, _Type __val, int __order, _Sco __scope)
{
  __cuda_atomic_store_dispatch(__cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
_CCCL_DEVICE_API void __atomic_store_n_cuda(volatile _Type* __ptr, _Type __val, int __order, _Sco __scope)
{
  __cuda_atomic_store_dispatch(__cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
[[nodiscard]] _CCCL_DEVICE_API _Type __atomic_exchange_n_cuda(_Type* __ptr, _Type __val, int __order, _Sco __scope)
{
  return __cuda_atomic_exchange_dispatch(
    __cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope);
}

template <class _Type, class _Sco>
[[nodiscard]] _CCCL_DEVICE_API _Type
__atomic_exchange_n_cuda(volatile _Type* __ptr, _Type __val, int __order, _Sco __scope)
{
  return __cuda_atomic_exchange_dispatch(
    __cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope);
}
...
template <class _Type, class _Sco>
_CCCL_DEVICE_API bool __atomic_compare_exchange_cuda(
  _Type* __ptr, _Type* __expected, _Type __desired, bool __weak, int __success, int __failure, _Sco __scope)
{
  return __cuda_atomic_compare_exchange_dispatch(
    __cuda_atomic_ptx_backend{},
    __ptr,
    __expected,
    __desired,
    __weak,
    static_cast<memory_order>(__success),
    static_cast<memory_order>(__failure),
    __scope);
}

template <class _Type, class _Sco>
_CCCL_DEVICE_API bool __atomic_compare_exchange_cuda(
  volatile _Type* __ptr, _Type* __expected, _Type __desired, bool __weak, int __success, int __failure, _Sco __scope)
{
  return __cuda_atomic_compare_exchange_dispatch(
    __cuda_atomic_ptx_backend{},
    __ptr,
...
#  define _CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(_Op)                                            \
    template <class _Type, class _Up, class _Sco>                                                \
    [[nodiscard]] _CCCL_DEVICE_API _Type __atomic_fetch_##_Op##_cuda(                            \
      _Type* __ptr, _Up __val, int __order, _Sco __scope)                                        \
    {                                                                                            \
      return __cuda_atomic_fetch_##_Op##_dispatch(                                               \
        __cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope); \
    }                                                                                            \
                                                                                                 \
    template <class _Type, class _Up, class _Sco>                                                \
    [[nodiscard]] _CCCL_DEVICE_API _Type __atomic_fetch_##_Op##_cuda(                            \
      volatile _Type* __ptr, _Up __val, int __order, _Sco __scope)                               \
    {                                                                                            \
      return __cuda_atomic_fetch_##_Op##_dispatch(                                               \
        __cuda_atomic_ptx_backend{}, __ptr, __val, static_cast<memory_order>(__order), __scope); \
    }

_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(add)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(sub)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(and)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(or)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(xor)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(min)
_CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER(max)

#  undef _CCCL_CUDA_ATOMIC_LEGACY_FETCH_WRAPPER
...
_CCCL_DEVICE static inline void __atomic_signal_fence_cuda(int __order)
{
  __cuda_atomic_signal_fence(__cuda_atomic_ptx_backend{}, static_cast<memory_order>(__order));
}

...
template <class _Tp>
_CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI constexpr _Tp __mul_sat_impl_generic(_Tp __x, _Tp __y) noexcept
{
...
template <class _Tp>
_CCCL_NODISCARD _CCCL_HIDE_FROM_ABI _CCCL_HOST _Tp __mul_sat_impl_host(_Tp __x, _Tp __y) noexcept
{
  if constexpr (_CCCL_TRAIT(is_signed, _Tp))
  {
...
template <class _Tp>
_CCCL_NODISCARD _CCCL_HIDE_FROM_ABI _CCCL_DEVICE _Tp __mul_sat_impl_device(_Tp __x, _Tp __y) noexcept
{
  if constexpr (_CCCL_TRAIT(is_signed, _Tp))
  {
    if constexpr (sizeof(_Tp) == sizeof(int8_t))
    ...
    else if constexpr (sizeof(_Tp) == sizeof(int32_t))
    {
      ...
      return static_cast<int32_t>(_CUDA_VSTD::clamp(
        __result,
        static_cast<int64_t>(numeric_limits<int32_t>::min()),
        static_cast<int64_t>(numeric_limits<int32_t>::max())));
    }
    else if constexpr (sizeof(_Tp) == sizeof(int64_t))
    {
  ...
  else // ^^^ signed types ^^^ / vvv unsigned types vvv
  {
    ...
    {
      uint16_t __result;
      asm("mul.lo.u16 %0, %1, %2;" : "=h"(__result) : "h"(static_cast<uint16_t>(__x)), "h"(static_cast<uint16_t>(__y)));
      return static_cast<uint8_t>(
        _CUDA_VSTD::min<uint16_t>(__result, static_cast<uint16_t>(numeric_limits<uint8_t>::max())));
    }
    else if constexpr (sizeof(_Tp) == sizeof(uint16_t))
    {
      uint32_t __result;
      ...
      return static_cast<uint16_t>(
        _CUDA_VSTD::min<uint32_t>(__result, static_cast<uint32_t>(numeric_limits<uint16_t>::max())));
    }
    else if constexpr (sizeof(_Tp) == sizeof(uint32_t))
    {
      uint64_t __result;
      ...
      return static_cast<uint32_t>(
        _CUDA_VSTD::min<uint64_t>(__result, static_cast<uint64_t>(numeric_limits<uint32_t>::max())));
    }
    else if constexpr (sizeof(_Tp) == sizeof(uint64_t))
    {
      uint64_t __lo, __hi;
...
_CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI constexpr _Tp mul_sat(_Tp __x, _Tp __y) noexcept
{
  ...
  return _CUDA_VSTD::__mul_sat_clamp_overflow<_Up>(__x, __y, __result, __overflow);
#else // ^^^ _CCCL_BUILTIN_MUL_OVERFLOW ^^^ / vvv !_CCCL_BUILTIN_MUL_OVERFLOW vvv
  if (!_CUDA_VSTD::__cccl_default_is_constant_evaluated())
  {
    NV_IF_ELSE_TARGET(NV_IS_HOST,
                      (return _CUDA_VSTD::__mul_sat_impl_host<_Up>(__x, __y);),
                      (return _CUDA_VSTD::__mul_sat_impl_device<_Up>(__x, __y);))
  }
  return _CUDA_VSTD::__mul_sat_impl_generic<_Up>(__x, __y);

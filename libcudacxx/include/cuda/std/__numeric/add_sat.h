...
template <class _Tp>
_CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI constexpr _Tp __add_sat_impl_generic(_Tp __x, _Tp __y) noexcept
{
...
template <class _Tp>
_CCCL_NODISCARD _CCCL_HIDE_FROM_ABI _CCCL_HOST _Tp __add_sat_impl_host(_Tp __x, _Tp __y) noexcept
{
  if constexpr (_CCCL_TRAIT(is_signed, _Tp))
  {
...
template <class _Tp>
_CCCL_NODISCARD _CCCL_HIDE_FROM_ABI _CCCL_DEVICE _Tp __add_sat_impl_device(_Tp __x, _Tp __y) noexcept
{
  if constexpr (_CCCL_TRAIT(is_signed, _Tp))
  {
    if constexpr (sizeof(_Tp) == sizeof(int8_t))
...
_CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI constexpr _Tp add_sat(_Tp __x, _Tp __y) noexcept
{
  ...
  return _CUDA_VSTD::__add_sat_clamp_overflow<_Up>(__x, __y, __result, __overflow);
#else // ^^^ _CCCL_BUILTIN_ADD_OVERFLOW ^^^ / vvv !_CCCL_BUILTIN_ADD_OVERFLOW vvv
  if (!_CUDA_VSTD::__cccl_default_is_constant_evaluated())
  {
    NV_IF_ELSE_TARGET(NV_IS_HOST,
                      (return _CUDA_VSTD::__add_sat_impl_host<_Up>(__x, __y);),
                      (return _CUDA_VSTD::__add_sat_impl_device<_Up>(__x, __y);))
  }
  return _CUDA_VSTD::__add_sat_impl_generic<_Up>(__x, __y);

...
// nvcc doesn't support __builtin_elementwise_add_sat in device code
#if _CCCL_CUDA_COMPILER(NVCC) && _CCCL_DEVICE_COMPILATION()
#  undef _CCCL_BUILTIN_ELEMENTWISE_ADD_SAT
...
[[nodiscard]] _CCCL_API constexpr _Tp __add_sat_impl_generic(_Tp __x, _Tp __y) noexcept
{
  ...
  {
    auto [__result, __overflow] = ::cuda::add_overflow(__x, __y);
    if (__overflow)
    {
...
selp.s64 %0, temp, %0, r;
  ...
  else // ^^^ signed types ^^^ / vvv unsigned types vvv
  {
    ...
    return static_cast<_Tp>(__x + ::cuda::std::min(__y, __bneg_x));
  } // ^^^ unsigned types ^^^
}

...
#include <cuda/std/__random/log2.h>
...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI __independent_bits_engine(_Engine& __e, size_t __w)
      : __e_(__e)
      , __w_(__w)
  {
  ...
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI result_type operator()()
  {
    return __eval(integral_constant<bool, _Rp != 0>());
  }

private:
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI result_type __eval(false_type)
  {
    return static_cast<result_type>(__e_() & __mask0_);
  }
  _CCCL_NODISCARD _LIBCUDACXX_HIDE_FROM_ABI result_type __eval(true_type)
  {
...
public:
  ...
  public:
    ...
    _LIBCUDACXX_HIDE_FROM_ABI explicit param_type(result_type __a = 0,
                                                  result_type __b = numeric_limits<result_type>::max())
        : __a_(__a)
        , __b_(__b)
    {}

    _LIBCUDACXX_HIDE_FROM_ABI result_type a() const
    {
      return __a_;
    }
    _LIBCUDACXX_HIDE_FROM_ABI result_type b() const
    {
      return __b_;
    }

    _LIBCUDACXX_HIDE_FROM_ABI friend bool operator==(const param_type& __x, const param_type& __y)
    {
...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI uniform_int_distribution()
      : uniform_int_distribution(0)
  {}
  _LIBCUDACXX_HIDE_FROM_ABI explicit uniform_int_distribution(
    result_type __a, result_type __b = numeric_limits<result_type>::max())
      : __p_(param_type(__a, __b))
  {}
  _LIBCUDACXX_HIDE_FROM_ABI explicit uniform_int_distribution(const param_type& __p)
      : __p_(__p)
  {}
  ...
  _LIBCUDACXX_HIDE_FROM_ABI result_type operator()(_URNG& __g)
  {
    return (*this)(__g, __p_);
  }
  template <class _URNG>
  _LIBCUDACXX_HIDE_FROM_ABI result_type operator()(_URNG& __g, const param_type& __p);
  ...
  _LIBCUDACXX_HIDE_FROM_ABI param_type param() const
  {
    return __p_;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void param(const param_type& __p)
  {
    __p_ = __p;
  }

  _LIBCUDACXX_HIDE_FROM_ABI result_type min() const
  {
    return a();
  }
  _LIBCUDACXX_HIDE_FROM_ABI result_type max() const
  {
    return b();
  }

  _LIBCUDACXX_HIDE_FROM_ABI friend bool
...
template <class _URNG>
_LIBCUDACXX_HIDE_FROM_ABI typename uniform_int_distribution<_IntType>::result_type
uniform_int_distribution<_IntType>::operator()(_URNG& __g, const param_type& __p)
  _LIBCUDACXX_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
{
  static_assert(__libcpp_random_is_valid_urng<_URNG>::value, "");
  typedef conditional_t<sizeof(result_type) <= sizeof(uint32_t), uint32_t, make_unsigned_t<result_type>> _UIntType;
  ...
  size_t __w = __dt - _CUDA_VSTD::__countl_zero(__rp) - 1;

...
struct __lce_ta<__a, __c, __m, (unsigned long long) (~0), true>
{
  ...
  _LIBCUDACXX_HIDE_FROM_ABI static result_type next(result_type __x)
  {
    // Schrage's algorithm
...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI static constexpr result_type min()
  {
    return _Min;
  }
  _LIBCUDACXX_HIDE_FROM_ABI static constexpr result_type max()
  {
    return _Max;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI linear_congruential_engine()
      : linear_congruential_engine(default_seed)
  {}
  _LIBCUDACXX_HIDE_FROM_ABI explicit linear_congruential_engine(result_type __s)
  {
    seed(__s);
  }
#else
  _LIBCUDACXX_HIDE_FROM_ABI explicit linear_congruential_engine(result_type __s = default_seed)
  {
    seed(__s);
  }
#endif
  template <class _Sseq, enable_if_t<__is_seed_sequence<_Sseq, linear_congruential_engine>::value, int> = 0>
  _LIBCUDACXX_HIDE_FROM_ABI explicit linear_congruential_engine(_Sseq& __q)
  {
    seed(__q);
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(result_type __s = default_seed)
  {
    seed(integral_constant<bool, __m == 0>(), integral_constant<bool, __c == 0>(), __s);
  }
  template <class _Sseq, enable_if_t<__is_seed_sequence<_Sseq, linear_congruential_engine>::value, int> = 0>
  _LIBCUDACXX_HIDE_FROM_ABI void seed(_Sseq& __q)
  {
    __seed(
      __q,
  ...
  _LIBCUDACXX_HIDE_FROM_ABI void discard(unsigned long long __z)
  {
    for (; __z; --__z)
    {
      operator()();
  ...
  friend _LIBCUDACXX_HIDE_FROM_ABI bool
...
private:
  _LIBCUDACXX_HIDE_FROM_ABI void seed(true_type, true_type, result_type __s)
  {
    __x_ = __s == 0 ? 1 : __s;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(true_type, false_type, result_type __s)
  {
    __x_ = __s;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(false_type, true_type, result_type __s)
  {
    __x_ = __s % __m == 0 ? 1 : __s % __m;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(false_type, false_type, result_type __s)
  {
    __x_ = __s % __m;
  }

  template <class _Sseq>
  _LIBCUDACXX_HIDE_FROM_ABI void __seed(_Sseq& __q, integral_constant<unsigned, 1>);
  template <class _Sseq>
  _LIBCUDACXX_HIDE_FROM_ABI void __seed(_Sseq& __q, integral_constant<unsigned, 2>);
...
linear_congruential_engine<_UIntType, __a, __c, __m>::__seed(_Sseq& __q, integral_constant<unsigned, 1>)
{
...
template <class _Sseq>
_LIBCUDACXX_HIDE_FROM_ABI void
linear_congruential_engine<_UIntType, __a, __c, __m>::__seed(_Sseq& __q, integral_constant<unsigned, 2>)
{

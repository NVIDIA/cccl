...
public:
  ...
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI static constexpr result_type min() noexcept
  {
    return _Min;
  }
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI static constexpr result_type max() noexcept
  {
    return _Max;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI linear_congruential_engine() noexcept
      : linear_congruential_engine(default_seed)
  {}
  _LIBCUDACXX_HIDE_FROM_ABI explicit linear_congruential_engine(result_type __s) noexcept
  {
    seed(__s);
  }

  template <class _Sseq, enable_if_t<__is_seed_sequence<_Sseq, linear_congruential_engine>, int> = 0>
  _LIBCUDACXX_HIDE_FROM_ABI explicit linear_congruential_engine(_Sseq& __q) noexcept
  {
    seed(__q);
  }
...
private:
  _LIBCUDACXX_HIDE_FROM_ABI void seed(true_type, true_type, result_type __s) noexcept
  {
    __x_ = __s == 0 ? 1 : __s;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(true_type, false_type, result_type __s) noexcept
  {
    __x_ = __s;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(false_type, true_type, result_type __s) noexcept
  {
    __x_ = __s % __m == 0 ? 1 : __s % __m;
  }
  _LIBCUDACXX_HIDE_FROM_ABI void seed(false_type, false_type, result_type __s) noexcept
  {
    __x_ = __s % __m;
  }

...
public:
  _CCCL_API __barrier_poll_tester_phase(_Barrier const* __this_, typename _Barrier::arrival_token&& __phase_)
      : __this(__this_)
      , __phase(_CUDA_VSTD::move(__phase_))
  {}

  [[nodiscard]] _CCCL_API bool operator()() const
  {
...
public:
  _CCCL_API __barrier_poll_tester_parity(_Barrier const* __this_, bool __parity_)
      : __this(__this_)
      , __parity(__parity_)
  {}
...
template <class _Barrier>
[[nodiscard]] _CCCL_API bool __call_try_wait(const _Barrier& __b, typename _Barrier::arrival_token&& __phase)
{
  return __b.__try_wait(_CUDA_VSTD::move(__phase));
}

template <class _Barrier>
[[nodiscard]] _CCCL_API bool __call_try_wait_parity(const _Barrier& __b, bool __parity)
{

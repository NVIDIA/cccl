...
public:
  ...
  _CCCL_API void count_down(ptrdiff_t __update = 1)
  {
  ...
  _CCCL_API bool try_wait() const noexcept
  {
    return __counter.load(memory_order_acquire) == 0;
  }
  _CCCL_API void wait() const
  {
    while (1)
    {
  ...
  _CCCL_API void arrive_and_wait(ptrdiff_t __update = 1)
  {
    count_down(__update);
    wait();
  }

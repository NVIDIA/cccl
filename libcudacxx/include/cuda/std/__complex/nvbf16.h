...
template <class _Tp>
_CCCL_API __nv_bfloat16 __convert_to_bfloat16(const _Tp& __value) noexcept
{
  return __value;
}

_CCCL_API __nv_bfloat16 __convert_to_bfloat16(const float& __value) noexcept
{
  return __float2bfloat16(__value);
}

_CCCL_API __nv_bfloat16 __convert_to_bfloat16(const double& __value) noexcept
{
...
public:
  ...
  _CCCL_API complex(const value_type& __re = value_type(), const value_type& __im = value_type())
      : __repr_(__re, __im)
  {}

  template <class _Up, enable_if_t<__cccl_internal::__is_non_narrowing_convertible<value_type, _Up>::value, int> = 0>
  _CCCL_API complex(const complex<_Up>& __c)
  ...
  _CCCL_API explicit complex(const complex<_Up>& __c)
      : __repr_(__convert_to_bfloat16(__c.real()), __convert_to_bfloat16(__c.imag()))
  {}

  _CCCL_API complex& operator=(const value_type& __re)
  {
    __repr_.x = __re;
  ...
  template <class _Up>
  _CCCL_API complex& operator=(const complex<_Up>& __c)
  {
...
#  if !_CCCL_COMPILER(NVRTC)
  template <class _Up>
  _CCCL_API complex(const ::std::complex<_Up>& __other)
      : __repr_(_LIBCUDACXX_ACCESS_STD_COMPLEX_REAL(__other), _LIBCUDACXX_ACCESS_STD_COMPLEX_IMAG(__other))
  {}

  template <class _Up>
  _CCCL_API complex& operator=(const ::std::complex<_Up>& __other)
  {
...
#  endif // !_CCCL_COMPILER(NVRTC)
  ...
  [[nodiscard]] _CCCL_API value_type real() const
  {
    return __repr_.x;
  }
  [[nodiscard]] _CCCL_API value_type imag() const
  {
    return __repr_.y;
  }

  _CCCL_API void real(value_type __re)
  {
    __repr_.x = __re;
  }
  _CCCL_API void imag(value_type __im)
  {
    __repr_.y = __im;
  }

  // Those additional volatile overloads are meant to help with reductions in thrust
  [[nodiscard]] _CCCL_API value_type real() const volatile
  {
    return __repr_.x;
  }
  [[nodiscard]] _CCCL_API value_type imag() const volatile
  {
    return __repr_.y;
  }
  ...
  _CCCL_API friend complex& operator+=(complex& __lhs, const complex& __rhs) noexcept
  {
  ...
  [[nodiscard]] _CCCL_API friend bool operator==(const complex& __lhs, const complex& __rhs) noexcept
  {
...
_CCCL_API complex<float>::complex(const complex<__nv_bfloat16>& __c)
...
template <> // complex<__half>
_CCCL_API complex<double>::complex(const complex<__nv_bfloat16>& __c)
    : __re_(__bfloat162float(__c.real()))
    , __im_(__bfloat162float(__c.imag()))
{}
...
_CCCL_API complex<float>& complex<float>::operator=(const complex<__nv_bfloat16>& __c)
{
  __re_ = __bfloat162float(__c.real());
  __im_ = __bfloat162float(__c.imag());
  return *this;
...
template <> // complex<double>
template <> // complex<__nv_bfloat16>
_CCCL_API complex<double>& complex<double>::operator=(const complex<__nv_bfloat16>& __c)
{
  __re_ = __bfloat162float(__c.real());
  __im_ = __bfloat162float(__c.imag());
  return *this;
...
[[nodiscard]] _CCCL_API __nv_bfloat16 arg(__nv_bfloat16 __re)
{
...
template <>
_CCCL_API complex<__nv_bfloat16> asinh(const complex<__nv_bfloat16>& __x)
{
  return complex<__nv_bfloat16>{_CUDA_VSTD::asinh(complex<float>{__x})};
}
template <>
_CCCL_API complex<__nv_bfloat16> acosh(const complex<__nv_bfloat16>& __x)
{
  return complex<__nv_bfloat16>{_CUDA_VSTD::acosh(complex<float>{__x})};
}
template <>
_CCCL_API complex<__nv_bfloat16> atanh(const complex<__nv_bfloat16>& __x)
{
  return complex<__nv_bfloat16>{_CUDA_VSTD::atanh(complex<float>{__x})};
}
template <>
_CCCL_API complex<__nv_bfloat16> acos(const complex<__nv_bfloat16>& __x)
{

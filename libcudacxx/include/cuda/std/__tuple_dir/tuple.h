...
public:
  ...
  template <class _Constraints = __tuple_constraints<_Tp...>, enable_if_t<_Constraints::__swappable, int> = 0>
  ...
  template <class _Constraints = __tuple_constraints<_Tp...>, enable_if_t<_Constraints::__swappable, int> = 0>
  _CCCL_API friend void swap(tuple& __t, tuple& __u) noexcept(_Constraints::__nothrow_swappable)
  {
    __t.swap(__u);
  }
};

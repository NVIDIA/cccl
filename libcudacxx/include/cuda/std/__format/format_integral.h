...
[[nodiscard]] _CCCL_API char* __fmt_insert_sign(char* __buf, bool __negative, __fmt_spec_sign __sign)
{
  if (__negative)
  {
    *__buf++ = '-';
  ...
  int __base)
{
  ...
  char* __last{};
  {
    const auto __r = _CUDA_VSTD::to_chars(__first, __array + __size, __value, __base);
  ...
  else
  {
    ...
    __specs.__width_ -= _CUDA_VSTD::min(static_cast<uint32_t>(__last - __first), __specs.__width_);
  }

  if (__specs.__std_.__type_ != __fmt_spec_type::__hexadecimal_upper_case)
  {
    return _CUDA_VSTD::__fmt_write(__first, __last, __ctx.out(), __specs);
  }
  return _CUDA_VSTD::__fmt_write_transformed(__first, __last, __ctx.out(), __specs, _CUDA_VSTD::__fmt_hex_to_upper);
}
...
__fmt_format_int(_Tp __value, _FmtCtx& __ctx, __fmt_parsed_spec<_CharT> __specs)
{
  ...
  switch (__specs.__std_.__type_)
  {
    case __fmt_spec_type::__binary_lower_case: {
      ...
      return _CUDA_VSTD::__fmt_format_int_impl(
...
__fmt_format_bool(bool __value, _FmtContext& __ctx, __fmt_parsed_spec<_CharT> __specs)
{
  ...
  return _CUDA_VSTD::__fmt_write(__str, __ctx.out(), __specs, static_cast<ptrdiff_t>(__str.size()));
}

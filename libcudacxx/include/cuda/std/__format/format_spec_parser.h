...
__throw_invalid_option_format_error(const char (&__id)[_IdSize], const char (&__option)[_OptSize])
{
  ...
  _CCCL_THROW(format_error(__msg));
}
...
struct __fmt_substitute_arg_id_visitor
{
  ...
  [[nodiscard]] _CCCL_API constexpr uint32_t operator()([[maybe_unused]] _Tp __arg)
  {
    ...
    {
      _CCCL_THROW(format_error("The argument index value is too large for the number of arguments supplied"));
    }
                       ...
                       is_same_v<_Tp, long long> || is_same_v<_Tp, unsigned long long>)
    {
      if constexpr (is_signed_v<_Tp>)
      {
        if (__arg < 0)
        {
          _CCCL_THROW(format_error("An argument index may not have a negative value"));
        }
      }
      ...
      {
        _CCCL_THROW(format_error("The value of the argument index exceeds its maximum value"));
      }
    ...
    {
      _CCCL_THROW(format_error("Replacement argument isn't a standard signed or unsigned integer type"));
    }
  }
};
...
[[nodiscard]] _CCCL_API constexpr uint32_t __fmt_spec_make_type_mask(__fmt_spec_type __t)
{
  ...
  if (__shift > 31)
  {
    _CCCL_THROW(format_error("The type does not fit in the mask"));
  }
  return 1 << __shift;
}
...
public:
  ...
  _CCCL_API constexpr typename _ParseContext::iterator __parse(_ParseContext& __ctx, __fmt_spec_fields __fields)
  {
    ...
    {
      _CCCL_THROW(format_error("The format specification does not allow the sign option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier does not allow the alternate form option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier does not allow the zero-padding option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier does not allow the precision option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier does not allow the locale-specific form option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier does not allow the n option"));
    }
    ...
    {
      _CCCL_THROW(format_error("The format specifier should consume the input or end with a '}'"));
    }
  ...
  __validate(__fmt_spec_fields __fields, const char (&__id)[_IdSize], uint32_t __type_mask = ~uint32_t{0}) const
  {
    if (!__fields.__sign_ && __fmt_spec_sign{__sign_} != __fmt_spec_sign::__default)
    {
      ...
      {
        _CCCL_THROW(format_error("The format specifier does not allow the sign option"));
      }
      else
      {
    ...
    if ((::cuda::std::__fmt_spec_make_type_mask(__type_) & __type_mask) == 0)
    {
      if (::cuda::std::is_constant_evaluated())
      {
        _CCCL_THROW(format_error("The format specifier uses an invalid value for the type option"));
      }
      else
      {
...
private:
  ...
  __parse_arg_id(_It __begin, _It __end, _ParseCtx& __ctx)
  {
    ...
    if (__begin == __end)
    {
      _CCCL_THROW(format_error("End of input while parsing an argument index"));
    }
    ...
    {
      _CCCL_THROW(format_error("The argument index is invalid"));
    }
  ...
  _CCCL_API constexpr void __validate_fill_character(_CharT __fill)
  {
    ...
    if (__fill == _CharT{'{'})
    {
      _CCCL_THROW(format_error("The fill option contains an invalid value"));
    }
  }
  ...
  [[nodiscard]] _CCCL_API constexpr bool __parse_width(_It& __begin, _It __end, _Ctx& __ctx)
  {
    if (*__begin == _CharT{'0'})
    {
      _CCCL_THROW(format_error("The width option should not have a leading zero"));
    }
  ...
  [[nodiscard]] _CCCL_API constexpr bool __parse_precision(_It& __begin, _It __end, _Ctx& __ctx)
  {
    ...
    if (__begin == __end)
    {
      _CCCL_THROW(format_error("End of input while parsing format specifier precision"));
    }
    ...
    if (*__begin < _CharT{'0'} || *__begin > _CharT{'9'})
    {
      _CCCL_THROW(format_error("The precision option does not contain a value or an argument index"));
    }
...
_CCCL_API constexpr void __fmt_process_display_type_str(__fmt_spec_type __type)
{
  switch (__type)
  {
    ...
    case __fmt_spec_type::__string:
      break;
    default:
      _CCCL_THROW(format_error("The type option contains an invalid value for a string formatting argument"));
  }
}

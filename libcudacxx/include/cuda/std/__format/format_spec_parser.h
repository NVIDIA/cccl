...
__throw_invalid_option_format_error(const char (&__id)[_IdSize], const char (&__option)[_OptSize])
{
  ...
  _CUDA_VSTD::__throw_format_error(__msg);
}
...
struct __fmt_substitute_arg_id_visitor
{
  ...
  [[nodiscard]] _CCCL_API uint32_t operator()([[maybe_unused]] _Tp __arg)
  {
    ...
    {
      _CUDA_VSTD::__throw_format_error("The argument index value is too large for the number of arguments supplied");
    }
                  ...
                  is_same_v<_Tp, long long> || is_same_v<_Tp, unsigned long long>)
    {
      if constexpr (is_signed_v<_Tp>)
      {
        if (__arg < 0)
        {
          _CUDA_VSTD::__throw_format_error("An argument index may not have a negative value");
        }
      }
    ...
    {
      _CUDA_VSTD::__throw_format_error("Replacement argument isn't a standard signed or unsigned integer type");
    }
  }
};
...
[[nodiscard]] _CCCL_API constexpr uint32_t __fmt_spec_make_type_mask(__fmt_spec_type __t)
{
  ...
  if (__shift > 31)
  {
    _CUDA_VSTD::__throw_format_error("The type does not fit in the mask");
  }
  return 1 << __shift;
}

inline constexpr uint32_t __fmt_spec_type_mask_int =
  _CUDA_VSTD::__fmt_spec_make_type_mask(__fmt_spec_type::__binary_lower_case) | //
  _CUDA_VSTD::__fmt_spec_make_type_mask(__fmt_spec_type::__binary_upper_case) | //
  _CUDA_VSTD::__fmt_spec_make_type_mask(__fmt_spec_type::__decimal) | //
  _CUDA_VSTD::__fmt_spec_make_type_mask(__fmt_spec_type::__octal) | //
...
public:
  ...
  _CCCL_API constexpr typename _ParseContext::iterator __parse(_ParseContext& __ctx, __fmt_spec_fields __fields)
  {
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_sign(__begin))
    {
      _CUDA_VSTD::__throw_format_error("The format specification does not allow the sign option");
    }
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_alternate_form(__begin))
    {
      _CUDA_VSTD::__throw_format_error("The format specifier does not allow the alternate form option");
    }
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_zero_padding(__begin))
    {
      _CUDA_VSTD::__throw_format_error("The format specifier does not allow the zero-padding option");
    }
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_precision(__begin, __end, __ctx))
    {
      _CUDA_VSTD::__throw_format_error("The format specifier does not allow the precision option");
    }
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_locale_specific_form(__begin))
    {
      _CUDA_VSTD::__throw_format_error("The format specifier does not allow the locale-specific form option");
    }
    ...
    else if (_CUDA_VSTD::is_constant_evaluated() && __parse_clear_brackets(__begin))
    {
      _CUDA_VSTD::__throw_format_error("The format specifier does not allow the n option");
    }
    ...
    {
      _CUDA_VSTD::__throw_format_error("The format specifier should consume the input or end with a '}'");
    }
  ...
  __validate(__fmt_spec_fields __fields, const char (&__id)[_IdSize], uint32_t __type_mask = ~uint32_t{0}) const
  {
    if (!__fields.__sign_ && __sign_ != __fmt_spec_sign::__default)
    {
      ...
      {
        _CUDA_VSTD::__throw_format_error("The format specifier does not allow the sign option");
      }
      else
      {
        _CUDA_VSTD::__throw_invalid_option_format_error(__id, "sign");
      }
    }
    ...
    if (!__fields.__alternate_form_ && __alternate_form_)
    {
      ...
      {
        _CUDA_VSTD::__throw_invalid_option_format_error(__id, "alternate form");
      }
    }
    ...
    if (!__fields.__zero_padding_ && __alignment_ == __fmt_spec_alignment::__zero_padding)
    {
      ...
      {
        _CUDA_VSTD::__throw_invalid_option_format_error(__id, "zero-padding");
      }
    }
    ...
    { // Works both when the precision has a value or an arg-id.
      ...
      {
        _CUDA_VSTD::__throw_invalid_option_format_error(__id, "precision");
      }
    }
    ...
    if (!__fields.__locale_specific_form_ && __locale_specific_form_)
    {
      ...
      {
        _CUDA_VSTD::__throw_invalid_option_format_error(__id, "locale-specific form");
      }
    }

    if ((_CUDA_VSTD::__fmt_spec_make_type_mask(__type_) & __type_mask) == 0)
    {
      if (_CUDA_VSTD::is_constant_evaluated())
      {
        _CUDA_VSTD::__throw_format_error("The format specifier uses an invalid value for the type option");
      }
      else
      {
        _CUDA_VSTD::__throw_invalid_type_format_error(__id);
      }
    }
  }
...
private:
  ...
  _CCCL_API constexpr void __validate_fill_character(_CharT __fill)
  {
    ...
    if (__fill == _CharT{'{'})
    {
      _CUDA_VSTD::__throw_format_error("The fill option contains an invalid value");
    }
  }
  ...
  [[nodiscard]] _CCCL_API constexpr bool __parse_width(_It& __begin, _It __end, _Ctx& __ctx)
  {
    if (*__begin == _CharT{'0'})
    {
      _CUDA_VSTD::__throw_format_error("The width option should not have a leading zero");
    }

    if (*__begin == _CharT{'{'})
    {
      const auto __r  = _CUDA_VSTD::__fmt_parse_arg_id(++__begin, __end, __ctx);
  ...
  [[nodiscard]] _CCCL_API constexpr bool __parse_precision(_It& __begin, _It __end, _Ctx& __ctx)
  {
    ...
    if (__begin == __end)
    {
      _CUDA_VSTD::__throw_format_error("End of input while parsing format specifier precision");
    }

    if (*__begin == _CharT{'{'})
    {
      const auto __arg_id = _CUDA_VSTD::__fmt_parse_arg_id(++__begin, __end, __ctx);
    ...
    if (*__begin < _CharT{'0'} || *__begin > _CharT{'9'})
    {
      _CUDA_VSTD::__throw_format_error("The precision option does not contain a value or an argument index");
    }
  ...
  template <class _Ctx>
  [[nodiscard]] _CCCL_API int32_t __get_width(_Ctx& __ctx) const
  {
    if (!__width_as_arg_)
    {
      return __width_;
    }
    return _CUDA_VSTD::__fmt_substitute_arg_id(__ctx.arg(__width_));
  }
  ...
  [[nodiscard]] _CCCL_API int32_t __get_precision(_Ctx& __ctx) const
  {
    ...
    return _CUDA_VSTD::__fmt_substitute_arg_id(__ctx.arg(__precision_));
  }
};
...
_CCCL_API constexpr void __fmt_process_display_type_str(__fmt_spec_type __type)
{
  switch (__type)
  {
    ...
    case __fmt_spec_type::__string:
      break;
    default:
      _CUDA_VSTD::__throw_format_error("The type option contains an invalid value for a string formatting argument");
  }
}
...
{
  _CUDA_VSTD::__fmt_process_display_type_bool_str(__parser, __id);
}
...
_CCCL_API constexpr void __fmt_process_parsed_char(__fmt_spec_parser<_CharT>& __parser)
{
  ...
  switch (__parser.__type_)
  {
    ...
    case __fmt_spec_type::__char:
      _CUDA_VSTD::__fmt_process_display_type_char(__parser, __id);
      break;
    case __fmt_spec_type::__binary_lower_case:
    case __fmt_spec_type::__binary_upper_case:
    case __fmt_spec_type::__octal:
...
_CCCL_API constexpr void __fmt_process_parsed_fp(__fmt_spec_parser<_CharT>& __parser)
{
  switch (__parser.__type_)
  {
    ...
    case __fmt_spec_type::__general_upper_case:
      ...
      break;
    default:
      _CUDA_VSTD::__throw_invalid_type_format_error("a floating-point");
  }
}
...
_CCCL_API constexpr void __fmt_process_display_type_ptr(__fmt_spec_type __type)
{
  switch (__type)
  {
    ...
    case __fmt_spec_type::__pointer_upper_case:
      break;
    default:
      _CUDA_VSTD::__throw_invalid_type_format_error("a pointer");
  }
}
...
__fmt_estimate_column_width(basic_string_view<_CharT> __str, size_t __maximum, __fmt_column_width_rounding) noexcept
{
  ...
  // need for rounding.
  size_t __width = _CUDA_VSTD::min(__str.size(), __maximum);

...
_CCCL_REQUIRES(same_as<typename _Context::char_type, typename _Tp::value_type> _CCCL_AND
...
[[nodiscard]] _CCCL_API basic_format_arg<_Context> __fmt_make_format_arg(_Tp& __value) noexcept
{
  ...
  using _CtxCharT = _Context::char_type;
...
__fmt_make_packed_storage(uint64_t& __types, __basic_format_arg_value<_Context>* __values, _Args&... __args) noexcept
{
  ...
  (
    [&] {
      basic_format_arg<_Context> __arg = _CUDA_VSTD::__fmt_make_format_arg<_Context>(__args);
      if (__shift != 0)
      {

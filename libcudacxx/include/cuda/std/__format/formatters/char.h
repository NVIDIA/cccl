...
struct __fmt_formatter_char
{
  ...
  {
    typename _ParseCtx::iterator __result = __parser_.__parse(__ctx, _CUDA_VSTD::__fmt_spec_fields_int());
    _CUDA_VSTD::__fmt_process_parsed_char(__parser_);
    return __result;
  }
  ...
  _CCCL_API typename _FmtCtx::iterator format(_CharT __value, _FmtCtx& __ctx) const
  {
    ...
    {
      return _CUDA_VSTD::__fmt_format_char(__value, __ctx.out(), __specs);
    }

    if constexpr (sizeof(_CharT) <= sizeof(unsigned))
    {
      return _CUDA_VSTD::__fmt_format_int(static_cast<unsigned>(static_cast<_Up>(__value)), __ctx, __specs);
    }
    else
    {
      return _CUDA_VSTD::__fmt_format_int(static_cast<_Up>(__value), __ctx, __specs);
    }
  }

...
struct __fmt_formatter_int
{
  ...
  _CCCL_API constexpr typename _ParseCtx::iterator parse(_ParseCtx& __ctx)
  {
    ...
    _CUDA_VSTD::__fmt_process_parsed_int(__parser_);
    return __result;
  }
  ...
  _CCCL_API typename _FmtCtx::iterator format(_Tp __value, _FmtCtx& __ctx) const
  {
    ...
    using _Type =
      __make_nbit_int_t<_CUDA_VSTD::max(sizeof(_Tp) * CHAR_BIT, sizeof(int32_t) * CHAR_BIT), is_signed_v<_Tp>>;

    // Reduce the number of instantiation of the integer formatter
    return _CUDA_VSTD::__fmt_format_int(static_cast<_Type>(__value), __ctx, __specs);
  }

...
{
  return _CUDA_VSTD::__fmt_copy(basic_string_view{__first, __last}, _CUDA_VSTD::move(__out_it));
}
...
{
  // todo: handle __fmt_output_buffer and __fmt_retarget_buffer when they are implemented
  return _CUDA_VSTD::fill_n(_CUDA_VSTD::move(__out_it), __n, __value);
}
...
__fmt_write(_It __first, _It __last, _OutIt __out_it, __fmt_parsed_spec<_ParserCharT> __specs, ptrdiff_t __size)
{
  ...
  return _CUDA_VSTD::__fmt_write(basic_string_view{__first, __last}, _CUDA_VSTD::move(__out_it), __specs, __size);
}
...
{
  _CCCL_ASSERT(__first <= __last, "Not a valid range");
  return _CUDA_VSTD::__fmt_write(__first, __last, _CUDA_VSTD::move(__out_it), __specs, __last - __first);
}
...
__fmt_write_string_no_precision(basic_string_view<_CharT> __str, _OutIt __out_it, __fmt_parsed_spec<_CharT> __specs)
{
  ...
  size_t __size =
    _CUDA_VSTD::__fmt_estimate_column_width(__str, __specs.__width_, __fmt_column_width_rounding::__up).__width_;
  return _CUDA_VSTD::__fmt_write(__str, _CUDA_VSTD::move(__out_it), __specs, __size);
}
...
__fmt_write_string(basic_string_view<_CharT> __str, _OutIt __out_it, __fmt_parsed_spec<_CharT> __specs)
{
  ...
  {
    return _CUDA_VSTD::__fmt_write_string_no_precision(__str, _CUDA_VSTD::move(__out_it), __specs);
  }
  int __size = _CUDA_VSTD::__fmt_truncate(__str, __specs.__precision_);
  return _CUDA_VSTD::__fmt_write(__str.begin(), __str.end(), _CUDA_VSTD::move(__out_it), __specs, __size);
}

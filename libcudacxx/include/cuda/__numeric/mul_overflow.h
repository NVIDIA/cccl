...
#  elif _CCCL_COMPILER(MSVC) && _CCCL_ARCH(X86_64)
    if constexpr (sizeof(_Tp) == sizeof(::cuda::std::int32_t))
    {
      const ::cuda::std::int64_t __result = ::__emul(__lhs, __rhs);
      return {static_cast<_Tp>(__result), !::cuda::std::in_range<_Tp>(__result)};
    }
    else if constexpr (sizeof(_Tp) == sizeof(::cuda::std::int64_t))
    {
      ::cuda::std::int64_t __hi = ::__mulh(__lhs, __rhs);
      ::cuda::std::int64_t __lo = __lhs * __rhs;
...
#  elif _CCCL_COMPILER(MSVC) && _CCCL_ARCH(X86_64)
    ...
    {
      const ::cuda::std::uint64_t __result = ::__emulu(__lhs, __rhs);
      return {static_cast<_Tp>(__result), !::cuda::std::in_range<_Tp>(__result)};
    }
    else if constexpr (sizeof(_Tp) == sizeof(::cuda::std::uint64_t))
    {

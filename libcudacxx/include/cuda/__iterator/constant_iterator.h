...
private:
  ...
  _CCCL_NO_UNIQUE_ADDRESS _CUDA_VSTD::ptrdiff_t __index_{};
...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr constant_iterator() noexcept(_CUDA_VSTD::is_nothrow_default_constructible_v<_Tp2>)
  {}
#endif // !_CCCL_HAS_CONCEPTS()

  //! @brief Creates \c constant_iterator from a \p __value. The index is value initialized
  //! @param __value The value to store in the \c constant_iterator
  _CCCL_EXEC_CHECK_DISABLE
  _LIBCUDACXX_HIDE_FROM_ABI constexpr explicit constant_iterator(_Tp __value) noexcept(
  ...
  _CCCL_EXEC_CHECK_DISABLE
  _LIBCUDACXX_HIDE_FROM_ABI constexpr explicit constant_iterator(_Tp __value, difference_type __index) noexcept(
    _CUDA_VSTD::is_nothrow_move_constructible_v<_Tp>)
      : __value_(_CUDA_VSTD::in_place, _CUDA_VSTD::move(__value))
      , __index_(__index)
  {}
  ...
  //! @brief Returns a const reference to the stored value
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI constexpr const _Tp& operator[](difference_type) const noexcept
  {
    return *__value_;
  }

  //! @brief Increments the stored index
  _LIBCUDACXX_HIDE_FROM_ABI constexpr constant_iterator& operator++() noexcept
  {
    ++__index_;
    return *this;
  }
  ...
  //! @brief Increments the stored index
  _CCCL_EXEC_CHECK_DISABLE
  _LIBCUDACXX_HIDE_FROM_ABI constexpr constant_iterator
  operator++(int) noexcept(_CUDA_VSTD::is_nothrow_copy_constructible_v<_Tp>)
  {
    auto __tmp = *this;
    ++*this;
    return __tmp;
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr constant_iterator& operator--() noexcept
  {
    _CCCL_ASSERT(__index_ > 0, "The value of bound must be greater than or equal to 0");
    --__index_;
    return *this;
  }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI constexpr constant_iterator& operator+=(difference_type __n) noexcept
  {
    _CCCL_ASSERT(__index_ + __n >= 0, "The value of bound must be greater than or equal to 0");
  ...
  //! @brief Compares a \c constant_iterator against default_sentinel_t. True if index is zero
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool
  operator==(const constant_iterator& __lhs, cuda::std::default_sentinel_t) noexcept
  {
    return __lhs.__index_ == 0;
  }

#if _CCCL_STD_VER <= 2017
  //! @brief Compares a \c constant_iterator against default_sentinel_t for equality. True if index() is zero
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool
  operator==(cuda::std::default_sentinel_t, const constant_iterator& __rhs) noexcept
  {
    return __rhs.__index_ == 0;
  }
  //! @brief Compares a \c constant_iterator against default_sentinel_t for inequality. True if index() is not zero
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr bool
  operator!=(const constant_iterator& __lhs, cuda::std::default_sentinel_t) noexcept
  {
    return __lhs.__index_ != 0;
  ...
  //! @param __n The amount of elements to advance
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr constant_iterator
  ...
  //! @brief Returns the distance between a \c constant_iterator a \p default_sentinel
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr difference_type
  operator-(const constant_iterator& __lhs, _CUDA_VSTD::default_sentinel_t) noexcept
  {
  ...
  //! @brief Returns the distance between a \c constant_iterator a \p default_sentinel
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI friend constexpr difference_type
  operator-(_CUDA_VSTD::default_sentinel_t, const constant_iterator& __rhs) noexcept
  {
...
make_constant_iterator(_Tp __value, _CUDA_VSTD::ptrdiff_t __index = 0)
{

...
_CCCL_CONCEPT __is_bijection = _CCCL_REQUIRES_EXPR((_Bijection), const _Bijection& __fun)(
  ...
  requires(_CUDA_VSTD::is_integral_v<typename _Bijection::index_type>),
  requires(_CUDA_VSTD::is_same_v<decltype(__fun.size()), typename _Bijection::index_type>),
  requires(_CUDA_VSTD::is_same_v<decltype(__fun(typename _Bijection::index_type(0))), typename _Bijection::index_type>));
...
//! @endcode
template <class _IndexType = _CUDA_VSTD::size_t, class _Bijection = random_bijection<_IndexType>>
...
private:
  ...
  static_assert(_CUDA_VSTD::is_integral_v<_IndexType>, "_IndexType must be an integral type");
...
public:
  ...
  using difference_type   = _CUDA_VSTD::make_signed_t<value_type>;
  ...
  _CCCL_API constexpr shuffle_iterator(_Bijection __bijection, value_type __start = 0) noexcept(
    _CUDA_VSTD::is_nothrow_move_constructible_v<_Bijection>)
      : __bijection_(_CUDA_VSTD::move(__bijection))
      , __current_(__start)
  {}
  ...
  _CCCL_API constexpr shuffle_iterator operator++(int) noexcept(_CUDA_VSTD::is_nothrow_copy_constructible_v<_Bijection>)
  {
    auto __tmp = *this;
    ++__current_;
    return __tmp;
  ...
  operator--(int) noexcept(_CUDA_VSTD::is_nothrow_copy_constructible_v<_Bijection>)
  {
    auto __tmp = *this;
    --__current_;
    return __tmp;
...
_CCCL_REQUIRES(__is_bijection<_Bijection> _CCCL_AND _CUDA_VSTD::is_integral_v<_Integral>)
...
{
  return shuffle_iterator<_IndexType, _Bijection>{__num_elements, _CUDA_VSTD::move(__fun)};
}

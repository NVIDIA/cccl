...
//! @brief `__equal_to_value` is a function object that checks if a value is equal to a stored value.
...
//! @brief `__equal_to_default` is a function object that checks if a value is equal to a default constructed `_Tp`
//! a specified value.
//! @tparam _Tp The type of the value to be compared.
template <typename _Tp>
struct __equal_to_default
{
  _CCCL_TEMPLATE(class _Up)
  _CCCL_REQUIRES(::cuda::std::__is_cpp17_equality_comparable_v<_Tp, _Up>)
  [[nodiscard]] _CCCL_API constexpr bool operator()(const _Up& __lhs) const
    noexcept(::cuda::std::__is_cpp17_nothrow_equality_comparable_v<_Tp, _Up>)
  {
    return static_cast<bool>(__lhs == _Tp{});
  }
};

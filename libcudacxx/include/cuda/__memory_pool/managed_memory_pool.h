...
struct managed_memory_pool : managed_memory_pool_ref
{
  ...
  [[nodiscard]] _CCCL_HOST_API managed_memory_pool_ref as_ref() const noexcept
  {
  ...
  //! @brief Returns a \c resource_ref for this \c managed_memory_pool.
  //! The result is the same as if this object was cast to a \c managed_memory_pool_ref and then converted to a
  //! \c resource_ref.
  _CCCL_TEMPLATE(class... _Properties)
  _CCCL_REQUIRES(__valid_execution_spaces<_Properties...>)
  [[nodiscard]] _CCCL_HOST_API operator ::cuda::mr::resource_ref<_Properties...>() const noexcept
  {

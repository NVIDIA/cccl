...
// This one site has to stay a preprocessor decision rather than target dispatch:
// cuda::atomic_ref<T> with sizeof(T) == 16 has no valid host instantiation (its host
// backend static_asserts on sizeof(T) > 8), and a target-dispatch macro compiles both
// arms on compilers that build host and device in a single pass. _CCCL_PTX_ARCH() is 0
// outside a device-only pass, so those builds get the named-error path instead.
...
#      else // ^^^ _CCCL_PTX_ARCH() >= 900 ^^^ / vvv _CCCL_PTX_ARCH() < 900 vvv
  // Pre-Hopper has no 128-bit compare-exchange: leave the named symbol undefined so
  // the failure is a link error stating the requirement, not a silent no-op.
  (void) __address_hi;
  (void) __address_lo;
  (void) __addition_hi;
  (void) __addition_lo;

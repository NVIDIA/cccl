...
_CCCL_DEVICE_API fpemu<double, _Acc> operator*(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_high_dmul_rn(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::mid)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_mid_dmul_rn(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::low)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_low_dmul_rn(__x.bits, __y.bits));
  }
  else
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_rn(__x.bits, __y.bits));
  }
} // operator*
...
_CCCL_API fpemu<double, _Acc> __dmul_rz(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_rz(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::mid)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_mid_dmul_rz(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::low)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_low_dmul_rz(__x.bits, __y.bits));
  }
  else
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_rz(__x.bits, __y.bits));
...
_CCCL_API fpemu<double, _Acc> __dmul_ru(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_ru(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::mid)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_mid_dmul_ru(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::low)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_low_dmul_ru(__x.bits, __y.bits));
  }
  else
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_ru(__x.bits, __y.bits));
...
_CCCL_API fpemu<double, _Acc> __dmul_rd(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_rd(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::mid)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_mid_dmul_rd(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::low)
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_low_dmul_rd(__x.bits, __y.bits));
  }
  else
  {
    return fpemu<double, _Acc>(__fpbits64_construct, __fp64emu_dmul_rd(__x.bits, __y.bits));
...
operator*(const fpemu_unpacked<double, _Acc>& __x, const fpemu_unpacked<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return fpemu_unpacked<double, _Acc>(__fpbits64_construct, __fp64emu_unpacked_high_dmul(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::mid)
  {
    return fpemu_unpacked<double, _Acc>(__fpbits64_construct, __fp64emu_unpacked_mid_dmul(__x.bits, __y.bits));
  }
  else if constexpr (_Acc == fpemu_accuracy::low)
  {
    return fpemu_unpacked<double, _Acc>(__fpbits64_construct, __fp64emu_unpacked_low_dmul(__x.bits, __y.bits));
  }
  else
  {
    return fpemu_unpacked<double, _Acc>(__fpbits64_construct, __fp64emu_unpacked_dmul(__x.bits, __y.bits));
  }
} // operator*

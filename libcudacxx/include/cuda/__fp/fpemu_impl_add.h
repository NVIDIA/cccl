...
_CCCL_TRIVIAL_HOST_DEVICE_API __fpbits64 __internal_fp64emu_high_dadd(__fpbits64 __x, __fpbits64 __y) noexcept
{
  uint64_t __a        = __x;
  uint64_t __b        = __y;
...
template <__fpemu_rounding _Rm = __fpemu_rounding::def, fpemu_accuracy _Acc = fpemu_accuracy::def, bool _IsSub = false>
_CCCL_TRIVIAL_HOST_DEVICE_API __fpbits64 __internal_fp64emu_mid_dadd(__fpbits64 __x, __fpbits64 __y) noexcept
{
  __uint32x2 __a_32x2            = ::cuda::std::bit_cast<__uint32x2>(__x);
  __uint32x2 __b_32x2            = ::cuda::std::bit_cast<__uint32x2>(__y);
  constexpr int32_t __extra_bits = (_Acc == fpemu_accuracy::low) ? 0 : _CCCL_FP64EMU_DADD_V2_EXTRA_BITS;
  ...
  // Find maximum from input exponents
  int32_t __exp_c = static_cast<int32_t>((__exp_a > __exp_b) ? __exp_a : __exp_b);
  // Find shifts to equlize mantissa A
  int32_t __delta_a = __exp_c - static_cast<int32_t>(__exp_a);
  // Find shifts to equlize mantissa B
  ...
  int32_t __nzeros = ::cuda::std::countl_zero(::cuda::std::bit_cast<uint64_t>(__c_32x2));
  ...
  {
    __exp_c  = 0;
    __c_32x2 = __zero_32x2;
  }
  // Check for finite exponent then set exponent to zero
  if ((__exp_c < 0x000007ff) && (__nzeros == 64))
  {
    __exp_c = 0;
  }
...
// Branch by carry bit to handle exponent overflow/underflow
  ...
  // Carry bit handling
  uint32_t __carry_bit = (__c_32x2.x[1] & (1 << (20 + 1 + __extra_bits))) != 0;
...
#  endif
  ...
  // Final result
  __fpbits64 __result = ::cuda::std::bit_cast<__fpbits64>(__c_32x2);
  return __result;
...
//! accuracy, range, and engine template parameters.
template <__fpemu_rounding _Rm = __fpemu_rounding::def, fpemu_accuracy _Acc = fpemu_accuracy::def, bool _IsSub = false>
_CCCL_TRIVIAL_HOST_DEVICE_API __fpbits64 __internal_fp64emu_low_dadd(__fpbits64 __x, __fpbits64 __y) noexcept
{
  __uint32x2 __a_32x2 = ::cuda::std::bit_cast<__uint32x2>(__x);
  __uint32x2 __b_32x2 = ::cuda::std::bit_cast<__uint32x2>(__y);
  __fpbits64 __result;
  ...
  int32_t __max_exp  = static_cast<int32_t>((__exp_a > __exp_b) ? __exp_a : __exp_b);
  int32_t __exp_diff = __max_exp - static_cast<int32_t>((__exp_a > __exp_b) ? __exp_b : __exp_a);
  ...
  if (__exp_a < __exp_b)
  {
    __exp_a_sp = __exp_a_sp - __exp_diff;
  }
  if (__exp_b < __exp_a)
  {
    __exp_b_sp = __exp_b_sp - __exp_diff;
  }
...
__internal_fp64emu_dadd_unpacked(__fpbits64_unpacked __a, __fpbits64_unpacked __b) noexcept
{
  ...
  if constexpr (__acc_used == fpemu_accuracy::high || __acc_used == fpemu_accuracy::mid)
  {
    ...
    __uint32x2 __man_a = ::cuda::std::bit_cast<__uint32x2>(__a.mantissa);
    __uint32x2 __man_b = ::cuda::std::bit_cast<__uint32x2>(__b.mantissa);
    ...
    int32_t __nz = ::cuda::std::countl_zero(::cuda::std::bit_cast<uint64_t>(__man_c));
    if (__nz >= 64)
    {
      __is_sign_c = (__is_sign_a && __is_sign_b);
    }
    ...
    uint64_t __m64   = ::cuda::std::bit_cast<uint64_t>(__man_c);
    uint64_t __norm  = __m64 << (__nz - 1); // leading bit -> 62
    uint64_t __res64 = __norm >> 1; // -> 61 (universal scale)
    if constexpr (__acc_used == fpemu_accuracy::high)
    {
      __res64 |= (__norm & 1ULL); // carry-case sticky (else 0)
    }
    __uint32x2 __res_x2 = ::cuda::std::bit_cast<__uint32x2>(__res64);
    ...
    __r.mantissa = ::cuda::std::bit_cast<uint64_t>(__res_x2);
  ...
  else
  {
    ...
    if (__exp_a < __exp_b)
    {
      __exp_a_sp -= __exp_diff;
    }
    if (__exp_b < __exp_a)
    {
      __exp_b_sp -= __exp_diff;
    }

    // Flush zero operands and underflowed alignments to +/-0.0f.
    if (__exp_a_sp < 1 || __a.mantissa == 0)
    {
      __exp_a_sp  = 0;
      __mant_a_sp = 0;
    }
    if (__exp_b_sp < 1 || __b.mantissa == 0)
    {
      __exp_b_sp  = 0;
      __mant_b_sp = 0;
    }
...
//! @return       Result of addition in packed form (__fpbits64)
template <__fpemu_rounding _Rm = __fpemu_rounding::def, fpemu_accuracy _Acc = fpemu_accuracy::def, bool _IsSub = false>
_CCCL_TRIVIAL_HOST_DEVICE_API __fpbits64 __internal_fp64emu_dadd(__fpbits64 __x, __fpbits64 __y) noexcept
{
...
_CCCL_FPEMU_BUILTIN_DECL __fpbits64_unpacked
__fp64emu_unpacked_dadd(__fpbits64_unpacked x, __fpbits64_unpacked y) noexcept;
_CCCL_FPEMU_BUILTIN_DECL __fpbits64_unpacked
__fp64emu_unpacked_high_dadd(__fpbits64_unpacked x, __fpbits64_unpacked y) noexcept;
_CCCL_FPEMU_BUILTIN_DECL __fpbits64_unpacked
__fp64emu_unpacked_mid_dadd(__fpbits64_unpacked x, __fpbits64_unpacked y) noexcept;
_CCCL_FPEMU_BUILTIN_DECL __fpbits64_unpacked
...
template <fpemu_accuracy _Acc>
_CCCL_HOST_DEVICE_API fpemu<double, _Acc>
__dadd_rn(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
...
template <fpemu_accuracy _Acc>
_CCCL_HOST_DEVICE_API fpemu<double, _Acc>
__dadd_rz(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return ::cuda::std::bit_cast<fpemu<double, _Acc>>(
...
template <fpemu_accuracy _Acc>
_CCCL_HOST_DEVICE_API fpemu<double, _Acc>
__dadd_ru(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return ::cuda::std::bit_cast<fpemu<double, _Acc>>(
...
template <fpemu_accuracy _Acc>
_CCCL_HOST_DEVICE_API fpemu<double, _Acc>
__dadd_rd(const fpemu<double, _Acc>& __x, const fpemu<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
    return ::cuda::std::bit_cast<fpemu<double, _Acc>>(
...
template <fpemu_accuracy _Acc>
_CCCL_HOST_DEVICE_API fpemu_unpacked<double, _Acc>
__dadd_rn(const fpemu_unpacked<double, _Acc>& __x, const fpemu_unpacked<double, _Acc>& __y) noexcept
{
  if constexpr (_Acc == fpemu_accuracy::high)
  {
...
_CCCL_HOST_DEVICE_API __fpemu_pick_t<_T1, _T2> __dadd_rn(const _T1& __x, const _T2& __y) noexcept
{
...
_CCCL_TEMPLATE(class _T1, class _T2)
_CCCL_REQUIRES(__fpemu_mixed_v<_T1, _T2>)
_CCCL_HOST_DEVICE_API __fpemu_pick_t<_T1, _T2> __dadd_rz(const _T1& __x, const _T2& __y) noexcept
{
  using _Fp = __fpemu_pick_t<_T1, _T2>;
...
_CCCL_TEMPLATE(class _T1, class _T2)
_CCCL_REQUIRES(__fpemu_mixed_v<_T1, _T2>)
_CCCL_HOST_DEVICE_API __fpemu_pick_t<_T1, _T2> __dadd_ru(const _T1& __x, const _T2& __y) noexcept
{
  using _Fp = __fpemu_pick_t<_T1, _T2>;
...
_CCCL_TEMPLATE(class _T1, class _T2)
_CCCL_REQUIRES(__fpemu_mixed_v<_T1, _T2>)
_CCCL_HOST_DEVICE_API __fpemu_pick_t<_T1, _T2> __dadd_rd(const _T1& __x, const _T2& __y) noexcept
{
  using _Fp = __fpemu_pick_t<_T1, _T2>;

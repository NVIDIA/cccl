...
public:
  ...
  _LIBCUDACXX_HIDE_FROM_ABI ~barrier()
  {
    NV_DISPATCH_TARGET(
      NV_PROVIDES_SM_90,
      (
        if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, &__barrier)) {
          asm volatile("mbarrier.inval.shared.b64 [%0];" ::"r"(static_cast<_CUDA_VSTD::uint32_t>(
            ::__cvta_generic_to_shared(&__barrier)))
                       : "memory");
        } else if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::cluster_shared, &__barrier)) {
          ::__trap();
        }),
      NV_PROVIDES_SM_80,
      (if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, &__barrier)) {
    ...
    barrier* __b, _CUDA_VSTD::ptrdiff_t __expected, _CUDA_VSTD::__empty_completion = _CUDA_VSTD::__empty_completion())
  {
    NV_DISPATCH_TARGET(
      NV_PROVIDES_SM_90,
      (
        if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, &__b->__barrier)) {
          asm volatile("mbarrier.init.shared.b64 [%0], %1;" ::"r"(
                       ...
                       : "memory");
        } else if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::cluster_shared, &__b->__barrier)) {
          ::__trap();
  ...
  [[nodiscard]] _LIBCUDACXX_HIDE_FROM_ABI arrival_token arrive(_CUDA_VSTD::ptrdiff_t __update = 1)
  {
    ...
    NV_DISPATCH_TARGET(
      NV_PROVIDES_SM_90,
      (
        if (!_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::cluster_shared, &__barrier)) {
          return __barrier.arrive(__update);
        } else if (!_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, &__barrier)) { ::__trap(); }
  ...
  _LIBCUDACXX_HIDE_FROM_ABI bool __try_wait([[maybe_unused]] arrival_token __token) const
  {
    NV_DISPATCH_TARGET(
      ...
      (
        ...
        } else if (!_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, &__barrier)) {
          ::__trap();
        } asm volatile("{\n\t"
                       ".reg .pred p;\n\t"

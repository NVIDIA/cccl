...
#    endif
  ...
  _CCCL_ASSERT(_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, __dest),
               "dest must point to shared memory.");
  _CCCL_ASSERT(_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::global, __src),
               "src must point to global memory.");

  NV_IF_ELSE_TARGET(
    NV_PROVIDES_SM_90,
    (
      if (_CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::shared, __dest)
          && _CUDA_DEVICE::is_address_from(_CUDA_DEVICE::address_space::global, __src)) {

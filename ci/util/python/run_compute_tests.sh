...
#
# Runs either in the CCCL devcontainer or, for lanes with `devcontainer: false`,
# inside the minimal container launched by run_in_minimal_container.sh. Anything
# added here must work with nothing but Python and the wheel's declared pip
# dependencies -- no host compiler, no system CUDA toolkit. The cuda_cccl wheel
# is already in wheelhouse/; the caller put it there.

...
# outer build resolves this before entering Docker so Git worktrees also work;
# retain a fallback for direct invocations from a regular checkout.
...
if [[ "${BUILD_CUDA_CCCL_METAPACKAGE:-1}" =~ ^(1|true|TRUE|on|ON)$ ]]; then
  rm -rf /workspace/python/cuda_cccl_meta/dist
  CMAKE_ARGS="" python -m pip wheel \
    --no-deps \
    --verbose \
    --wheel-dir /workspace/python/cuda_cccl_meta/dist \
    /workspace/python/cuda_cccl_meta
  mv /workspace/python/cuda_cccl_meta/dist/cuda_cccl-*.whl /workspace/wheelhouse/
fi
...
# copy is created under python/cuda_cccl.

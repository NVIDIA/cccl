  ...
  complex max)
{
  switch (entropy)
  {
    case bit_entropy::_1_000: {
      const double* uniform_distribution = dist.new_uniform_distribution(seed, span.size());
      thrust::for_each_n(
        exec, ::cuda::make_counting_iterator(0), span.size(), set_real_t{min, max, span.data(), uniform_distribution});
      ++seed;

      uniform_distribution = dist.new_uniform_distribution(seed, span.size());
      thrust::for_each_n(
        exec, ::cuda::make_counting_iterator(0), span.size(), set_imag_t{min, max, span.data(), uniform_distribution});
      ++seed;
      return;
    }
    case bit_entropy::_0_000: {

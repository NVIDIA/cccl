...
int main(int, char**)
{
  ...
  // this builtin has different diagnostic messages than the fallback implementation.
    ...
    MakeSeqT i; // expected-error@__utility/integer_sequence.h:* {{integer sequences must have non-negative sequence length}}
#else
    MakeSeqT i; // expected-error@__utility/integer_sequence.h:* {{{{(static_assert|static assertion)}} failed "std::make_integer_sequence must have a non-negative sequence length"}}
#endif

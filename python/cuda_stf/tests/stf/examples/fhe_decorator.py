...
# Threads-per-block; the launch grid is sized from the operand length so the
# circuit works for arbitrary-length ciphertexts, not only the demo vectors.
...
class Ciphertext:
    ...
    def __add__(self, other):
        ...
        add_kernel[blocks, tpb](self.l.read(), other.l.read(), result.l.write())
        return result
    ...
    def __sub__(self, other):
        ...
        result = self.empty_like()
        blocks, tpb = _launch_grid(self.size)
        sub_kernel[blocks, tpb](self.l.read(), other.l.read(), result.l.write())
        return result
    ...
    def decrypt(self, num_operands=2):
        ...
        blocks, tpb = _launch_grid(self.size)
        sub_scalar_kernel[blocks, tpb](self.l.read(), result.l.write(), total_key)
...
def _run_fhe_decorator(vA=None, vB=None):
...
def test_fhe_decorator_arbitrary_lengths(monkeypatch, length):
    """The decorator circuit must work for lengths beyond a single block."""
    monkeypatch.setattr(numba.cuda.config, "CUDA_LOW_OCCUPANCY_WARNINGS", 0)
    rng = random.Random(length)
    vA = [rng.randint(0, 255) for _ in range(length)]
    vB = [rng.randint(0, 255) for _ in range(length)]
    _run_fhe_decorator(vA, vB)


def test_fhe_decorator_rejects_mismatched_operands(monkeypatch):

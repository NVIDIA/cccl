...
def _make_extracted_wheel(root: Path, cuda_major: int, shared: str) -> Path:
    ...
    (root / "cuda/compute/_cccl/clang").mkdir(parents=True)
    (root / "cuda/compute/_cccl/clang/header.h").write_text(shared)
...
def test_divergent_shared_payload_is_rejected(tmp_path: Path) -> None:
    ...
    divergent_header = cu13 / "cuda/compute/_cccl/clang/header.h"
    ...
    with pytest.raises(
        ...
        match=(
            ...
            r"\(missing=\[\], extra=\[\], "
            r"changed=\['cuda/compute/_cccl/clang/header.h'\]\)"
        ),
    ):
        merger._validate_shared_contents(
            [cu12, cu13],

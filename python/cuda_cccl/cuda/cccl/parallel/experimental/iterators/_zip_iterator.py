...
from ._iterators import IteratorBase, IteratorIOKind, IteratorKind, pointer
...
def make_zip_iterator(*iterators):
    ...
    n_iterators = len(processed_iterators)

    # ctypes struct for the combined state:
    fields = [
        (f"iter_{i}", it.cvalue.__class__) for i, it in enumerate(processed_iterators)
    ]
    ZipStateStruct = type("ZipStateStruct", (ctypes.Structure,), {"_fields_": fields})
    combined_cvalue = ZipStateStruct(
        **{f"iter_{i}": it.cvalue for i, it in enumerate(processed_iterators)}
    )

    # this iterator's state is a struct composed of the states of the input iterators:
    ZipState = gpu_struct_from_numba_types(
        "ZipState", [it.state_type for it in processed_iterators]
    )
    ...
    ZipValue = gpu_struct_from_numba_types(
        "ZipValue", [it.value_type for it in processed_iterators]
    )
...
"""
        ...
        field_ptr_functions[field_idx] = globals()[func_name]

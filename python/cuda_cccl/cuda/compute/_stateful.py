...
class _AddStateParameters(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> ast.FunctionDef:
        # Add state parameters to the function arguments
...
def _transform_function_ast(func: Callable, state_names: List[str]) -> Callable:
    """
    Transform a function to add device array state as explicit parameters.

    Uses AST transformation to modify the function's signature, adding
    state array names as additional parameters. This allows the transformed
    function to be compiled by numba-cuda.

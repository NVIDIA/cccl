...
def test_reduce_nested_struct_direct():
    ...
    reducer = cuda.compute.make_reduce_into(
        d_input, d_output, sum_nested, h_init
    )
    ...
    reducer.compute(
        d_temp_storage, d_input, d_output, num_items, h_init=h_init
    )
...
def test_nested_struct_in_zip_iterator():
    ...
    reducer = cuda.compute.make_reduce_into(
        zip_it, d_output, sum_pixels, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        zip_it, d_output, num_items, h_init=h_init
    )
    d_temp_storage = cp.empty(temp_storage_bytes, dtype=np.uint8)
    reducer.compute(
        d_temp_storage, zip_it, d_output, num_items, h_init=h_init
    )
...
def test_nested_struct_tuple_construction():
    ...
    reducer = cuda.compute.make_reduce_into(
        d_input, d_output, sum_nested_with_tuples, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        d_input, d_output, num_items, h_init=h_init
    )
...
def test_deeply_nested_tuple_construction():
    ...
    reducer = cuda.compute.make_reduce_into(
        d_input, d_output, sum_deeply_nested, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        d_input, d_output, num_items, h_init=h_init
    )
...
def test_mixed_tuple_and_direct_construction():
    ...
    reducer = cuda.compute.make_reduce_into(
        d_input, d_output, sum_mixed, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        d_input, d_output, num_items, h_init=h_init
    )
...
def test_tuple_construction_in_zip_iterator():
    ...
    reducer = cuda.compute.make_reduce_into(
        zip_it, d_output, sum_pixels_with_tuples, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        zip_it, d_output, num_items, h_init=h_init
    )
...
def test_all_tuple_construction():
    ...
    reducer = cuda.compute.make_reduce_into(
        d_input, d_output, sum_all_tuples, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        d_input, d_output, num_items, h_init=h_init
    )

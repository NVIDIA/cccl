...
def test_nested_zip_iterators():
    ...
    InnerPair = gpu_struct({"first": np.int32, "second": np.int32})
    ...
    d_input_b = cp.arange(num_items, dtype=np.int32) * 2
...
def test_dict_init_flat_struct():
    """Test initializing a flat struct with a dictionary."""
    Point = gpu_struct({"x": np.int32, "y": np.int32})
    ...
    assert p.x == 10
    assert p.y == 20
    assert p._data[0]["x"] == 10
    assert p._data[0]["y"] == 20

...
temp_storage_bytes = selector.get_temp_storage_bytes(
    d_in, d_out, d_num_selected, len(d_in), cond=greater_than_5
)
d_temp_storage = cp.empty(temp_storage_bytes, dtype=cp.uint8)

# Execute select
selector.compute(d_temp_storage, d_in, d_out, d_num_selected, len(d_in), cond=greater_than_5)
...
selector.compute(
    d_temp_storage, d_in2, d_out2, d_num_selected2, len(d_in2), cond=greater_than_5
)

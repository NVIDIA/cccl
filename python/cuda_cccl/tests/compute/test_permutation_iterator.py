...
def test_permutation_iterator_with_array_values():
    ...
    reducer = cuda.compute.make_reduce_into(
        perm_it, d_output, cuda.compute.OpKind.PLUS, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        perm_it, d_output, len(indices), h_init=h_init
    )
    d_temp_storage = cp.empty(temp_storage_bytes, dtype=np.uint8)
    reducer.compute(
        d_temp_storage, perm_it, d_output, len(indices), h_init=h_init
    )
...
def test_permutation_iterator_of_zip_iterator():
    ...
    reducer = cuda.compute.make_reduce_into(
        perm_it, d_output, sum_both_fields, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(
        perm_it, d_output, len(indices), h_init=h_init
    )
...
def test_zip_iterator_of_permutation_iterators():
    ...
    reducer = cuda.compute.make_reduce_into(
        zip_it, d_output, sum_both_fields, h_init
    )
    temp_storage_bytes = reducer.get_temp_storage_bytes(

...
def test_select_stateful_threshold():
    """Test stateful select that uses state for threshold (auto-captured)."""
    num_items = 1000
    ...
    # Define condition that references state as closure - auto-captured
    ...
    cuda.compute.select(
        ...
        d_out,
        d_num_selected,
        threshold_select,  # Pass function directly - auto-capture handles state
        num_items,
    )
...
def test_select_stateful_atomic():
    """Test stateful select with atomic operations to count rejected items (auto-captured)."""
    ...
    cuda.compute.select(
        ...
        d_out,
        d_num_selected,
        count_rejects,  # Pass function directly - auto-capture handles state
        num_items,
    )
...
def test_select_stateful_counting_rejects():
    """Test select that counts rejected items using auto-captured state."""
    from numba import cuda as numba_cuda

...
@pytest.mark.parametrize("T", [types.int8, types.int16, types.uint32, types.uint64])
@pytest.mark.parametrize("items_per_thread", [1, 3])
def test_warp_merge_sort_single_phase(T, items_per_thread):
...
def test_warp_merge_sort_multiple_warps_single_phase():
    T = types.int32
    items_per_thread = 3
    block_threads = 1024
    warp_threads = 32
...
def test_warp_merge_sort_temp_storage_single_phase():
    T = types.int32
    items_per_thread = 2
    threads_in_warp = 32
...
def test_warp_merge_sort_pairs_single_phase():
    T = types.int32
    items_per_thread = 4
    threads_in_warp = 32
    ...
    items_per_tile = threads_in_warp * items_per_thread
    h_keys = random_int(items_per_tile, dtype)
    h_values = (h_keys * 5 + 3).astype(dtype)
    d_keys = cuda.to_device(h_keys)
    d_values = cuda.to_device(h_values)
    d_keys_out = cuda.device_array(items_per_tile, dtype=dtype)
    d_values_out = cuda.device_array(items_per_tile, dtype=dtype)
    ...
    expected = sorted(zip(h_keys, h_values), key=lambda kv: kv[0], reverse=True)
    exp_keys = [kv[0] for kv in expected]
    exp_values = [kv[1] for kv in expected]

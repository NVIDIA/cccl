...
"""Thread-group reduce example for cuda.coop.cutlass."""
...
def make_runner() -> tuple[Any, Any, Any, Any]:
    ...
    # docs: start cutlass-block-sum
    @cute.kernel
    def _thread_group_reduce_kernel(
        values: cute.Tensor,
        block_sum_out: cute.Tensor,
        block_items_sum_out: cute.Tensor,
        warp_max_out: cute.Tensor,
    ):
        tidx_x, tidx_y, tidx_z = cute.arch.thread_idx()
        tidx = tidx_x + BLOCK_DIM[0] * (tidx_y + BLOCK_DIM[1] * tidx_z)

        # The group descriptors recover their static hierarchy from the CuTe
        # kernel's launch attributes when the reduction is lowered.
    ...
    # docs: end cutlass-block-sum

    @cute.jit
    def _run_thread_group_reduce(
        values: cute.Tensor,
        block_sum_out: cute.Tensor,
        block_items_sum_out: cute.Tensor,
        warp_max_out: cute.Tensor,
    ):
        _thread_group_reduce_kernel(
            values,
            block_sum_out,
            block_items_sum_out,
            warp_max_out,
    ...
    return _run_thread_group_reduce, torch, from_dlpack, cutlass
...
def prepare_example() -> PreparedExample:
    ...
    run_thread_group_reduce, torch, from_dlpack, cutlass = make_runner()
...
def run_example() -> dict[str, Any]:
    """Run the thread-group reduce example and validate the result."""

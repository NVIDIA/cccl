...
"""Final-link evidence for common CUTLASS Radix Rank."""
...
@pytest.mark.evidence_for("group.radix_rank", backend="cutlass", evidence="link")
def test_common_and_qualified_radix_rank_wrappers_are_eliminated(
    ...
    _run_example_subprocess("_common_root_radix_rank_codegen_probe")
    ...
    provider_symbols = tuple(
        sorted(
            set(
                re.findall(
                    r"\b(cuda_coop_cutlass_radix_rank_[A-Za-z0-9_]+)\(",
                    provider_source,
                )
            )
        )
    ...
    assert "^ 0x80000000u" in provider_source

...
"""Final-link evidence for common-root CUTLASS Histogram."""
...
@pytest.mark.evidence_for("group.histogram", backend="cutlass", evidence="link")
def test_common_and_qualified_histogram_wrappers_are_eliminated(
    ...
    _run_example_subprocess("_group_histogram_codegen_probe")
    ...
    provider_symbols = tuple(
        sorted(
            set(
                re.findall(
                    r"\b(cuda_coop_cutlass_cub_histogram_[A-Za-z0-9_]+)\(",
                    provider_source,
                )
            )
        )

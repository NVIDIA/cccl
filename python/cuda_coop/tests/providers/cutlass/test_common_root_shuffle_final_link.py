...
"""Final-link evidence for common-root CUTLASS Shuffle."""
...
@pytest.mark.evidence_for("group.shuffle", backend="cutlass", evidence="link")
def test_common_and_qualified_shuffle_wrappers_are_eliminated(
    ...
    _run_example_subprocess("_group_shuffle_codegen_probe")
    ...
    provider_symbols = tuple(
        sorted(
            set(
                re.findall(
                    r"\b(cuda_coop_cutlass_shuffle_[A-Za-z0-9_]+)\(",
                    provider_source,
                )
            )
        )

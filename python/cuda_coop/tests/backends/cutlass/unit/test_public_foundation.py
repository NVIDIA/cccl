...
def test_private_dsl_package_stays_lazy():
    script = textwrap.dedent(
        """
        import sys
        ...
        assert dsl.__all__ == ()
        assert "cuda.coop.cutlass._dsl._provider" not in sys.modules
        assert "cuda.coop.cutlass._dsl._provider_bundle" not in sys.modules
        assert "cuda.coop.cutlass._dsl._provider_finalizer" not in sys.modules
        """
    )

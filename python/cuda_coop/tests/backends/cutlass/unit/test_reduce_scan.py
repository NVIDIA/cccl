...
def test_public_reduce_scan_exports_and_frontends(monkeypatch) -> None:
    ...
    from cuda.coop.cutlass._dsl import _cub_scan_provider as scan_provider
    from cuda.coop.cutlass._dsl import _cudax_reduce_provider as reduce_provider
...
def test_reduce_plans_cover_cudax_mappings_and_direct_cub() -> None:
    ...
    from cuda.coop._core.block import BlockReduceAlgorithm
    from cuda.coop.cutlass import _group_reduce as frontend
    ...
    plans = []
    for group in groups:
        plan = frontend._make_group_reduce_plan(
            group=group,
            launch=launch,
            dtype=Int32,
    ...
    array_plan = frontend._make_group_reduce_plan(
...
def test_scan_plans_render_aggregate_storage_and_distinct_widths(monkeypatch) -> None:
    ...
    from cuda.coop.cutlass import _group_scan as frontend
    ...
    def logical_request(width: int):
        plan = frontend._make_group_scan_plan(
            group=coop.this_warp().group_by(width),
            launch=launch,
            dtype=Int32,
    ...
    for value_type, zero_literal in (
        ...
        partial_exclusive_plan = frontend._make_group_scan_plan(
    ...
    for feature in ("reduce", "scan"):
        for op in ("bit_and", "bit_or", "bit_xor"):
            provider_support.validate_scan_reduce_op_for_type(
                op,
                Uint8,
    ...
    block_plan = frontend._make_group_scan_plan(
        group=coop.this_block(),
        launch=launch,
        dtype=Int32,

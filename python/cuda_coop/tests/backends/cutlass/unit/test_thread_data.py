...
def test_thread_data_from_vector_like_payload():
    script = textwrap.dedent(
        ...
        assert items.values("test") == ("v0", "v1", "v2")
        """
    )
...
def test_thread_data_from_payload_adapts_backend_payloads():
    script = textwrap.dedent(
        ...
        assert from_vector.values("vector") == ("v0", "v1", "v2")
        ...
        assert_raises(
            TypeError,
            "a scoped load for cutlass.Array values",
        ...
        direct_register = coop._block.radix_sort_keys(
            RegisterTensor(),
            begin_bit=0,
            end_bit=32,
        )
        assert isinstance(direct_register, coop.ThreadData)
        assert direct_register.dtype == "i32"
        assert direct_register.values("direct-register") == ("r0", "r1")

        direct_vector = coop._block.radix_sort_keys(
            Vector(),
            begin_bit=0,
            end_bit=32,
        )
        assert isinstance(direct_vector, coop.ThreadData)
        assert direct_vector.dtype == "i64"
        assert direct_vector.values("direct-vector") == ("v0", "v1", "v2")

        direct_cutlass_vector = coop._block.radix_sort_keys(
            Vector(),
            begin_bit=0,
            end_bit=32,
        )
        assert isinstance(direct_cutlass_vector, coop.ThreadData)
        assert direct_cutlass_vector.dtype == "i64"
        assert direct_cutlass_vector.values("direct-cutlass-vector") == (
            "v0",
            "v1",
            "v2",
        )

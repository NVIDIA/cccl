...
def test_frontends_delegate_resolved_groups_and_plans(monkeypatch):
    ...
    from cuda.coop.cutlass._dsl import _cub_exchange_provider as exchange_provider
    from cuda.coop.cutlass._dsl import _cub_load_store_provider as load_store_provider
    from cuda.coop.cutlass._dsl import _cub_shuffle_provider as shuffle_provider
...
def test_load_store_plans_bindings_rendering_and_layout_proofs():
    ...
    block_plan = frontend._make_group_load_store_plan(
    ...
    warp_plan = frontend._make_group_load_store_plan(
...
def test_exchange_requests_render_block_logical_warp_ranks_and_flags():
    ...
    from cuda.coop.cutlass import _group_exchange as frontend
    from cuda.coop.cutlass._dsl import _cub_exchange_provider as provider

    block_plan = frontend._make_group_exchange_plan(
        group=coop.this_block(),
        launch=LaunchFacts(exact_block_dim=64),
        dtype=Int32,
        items_per_thread=2,
    ...
    assert "::cub::BlockExchange<int, 64, 2, 1, 1, 1>" in block_source
    ...
    logical_plan = frontend._make_group_exchange_plan(
...
def test_exchange_symbols_distinguish_lowering_plans():
    ...
    def request(group, *, warp_time_slicing=False):
        plan = frontend._make_group_exchange_plan(
            group=group,
            launch=LaunchFacts(exact_block_dim=64),
            dtype=Int32,
            items_per_thread=2,
...
def test_deferred_load_store_requests_do_not_collide():
    ...
    plan = frontend._make_group_load_store_plan(
        group=coop.this_block(),
        launch=LaunchFacts(exact_block_dim=64),
        kind=GroupLoadStoreKind.LOAD,
        dtype=Int32,
...
def test_deferred_load_failure_rolls_back_provider_session(monkeypatch):
    ...
    monkeypatch.setattr(
        provider_support, "snapshot_active_session_state", lambda: snapshot
    )
    monkeypatch.setattr(
        provider_support, "restore_active_session_state", restored.append
    )

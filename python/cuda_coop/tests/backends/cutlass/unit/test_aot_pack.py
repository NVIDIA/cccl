...
def provider_bundle(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_provider_bundle, "cuda_nvrtc", _FakeNvrtc())
    ...
    def resolver(request):
        return provider_bundle.BundleResolution(
            ...
            layouts_by_expression={
                expression: provider_bundle.StorageLayout(16, 8)
...
@pytest.mark.evidence_for("aot.pack", backend="cutlass", evidence="semantics")
...
def test_same_identity_and_symbols_reject_conflicting_artifacts(
    ...
    fake_nvrtc = provider_bundle.cuda_nvrtc
...
@pytest.mark.parametrize(
    "kind",
    ["artifact-digest", "artifact-size", "artifact-oversize", "source"],
)
def test_pack_rejects_corrupt_content(tmp_path, provider_bundle, kind):
    output, _ = _capture_pack(tmp_path, provider_bundle)
    payload = _manifest_payload(output)

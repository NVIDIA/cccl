...
def _isolated_state(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(provider_bundle, "cuda_nvrtc", _FakeNvrtc())
...
def test_default_jit_path_does_not_import_or_time_aot_dispatch(monkeypatch):
    ...
    assert observed[0].route == provider_bundle.RESOLUTION_ROUTE_NVRTC
...
@pytest.mark.evidence_for("aot.pack", backend="cutlass", evidence="compile")
def test_required_hit_skips_nvrtc_and_mutable_jit_io(
    ...
    with monkeypatch.context() as hit_patch:
        ...
        hit_patch.setattr(provider_bundle, "ensure_cache_dir", forbidden)
        hit_patch.setattr(provider_bundle, "cuda_nvrtc", forbidden)
        with (
    ...
    assert observed[0].route == provider_bundle.RESOLUTION_ROUTE_AOT_PACK
...
def test_environment_selection_and_explicit_off_override(
    ...
    assert observed[-1].route == provider_bundle.RESOLUTION_ROUTE_AOT_PACK
    ...
    assert observed[-1].route == provider_bundle.RESOLUTION_ROUTE_PRECOMPILED
...
def test_typed_resolver_rejects_a_resolution_from_another_route(tmp_path):
    ...
    resolver = provider_bundle._bundle_precompile_resolver(
        ...
        route=provider_bundle.RESOLUTION_ROUTE_AOT_PACK,
        phase="pack_lookup",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ((13, 0), "required", provider_bundle.RESOLUTION_ROUTE_AOT_PACK),
        ((13, 1), "required", provider_bundle.RESOLUTION_ROUTE_AOT_PACK),
...
def test_nvjitlink_compatibility_matrix(
    ...
    if route_or_error == provider_bundle.RESOLUTION_ROUTE_AOT_PACK:
        with (
            context,
            provider_bundle.activate_bundle_resolution_observer(observed.append),
        ):
...
def test_required_rejects_older_consumer_minor(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(provider_bundle, "cuda_nvrtc", _FakeNvrtc((13, 1)))
...
def test_capture_reexports_aot_hit_and_preserves_producer_version(
    ...
    with aot.capture(second_pack):
        with aot.use(first_pack, mode="required"):
            provider_bundle.compile_bundle_source(
                'extern "C" __device__ void provider_a() {}\n',
                **_compile_kwargs(),
            )
...
def test_capture_rejects_missing_legacy_producer_version(tmp_path):
    ...
    with pytest.raises(aot.CaptureError, match="legacy provider disk cache"):
        with aot.capture(output):
            provider_bundle.compile_bundle_source(
                source,
                **_compile_kwargs(),

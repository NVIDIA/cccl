...
from cuda.coop.cutlass._dsl import _provider as provider
...
def _render(requests, include_lines):
    return provider.render_bundle_source(
        requests,
...
def test_warp_exchange_identity_includes_exact_block_threads():
    from cutlass.base_dsl.typing import Int32

    from cuda.coop.cutlass._dsl.warp import _provider as warp_provider

    requests = tuple(
        warp_provider._warp_exchange_request(
        ...
        for block_threads in (64, 128)
    )

    assert requests[0].symbol_name != requests[1].symbol_name
    assert provider.canonical_bundle_requests(reversed(requests)) == tuple(
        sorted(requests, key=lambda request: request.symbol_name)
    )
...
def test_registered_headers_are_canonical_and_reject_conflicts(monkeypatch):
    ...
    renderer_a = provider.BundleRenderer(
        include_lines=(),
        cccl_headers=(
            ("#include <z.cuh>", "z.cuh"),
            ("#include <a.cuh>", "a.cuh"),
    ...
    renderer_z = provider.BundleRenderer(
        include_lines=(),
    ...
    assert list(provider.registered_bundle_headers()) == [
        "#include <a.cuh>",
        "#include <m.cuh>",
        "#include <z.cuh>",
    ]

    conflicting = provider.BundleRenderer(
        include_lines=(),
        cccl_headers=(("#include <a.cuh>", "different/a.cuh"),),
        render=render,
    )
    monkeypatch.setitem(provider._BUNDLE_RENDERERS, "conflict", conflicting)
    with pytest.raises(ValueError, match="conflicting CCCL headers"):
        provider.registered_bundle_headers()
...
source = provider.render_bundle_source(
    requests,
    scope="cuda.coop.cutlass",
    include_lines=lines,
    render_local_request=lambda request: [
...
"""
    ...
    for seed in ("1", "7", "100"):
        env = os.environ.copy()
        env["CUDA_COOP_SOURCE_PATH"] = str(Path(provider.__file__).resolve().parents[2])
...
def test_bundle_cache_identity_uses_the_full_canonical_contract():
    ...
    compiler_options = provider_bundle.bundle_compiler_options(
        "ltoir",
        "compute_100a",
    )
    identity = provider_bundle.make_bundle_identity(
    ...
    cache_identity = provider_bundle.make_bundle_cache_identity(
        identity,
        include_key="0123456789abcdef",
        producer_compiler_version="13.0",
    )

    assert identity.version == provider_bundle.BUNDLE_IDENTITY_VERSION == 1
    assert (
        identity.provider_abi_version
        == provider_bundle.PROVIDER_BUNDLE_ABI_VERSION
        == 1
    )
    ...
    assert (
        ...
        == provider_bundle.BUNDLE_CACHE_SCHEMA_VERSION
        == 3
    )
    expected_payload = {
        "bundle": {
...
def test_bundle_cache_contract_has_no_identity_field_collisions():
    ...
    bundle_variants = (
        identity,
        replace(identity, version=identity.version + 1),
        replace(
            identity,
    ...
    cache_identities = tuple(
        provider_bundle.make_bundle_cache_identity(
            bundle,

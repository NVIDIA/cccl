...
def test_generic_finalizer_uses_registered_group_renderer():
    ...
    from cuda.coop.cutlass._dsl import _provider_finalizer, _thread_group_provider
    ...
    assert provider_support._ensure_bundle_finalizer() is (
        _provider_finalizer._trace_finalize_hook
    )
    assert "cuda.coop.cutlass._dsl.block._provider" not in sys.modules

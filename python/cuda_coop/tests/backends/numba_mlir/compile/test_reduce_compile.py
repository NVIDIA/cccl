...
def test_deterministic_cub_reduce_variants_compile_with_scoped_storage(
    ...
    assert "if (num_valid < 1 || num_valid > 64)" in block_runtime_prefix
    assert "if (valid_items < 1 || valid_items > 8)" in warp_runtime_prefix
    assert ".Sum(src, 37);" in _cub_source(collected[5])
...
def test_stateless_cub_callbacks_compile_for_block_arrays_and_warp_prefixes(
    ...
    for name, collected in cases:
        ...
        assert "auto binary_op = []" in source
        assert f"return {callback_name}(wp_0, wp_1);" in source
        if name == "block-array":
            assert "cub::BlockReduce<" in source
            assert "(*)[2]>(src), binary_op);" in source
            assert "__syncthreads();" in source
            assert "__syncwarp" not in source
        else:
            assert "cub::WarpReduce<::cuda::std::int32_t, 8>" in source
            assert ".Reduce(input, binary_op, 5);" in source
...
def test_parameter_name_cannot_shadow_callback_symbol(
    compile_context: _nvrtc.CompileContext,
    ...
    monkeypatch.setattr(
        _types,
        "_python_operator_symbol_name",
        lambda *args, **kwargs: "custom_callback",
    )
    collected = _collect_cub(
        ...
        dtype=types.int32,
        threads_per_block=_BLOCK_THREADS,
        binary_op=cuda.jit(device=True)(maximum),
    )
    algorithm = collected[0]
    for method in algorithm.parameters:
        for parameter in method:
            if isinstance(parameter, _types.StatelessOperator):
                parameter.parameter_name = parameter.name

    source = _cub_source(collected)
    assert "return custom_callback(wp_0, wp_1);" in source
    bundle = _compile_cub_bundle([collected], name="callback_name_collision")
    invocable = _types.make_invocable_from_specialization(
        algorithm,
        threads=collected[1],
        block_threads=collected[2],
    ...
    assert Path(invocable.files[0]).read_bytes() == bundle
    assert ".version" in _link_ltoir_files(
        invocable.files, name="callback_name_collision"
    )

...
def test_direct_load_store_compile_without_temp_storage_or_barriers(
    ...
    for source in sources:
        assert "::cuda::std::int64_t num_valid_items" in source
        assert "if (num_valid_items < 0 || num_valid_items > 64)" in source
        assert 'asm volatile("trap;" : : :);' in source
        assert "static_cast<::cuda::std::int32_t>(num_valid_items)" in source
...
def test_unguarded_vectorize_compiles_the_full_tile_overload(
    ...
    for operation, specialization in zip(("load", "store"), algorithms):
        ...
        assert f"().{operation.title()}(" in source
        assert "checked_num_valid_items" not in source
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("input", "transformed_input", "cast_input", "temp_storage"),
        ("out", "ref_out", "cast_out", "out"),
...
def test_wrapper_parameters_do_not_shadow_function_type_or_return_local(
    ...
    source_buffer.write(
        ...
        "struct scratch_t {};\n"
        "__device__ void invoke(scratch_t *, ::cuda::std::int32_t &input, "
        "::cuda::std::int32_t value, ::cuda::std::int32_t &output) {\n"
        "  output = input + value;\n"
        "}\n"
    )
    _algorithm(compile_context)._emit_abi_wrapper(
        source_buffer.write,
        method,
        exported_name="named_wrapper",
        internal_name="invoke",
        temp_storage_type_name="scratch_t",
    )
    source = source_buffer.getvalue()
    assert "named_wrapper__abi(void *__ret," in source
    assert "*reinterpret_cast<::cuda::std::int32_t *>(__ret) = out;" in source
    _, ltoir = _nvrtc.compile(
        cpp=source,
        cc=90,
        rdc=True,

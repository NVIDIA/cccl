...
def _named_operator_algorithm(operator):
    ...
    specification = Algorithm(
        ...
        method_name="Run",
        c_name="named_operator_provider",
        includes=(),
        template_parameters=(),
        parameters=((Reference(types.int32, name="input"), operator),),
    ).specialize({})
    return NumbaMlirCoreAdapter().materialize(
        specification,
...
def test_core_adapter_lowers_stateless_operator_and_emits_scalar_wrapper(
    ...
    named_algorithm = _named_operator_algorithm(operator)
    named_operator = named_algorithm.parameters[0][1]
    assert named_operator.parameter_name == "binary_op"
    assert named_operator.name == specialized.name
    named_source = named_algorithm._source_code(
        compile_identity=(90, True, "lto", (), "test-toolchain")
    )[0]
    assert "auto binary_op = []" in named_source
...
def test_core_adapter_lowers_stateful_operator_and_emits_state_capture(
    ...
    named_algorithm = _named_operator_algorithm(operator)
    named_operator = named_algorithm.parameters[0][1]
    assert named_operator.parameter_name == "running_prefix"
    assert named_operator.name == specialized.name
    named_source = named_algorithm._source_code(
        compile_identity=(90, True, "lto", (), "test-toolchain")
    )[0]
    assert "auto running_prefix = [running_prefix_state]" in named_source
    assert "void *abi_running_prefix" in named_source

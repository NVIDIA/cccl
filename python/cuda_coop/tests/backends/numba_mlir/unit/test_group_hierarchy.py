...
from cuda.coop.numba_mlir import _group_provider
...
def test_planner_lowers_group_method_without_primitive_rewrites(monkeypatch):
    captured = []

    def query():
        return coop.this_block().count("thread")
    ...
    assert captured[0]["operation"] == "count"
    assert captured[0]["level"] == "thread"
    assert captured[0]["group"].kind == "block"
    assert captured[0]["group"].thread_count == 64
...
def test_mapped_parent_queries_render_the_parent_group():
    ...
    assert _group_provider._query_expr(mapped, "rank", "warp") == (
...
def test_current_physical_group_rendering_uses_implicit_hierarchy(kind):
    ...
    assert _group_provider._group_prelude(group) == [

...
def test_data_movement_retains_names_through_dependent_specialization(operation):
    ...
    assert "void *abi_src" in source
    assert "void *abi_dst" in source
    assert "::cuda::std::int64_t abi_offset" in source
...
def test_input_transform_retains_the_source_parameter_name():
    ...
    assert "void *abi_flags" in source
...
def test_parameter_renaming_preserves_provider_identity():
    ...
    assert "abi_value" in original_source
    assert "abi_input" in renamed_source

...
from cuda.coop._core import PythonOperator, semantic_token
from cuda.coop._core._symbols import _dependency_token, _TokenState
...
def test_callable_symbol_uses_numba_dispatcher_python_function():
    ...
    assert semantic_token(
        PythonOperator(
            ret_dtype=_types.types.int32,
            arg_dtypes=(_types.types.int32,),
            op=dispatcher,
        )
    ) == semantic_token(
        PythonOperator(
            ret_dtype=_types.types.int32,
            arg_dtypes=(_types.types.int32,),
            op=_callable_cache_op,
        )
    )
    assert semantic_token(_capture_callable(dispatcher)) == semantic_token(
        _capture_callable(_callable_cache_op)
    )
...
def test_symbol_hash_uses_numba_type_identity():
    assert semantic_token(_types.types.int32) == (
...
def test_numba_type_tokens_remain_distinct_in_one_walk():
    values = (
        _types.types.int32,
    ...
    expected = tuple(semantic_token(value) for value in values)

    assert semantic_token(values) == expected
    assert _dependency_token(values, _TokenState()) == expected

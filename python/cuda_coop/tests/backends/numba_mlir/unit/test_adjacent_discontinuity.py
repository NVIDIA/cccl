...
def test_common_and_qualified_adjacent_discontinuity_lower_to_same_factories():
    ...
    import cuda.coop.numba_mlir._lowering as block_providers
    ...
    for function, adjacent_count, discontinuity_count in expected_counts:
        ...
        assert factories[block_providers.adjacent_difference] == adjacent_count
        assert factories[block_providers.discontinuity] == discontinuity_count


@pytest.mark.parametrize(
    ("module_name", "factory_name", "spec_factory_name", "operator_name"),
    [
        (
...
def test_comparison_factories_normalize_dispatchers(
    monkeypatch,
    module_name,
    factory_name,
    spec_factory_name,
    ...
    module = importlib.import_module(f"cuda.coop.numba_mlir._lowering.{module_name}")
    dispatcher = cuda.jit(device=True)(lambda left, right: left != right)

    def capture_spec(**kwargs):
        return SimpleNamespace(specialization=kwargs[operator_name])

    monkeypatch.setattr(module, spec_factory_name, capture_spec)
    monkeypatch.setattr(module, "numba_type_to_wrapper", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        module.NumbaMlirCoreAdapter,
        "materialize",
    ...
    keyword = "difference_op" if factory_name == "adjacent_difference" else "flag_op"
    descriptor = getattr(module, factory_name)(
        dtype=types.int32,
        threads_per_block=32,
        **{keyword: dispatcher},
    )

    assert descriptor.op is dispatcher.py_func
...
def test_common_comparison_rejects_scalar_but_qualified_scalar_remains_available(
    ...
    with pytest.raises(
        ...
        match=(
            ...
            r"in the portable API; use cuda\.coop\.numba_mlir for "
            r"qualified scalar or local-array support"
        ),
    ):
...
@pytest.mark.parametrize("valid_items", [True, -1, 65])
def test_adjacent_difference_rejects_invalid_static_valid_items(valid_items):
    pytest.importorskip("numba_cuda_mlir")
    ...
    error = TypeError if isinstance(valid_items, bool) else ValueError
    with pytest.raises(error, match="valid_items"):
        _GroupCallPlanner(
            state,
            {"block": (32, 1, 1), "grid": (1, 1, 1), "cluster": None},
        ).run()
...
@pytest.mark.parametrize(
    ("dtype_name", "accepted"),
    [("int32", True), ("float32", False), ("boolean", False)],
)
def test_adjacent_difference_dynamic_valid_items_requires_integer_dtype(
    dtype_name,
    accepted,
):
    pytest.importorskip("numba_cuda_mlir")
    ...
    runtime_args = [
        ir.Var(scope, "input", loc),
        ir.Var(scope, "output", loc),
        ir.Var(scope, "valid_items", loc),
    ]
    rewrite = object.__new__(CoopSinglePhaseRewrite)
    dtype = getattr(types, dtype_name)
    rewrite._resolve_var_numba_type = lambda _value: dtype
    rewrite._resolve_var_dtype = lambda _value: None

    def validate():
        rewrite._validate_integer_runtime_controls(
            op_name="adjacent_difference",

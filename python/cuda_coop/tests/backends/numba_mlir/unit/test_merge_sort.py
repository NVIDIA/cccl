...
@pytest.mark.parametrize(
    ("factory_name", "spec_factory_name"),
    [
        ("merge_sort_keys", "make_block_merge_sort_spec"),
        ("merge_sort_pairs", "make_block_merge_sort_spec"),
        ("warp_merge_sort_keys", "make_warp_merge_sort_spec"),
        ("warp_merge_sort_pairs", "make_warp_merge_sort_spec"),
    ],
)
def test_merge_sort_factories_normalize_dispatchers(
    monkeypatch,
    factory_name,
    spec_factory_name,
):
    ...
    module = importlib.import_module("cuda.coop.numba_mlir._lowering._merge_sort")
    dispatcher = cuda.jit(device=True)(_less)

    def capture_spec(**kwargs):
        return SimpleNamespace(specialization=kwargs["compare_operator"])
    ...
    kwargs = {
        "compare_op": dispatcher,
        "items_per_thread": 1,
    }
    if factory_name.startswith("warp_"):
        if factory_name.endswith("_pairs"):
            kwargs.update(keys=types.int32, values=types.int32)
        else:
            kwargs["dtype"] = types.int32
    else:
    ...
    descriptor = getattr(module, factory_name)(**kwargs)

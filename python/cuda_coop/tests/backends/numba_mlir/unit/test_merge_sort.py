...
def test_public_exports_are_qualified_and_providers_remain_private():
    ...
    assert private_warp.__all__ == ()
    assert coop.merge_sort_keys.__module__.endswith("._group_ops")
    assert coop.merge_sort_pairs.__module__.endswith("._group_ops")
...
def test_common_root_merge_sort_reaches_the_private_provider(group_kind, pairs):
    ...
    expected = (
        private_block.merge_sort_pairs if pairs else private_block.merge_sort_keys
    )
    if group_kind != "block":
        expected = (
            private_warp.merge_sort_pairs if pairs else private_warp.merge_sort_keys
        )
    provider_call = next(
        call
    ...
    marker = dict(provider_call.kws)["_common_profile_operation"]
...
def test_common_root_merge_sort_rejects_floating_point_keys():
    ...
    with pytest.raises(CoopSinglePhaseRewriteError, match="common V1 supports"):

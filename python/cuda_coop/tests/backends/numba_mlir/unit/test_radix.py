...
def test_public_radix_exports_keep_factories_private() -> None:
    ...
    assert private_block.__all__ == ()
    for name in ("radix_rank", "radix_sort_keys", "radix_sort_pairs"):
        assert getattr(coop, name).__module__.endswith("._group_ops")
    for name in (
        "radix_rank",
        "radix_sort_keys",
        ...
        assert callable(getattr(private_block, name))
...
def test_qualified_thread_data_radix_lowers_to_block_factories() -> None:
    ...
    assert counts[private_block.radix_sort_keys_descending] == 1
    assert counts[private_block.radix_sort_pairs] == 1
    assert counts[private_block.radix_rank] == 1
    ...
    provider_calls = [
        ...
        if factory
        in {
            private_block.radix_sort_keys_descending,
            private_block.radix_sort_pairs,
            private_block.radix_rank,
        }
    ]
...
def test_qualified_scalar_radix_boxes_and_projects_fresh_results(operation) -> None:
    ...
    import cuda.coop.numba_mlir as coop
    from cuda.coop.numba_mlir._group_rewrites import _typed_group_payload_like
...
def test_common_thread_data_radix_uses_profile_factories() -> None:
    ...
    from cuda.coop.numba_mlir._block._block_radix_rank import _common_radix_rank
    from cuda.coop.numba_mlir._block._block_radix_sort import (

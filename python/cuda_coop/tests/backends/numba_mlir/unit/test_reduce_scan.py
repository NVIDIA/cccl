...
def test_block_scan_factory_normalizes_dispatcher_prefix(monkeypatch):
    from numba_cuda_mlir import cuda

    from cuda.coop._core import PythonOperator

    dispatcher = cuda.jit(device=True)(_prefix_from_aggregate)
    specialization = _capture_block_scan_factory(
        monkeypatch,
        prefix_op=dispatcher,
    )

    prefix = specialization.parameters[0][-1]
    assert isinstance(prefix, PythonOperator)
    assert prefix.op is dispatcher.py_func
...
def test_stateful_function_normalizes_dispatcher(monkeypatch):
    ...
    prefix = specialization.parameters[0][-1]
    assert running_prefix.op is dispatcher.py_func
    assert isinstance(prefix, StatefulOperator)
    assert prefix.op is running_prefix


@pytest.mark.parametrize("factory_name", ["reduce", "warp_reduce"])
def test_reduce_factories_normalize_dispatchers(monkeypatch, factory_name):
    import importlib
    ...
    module = importlib.import_module("cuda.coop.numba_mlir._lowering._reduce")
    materializer_name = (
        "_materialize_block" if factory_name == "reduce" else "_materialize_warp"
    )
    monkeypatch.setattr(
        module,
        materializer_name,
        lambda **kwargs: kwargs["reduce_operator"],
    )
    dispatcher = cuda.jit(device=True)(_multiply)
    kwargs = (
        {"threads_per_block": 32}
        if factory_name == "reduce"
        else {"threads_in_warp": 32}
    )

    descriptor = getattr(module, factory_name)(
        types.int32,
        binary_op=dispatcher,
        **kwargs,
    )

    assert isinstance(descriptor, PythonOperator)
...
@pytest.mark.parametrize(
    ("operation", "expected_op", "binding_name"),
    [
        ("reduce", "warp_sum", "valid_items"),
        ("scan", "warp_inclusive_scan", "valid_items"),
    ],
)
def test_static_partial_controls_remain_factory_bound(
    operation,
    expected_op,
    binding_name,
):
    ...
    def scan_kernel(value):
        return coop.inclusive_scan(group, value, valid_items=5)
    ...
    match = next(
        match
        for match in _single_phase_matches(func_ir, arg_types)
        if match.op_name == expected_op
    )
    assert match.factory_kwargs[binding_name] == ArgumentBinding.static(5)
    assert len(match.runtime_args) == 1


def test_static_warp_scan_valid_items_do_not_shift_runtime_aggregate():
    from numba_cuda_mlir import types
    ...
    match = next(
        match
        for match in _single_phase_matches(func_ir, arg_types)
        if match.op_name == "warp_inclusive_scan"
    )
    assert match.factory_kwargs["valid_items"] == ArgumentBinding.static(5)
    assert match.factory_kwargs["warp_aggregate"] is True

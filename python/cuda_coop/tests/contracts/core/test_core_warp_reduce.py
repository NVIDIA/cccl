...
def test_warp_builtin_semantics_materialize_once_when_reused(operation, materialized):
    ...
    assert lowered.count(materialized) == 1
...
def test_warp_reduce_accepts_numpy_static_valid_items():
    spec = make_warp_reduce_spec(
        dtype="int",
        threads_in_warp=8,
        operation="sum",
    ...
    valid_items = spec.specialization.parameters[0][-2]
...
def test_warp_reduce_can_retain_full_signature_with_partial_support():
    ...
    assert [
        ...
        ["temp_storage", "input", "valid_items", "output"],
    ]

...
@pytest.mark.parametrize("kind", sorted(PHYSICAL_GROUP_KINDS))
def test_current_group_rendering_uses_implicit_hierarchy(kind):
    group = make_thread_group(kind)
    assert _thread_group.render_group_decl(group) == (
...
def test_physical_group_renderer_rejects_mapped_groups(group):
    with pytest.raises(ValueError, match="render_group_decl_lines"):
        _thread_group.render_group_decl(group)


def test_resolved_hierarchy_and_physical_group_rendering_are_exact():
    block_only = _resolved_hierarchy(64)
    multilevel = _resolved_hierarchy(
        block_dim=(8, 4),
        cluster_dim=2,
        grid_dim=3,
    )

    assert _thread_group.render_hierarchy_decl(block_only) == [
        "  auto hierarchy = ::cuda::hierarchy{::cuda::block_dims<64>()};"
    ]
    assert _thread_group.render_hierarchy_decl(multilevel) == [
        "  auto hierarchy = ::cuda::hierarchy{",
        "      ::cuda::grid_dims<3>(),",
        "      ::cuda::cluster_dims<2>(),",
        "      ::cuda::block_dims<8, 4>()",
        "  };",
    ]
    assert _thread_group.render_group_decl_lines(
        ThreadGroup(kind="block", hierarchy=multilevel)
    ) == ["  ::cuda::experimental::this_block group{hierarchy};"]


def test_resolved_mapped_group_rendering_keeps_membership_and_sync():
    hierarchy = _resolved_hierarchy(64)
    group = ThreadGroup(kind="warp", hierarchy=hierarchy).group_by(8)

    assert _thread_group.render_group_decl_lines(group) == [
        "  ::cuda::experimental::this_warp group_parent{hierarchy};",
        "  ::cuda::experimental::generic_group group{",
        "      ::cuda::gpu_thread, group_parent,",
        "      ::cuda::experimental::group_by<8, true>{},",
        "      ::cuda::experimental::lane_synchronizer{}};",
    ]


def test_resolved_mapped_warp_group_rendering_allocates_barriers():
    hierarchy = _resolved_hierarchy(160)
    group = ThreadGroup(kind="block", hierarchy=hierarchy).group_by(
        2,
        exhaustive=False,
    )

    assert _thread_group.render_group_decl_lines(group) == [
        "  ::cuda::experimental::this_block group_parent{hierarchy};",
        "  using group_barriers_type =",
        "      ::cuda::barrier<::cuda::thread_scope_block>[2];",
        "  __shared__ ::cuda::std::aligned_storage_t<",
        "      sizeof(group_barriers_type),",
        "      alignof(group_barriers_type)> group_barriers_storage;",
        "  auto& group_barriers =",
        "      reinterpret_cast<group_barriers_type&>(",
        "          group_barriers_storage);",
        "  ::cuda::experimental::generic_group group{",
        "      ::cuda::warp, group_parent,",
        "      ::cuda::experimental::group_by<2, false>{},",
        "      ::cuda::experimental::barrier_synchronizer{group_barriers}};",
    ]
...
def test_group_by_revalidates_deferred_constraints_after_hierarchy_resolution(
    block_threads,
    count,
    exhaustive,
    message,
    ...
    with pytest.raises(ValueError, match=message):
        current.with_hierarchy(_resolved_hierarchy(block_threads))

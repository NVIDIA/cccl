...
def test_launch_fact_verification_is_semantic_but_its_origin_is_diagnostic():
    ...
    same_verification = LaunchFacts(
        cooperative_launch=True,
        provenance=LaunchFactOrigin(
            "cooperative_launch",
            "driver_attribute",
            detail="alternate compiler path",
            verified=True,
        ),
    )

    assert asserted != verified
    assert asserted.semantic_key != verified.semantic_key
    assert verified == same_verification
    assert hash(verified) == hash(same_verification)
    assert verified.semantic_key == same_verification.semantic_key
...
def test_launch_fact_verification_changes_grid_lowering_support():
    common = {
        "exact_block_dim": 32,
        "exact_grid_dim": 4,
    ...
    asserted = LaunchFacts(
        **common,
        provenance=(
            ...
            LaunchFactOrigin("cooperative_launch", "call_metadata"),
        ),
    )
    verified = LaunchFacts(
        **common,
    ...
    asserted_plan = _plan(this_grid(), _reduce(), asserted)
    verified_plan = _plan(this_grid(), _reduce(), verified)

    assert asserted_plan.target is GroupLoweringTarget.UNSUPPORTED
    assert asserted_plan.unsupported.code is UnsupportedReasonCode.LAUNCH_CAPABILITY
    assert verified_plan.target is GroupLoweringTarget.CUDAX_GROUP


@pytest.mark.parametrize("fact", ["cluster_launch", "cooperative_launch"])
def test_verified_launch_fact_origin_requires_its_value_in_the_same_record(fact):
    with pytest.raises(ValueError, match="same LaunchFacts record"):
        LaunchFacts(
            provenance=LaunchFactOrigin(
                fact,
                "detached_verifier",
...
def test_verified_launch_fact_origin_rejects_unknown_fact_names():
    with pytest.raises(ValueError, match="must name a LaunchFacts value field"):
        LaunchFacts(
            exact_block_dim=32,
            provenance=LaunchFactOrigin(
                "block",
...
def test_merge_launch_facts_only_verifies_surviving_maximums():
    verified_loose_origin = LaunchFactOrigin(
        "max_block_dim",
        "verified:loose",
        verified=True,
    )
    merged = merge_launch_facts(
        LaunchFacts(
            max_block_dim=(256, 8, 2),
            provenance=verified_loose_origin,
        ),
        LaunchFacts(max_block_dim=(128, 16, 1)),
    )

    assert merged.max_block_dim == (128, 8, 1)
    assert not merged.is_verified("max_block_dim")
    assert LaunchFactOrigin("max_block_dim", "verified:loose") in merged.provenance

    verified_tight_origin = LaunchFactOrigin(
        "max_block_dim",
        "verified:tight",
        verified=True,
    )
    merged = merge_launch_facts(
        LaunchFacts(
            max_block_dim=(128, 8, 1),
            provenance=verified_tight_origin,
        ),
        LaunchFacts(max_block_dim=(256, 16, 2)),
    )

    assert merged.max_block_dim == (128, 8, 1)
    assert merged.is_verified("max_block_dim")
    assert verified_tight_origin in merged.provenance

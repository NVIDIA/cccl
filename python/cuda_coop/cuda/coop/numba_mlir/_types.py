...
def _compile_device_ltoir(fn, *, sig, abi_info, semantic_identity=None):
    ...
    # multiple coop specializations. Aggregate ABI adapters close over a fresh
    # dispatcher solely to compile their nested call, so key those adapters by
    # the caller-provided source identity instead of mutable compiler state.
...
class DependentPythonOperator:
    ...
    def specialize(self, template_arguments):
        ...
        if isinstance(op, StatefulFunction):
            ...
            operator_py_func = getattr(binary_op, "py_func", binary_op)
...
class Algorithm:
    ...
    def _ignore_codegen_param(param):
        return isinstance(param, (StatelessOperator, CxxFunction)) or (
    ...
    def codegen_method(self, func_to_overload, method, mangled_name):
        ...
        def ignore_param(param):
            ...
            ignore = isinstance(param, (StatelessOperator, CxxFunction)) or (
                isinstance(param, PointerOffset) and param.static_value is not None
            )
            return ignore

...
from numba_cuda_mlir.descriptor import MLIRDispatcher, mlir_target
...
from cuda.coop._core._symbols import _normalize_semantic_value
...
@_normalize_semantic_value.register(types.Type)
def _normalize_numba_semantic_type(value):
    return (
...
@_normalize_semantic_value.register(MLIRDispatcher)
def _normalize_numba_semantic_dispatcher(value):
    # Device dispatchers are compiled from ``py_func`` below. Their mutable
    # compiler contexts and overload caches are not callable semantics.
...
def _hash_symbol_value(hasher, value, depth=0):
    del depth
    hasher.update(
        repr(semantic_token(value)).encode("utf-8", errors="backslashreplace")
    )
...
def _python_operator_symbol_name(
    ...
    op_component = (
        ...
        else f"{_symbol_component(stateful_name)}_{callable_component}"
    )
...
def _compile_device_ltoir(fn, *, sig, abi_info):
    ...
    cache_key = (
        semantic_token(py_func),
        repr(sig),
        tuple(sorted(abi_info.items())),
    )

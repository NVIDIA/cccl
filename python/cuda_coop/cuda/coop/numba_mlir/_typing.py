...
DtypeType = Union[str, type, "np.number", "np.dtype", "numba_mlir_types.Type"]
"""
.. _DtypeType:
...
- NumPy scalar types (e.g., ``np.float32``) or NumPy dtypes (e.g.,
  ``np.dtype('float32')``).
- Numba-CUDA-MLIR types (e.g., ``numba_cuda_mlir.types.int32``).
...
.. code-block:: python
    ...
    dtype = np.float32
    dtype = "int32"
    dtype = numba_cuda_mlir.types.float64

See Also
--------
- :mod:`numpy`
- :mod:`numba_cuda_mlir.types`
"""
...
ScanOpType = Union[
    ...
    # Callable objects.
    Callable[
        ["numba_mlir_types.Number", "numba_mlir_types.Number"],
        "numba_mlir_types.Number",
    ],

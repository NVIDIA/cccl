...
"""Planner-private warp Merge Sort providers."""
...
def _normalize_args(dtype, items_per_thread, compare_op):
    if dtype is None:
...
def warp_merge_sort_keys(
    ...
    dtype, items_per_thread = _normalize_args(
        dtype,
        items_per_thread,
        compare_op,
    )
...
def warp_merge_sort_pairs(
    ...
    if (valid_items is None) != (oob_default is None):
        raise ValueError("valid_items and oob_default must be provided together")
    keys, items_per_thread = _normalize_args(keys, items_per_thread, compare_op)
    threads_in_warp = _positive_int(threads_in_warp, name="threads_in_warp")
    if values is None:

...
class BlockAdjacentDifferenceType(IntEnum):
    """Direction used by ``block.adjacent_difference`` comparisons."""
...
def adjacent_difference(
    ...
    """Build a block-wide adjacent-difference invocable.

    The returned invocable wraps CUB ``BlockAdjacentDifference`` and is used by
    the Numba-CUDA-MLIR single-phase rewrite for ``coop._block.adjacent_difference`` calls.
    ``dtype`` and ``threads_per_block`` describe the tile, ``items_per_thread``
    describes each thread's local item array, and ``difference_op`` is a device
    callable used to compare neighboring items.
    """
    if dtype is None and not isinstance(
        block_adjacent_difference_type, (BlockAdjacentDifferenceType, int)
    ):
        dtype = block_adjacent_difference_type
        block_adjacent_difference_type = BlockAdjacentDifferenceType.SubtractLeft

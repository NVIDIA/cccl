...
class _StorageRewrite:
    ...
    def _ensure_temp_storage_global_plan(self) -> _TempStorageGlobalPlan:
        ...
        if implicit.uses:
            implicit_alignment = max(
                _MIN_TEMP_STORAGE_ALIGNMENT,
                int(implicit.max_alignment),
                *(int(entry.alignment) for entry in implicit.uses),
            )
            implicit_size = max(
                int(implicit.max_size_in_bytes),
                *(int(entry.size_in_bytes) for entry in implicit.uses),
            )
            offset = _align_up(offset, implicit_alignment)
            implicit_base_offset = offset
            implicit_slices = {
                id(entry.call_assign): _TempStorageSlice(
                    offset=0, size_in_bytes=int(entry.size_in_bytes)
                )
                for entry in implicit.uses
            }
    ...
    def _implicit_temp_storage_arg_for_call(
        ...
        start = plan.base_offset + slice_info.offset
        self._emit_array_slice(
            block,
            source_var=backing,
            target_var=sliced_var,
            start=start,
            stop=start + slice_info.size_in_bytes,
            loc=call_assign.loc,
        )

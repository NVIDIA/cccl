...
"""Scan payload inference, callback, and aggregate factory finalization.
...
class _ScanRewrite:
    def _infer_scan_payload(self, inference: PayloadInference) -> None:
        ...
        if inference.factory_kwargs.get("warp_aggregate"):
            valid_items = inference.factory_kwargs.get("valid_items")
            valid_items_is_runtime = (
                valid_items.kind is BindingKind.RUNTIME
                if isinstance(valid_items, ArgumentBinding)
                else bool(valid_items)
            )
            aggregate_index = (
                2
                if valid_items_is_runtime

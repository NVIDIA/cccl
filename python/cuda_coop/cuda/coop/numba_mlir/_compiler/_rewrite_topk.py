...
"""TopK payload inference and runtime-control validation.
...
class _TopKRewrite:
    ...
    def _validate_topk_runtime_controls(
        ...
        *,
        ...
        def normalize_static_index(name: str, value: object) -> int:
        ...
        def static_index(name: str) -> int | None:
            binding = factory_kwargs.get(name, _UNRESOLVED)
            if isinstance(binding, ArgumentBinding):
                if binding.kind is BindingKind.OMITTED:
                    return None
                if binding.kind is BindingKind.STATIC:
                    return normalize_static_index(name, binding.value)
            elif (
                binding is not _UNRESOLVED
                and binding is not None
                and binding is not True
            ):
                return normalize_static_index(name, binding)
            ...
            if value is _UNRESOLVED:
                return None
            return normalize_static_index(name, value)

        def is_omitted_control(name: str) -> bool:
            value = factory_kwargs.get(name, _UNRESOLVED)
            return (
                value is _UNRESOLVED
                or value is None
                or (
                    isinstance(value, ArgumentBinding)
                    and value.kind is BindingKind.OMITTED
                )
            )
        ...
        static_valid = (
            tile_size if is_omitted_control("num_valid") else static_index("num_valid")
        )
        ...
        static_begin = (
            0 if is_omitted_control("begin_bit") else static_index("begin_bit")
        )
        static_end = (
            key_width if is_omitted_control("end_bit") else static_index("end_bit")
        )

...
def _materialize_block(
    *,
    ...
    valid_items_binding = _optional_binding(num_valid)
    if valid_items_binding.kind is not BindingKind.OMITTED and items_per_thread != 1:
...
def _materialize_warp(
    *,
    ...
    core_spec = make_warp_reduce_spec(
        ...
        valid_items=valid_items_binding,
        include_full_warp=(
            not isinstance(valid_items, ArgumentBinding) and valid_items is not None
        ),
    )
    specialization = NumbaMlirCoreAdapter().materialize(
        core_spec.specialization,

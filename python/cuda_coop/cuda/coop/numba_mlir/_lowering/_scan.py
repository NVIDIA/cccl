...
def scan(
    ...
    if prefix_callback is not None:
        descriptor_kwargs = {
            ...
            "op": _normalize_numba_callable(prefix_callback),
            "name": "prefix_op",
        }
        prefix_descriptor = (
            StatefulOperator(
...
def _scan(
    *,
    ...
    has_valid_items = valid_items_binding.kind is not BindingKind.OMITTED
    if mode == "inclusive" and initial_value is not None:
        raise ValueError("inclusive scan does not accept initial_value")
    normalized_op = ScanOp(scan_op)
    ...
    if (
        ...
        initial_value = 0
    use_sum_method = (
        normalized_op.is_sum and initial_value is None and not has_valid_items
    )

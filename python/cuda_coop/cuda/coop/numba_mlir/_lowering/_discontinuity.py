...
"""Discontinuity provider lowering for Numba-CUDA-MLIR.

This semantic module owns physical-block provider materialization.  Callable
and boundary-operand analysis remain compiler-planner responsibilities.
"""
...
class BlockDiscontinuityType(IntEnum):
    """Planner output mode for block discontinuity."""
...
def discontinuity(
    ...
    """Build the direct CUB discontinuity invocable selected by planning."""
    ...
    core_spec = make_block_discontinuity_spec(
        ...
        flag_operator=PythonOperator(
            ...
            op=_normalize_numba_callable(flag_op),
            name="flag_op",
        ),

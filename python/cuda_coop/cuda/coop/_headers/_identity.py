...
def _recursive_include_root_identity(root: Path) -> str:
    ...
    def hash_file(path: Path) -> None:
        ...
        digest.update(str(content_length).encode("ascii"))
        digest.update(b":")
...
def include_dirs_identity(include_dirs: Iterable[str]) -> IncludeDirsIdentity:
    ...
    if not resolved_roots:
        digest = hashlib.sha256(b"ordered-include-roots-v3\0none").hexdigest()
    ...
    for repo, roots in roots_by_repo.items():
        ...
        try:
            repo_identities = _git_include_root_identities(repo, tuple(roots))
        except OSError:
            repo_identities = {}
        git_identities.update(repo_identities)
    ...
    root_identities: list[IncludeRootIdentity] = []
    recursive_walks = 0
    for root in resolved_roots:
        method = "git-tree"
    ...
    aggregate_digest.update(b"ordered-include-roots-v3\0")
    for index, identity in enumerate(root_identities):
        ...
        aggregate_digest.update(b"\0")
        aggregate_digest.update(identity.digest.encode("ascii"))
        aggregate_digest.update(b"\0")
    return IncludeDirsIdentity(
        roots=tuple(root_identities),
        digest=aggregate_digest.hexdigest(),

...
def _toolkit_version(
    ...
    for include_dir in cuda_include_dirs:
        ...
        if match is None:
            raise RuntimeError(
                f"failed parsing CUDART_VERSION from CUDA Toolkit version header: "
                f"{header}"
            )
...
def _load_exact_candidate(candidate: Path) -> str:
    """Load one exact library path while ``_PRELOAD_LOCK`` is held."""
...
def _preload_exact_nvrtc(
    *,
    major: int,
    minor: int,
    library_dirs: tuple[Path, ...],
) -> tuple[str, str] | None:
    """Load one same-root NVRTC and builtins pair."""
    ...
    for library_dir in library_dirs:
        for name in _library_names("nvrtc", major):
            ...
            builtins = next(
                (
                    library_dir / builtins_name
                    for builtins_name in _nvrtc_builtins_names(major, minor)
                    if (library_dir / builtins_name).is_file()
                ),
                None,
            )
            if builtins is None:
                expected = ", ".join(_nvrtc_builtins_names(major, minor))
                failures.append(
                    (
                        candidate,
                        FileNotFoundError(
                            f"missing adjacent NVRTC builtins ({expected})"
                        ),
                    )
                )
                continue
            try:
                builtins_path = _load_exact_candidate(builtins)
            except OSError as exc:
                failures.append((candidate, exc))
                continue
            try:
                nvrtc_path = _load_exact_candidate(candidate)
            except OSError as exc:
                raise RuntimeError(
                    "failed loading same-root NVRTC after its builtins library "
                    f"was made process-global: {candidate}: {exc}"
                ) from exc
            return nvrtc_path, builtins_path
    if failures:
        rendered = "; ".join(f"{candidate}: {error}" for candidate, error in failures)
        raise RuntimeError(
            "failed loading all resolved CUDA Toolkit NVRTC and builtins "
    ...
    for library_dir in library_dirs:
        for name in _library_names(kind, major):
            ...
            if not candidate.is_file():
                continue
            try:
                return _load_exact_candidate(candidate)
            except OSError as exc:
                failures.append((candidate, exc))
                continue
    if failures:
...
def preload_toolkit_compiler_libraries(
    ...
    nvrtc_builtins_path: str | None = None

    with _PRELOAD_LOCK:
        if toolkit_version is not None:
            major, minor = toolkit_version
            nvrtc_pair = _preload_exact_nvrtc(
                major=major,
                minor=minor,
                library_dirs=library_dirs,
            )
            if nvrtc_pair is None:
                expected = ", ".join(_nvrtc_builtins_names(major, minor))
                raise RuntimeError(
                    "resolved CUDA headers require a same-root NVRTC and "
                    f"builtins pair ({expected})"
                )
            exact_paths["nvrtc"], nvrtc_builtins_path = nvrtc_pair
            for kind in ("nvJitLink",):
        ...
        libraries = ToolkitCompilerLibraries(
            ...
            nvrtc_builtins_path=nvrtc_builtins_path,

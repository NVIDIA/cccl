...
class ThreadGroup:
    ...
    # Provenance is diagnostic-only; equivalent group semantics share caches.
        ...
        *,
        ...
        if self.kind == "warp":
            ...
            if block_threads is not None and block_threads % 32 != 0:
                raise ValueError(
                    "block group_by requires a parent composed of complete warps"
                )
            parent_units = None if block_threads is None else block_threads // 32
            if parent_units is not None and count > parent_units:
                raise ValueError(
        ...
        if parent_units is not None and exhaustive and parent_units % count != 0:
            raise ValueError(
...
def render_group_decl(
    ...
    *,
    ...
    if group.hierarchy.implicit:
        return f"{indent}::cuda::experimental::this_{group.kind} {var_name}{{}};"
    return (

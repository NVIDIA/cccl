...
does not prevent another compatible DSL from registering.

Set ``CUDA_COOP_DISABLE_AUTO_DSL_REGISTRATION`` to a truthy value to skip
every automatic probe and register backends explicitly instead.
"""
...
_INSTALL_GUIDE_URL = "https://nvidia.github.io/cccl"

# Keep this an explicit allowlist.
...
def _warn_incompatible(candidate: _Candidate, error: Exception) -> None:
    ...
    warnings.warn(
        ...
        "continued and other DSL backends were unaffected. Install the "
        f"matched cuda-coop[{candidate.extra}] extra; installation "
        f"instructions are at {_INSTALL_GUIDE_URL}. "

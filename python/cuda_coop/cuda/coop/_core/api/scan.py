...
_SUM_SCAN_OPERATORS = frozenset({"+", "add", "plus", "sum"})
_WARP_GROUP_KINDS = frozenset({"warp", "threads_within_warp"})


def _is_sum_scan_operator(scan_op: Any) -> bool:
    if scan_op is None:
        return True
    token = getattr(scan_op, "value", scan_op)
    if isinstance(token, str):
        token = token.strip().lower()
    return token in _SUM_SCAN_OPERATORS
...
def _validate_portable_scan_options(
    ...
    *,
    ...
    """Enforce the group- and operator-dependent portable overload matrix."""
    ...
    if (
        ...
        and not _is_sum_scan_operator(scan_op)
    ):
        raise ValueError(
            f"cuda.coop.{operation} non-sum exclusive scans require initial_value"
        )
    if group.kind not in _WARP_GROUP_KINDS:
        if temp_storage is not None:
            _validate_common_temp_storage(operation, temp_storage)
        return
    _validate_common_numeric_scalar(operation, "value", value)
    if algorithm is not None:
        raise ValueError(
            f"cuda.coop.{operation} algorithm selection is supported only for blocks"
        )
    if temp_storage is not None:
        raise ValueError(
...
def scan(
    ...
    *,
    ...
    scan_op = _portable_operator("scan", "scan_op", scan_op)
    if (
    ...
    if _backend_module_name() is not None:
        ...
        if initial_value is not None:
            _validate_common_numeric_scalar("scan", "initial_value", initial_value)
...
def exclusive_scan(
    ...
    *,
    ...
    scan_op = _portable_operator("exclusive_scan", "scan_op", scan_op)
    algorithm = _portable_selector(
        "exclusive_scan",
        "algorithm",
        algorithm,
...
def inclusive_scan(
    ...
    *,
    ...
    scan_op = _portable_operator("inclusive_scan", "scan_op", scan_op)
    algorithm = _portable_selector(
        "inclusive_scan",
        "algorithm",
        algorithm,

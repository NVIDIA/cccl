...
def _normalize_semantic_value(value: Any) -> Any:
    """Return a backend-defined stable representation of ``value``."""
...
class _TokenState:
    ...
    completed: dict[tuple[str, int], Any] = dataclasses.field(default_factory=dict)
    normalized_values: list[Any] = dataclasses.field(default_factory=list)
    cycle_hits: int = 0


def _normalized_token_value(value: Any, state: _TokenState) -> Any:
    normalized = _normalize_semantic_value(value)
    if normalized is not value:
        # Memoization keys include object identities, so keep replacement
        # values alive until the complete token walk finishes.
        state.normalized_values.append(normalized)
    return normalized
...
def _object_state_token(value: Any, state: _TokenState) -> tuple[Any, ...] | None:
    ...
    for cls in type(value).__mro__:
        ...
        for slot in slots:
            ...
            object_state.append((slot, token))
...
def _type_dependency_token(value: type, state: _TokenState) -> Any:
    ...
    if memo_key in state.completed:
        return state.completed[memo_key]
    ...
    if state.cycle_hits == cycle_hits_before:
        state.completed[memo_key] = token
    return token
...
def _dependency_token(value: Any, state: _TokenState) -> Any:
    ...
    normalized = _normalized_token_value(value, state)
    if normalized is not value:
    ...
    try:
        if isinstance(value, Mapping):
            ...
            token = tuple(sorted(items, key=lambda item: repr(item[0])))
        elif isinstance(value, (tuple, list)):
            token = tuple(_dependency_token(item, state) for item in value)
        elif isinstance(value, (set, frozenset)):
            token = tuple(
                sorted((_dependency_token(item, state) for item in value), key=repr)
            )
        else:
            token = (
...
def _semantic_token(value: Any, state: _TokenState) -> Any:
    ...
    try:
        if isinstance(value, Mapping):
            ...
            token = tuple(_semantic_token(item, state) for item in value)
        elif isinstance(value, (set, frozenset)):
            token = tuple(
                sorted((_semantic_token(item, state) for item in value), key=repr)
            )

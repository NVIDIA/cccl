...
The planner preserves portable scan meaning while selecting the matching CUB
block or warp specialization. It does not own backend activation, compiler
rewrites, or provider rendering.
"""
...
def _plan_scan(
    ...
    if (
        ...
        return _unsupported(
            ...
            resolved,
            UnsupportedReasonCode.OPERATION_VARIANT,
            "inclusive scans do not accept an initial value in the portable profile",
        )
    ...
    if (
        ...
        and (
            operation.scan_operator is not None
            or operation.valid_items.kind is not BindingKind.OMITTED
        )
    ):
        return _unsupported(
            call,
            resolved,
            UnsupportedReasonCode.OPERATION_VARIANT,
            "group exclusive scans that select a CUB Scan overload require an "
            "initial value because the no-initial overload leaves group rank "
            "zero undefined",
        )

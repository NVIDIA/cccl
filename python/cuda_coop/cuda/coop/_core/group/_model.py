...
def _lowered_operation_semantic_key(
    ...
    from ..block.reduce import BlockReduceAlgorithm
    from ..block.scan import BlockScanAlgorithm
    from .reduce import GroupReduceSemantics
    from .scan import GroupScanSemantics, _canonical_cub_scan_operation

    if (
        isinstance(operation, GroupReduceSemantics)
        and target is GroupLoweringTarget.CUB_BLOCK
    ):
        algorithm = operation.cub_algorithm or BlockReduceAlgorithm.WARP_REDUCTIONS
        return operation.primitive.semantic_key, operation.broadcast, algorithm.value
    if isinstance(operation, GroupScanSemantics) and target in {
        GroupLoweringTarget.CUB_BLOCK,
        GroupLoweringTarget.CUB_WARP,
    }:
        operation = _canonical_cub_scan_operation(operation)
        if target is GroupLoweringTarget.CUB_BLOCK:

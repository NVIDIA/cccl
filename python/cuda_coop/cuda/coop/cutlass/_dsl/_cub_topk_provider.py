...
_resolve_topk_type = _provider_support.make_provider_type_resolver(
    scope=__name__,
...
def _resolve_topk_thread_data_value_type(
    ...
    *,
    ...
    return _provider_support.resolve_thread_data_value_type(
        value,
        allowed=allowed,
        feature=feature,
        scope=_ROOT_SCOPE,
...
def _resolve_topk_thread_data_pair_types(
    *,
    ...
    return _provider_support.resolve_thread_data_pair_types(
        key=key,
        value=value,
...
def _topk_temp_storage_requirement(
    *,
    ...
    return _topk_cub_temp_storage_requirement(
        ...
        key_bytes=_provider_support.type_size_bytes(key_type),
        value_bytes=(
            0 if value_type is None else _provider_support.type_size_bytes(value_type)
        ),
    )
...
def _storage_ffi_args(primitive_name: str) -> tuple[bool, tuple[Any, ...]]:
    ...
    if context is None or context.temp_storage is None:
        return False, ()
    return True, _provider_support.temp_storage_ffi_args(
        primitive_name,
        scope=_ROOT_SCOPE,
    )
...
def provider_topk_keys(
    *,
    ...
    ffi(
        ...
        *key_values,
        _provider_support.as_int32(k),
        _provider_support.as_valid_items_arg(num_valid, scope=_ROOT_SCOPE),
        _provider_support.as_int32(begin_bit),
        _provider_support.as_int32(resolved_end),
        *storage_args,

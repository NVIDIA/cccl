...
class _CudaxGroupRequest:
    ...
    def symbol_name(self) -> str:
        ...
        if self.op in _QUERY_OPS:
            ...
            parts.append(_provider_support.TYPE_SPECS[self.result_type].token)
        return "_".join(parts)


def _result_type(result_type: type | None) -> type:
    if result_type is None:
        return _provider_support.Int32
    ...
    if result_type not in _provider_support.TYPE_SPECS:
        raise TypeError(
            f"{_ROOT_SCOPE}.ThreadGroup query dtype must be one of "
            f"{_provider_support.supported_names(_provider_support.ALL_PROVIDER_TYPES)}"
        )
    return result_type
...
def _render_cudax_group(request: _CudaxGroupRequest) -> list[str]:
    if request.op in _QUERY_OPS:
        spec = _provider_support.TYPE_SPECS[request.result_type]
        return [
...
def provider_group_membership(*, group: ThreadGroup) -> Any:
    ...
    return ffi(
        name=request.symbol_name,
        params_types=[],
        return_type=_provider_support.Uint8,
    )()

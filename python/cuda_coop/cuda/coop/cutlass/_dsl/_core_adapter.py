...
def resolve_deferred_core_temp_storage(
    *,
    group: Any,
    primitive_name: str,
    source: str,
    ...
    """Resolve public root/scoped block storage without changing legacy calls."""
    ...
    if not temp_storage.is_deferred:
        return None
    if getattr(group, "kind", None) != "block":
        raise ValueError(
            f"{_ROOT_SCOPE}.{primitive_name} deferred "
            "TempStorage is supported only for block groups"
        )
    if source not in {"cutlass_root", "scoped_block", "cutlass_scoped_block"}:
        raise ValueError(
            f"{_ROOT_SCOPE}.{primitive_name} deferred "
            "TempStorage requires a public root or scoped block call"
        )
    return temp_storage
...
def _render_output_initializers(
    ...
    for output_name, input_name in artifact.output_initializers:
        ...
        if isinstance(output_parameter, Array) and isinstance(input_parameter, Array):
            output_size = adapter._resolve(  # noqa: SLF001
                output_parameter.size,
                artifact.specialization,
            )
            input_size = adapter._resolve(  # noqa: SLF001
    ...
    *,
    ...
    _provider_support.register_bundle_renderer(
        ...
        scratch_layout_probe=lambda artifact: (
            _provider_support.make_scratch_layout_probe(

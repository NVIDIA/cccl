...
def _validate_scan_request_plan(
    ...
    *,
    ...
    if scan_operator is None:
        ...
        expected_cpp = _normalize_cpp_operator(_provider_support.cub_op_expr(op))
    ...
    if external_scratch:
        if plan.target is not GroupLoweringTarget.CUB_BLOCK:
            raise ValueError("deferred CUB Scan scratch is block-scoped only")
        if (
            ...
            raise ValueError(
                "deferred CUB BlockScan requires exact caller-owned shared storage"
            )
...
class _CubScanRequest:
    ...
    def symbol_name(self) -> str:
        ...
        symbol = (
            ...
            f"{self.group.kind}_{self.group.block_dim_token}_"
            f"{implementation.method_name.lower()}_{self.op}_"
            f"{_TYPE_SPECS[self.value_type].token}_{value_kind}_"
            f"{self._algorithm_suffix}_{initial}_{aggregate}_{valid_items}"
        )
...
def _deferred_temp_storage_for_scan(
    *,
    group: ThreadGroup,
    source: str,
    ...
    if group.kind != "block":
        raise ValueError(
            f"{_ROOT_SCOPE}.scan deferred TempStorage is supported only for "
            "block groups"
        )
    if source not in {"cutlass_root", "scoped_block"}:
        raise ValueError(
            f"{_ROOT_SCOPE}.scan deferred TempStorage requires a public root "
            "or scoped block call"
        )
    return temp_storage
...
def _materialize_scan(
    *,
    ...
    valid_items: Any,
    deferred_temp_storage: Any | None,
) -> Any:
    request = _CubScanRequest(
        plan=plan,
        op=op,
...
def provider_scan(
    *,
    ...
    source: str = "cutlass_group_scan_provider",
) -> Any:
    ...
    deferred_temp_storage = _deferred_temp_storage_for_scan(
        group=group,
        source=source,
    ...
    def materialize(
        *,
        ...
        from .. import _group_scan as _group_frontend

        plan = _group_frontend._make_group_scan_plan(
            group=group,
            launch=launch,
            dtype=value_type,
        ...
        return _materialize_scan(
            ...
            valid_items=valid_items,
            deferred_temp_storage=deferred_temp_storage,
        )

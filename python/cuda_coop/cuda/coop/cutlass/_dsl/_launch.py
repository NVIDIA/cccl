...
"""Launch-metadata normalization for CuTe cooperative primitives."""
...
def _reject_launch_metadata_kwargs(
    scope: str,
    primitive_name: str,
    kwargs: Mapping[str, Any],
) -> None:
    present = tuple(name for name in LAUNCH_METADATA_KEYS if name in kwargs)
    if present:
        names = ", ".join(present)
        raise TypeError(
            f"{scope}.{primitive_name} does not accept launch metadata "
...
def resolve_block_threads(
    ...
    *,
    ...
    if threads_per_block is not None and dim is not None and threads_per_block != dim:
        raise TypeError(
...
def bind_block_launch_metadata(
    ...
    *,
    ...
    present = [name for name in LAUNCH_METADATA_KEYS if name in kwargs]
    if len(present) > 1:
        names = ", ".join(present)
        raise TypeError(
            f"{scope}.{primitive_name} got multiple launch metadata aliases: {names}"

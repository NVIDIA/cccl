...
class ThreadGroup(_CoreThreadGroup):
    ...
    def rank_as(self, dtype: Any = None, level: str = "thread") -> Any:
        ...
        from ._dsl import _thread_group_provider as _provider
...
def _require_resolved_group(
    ...
    *,
    ...
    from ._dsl._launch import current_kernel_launch_facts
...
def _resolve_collective_group(
    group: ThreadGroup,
    launch_kwargs: dict[str, Any],
    *,
    feature: str,
) -> ThreadGroup:
    """Resolve a collective group through the shared launch-fact path."""
    ...
    try:
        launch = infer_launch_facts(
            launch_kwargs,
            scope=_ROOT_SCOPE,
            primitive_name=feature,
        )
    except LaunchFactConflict:
        raise
    except ValueError as exc:
        raise NotImplementedError(
            _collective_group_resolution_error(group, feature=feature)
        ) from exc
    return _resolve_collective_group_from_launch(
        group,
        launch,
        feature=feature,
    )
...
def _validate_sync_launch(group: ThreadGroup, *, feature: str) -> ThreadGroup:
    ...
    if group.kind == "grid" and group.source == "common_root":
        raise NotImplementedError(
            ...
            "the common V1 profile; use cuda.coop.cutlass.this_grid() under a "

...
from ._core import CutlassCoreAdapter, CutlassCoreArtifact
...
def _static_positive_int(name: str, value: Any) -> int:
    return normalize_block_histogram_positive_int(
        name,
        value,
        scope=f"{_ROOT_SCOPE}.histogram",
    )


def _normalize_algorithm(algorithm: Any) -> Any:
    try:
        return normalize_block_histogram_algorithm(algorithm)
    except ValueError as exc:
        raise ValueError(
            f"{_ROOT_SCOPE}.histogram algorithm must be 'atomic' or 'sort'"
        ) from exc
...
def _make_group_histogram_plan(
    *,
    ...
    """Build the canonical shared-core histogram plan."""
...
def _make_request(
    *,
    ...
    plan = _make_group_histogram_plan(
        group=group,
        launch=launch,
...
def provider_histogram(
    *,
    ...
    if isinstance(samples, ThreadData):
        sample_type, sample_values = _provider_types.resolve_thread_data_value_type(
            samples,

...
from .._compiler import _state, _types
...
def _normalize_mode(mode: Any) -> BlockShuffleMode:
    try:
        ...
        try:
            ...
            raise ValueError(
                f"{_ROOT_SCOPE}.shuffle mode must be 'up', 'down', 'offset', "
                "or 'rotate'"
            ) from exc
...
def _normalize_shuffle_route(
    ...
    *,
    ...
    if isinstance(distance, bool):
        raise TypeError(f"{_ROOT_SCOPE}.shuffle distance must be an integer, not bool")
    if is_thread_data:
        if not isinstance(distance, Integral):
            raise TypeError(
                f"{_ROOT_SCOPE}.shuffle ThreadData Up/Down requires the "
        ...
        if mode not in {BlockShuffleMode.UP, BlockShuffleMode.DOWN}:
            raise NotImplementedError(
                f"{_ROOT_SCOPE}.shuffle ThreadData supports only public-CUB "
                "Up/Down unit-shift forms"
            )
        if distance != 1:
            raise NotImplementedError(
                f"{_ROOT_SCOPE}.shuffle ThreadData Up/Down supports only distance=1"
            )
        if block_prefix is not None and block_suffix is not None:
            raise ValueError(
                f"{_ROOT_SCOPE}.shuffle accepts only one of block_prefix or "
                "block_suffix"
            )
        if block_prefix is not None and mode is not BlockShuffleMode.DOWN:
            raise ValueError(
                f"{_ROOT_SCOPE}.shuffle block_prefix is valid only for ThreadData Down"
            )
        if block_suffix is not None and mode is not BlockShuffleMode.UP:
            raise ValueError(
                f"{_ROOT_SCOPE}.shuffle block_suffix is valid only for ThreadData Up"
            )
        return distance

    if mode not in {BlockShuffleMode.OFFSET, BlockShuffleMode.ROTATE}:
        raise NotImplementedError(
            f"{_ROOT_SCOPE}.shuffle scalar values support only public-CUB Offset/Rotate"
        )
    if block_prefix is not None or block_suffix is not None:
        raise NotImplementedError(
            f"{_ROOT_SCOPE}.shuffle scalar Offset/Rotate does not return block "
    ...
    if mode is BlockShuffleMode.ROTATE and isinstance(distance, int) and distance < 0:
        raise ValueError(f"{_ROOT_SCOPE}.shuffle Rotate distance must be non-negative")
    return distance
...
def provider_shuffle(
    *,
    ...
    if items_per_thread is None:
        ...
        runtime_values = {
            "input_item": values[0],
            "distance": _provider_types.as_int32(distance),
        }
    else:

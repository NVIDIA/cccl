...
"""Private warp-scope compatibility primitives for the CUTLASS backend."""

from .. import _root_load_store
from .._dsl import warp as _backend
...
_SOURCE_SCOPE = "cuda.coop.cutlass._dsl.warp"
...
def make_load(*args, **kwargs):
    ...
    """
    return _root_load_store.make_warp_load_factory(
        _tensor_make_load,
        load,
        *args,
        scope=_TARGET_SCOPE,
...
def make_store(*args, **kwargs):
    ...
    deferred-call defaults. ``dtype=`` and ``offset=`` can stay on canonical CUB.
    """
    return _root_load_store.make_warp_store_factory(
        _tensor_make_store,
        store,
        *args,
        scope=_TARGET_SCOPE,

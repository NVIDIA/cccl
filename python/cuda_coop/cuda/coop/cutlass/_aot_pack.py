...
def _parse_identity(value: Any, *, entry_index: int) -> _ManifestIdentity:
    ...
    identity = _ManifestIdentity(
        version=_require_integer(
            payload["version"],
            description=f"entry {entry_index} identity version",
            minimum=1,
        ),
    ...
    if identity.version != BUNDLE_IDENTITY_VERSION:
        raise PackIntegrityError(
            f"entry {entry_index} uses unsupported bundle identity version "
            f"{identity.version}."
        )
    ...
    *,
    ...
    try:
        if loaded is None:
            ...
            from ._dsl import _provider_bundle
...
def _precompile_resolver() -> Any:
    ...
    return _provider_bundle._bundle_precompile_resolver(
        ...
        route=_provider_bundle.RESOLUTION_ROUTE_AOT_PACK,
        phase="pack_lookup",
    )
...
def _assert_provider_constants(provider_bundle: Any) -> None:
    if provider_bundle.BUNDLE_IDENTITY_VERSION != BUNDLE_IDENTITY_VERSION:
        raise PackIntegrityError(
            "AOT pack bundle identity version is out of sync with the provider."
        )
    if provider_bundle.PROVIDER_BUNDLE_ABI_VERSION != PROVIDER_ABI_VERSION:
        raise PackIntegrityError(
...
def resolve_precompiled_bundle(request: Any) -> Any | None:
    ...
    _assert_provider_constants(_provider_bundle)
    ...
    return _provider_bundle.BundleResolution(
        request=request,
        path=artifact_path,
        layouts_by_expression={
            layout.expression: _provider_bundle.StorageLayout(
                size_in_bytes=layout.size_in_bytes,
                alignment=layout.alignment,
            )
...
def _read_resolution_artifact(resolution: Any, provider_bundle: Any) -> bytes:
    if resolution.route == provider_bundle.RESOLUTION_ROUTE_AOT_PACK:
...
def _capture_resolution_routes(provider_bundle: Any) -> frozenset[str]:
    return frozenset(
        {
            provider_bundle.RESOLUTION_ROUTE_AOT_PACK,
...
def _require_trusted_capture_resolution(
    ...
    if resolution.route not in _capture_resolution_routes(provider_bundle):
        raise CaptureError(
...
class _CaptureSink:
    ...
    def record(self, resolution: Any) -> None:
        ...
        _require_trusted_capture_resolution(resolution, _provider_bundle)
        ...
        if (
            resolution.route == _provider_bundle.RESOLUTION_ROUTE_DISK
        ...
        artifact = _read_resolution_artifact(resolution, _provider_bundle)

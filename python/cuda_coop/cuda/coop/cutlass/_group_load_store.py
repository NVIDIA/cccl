...
def _launch_and_resolve_group(
    group: ThreadGroup,
    launch_kwargs: Mapping[str, Any] | None,
    *,
    primitive_name: str,
    ...
    launch = infer_launch_facts(
        {} if launch_kwargs is None else launch_kwargs,
        scope=_SCOPE,
        primitive_name=primitive_name,
    )
    if launch_kwargs is None and not launch.is_verified("exact_block_dim"):
        raise NotImplementedError(
...
def _load(
    ...
    *,
    ...
    _launch_kwargs: Mapping[str, Any] | None = None,
) -> ThreadData:
    """Collectively load one block or physical-warp tile into ``output``.
    ...
    from ._dsl import _cub_load_store_provider as _provider

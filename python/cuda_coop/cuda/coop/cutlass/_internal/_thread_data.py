...
from .._value_metadata import (
    attach_thread_data_metadata as _attach_group_metadata,
)
...
_ROOT_SCOPE = __name__.split("._internal", 1)[0]
_UNSET = object()
...
class ThreadData:
    ...
    CUTLASS DSL kernel. Block and warp primitives infer ``items_per_thread``
    from this object, so users specify the item count once when constructing
    the payload.
    """
    ...
    def __init__(
        ...
        *,
        ...
        common_v1 = _common_root_operation_name() == "ThreadData"
        if dtype is not None and common_v1:
            from .._dsl._provider import _validate_common_root_numeric_dtype
    ...
    def from_values(cls, *values: Any, dtype: Any = None) -> ThreadData:
        ...
        result = cls(len(values), dtype=dtype, values=list(values))
        return _attach_group_metadata(
            result,
            _merge_group_metadata(_value_group_metadata(value) for value in values),
        )
        ...
        *,
        ...
        if _is_memory_backed_payload(vector):
            raise TypeError(
                ...
                "CuTe register fragments, or a scoped load for cutlass.Array "
    ...
    def from_payload(
        ...
        *,
        ...
        scoped load/store helpers to move them into per-thread registers first.
        """
        if isinstance(payload, cls):
            ...
            result = payload._preserve_common_v1(result)
            return _attach_group_metadata(result, _thread_data_metadata(payload))
        ...
        if _is_memory_backed_payload(payload):
            raise TypeError(
                ...
                "or a scoped load for cutlass.Array values and memory tensors"
            )
    ...
    def __setitem__(self, idx: int, value: Any) -> None:
        ...
        self._group_metadata = _merge_group_metadata(
            (
                _thread_data_metadata(self),
                _value_group_metadata(value),
            )
        )

    def _preserve_common_v1(self, result: ThreadData) -> ThreadData:
        result._common_v1 = self._common_v1
        return result
    ...
    def __copy__(self) -> ThreadData:
        ...
        result = self._preserve_common_v1(result)
        return _attach_group_metadata(result, _thread_data_metadata(self))
    ...
    def _new_uninitialized(self, *, dtype: Any = None) -> ThreadData:
        ...
        if self._common_v1 and resolved_dtype is not None:
            from .._dsl._provider import _validate_common_root_numeric_dtype
        ...
        return self._preserve_common_v1(result)
            ...
            if common_root_payload_kind == "thread_data":
                if not isinstance(value, ThreadData):
                    raise TypeError(
                        ...
                        f"ThreadData {arg_name} payload in common V1; use "
                ...
                if not isinstance(value, ThreadData) and _is_thread_payload_candidate(
                    ...
                    raise TypeError(
                        ...
                        f"fixed-size ThreadData {arg_name} payload in common V1; "

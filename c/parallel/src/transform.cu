...
struct input_iterator1_t;
struct input_iterator2_t;
...
struct transform_runtime_tuning_policy
{
  ...
  static constexpr cub::detail::transform::Algorithm GetAlgorithm() // TODO this can't be constexpr
  {
...
std::string get_input_iterator1_name()
{
  std::string iterator_t;
  check(nvrtcGetTypeName<input_iterator1_t>(&iterator_t));
  return iterator_t;
}

std::string get_input_iterator2_name()
{
  std::string iterator_t;
  check(nvrtcGetTypeName<input_iterator2_t>(&iterator_t));
  return iterator_t;
}
...
struct transform_kernel_source
{
  ...
  constexpr It make_iterator_kernel_arg(It it)
  {
    return it;
  }
};
...
CUresult cccl_device_transform_build(
...
struct prefetch_policy_t {{
  ...
  static constexpr int items_per_thread_no_input = {3};
  static constexpr int min_items_per_thread      = {4};
  static constexpr int max_items_per_thread      = {5};
}};
struct device_transform_policy {{
  struct ActivePolicy {{
...
{8}
    ...
    const std::string& src = std::format(
      ...
      policy.max_items_per_thread, // 5
      input_iterator_src, // 6
    ...
    build_ptr->loaded_bytes_per_iteration = input_it.value_type.size; // TODO: this needs to be computed correctly
    build_ptr->cc                         = cc;
...
CUresult cccl_device_transform(
  cccl_device_transform_build_result_t build,
  ...
  CUstream stream)
{
  ...
  try
  {
    ...
    auto cuda_error = cub::detail::transform::dispatch_t<
      ...
      ::cuda::std::int32_t, // TODO generalize?

...
struct op_wrapper;
struct device_scan_by_key_policy;
...
std::string get_init_kernel_name(
  cccl_iterator_t input_keys_it, cccl_iterator_t input_it, cccl_op_t op, cccl_type_info init)
{
  const cccl_type_info accum_t  = scan_by_key::get_accumulator_type(op, input_it, init);
  const std::string accum_cpp_t = cccl_type_enum_to_name(accum_t.type);
  const std::string input_keys_iterator_t =
    input_keys_it.type == cccl_iterator_kind_t::CCCL_POINTER
      ? cccl_type_enum_to_name(input_keys_it.value_type.type, true)
      : scan_by_key::get_input_keys_iterator_name();

  std::string offset_t;
  check(cccl_type_name_from_nvrtc<OffsetT>(&offset_t));

  return std::format(
    "cub::detail::scan_by_key::DeviceScanByKeyInitKernel<cub::ReduceByKeyScanTileState<{0}, int>, {1}, {2}>",
    accum_cpp_t,
  ...
  cccl_init_kind_t init_kind)
{
  ...
  check(cccl_type_name_from_nvrtc<device_scan_by_key_policy>(&policy_selector_t));
  ...
  const std::string input_iterator_t =
    input_it.type == cccl_iterator_kind_t::CCCL_POINTER
      ...
      : scan_by_key::get_input_iterator_name();
  const std::string output_iterator_t =
    output_it.type == cccl_iterator_kind_t::CCCL_POINTER
      ...
      : scan_by_key::get_output_iterator_name();
  ...
  const std::string init_value_t =
    init_kind == cccl_init_kind_t::CCCL_NO_INIT ? "cub::NullType" : cccl_type_enum_to_name(init.type);

  return std::format(
    "cub::detail::scan_by_key::DeviceScanByKeyKernel<{0}, {1}, {2}, {3}, "
    "cub::ReduceByKeyScanTileState<{4}, int>, {5}, {6}, {7}, {8}, {4}>",
    policy_selector_t, // 0
...
// The JIT-compiled kernel names cub::ReduceByKeyScanTileState<accum, int>, which resolves to one of
// two layouts depending on the pair size. The launcher passes the tile state by value, so the host
// has to hand back a type of the same width.
template <int TxnWordSize>
struct scan_by_key_single_word_kernel_source
{
  cccl_device_scan_by_key_build_result_t& build;

  using ScanByKeyTileStateT = reduce_by_key_single_word_tile_state<TxnWordSize>;
  ...
  ScanByKeyTileStateT TileState() const
  {
    return {};
  }
};

struct scan_by_key_multi_word_kernel_source
{
  cccl_device_scan_by_key_build_result_t& build;

  using ScanByKeyTileStateT = scan_tile_state;
...
CUresult cccl_device_scan_by_key_compile(
...
try
{
  ...
  const std::string input_keys_iterator_src = make_kernel_input_iterator(
    offset_t, "input_keys_iterator_state_t", input_keys_it_value_t, input_keys_it);
  ...
  // The equality operator compares keys and yields a predicate, so it takes the comparison form
  // under its own wrapper name -- one program can carry only one op_wrapper.
  const std::string equality_op_src =
    make_kernel_user_comparison_operator(key_cpp, equality_op, "equality_op_wrapper");

  const auto policy_sel = [&] {
    using cub::detail::scan_by_key::policy_selector;

    const auto is_trivial_type = [](cccl_type_enum) { return true; };

    return policy_selector{
      static_cast<int>(input_keys_it.value_type.size),
      static_cast<int>(input_it.value_type.size),
      static_cast<int>(accum_t.size),
      true, // value_is_primitive: every type the C API accepts today is primitive
      is_trivial_type(input_it.value_type.type),
      true, // accum_is_primitive
      is_trivial_type(accum_t.type),
      cccl_type_enum_to_cub_type(input_keys_it.value_type.type),
      cccl_type_enum_to_cub_type(input_it.value_type.type),
      cccl_type_enum_to_cub_type(accum_t.type),
      cccl_op_kind_to_cub_op(op.type)};
  }();
  ...
  std::string policy_selector_expr = std::format(
    "cub::detail::scan_by_key::policy_selector_from_types<{}, {}, {}, {}>",
    input_keys_iterator_t,
    accum_cpp,
    input_iterator_t,
    "op_wrapper");
...
{4}
{8}
using device_scan_by_key_policy = {5};
using namespace cub;
using namespace cub::detail::scan_by_key;
using cub::LookbackDelayPolicy;
using cub::LookbackDelayAlgorithm;
static_assert(device_scan_by_key_policy()(detail::current_tuning_cc()) == {6}, "Host generated and JIT compiled policy mismatch");
)XXX",
  ...
  if (scan_by_key::nvrtc_miscompiles_bf16_simd_intrinsics())
  {
  ...
  std::string init_kernel_name = scan_by_key::get_init_kernel_name(input_keys_it, input_it, op, init);
  std::string scan_kernel_name =
    scan_by_key::get_scan_kernel_name(input_keys_it, input_it, output_it, op, init, init_kind);
  ...
  if (active_policy.algorithm == cub::ScanByKeyAlgorithm::lookback)
  {
  ...
  appender.append_operation(equality_op);
  ...
  static_assert(::cuda::is_trivially_copyable_v<cub::detail::scan_by_key::policy_selector>);
...
{
  fflush(stderr);
  printf("\nEXCEPTION in cccl_device_scan_by_key_compile(): %s\n", exc.what());
  fflush(stdout);

  return CUDA_ERROR_UNKNOWN;
}

CUresult cccl_device_scan_by_key_load(cccl_device_scan_by_key_build_result_t* build_ptr)
try
{
...
{
  fflush(stderr);
  printf("\nEXCEPTION in cccl_device_scan_by_key_load(): %s\n", exc.what());
  fflush(stdout);
  return CUDA_ERROR_UNKNOWN;
}

template <typename InitValueT>
CUresult cccl_device_scan_by_key_launch(
  cccl_device_scan_by_key_build_result_t build,
  void* d_temp_storage,
  ...
  CUstream stream)
{
  ...
  try
  {
    ...
    auto policy_selector  = *static_cast<cub::detail::scan_by_key::policy_selector*>(build.runtime_policy);
    ...
    auto launch = [&](auto kernel_source) {
      ...
      return cub::detail::scan_by_key::dispatch<
        char,
        indirect_arg_t,
        indirect_arg_t,
        indirect_arg_t,
        ...
        OffsetT,
        char,
        cub::detail::scan_by_key::policy_selector,
        KernelSourceT,
    ...
    // The compiled kernel names cub::ReduceByKeyScanTileState<accumulator, int>: the tile state holds
    // the aggregate next to an int run index, so its width follows the accumulator, not the key.
    constexpr int run_index_size = static_cast<int>(sizeof(int));
    const int accum_size         = static_cast<int>(build.accumulator_type.size);
    cudaError_t exec_status;
    if (reduce_by_key_is_single_word(run_index_size, accum_size))
    {
      switch (reduce_by_key_txn_word_size(run_index_size, accum_size))
      {
        case 4:
          exec_status = launch(scan_by_key::scan_by_key_single_word_kernel_source<4>{build});
          break;
        case 8:
          exec_status = launch(scan_by_key::scan_by_key_single_word_kernel_source<8>{build});
          break;
        default:
          exec_status = launch(scan_by_key::scan_by_key_single_word_kernel_source<16>{build});
          break;
      }
    }
    else
    {
      exec_status = launch(scan_by_key::scan_by_key_multi_word_kernel_source{build});
    }
  ...
  {
    fflush(stderr);
    printf("\nEXCEPTION in cccl_device_scan_by_key(): %s\n", exc.what());
    fflush(stdout);
  ...
  CUstream stream)
{
  ...
  return cccl_device_scan_by_key_launch<cccl_value_t>(
    build, d_temp_storage, temp_storage_bytes, d_keys_in, d_values_in, d_values_out, num_items, op, equality_op, init,
    stream);
}
  ...
  CUstream stream)
{
  ...
  return cccl_device_scan_by_key_launch<cub::NullType>(
    build, d_temp_storage, temp_storage_bytes, d_keys_in, d_values_in, d_values_out, num_items, op, equality_op,
    cub::NullType{}, stream);
}
...
CUresult cccl_device_scan_by_key_build_ex(
  ...
  cccl_build_config* config)
{
  CUresult r = cccl_device_scan_by_key_compile(
    build_ptr,
    d_keys_in,
    d_values_in,
    d_values_out,
  ...
  CUresult load_r = cccl_device_scan_by_key_load(build_ptr);
  if (load_r != CUDA_SUCCESS)
  {
    cccl_device_scan_by_key_cleanup(build_ptr);
  }
  return load_r;
}
...
CUresult cccl_device_scan_by_key_link_ltoir(
  cccl_device_scan_by_key_build_result_t* build_ptr, const void** input_blobs, const size_t* input_sizes, size_t num_inputs)
try
{
  if (build_ptr == nullptr || build_ptr->payload == nullptr || build_ptr->payload_size == 0
      || build_ptr->payload_kind != CCCL_PAYLOAD_LTOIR)
...
{
  printf("\nEXCEPTION in cccl_device_scan_by_key_link_ltoir(): %s\n", exc.what());
  return CUDA_ERROR_UNKNOWN;
}

CUresult cccl_device_scan_by_key_serialize(const cccl_device_scan_by_key_build_result_t* build_ptr, void** out_buf, size_t* out_size)
try
{
...
{
  fflush(stderr);
  printf("\nEXCEPTION in cccl_device_scan_by_key_serialize(): %s\n", exc.what());
  fflush(stdout);
  return CUDA_ERROR_UNKNOWN;
}

CUresult cccl_device_scan_by_key_deserialize(cccl_device_scan_by_key_build_result_t* build_ptr, const void* buf, size_t size)
try
{
  ...
  std::unique_ptr<cub::detail::scan_by_key::policy_selector, decltype(&std::free)> policy(
    static_cast<cub::detail::scan_by_key::policy_selector*>(std::malloc(sizeof(cub::detail::scan_by_key::policy_selector))),
    std::free);
  if (!policy)
  {
    return CUDA_ERROR_OUT_OF_MEMORY;
  }
  r.read_into(policy.get(), sizeof(cub::detail::scan_by_key::policy_selector));

  std::unique_ptr<char[]> n_init{r.read_cstring_dup()};
  std::unique_ptr<char[]> n_scan{r.read_cstring_dup()};

  cccl_device_scan_by_key_build_result_t result{};
  ...
  result.runtime_policy_size        = sizeof(cub::detail::scan_by_key::policy_selector);
...
{
  fflush(stderr);
  printf("\nEXCEPTION in cccl_device_scan_by_key_deserialize(): %s\n", exc.what());
  fflush(stdout);
  return CUDA_ERROR_UNKNOWN;
}

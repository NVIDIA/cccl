...
namespace merge_sort_aot
{
inline uint64_t aot_abi_hash()
{
  using namespace cccl::aot;
  uint64_t h = fnv1a64("cccl_device_merge_sort");
  h          = fnv1a64_mix(h, CCCL_VERSION);
  h          = fnv1a64_mix(h, sizeof(cccl_device_merge_sort_build_result_t));
  h          = fnv1a64_mix(h, sizeof(cub::detail::merge_sort::policy_selector));
  return h;
}
} // namespace merge_sort_aot
...
try
{
  ...
  using namespace cccl::aot;
  buffer_writer w;
  write_header(w, CCCL_AOT_ALGO_MERGE_SORT, merge_sort_aot::aot_abi_hash(), build_ptr->payload_kind, build_ptr->cc);
...
try
{
  ...
  const auto h = read_and_validate_header(r, CCCL_AOT_ALGO_MERGE_SORT, merge_sort_aot::aot_abi_hash());

...
// Diagnostics for the AoT deserialize path. Declared in a standalone header
// (rather than aot.h, which is pulled in transitively by every algorithm
// translation unit) so adding it does not trigger a rebuild of the whole C
// parallel library.

CCCL_C_EXTERN_C_BEGIN

// Returns a human-readable description of the most recent AoT failure on the
// calling thread, or "" if none. The returned pointer is owned by the library
// and is valid until the next AoT call on the same thread. Callers can surface
// this so a deserialize failure carries an actionable message instead of only an
// opaque CUDA error code.
CCCL_C_API const char* cccl_aot_last_error(void);

// Validates that an AoT blob (a build_result blob, i.e. what is passed to
...
// message retrievable via cccl_aot_last_error(). This never executes device
// code; it only inspects the header.
CCCL_C_API CUresult cccl_aot_validate_blob(const void* buf, size_t size);

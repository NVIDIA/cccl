...
CUresult
cccl_device_three_way_partition_load(cccl_device_three_way_partition_build_result_t* build_ptr, const char* ctk_path)
try
{
  ...
  {
    fprintf(stderr, "\nERROR in cccl_device_three_way_partition_load(): %s\n", compiler->getLastError().c_str());
    return CUDA_ERROR_UNKNOWN;
  }

  auto fn = compiler->getFunction<three_way_partition_fn_t>("cccl_jit_three_way_partition");
  if (!fn)
  {
    fprintf(stderr, "\nERROR in cccl_device_three_way_partition_load(): %s\n", compiler->getLastError().c_str());
    return CUDA_ERROR_UNKNOWN;
  ...
  build_ptr->three_way_partition_fn = reinterpret_cast<void*>(fn);
...
try
{
  ...
  *out_size = 0;
  ...
  std::vector<char> blob = cccl::serialization_v2::write_blob(
    CCCL_SERIALIZATION_V2_ALGO_THREE_WAY_PARTITION,
    build_ptr->cc,
    {"cccl_jit_three_way_partition"},
    /*extra=*/{},
    build_ptr->payload,
...
try
{
  ...
  cccl::serialization_v2::parsed_blob parsed =
    cccl::serialization_v2::read_blob(CCCL_SERIALIZATION_V2_ALGO_THREE_WAY_PARTITION, buf, size);
  ...
  result.three_way_partition_fn = nullptr;
